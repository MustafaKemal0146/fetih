"""Windows UTF-8 bootstrap for FETIH entry points.
...
should be imported at the very top of every FETIH entry point
(``fetih``, ``fetih-agent``, ``fetih-acp``, ``python -m gateway.run``,
...
# Apply on import — entry points just need ``import fetih_bootstrap``
# (or ``from fetih_bootstrap import apply_windows_utf8_bootstrap``) at
