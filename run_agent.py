...
from fetih_cli.timeouts import (
...
_loaded_env_paths = load_fetih_dotenv(fetih_home=_fetih_home, project_env=_project_env)
...
class AIAgent:
    ...
    _TOOL_CALL_ARGUMENTS_CORRUPTION_MARKER = (
        "[fetih-agent: tool call arguments were corrupted in this session and "
    ...
    def _ensure_db_session(self) -> None:
        ...
        try:
            self._session_db.create_session(
                ...
                source=self.platform or os.environ.get("FETIH_SESSION_SOURCE", "cli"),
                model=self.model,
    ...
    def _ensure_lmstudio_runtime_loaded(self, config_context_length: Optional[int] = None) -> None:
        """
        Preload the LM Studio model with at least FETIH' minimum context.
        """
        if (self.provider or "").strip().lower() != "lmstudio":
            return
        try:
            from agent.model_metadata import MINIMUM_CONTEXT_LENGTH
            from fetih_cli.models import ensure_lmstudio_model_loaded
    ...
    def _vprint(self, *args, force: bool = False, **kwargs):
        ...
        parseable single-query flows such as ``fetih chat -q``. In that mode,
    ...
    def _resolved_api_call_timeout(self) -> float:
        ...
        Priority:
          ...
          3. ``FETIH_API_TIMEOUT`` env var (legacy escape hatch)
          4. 1800.0s default
        ...
        helper, the hardcoded ``FETIH_API_TIMEOUT`` fallback would always be
        ...
        if cfg is not None:
            return cfg
        return float(os.getenv("FETIH_API_TIMEOUT", 1800.0))
    ...
    def _resolved_api_call_stale_timeout_base(self) -> tuple[float, bool]:
        ...
        Priority:
          ...
          3. ``FETIH_API_CALL_STALE_TIMEOUT`` env var
          4. 300.0s default
        ...
        env_timeout = os.getenv("FETIH_API_CALL_STALE_TIMEOUT")
    ...
    def _try_refresh_codex_client_credentials(self, *, force: bool = True) -> bool:
        ...
        # pool entry (``fetih auth add xai-oauth``) whose tokens belong to
    ...
    def _try_refresh_copilot_client_credentials(self) -> bool:
        ...
        try:
            from fetih_cli.copilot_auth import resolve_copilot_token
    ...
    def _supports_reasoning_extra_body(self) -> bool:
        ...
        known reasoning-capable model families and direct FETIH Portal.
        """
        if base_url_host_matches(self._base_url_lower, "github.com/MustafaKemal0146/fetih"):
            return True
        ...
        if (
            ...
            try:
                from fetih_cli.models import github_model_reasoning_efforts
    ...
    def _lmstudio_reasoning_options_cached(self) -> list[str]:
        ...
        if cached is not None:
            ...
            if opts or (_time.monotonic() - ts) < 60:
                return opts
        try:
            from fetih_cli.models import lmstudio_model_reasoning_options
