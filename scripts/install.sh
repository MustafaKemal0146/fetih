...
# Python-driven tool session (e.g. FETIH terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:MustafaKemal0146/fetih.git"
REPO_URL_HTTPS="https://github.com/MustafaKemal0146/fetih.git"
FETIH_HOME="${FETIH_HOME:-$HOME/.fetih}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${FETIH_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$FETIH_INSTALL_DIR"
...
#   code at /usr/local/lib/fetih, command at /usr/local/bin/fetih,
#   data still at /root/.fetih (FETIH_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   default (non-root):  ~/Desktop/fetih"
            echo "                   default (root, Linux): /usr/local/lib/fetih"
            echo "  --fetih-home PATH  Data directory (default: ~/.fetih, or \$FETIH_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, FETIH installs the code under"
            echo "  /usr/local/lib/fetih and links the command into"
            echo "  /usr/local/bin/fetih (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$FETIH_HOME"
            echo "  (default /root/.fetih).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$FETIH_HOME/fetih are preserved in-place."
            echo "  --ensure DEPS  Install only specified deps (comma-separated)"
            echo "                   Supported: node, browser, ripgrep, ffmpeg"
            echo "                   Does NOT clone repo or create venv"
            echo "  --postinstall  Run post-install setup only (for pip users)"
            echo "                   Installs optional deps + runs fetih setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
print_banner() {
    ...
    echo "│             ⚕ FETIH Installer                    │"
    echo "├─────────────────────────────────────────────────────────┤"
    echo "│  An open source AI agent by FETIH.              │"
    echo "└─────────────────────────────────────────────────────────┘"
    echo -e "${NC}"
}
...
# Decide where the repo checkout + venv live, and where the `fetih` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $FETIH_HOME/fetih
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $FETIH_HOME/fetih
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/fetih
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $FETIH_HOME/fetih — then preserve it)
#
# Always no-op when the user set --dir or $FETIH_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in FETIH_HOME.
    if is_termux; then
        INSTALL_DIR="$FETIH_HOME/fetih"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$FETIH_HOME/fetih/.git" ]; then
            INSTALL_DIR="$FETIH_HOME/fetih"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/fetih)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/fetih"
        ROOT_FHS_LAYOUT=true
        log_info "Root install on Linux — using FHS layout"
        log_info "  Code:    $INSTALL_DIR"
        log_info "  Command: /usr/local/bin/fetih"
        log_info "  Data:    $FETIH_HOME (unchanged)"
        return 0
    fi
...
get_fetih_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/fetih" ]; then
        echo "$link_dir/fetih"
    else
        echo "fetih"
    fi
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  irm https://raw.githubusercontent.com/MustafaKemal0146/fetih/main/scripts/install.ps1 | iex"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/fetih-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/fetih-uv-installer.$$.sh")"
...
check_node() {
    ...
    if [ -x "$FETIH_HOME/node/bin/node" ]; then
        export PATH="$FETIH_HOME/node/bin:$PATH"
        local found_ver=$("$FETIH_HOME/node/bin/node" --version)
        log_success "Node.js $found_ver found (FETIH-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    log_info "Extracting to ~/.fetih/node/..."
    ...
    # Place into ~/.fetih/node/ and symlink binaries to ~/.local/bin/
    rm -rf "$FETIH_HOME/node"
    mkdir -p "$FETIH_HOME"
    mv "$extracted_dir" "$FETIH_HOME/node"
    rm -rf "$tmp_dir"

    mkdir -p "$HOME/.local/bin"
    ln -sf "$FETIH_HOME/node/bin/node" "$HOME/.local/bin/node"
    ln -sf "$FETIH_HOME/node/bin/npm"  "$HOME/.local/bin/npm"
    ln -sf "$FETIH_HOME/node/bin/npx"  "$HOME/.local/bin/npx"

    export PATH="$FETIH_HOME/node/bin:$PATH"

    local installed_ver
    installed_ver=$("$FETIH_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.fetih/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. FETIH install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "FETIH itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                local stash_name
                stash_name="fetih-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if FETIH behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up fetih command..."

    if [ "$USE_VENV" = true ]; then
        FETIH_BIN="$INSTALL_DIR/venv/bin/fetih"
    else
        FETIH_BIN="$(which fetih 2>/dev/null || echo "")"
        if [ -z "$FETIH_BIN" ]; then
            log_warn "fetih not found on PATH after install"
            return 0
        fi
    fi
    ...
    if [ ! -x "$FETIH_BIN" ]; then
        log_warn "fetih entry point not found at $FETIH_BIN"
    ...
    # Create a user-facing shim for the fetih command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $FETIH_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$FETIH_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/fetih"
    cat > "$command_link_dir/fetih" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$FETIH_BIN" "\$@"
EOF
    chmod +x "$command_link_dir/fetih"
    log_success "Installed fetih launcher → $command_link_display_dir/fetih"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "fetih command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v fetih' \
                >/dev/null 2>&1; then
        ...
        log_info "fetih not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# FETIH — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# FETIH — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# FETIH — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so fetih works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.fetih directory structure (config at top level, code in subdir)
    mkdir -p "$FETIH_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.fetih/.env (top level, easy to find)
    if [ ! -f "$FETIH_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$FETIH_HOME/.env"
            log_success "Created ~/.fetih/.env from template"
        else
            touch "$FETIH_HOME/.env"
            log_success "Created ~/.fetih/.env"
        fi
    else
        log_info "~/.fetih/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$FETIH_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.fetih/config.yaml (top level, easy to find)
    if [ ! -f "$FETIH_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$FETIH_HOME/config.yaml"
            log_success "Created ~/.fetih/config.yaml from template"
        fi
    else
        log_info "~/.fetih/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$FETIH_HOME/SOUL.md" ]; then
        cat > "$FETIH_HOME/SOUL.md" << 'SOUL_EOF'
# FETIH Persona

<!--
This file defines the agent's personality and tone.
The agent will embody whatever you write here.
Edit this to customize how FETIH communicates with you.
...
-->
SOUL_EOF
        log_success "Created ~/.fetih/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.fetih/"

    # Seed bundled skills into ~/.fetih/skills/ (manifest-based, one-time per skill)
    log_info "Syncing bundled skills to ~/.fetih/skills/ ..."
    if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
        log_success "Skills synced to ~/.fetih/skills/"
    else
        # Fallback: simple directory copy if Python sync fails
        if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$FETIH_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
            cp -r "$INSTALL_DIR/skills/"* "$FETIH_HOME/skills/" 2>/dev/null || true
            log_success "Skills copied to ~/.fetih/skills/"
        fi
    fi
}
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# FETIH browser tools — use the system Chrome/Chromium binary."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        if [ -n "$DETECTED_BROWSER_EXECUTABLE" ]; then
            ...
            log_info "Skipping Playwright browser download; FETIH will use the system browser."
        else
            case "$DISTRO" in
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (fetih --tui may not work)"
        }
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'fetih setup' after install."
        return 0
    fi
    ...
    # Run fetih setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m fetih_cli.main setup < /dev/tty
    else
        python -m fetih_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$FETIH_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for FETIH to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$FETIH_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'fetih whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                FETIH_CMD="$(get_fetih_command_path)"
                $FETIH_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'fetih whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'fetih gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $FETIH_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $FETIH_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: fetih gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: fetih gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $FETIH_CMD gateway > "$FETIH_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.fetih/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: fetih gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: fetih gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $FETIH_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $FETIH_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $FETIH_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}fetih${NC}              Start chatting"
    echo -e "   ${GREEN}fetih setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}fetih config${NC}       View/edit configuration"
    echo -e "   ${GREEN}fetih config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}fetih gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}fetih update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'fetih' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'fetih' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'fetih' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$FETIH_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$FETIH_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$FETIH_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    local log_file
    log_file="$(mktemp)"
    if ! "$npm_bin" install -g --prefix "$FETIH_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$FETIH_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up FETIH for pip install"
    ...
    FETIH_CMD="$(command -v fetih 2>/dev/null || echo "")"
    if [ -n "$FETIH_CMD" ]; then
        log_info "Running fetih setup..."
        "$FETIH_CMD" setup
    else
        log_warn "fetih command not found on PATH"
        log_info "Try: python -m fetih_cli.main setup"
    fi
}
...
main() {
    ...
    echo "git" > "$FETIH_HOME/.install_method"
}
