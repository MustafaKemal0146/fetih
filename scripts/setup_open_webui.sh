...
# Bootstrap Open WebUI against FETIH's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.fetih/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-fetih.sh
...
#   FETIH_API_PORT=8642
#   FETIH_API_HOST=127.0.0.1
#   FETIH_API_MODEL_NAME='FETIH'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-FETIH WebUI}"
...
FETIH_ENV_FILE="${FETIH_ENV_FILE:-$HOME/.fetih/.env}"
FETIH_API_PORT="${FETIH_API_PORT:-8642}"
FETIH_API_HOST="${FETIH_API_HOST:-127.0.0.1}"
FETIH_API_CONNECT_HOST="${FETIH_API_CONNECT_HOST:-127.0.0.1}"
FETIH_API_MODEL_NAME="${FETIH_API_MODEL_NAME:-FETIH}"
FETIH_API_BASE_URL="http://${FETIH_API_CONNECT_HOST}:${FETIH_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-fetih.sh"
LOG_DIR="$HOME/.fetih/logs"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$FETIH_API_BASE_URL")"
...
ensure_env_permissions() {
  chmod 600 "$FETIH_ENV_FILE" 2>/dev/null || true
}

install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.fetih.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.fetih</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.fetih"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.fetih"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-fetih.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to FETIH
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-fetih.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.fetih/logs/openwebui.log
StandardError=append:%h/.fetih/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-fetih.service
}
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$FETIH_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring FETIH API server is configured...'
  upsert_env API_SERVER_ENABLED true "$FETIH_ENV_FILE"
  upsert_env API_SERVER_HOST "$FETIH_API_HOST" "$FETIH_ENV_FILE"
  upsert_env API_SERVER_PORT "$FETIH_API_PORT" "$FETIH_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$FETIH_API_MODEL_NAME" "$FETIH_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$FETIH_ENV_FILE"
  ensure_env_permissions

  log 'Restarting FETIH gateway so API server settings take effect...'
  fetih gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${FETIH_API_CONNECT_HOST}:${FETIH_API_PORT}/health" >/dev/null; then
    log 'FETIH API server did not answer on the first check. Trying to start gateway in the background...'
    nohup fetih gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  curl -fsS "http://${FETIH_API_CONNECT_HOST}:${FETIH_API_PORT}/health" >/dev/null
  ...
  log "FETIH API endpoint: ${FETIH_API_BASE_URL}"
