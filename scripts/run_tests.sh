#!/usr/bin/env bash
# Canonical test runner for fetih. Run this instead of calling
...
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/Desktop/fetih/venv"; do
...
# Unset FETIH_* behavioral vars too.
unset FETIH_YOLO_MODE FETIH_INTERACTIVE FETIH_QUIET FETIH_TOOL_PROGRESS \
      FETIH_TOOL_PROGRESS_MODE FETIH_MAX_ITERATIONS FETIH_SESSION_PLATFORM \
      FETIH_SESSION_CHAT_ID FETIH_SESSION_CHAT_NAME FETIH_SESSION_THREAD_ID \
      FETIH_SESSION_SOURCE FETIH_SESSION_KEY FETIH_GATEWAY_SESSION \
      FETIH_CRON_SESSION \
      FETIH_PLATFORM FETIH_INFERENCE_PROVIDER FETIH_MANAGED FETIH_DEV \
      FETIH_CONTAINER FETIH_EPHEMERAL_SYSTEM_PROMPT FETIH_TIMEZONE \
      FETIH_REDACT_SECRETS FETIH_BACKGROUND_NOTIFICATIONS FETIH_EXEC_ASK \
      FETIH_HOME_MODE 2>/dev/null || true
...
# If a system-wide fetih pytest_live_guard plugin is installed at
# $HOME/.fetih/pytest_live_guard.py, force-load it here so every test run
...
if [ -f "$HOME/.fetih/pytest_live_guard.py" ]; then
  case ":${PYTHONPATH:-}:" in
    *":$HOME/.fetih:"*) ;;
    *) export PYTHONPATH="${PYTHONPATH:+$PYTHONPATH:}$HOME/.fetih" ;;
  esac
