# ============================================================================
# FETIH Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [switch]$SkipSetup,
    [string]$Branch = "main",
    [string]$FetihHome = "$env:LOCALAPPDATA\fetih",
    [string]$InstallDir = "$env:LOCALAPPDATA\fetih\fetih-agent",
...
$RepoUrlSsh = "git@github.com:MustafaKemal0146/fetih.git"
$RepoUrlHttps = "https://github.com/MustafaKemal0146/fetih.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $FetihHome)) {
        New-Item -ItemType Directory -Force -Path $FetihHome | Out-Null
    }
    $envFile = Join-Path $FetihHome ".env"
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $FetihHome "node"
...
function Install-Git {
    ...
    which FETIH uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\fetih\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    FETIH needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the FETIH copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\fetih\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``FETIH_GIT_BASH_PATH`` (User scope) so FETIH can find it in a fresh
    ...
    # Download PortableGit into $FetihHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $FetihHome\git\ ..."
    ...
    try {
        ...
        $release = Invoke-RestMethod -Uri $releaseApi -UseBasicParsing -Headers @{ "User-Agent" = "fetih-installer" }

        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent FETIH features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  FETIH needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``FETIH_GIT_BASH_PATH`` (User env scope) so FETIH can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $FetihHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $FetihHome\git\usr\bin\bash.exe
    $candidates += "$FetihHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$FetihHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("FETIH_GIT_BASH_PATH", $candidate, "User")
            $env:FETIH_GIT_BASH_PATH = $candidate
            Write-Info "Set FETIH_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- FETIH may not find Git Bash."
    Write-Info "If needed, set FETIH_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    $managedNode = "$FetihHome\node\node.exe"
    if (Test-Path $managedNode) {
        $version = & $managedNode --version
        $env:Path = "$FetihHome\node;$env:Path"
        Write-Success "Node.js $version found (FETIH-managed)"
        $script:HasNode = $true
        return $true
    }
    ...
    # The portable zip path drops node.exe + npm into $FetihHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $FetihHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\fetih-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$FetihHome\node") { Remove-Item -Recurse -Force "$FetihHome\node" }
                Move-Item $extractedDir.FullName "$FetihHome\node"
                ...
                $version = & "$FetihHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $FetihHome\node\ (portable, user-scoped)"
                $script:HasNode = $true
...
function Install-Repository {
    ...
    if (Test-Path $InstallDir) {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                Write-Info "terminals, running fetih processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                $zipUrl = "https://github.com/MustafaKemal0146/fetih/archive/refs/heads/$Branch.zip"
                $zipPath = "$env:TEMP\fetih-agent-$Branch.zip"
                $extractPath = "$env:TEMP\fetih-agent-extract"
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `fetih.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install fetih-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and fetih.exe broken with
    ...
    # here, not 30 seconds later when the user runs `fetih`.
    if (-not $NoVenv) {
    ...
    # users hit and lazy-import errors from `fetih dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `fetih dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `fetih dashboard` should now work."
            } else {
...
function Set-PathVariable {
    Write-Info "Setting up fetih command..."

    if ($NoVenv) {
        $fetihBin = "$InstallDir"
    } else {
        $fetihBin = "$InstallDir\venv\Scripts"
    }

    # Add the venv Scripts dir to user PATH so fetih is globally available
    # On Windows, the fetih.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")

    if ($currentPath -notlike "*$fetihBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$fetihBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $fetihBin"
    } else {
        Write-Info "PATH already configured"
    }

    # Set FETIH_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\fetih instead
    # of the Unix default ~/.fetih
    $currentFetihHome = [Environment]::GetEnvironmentVariable("FETIH_HOME", "User")
    if (-not $currentFetihHome -or $currentFetihHome -ne $FetihHome) {
        [Environment]::SetEnvironmentVariable("FETIH_HOME", $FetihHome, "User")
        Write-Success "Set FETIH_HOME=$FetihHome"
    }
    $env:FETIH_HOME = $FetihHome

    # Update current session
    $env:Path = "$fetihBin;$env:Path"

    Write-Success "fetih command ready"
}

function Copy-ConfigTemplates {
    Write-Info "Setting up configuration files..."
    
    # Create ~/.fetih directory structure
    New-Item -ItemType Directory -Force -Path "$FetihHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$FetihHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$FetihHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$FetihHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$FetihHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$FetihHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$FetihHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$FetihHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$FetihHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$FetihHome\.env"
    if (-not (Test-Path $envPath)) {
        $examplePath = "$InstallDir\.env.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $envPath
            Write-Success "Created ~/.fetih/.env from template"
        } else {
            New-Item -ItemType File -Force -Path $envPath | Out-Null
            Write-Success "Created ~/.fetih/.env"
        }
    } else {
        Write-Info "~/.fetih/.env already exists, keeping it"
    }
    
    # Create config.yaml
    $configPath = "$FetihHome\config.yaml"
    if (-not (Test-Path $configPath)) {
        $examplePath = "$InstallDir\cli-config.yaml.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $configPath
            Write-Success "Created ~/.fetih/config.yaml from template"
        }
    } else {
        Write-Info "~/.fetih/config.yaml already exists, keeping it"
    }
    ...
    # (the default PS5 behaviour), and FETIH's prompt-injection scanner
...
"@
        ...
        Write-Success "Created ~/.fetih/SOUL.md (edit to customize personality)"
    }
    
    Write-Success "Configuration directory ready: ~/.fetih/"

    # Seed bundled skills into ~/.fetih/skills/ (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to ~/.fetih/skills/ ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to ~/.fetih/skills/"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$FetihHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to ~/.fetih/skills/"
            }
        }
    }
}
...
function Install-NodeDeps {
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'fetih setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\fetih-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\fetih-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\fetih-npm-tui-$(Get-Random).log"
...
function Install-PlatformSdks {
    ...
    # ~/.fetih/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'fetih setup'."
        return
    }
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m fetih_cli.main setup
    } else {
        python -m fetih_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $fetihCmd = "$InstallDir\venv\Scripts\fetih.exe"
    if (-not (Test-Path $fetihCmd)) {
        $fetihCmd = "fetih"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$FetihHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'fetih whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: fetih gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$FetihHome\logs\gateway.log"
            Start-Process -FilePath $fetihCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$FetihHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: fetih gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: fetih gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$FetihHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$FetihHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$FetihHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$FetihHome\fetih-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   fetih              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   fetih setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   fetih config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   fetih config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   fetih gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   fetih update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning FETIH repository";             Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
    ...
    @{ Name = "path";             Title = "Adding FETIH to PATH";                 Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/MustafaKemal0146/fetih/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
