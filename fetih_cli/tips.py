...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or fetih -c.",
    ...
    "Ctrl+Z suspends FETIH to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "fetih -c resumes your most recent CLI session. fetih -c \"project name\" resumes by title.",
    "fetih -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "fetih -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "fetih chat -t web,terminal enables only specific toolsets for a focused session.",
    "fetih chat -s github-pr-workflow preloads a skill at launch.",
    "fetih chat -q \"query\" runs a single non-interactive query and exits.",
    "fetih chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "fetih chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "fetih --yolo bypasses all dangerous command approval prompts for the entire session.",
    "fetih chat --source telegram tags the session for filtering in fetih sessions list.",
    "fetih -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "fetih doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "fetih dump outputs a compact setup summary — great for bug reports.",
    "fetih config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "fetih config edit opens config.yaml in your default editor.",
    "fetih config check scans for missing or stale configuration options.",
    "fetih sessions browse opens an interactive session picker with search.",
    "fetih sessions stats shows session counts by platform and database size.",
    "fetih sessions prune --older-than 30 cleans up old sessions.",
    "fetih skills search react --source skills-sh searches the skills.sh public directory.",
    "fetih skills check scans installed hub skills for upstream updates.",
    "fetih skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "fetih skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "fetih mcp add github --command npx adds MCP servers from the command line.",
    "fetih mcp serve runs FETIH itself as an MCP server for other agents.",
    "fetih auth add lets you add multiple API keys for credential pool rotation.",
    "fetih completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "fetih logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "fetih backup creates a zip backup of your entire FETIH home directory.",
    "fetih profile create coder creates an isolated profile that becomes its own command.",
    "fetih profile create work --clone copies your current config and keys to a new profile.",
    "fetih update syncs new bundled skills to ALL profiles automatically.",
    "fetih gateway install sets up FETIH as a system service (systemd/launchd).",
    "fetih memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "fetih webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: fetih tools disables unused tools, fetih skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "fetih models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as fetih -w).",
    ...
    "FETIH_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call FETIH tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'fetih profile create coder' creates the 'coder' command.",
    "fetih profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, FETIH prints a resume command with session ID and stats.",
    "fetih sessions export backup.jsonl exports all sessions for backup or analysis.",
    "fetih -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "fetih skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .fetih/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: fetih cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "FETIH runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "fetih gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to FETIH via a local macOS server.",
    ...
    "Context files (.fetih.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "SOUL.md at ~/.fetih/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    "FETIH loads project context from .fetih.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "MCP servers auto-generate toolsets at runtime — fetih tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.fetih/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "fetih chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "fetih plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "fetih acp runs FETIH as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "FETIH_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "fetih login supports OAuth-based auth for Nous and OpenAI Codex providers.",
    ...
    "fetih status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.fetih/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make FETIH your own.",
    "SOUL.md is auto-seeded with a default personality on first run. Edit ~/.fetih/SOUL.md to customize.",
    ...
    "Before a gateway session reset, FETIH auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.fetih/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at FETIH_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "FETIH_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in FETIH_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.fetih/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'fetih profile create ls' would be rejected.",
    "fetih profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "fetih claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "FETIH_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via fetih tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.fetih/.env.",
    "Skills from trusted repos (FETIH) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.fetih/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — FETIH auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of FETIH config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while FETIH is working.',
    ...
    '/reload reloads ~/.fetih/.env into the running session — pick up new API keys without restarting.',
    ...
    "FETIH_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.fetih/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.fetih/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'fetih curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "fetih curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'fetih curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'fetih auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    # --- TUI & Dashboard ---
    'FETIH_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "FETIH_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'FETIH_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'FETIH_WRITE_SAFE_ROOT restricts write_file and patch to a directory prefix; writes outside require approval.',
    'FETIH_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'FETIH_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.fetih/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.fetih/skills/ so drop-in skills appear without restarting the session.',
    ...
    'fetih -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'fetih chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'fetih chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'fetih chat --ignore-user-config skips ~/.fetih/config.yaml — reproducible bug reports and CI runs.',
    "fetih chat --source tool tags programmatic chats so they don't clutter fetih sessions list.",
    'fetih dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'fetih sessions rename <ID> "new title" renames any past session; fetih sessions delete <ID> removes one.',
    'fetih import restores a session export or profile archive produced by sessions export or profile export.',
    'fetih fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'fetih pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'fetih setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'fetih status --deep runs the full health sweep across every component; plain fetih status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'FETIH_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'FETIH_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.fetih/plugins/ — trust-gated by design.',
    "FETIH_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'FETIH_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'FETIH_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'FETIH_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'FETIH_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'FETIH_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'FETIH_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'FETIH_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'FETIH_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'FETIH_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'FETIH_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes FETIH block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from fetih sessions list by default — set --source explicitly to see them.',
