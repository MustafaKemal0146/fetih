#!/usr/bin/env python3
"""
FETIH CLI - Main entry point.

Usage:
    fetih                     # Interactive chat (default)
    fetih chat                # Interactive chat
    fetih gateway             # Run gateway in foreground
    fetih gateway start       # Start gateway as service
    fetih gateway stop        # Stop gateway service
    fetih gateway status      # Show gateway status
    fetih gateway install     # Install gateway service
    fetih gateway uninstall   # Uninstall gateway service
    fetih setup               # Interactive setup wizard
    fetih logout              # Clear stored authentication
    fetih status              # Show status of all components
    fetih cron                # Manage cron jobs
    fetih cron list           # List cron jobs
    fetih cron status         # Check if cron scheduler is running
    fetih doctor              # Check configuration and dependencies
    fetih honcho setup                    # Configure Honcho AI memory integration
    fetih honcho status                   # Show Honcho config and connection status
    fetih honcho sessions                 # List directory → session name mappings
    fetih honcho map <name>               # Map current directory to a session name
    fetih honcho peer                     # Show peer names and dialectic settings
    fetih honcho peer --user NAME         # Set user peer name
    fetih honcho peer --ai NAME           # Set AI peer name
    fetih honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    fetih honcho mode                     # Show current memory mode
    fetih honcho mode [hybrid|honcho|local]  # Set memory mode
    fetih honcho tokens                   # Show token budget settings
    fetih honcho tokens --context N       # Set session.context() token cap
    fetih honcho tokens --dialectic N     # Set dialectic result char cap
    fetih honcho identity                 # Show AI peer identity representation
    fetih honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    fetih honcho migrate                  # Step-by-step migration guide: OpenClaw native → FETIH + Honcho
    fetih version             Show version
...
# IMPORTANT: fetih_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``fetih_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``fetih update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``fetih_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, fetih crashes on import and the user can't run
# ``fetih update`` to recover.  Missing the bootstrap means UTF-8 stdio
...
def _add_accept_hooks_flag(parser) -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(equivalent to FETIH_ACCEPT_HOOKS=1 / hooks_auto_accept: true)."
        ),
    )
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (fetih tools, fetih setup, fetih model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'fetih {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any fetih module import.
#
# Many modules cache FETIH_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("FETIH_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.fetih/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set FETIH_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # Mirrors fetih_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If FETIH_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.fetih/profiles/coder or
    # /opt/data/profiles/coder).  If FETIH_HOME points to the fetih root
    # instead (e.g. systemd hardcodes FETIH_HOME=/root/.fetih), we must
    # still read active_profile — the user may have switched profiles via
    # `fetih profile use` and the gateway should honour that choice.
    # See issue #22502.
    fetih_home_env = os.environ.get("FETIH_HOME", "")
    if profile_name is None and fetih_home_env:
        if Path(fetih_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the fetih root
    if profile_name is None:
        try:
            from fetih_constants import get_default_fetih_root

            active_path = get_default_fetih_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set FETIH_HOME
    if profile_name is not None:
        try:
            from fetih_cli.profiles import resolve_profile_env

            fetih_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent fetih from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["FETIH_HOME"] = fetih_home
...
# Load .env from ~/.fetih/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from fetih_cli.config import get_fetih_home
from fetih_cli.env_loader import load_fetih_dotenv

load_fetih_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → FETIH_REDACT_SECRETS env
# var BEFORE fetih_logging imports agent.redact (which snapshots the flag at
# module-import time). Without this, config.yaml's toggle is ignored because
# the setup_logging() call below imports agent.redact, which reads the env var
# exactly once. Env var in .env still wins — this is config.yaml fallback only.
try:
    if "FETIH_REDACT_SECRETS" not in os.environ:
        import yaml as _yaml_early

        _cfg_path = get_fetih_home() / "config.yaml"
        if _cfg_path.exists():
            ...
            if isinstance(_early_sec_cfg, dict):
                ...
                if _early_redact is not None:
                    os.environ["FETIH_REDACT_SECRETS"] = str(_early_redact).lower()
            del _early_sec_cfg
        del _cfg_path
except Exception:
    pass  # best-effort — redaction stays at default (enabled) on config errors

# Initialize centralized file logging early — all `fetih` subcommands
# (chat, setup, gateway, config, etc.) write to agent.log + errors.log.
try:
    from fetih_logging import setup_logging as _setup_logging
...
# Also initialize the skin from config so banner colors are correct.
try:
    from fetih_cli.config import load_config as _load_config_early
    from fetih_constants import apply_ipv4_preference as _apply_ipv4
    ...
    try:
        from fetih_cli.skin_engine import init_skin_from_config as _init_skin
        _init_skin(_early_cfg)
    except Exception:
        pass
    del _early_cfg, _net
except Exception:
...
from fetih_cli import __version__, __release_date__
from fetih_constants import AI_GATEWAY_BASE_URL, OPENROUTER_BASE_URL
...
def _has_any_provider_configured() -> bool:
    ...
    from fetih_cli.config import get_env_path, get_fetih_home, load_config
    from fetih_cli.auth import get_auth_status

    # Determine whether FETIH itself has been explicitly configured (model
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_fetih_config = _model_name and _model_name != _DEFAULT_MODEL
    ...
    # Check for FETIH Portal OAuth credentials
    auth_file = get_fetih_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if FETIH has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants FETIH to use their tokens.
    if _has_fetih_config:
        try:
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, fetih_bin
        cli_args: the original CLI arguments (everything after 'fetih')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    fetih_bin = container_info["fetih_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo fetih {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo fetih {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @fetih/ink is missing or node_modules is behind package-lock.json.
    ...
    ink = root / "node_modules" / "@fetih" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
def _ensure_tui_node() -> None:
    ...
    ``FETIH_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("FETIH_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    fetih_home = os.environ.get("FETIH_HOME") or str(Path.home() / ".fetih")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "FETIH_HOME": fetih_home},
            capture_output=True,
            text=True,
            check=False,
        )
    ...
    extras.extend([Path(fetih_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(fetih_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if fetih_cli_dir is None:
        fetih_cli_dir = Path(__file__).parent
    bundled = fetih_cli_dir / "tui_dist" / "entry.js"
    return bundled if bundled.is_file() else None


def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (FETIH_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("FETIH_NODE")
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        path = shutil.which(bin)
        if not path and bin == "node":
            try:
                from fetih_cli.dep_ensure import ensure_dependency
    ...
    ext_dir = os.environ.get("FETIH_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with FETIH_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset FETIH_TUI_DIR (e.g. `unset FETIH_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if _tui_need_npm_install(tui_dir):
        npm = _node_bin("npm")
        if not os.environ.get("FETIH_QUIET"):
    ...
    if tui_dev:
        # Keep the local @fetih/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @fetih/ink resolves through
        # packages/fetih-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "fetih-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from fetih_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="fetih-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["FETIH_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["FETIH_PYTHON_SRC_ROOT"] = os.environ.get(
        "FETIH_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("FETIH_PYTHON", sys.executable)
    env.setdefault("FETIH_CWD", os.getcwd())
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["FETIH_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    if model:
        env["FETIH_MODEL"] = model
        env["FETIH_INFERENCE_MODEL"] = model
    if provider:
        env["FETIH_TUI_PROVIDER"] = provider
        env["FETIH_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["FETIH_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["FETIH_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["FETIH_TUI_SKILLS"] = value
    if query:
        env["FETIH_TUI_QUERY"] = query
    if image:
        env["FETIH_TUI_IMAGE"] = image
    if checkpoints:
        env["FETIH_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["FETIH_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["FETIH_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["FETIH_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["FETIH_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["FETIH_ACCEPT_HOOKS"] = "1"
    ...
    if resume_session_id:
        env["FETIH_TUI_RESUME"] = resume_session_id
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``FETIH_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``fetih kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``fetih kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("FETIH_KANBAN_BOARD"):
        return
    try:
        from fetih_cli.kanban_db import get_current_board

        os.environ["FETIH_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass
...
def cmd_chat(args):
    ...
    use_tui = getattr(args, "tui", False) or os.environ.get("FETIH_TUI") == "1"
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'fetih sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like FETIH isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  fetih setup")
        print()

        from fetih_cli.setup import (
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'fetih setup' at any time to configure.")
        sys.exit(1)

    # Start update check in background (runs while other init happens)
    try:
        from fetih_cli.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        os.environ["FETIH_YOLO_MODE"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.fetih/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["FETIH_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["FETIH_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["FETIH_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from fetih_cli.gateway import gateway_command
...
def cmd_whatsapp(args):
    ...
    from fetih_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with FETIH?")
        print()
    ...
    # has no creds.json.  Every subsequent `fetih gateway` then paid a 30s
    ...
    session_dir = get_fetih_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: fetih gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `fetih gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  fetih gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚔ FETIH'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  fetih gateway")
        ...
        print()
        print("  Or install as a service: fetih gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'fetih whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    from fetih_cli.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    from fetih_cli.config import stamp_install_method
    from fetih_cli.dep_ensure import ensure_dependency

    stamp_install_method("pip")

    print("⚕ FETIH post-install bootstrap")
    print()
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``fetih model``) and the setup wizard
    ...
    from fetih_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("FETIH_INFERENCE_PROVIDER") or "auto"
    )
    ...
    if effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'fetih model' for "
    ...
    from fetih_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from fetih_cli.config import read_raw_config
    ...
    # a leftover OPENAI_BASE_URL in ~/.fetih/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.fetih/.env if the active provider is not 'custom'.
    ...
    """
    from fetih_cli.config import get_env_value, save_env_value, load_config
...
#
# FETIH uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `fetih model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `fetih model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  FETIH only falls back to a lightweight backend (OpenRouter,")
        print("  FETIH Portal) if the main model is unavailable.  Override a")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``fetih model`` flow, then route aux tasks to them here.
    """
    from fetih_cli.config import load_config
    from fetih_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from fetih_cli.auth import _prompt_model_selection
    from fetih_cli.models import get_pricing_for_provider
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from fetih_cli.config import get_env_value

    # Route through _prompt_api_key so users can replace a stale/broken key
    # in-flow (K/R/C) instead of having to edit ~/.fetih/.env by hand. The
    # previous bypass-when-key-exists branch left no way to recover from a
    # bad paste short of re-running `fetih setup` from scratch. OpenRouter
    ...
    from fetih_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    # Route through _prompt_api_key so users can replace a stale/broken key
    # in-flow (K/R/C) instead of having to edit ~/.fetih/.env by hand.
    ...
    from fetih_cli.models import ai_gateway_model_ids, get_pricing_for_provider
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from fetih_cli.codex_models import get_codex_model_ids
    ...
    _codex_token = None
    # Prefer credential pool (where `fetih auth` stores device_code tokens),
    ...
    if not _codex_token:
        try:
            from fetih_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_xai_oauth(_config, current_model=""):
    ...
    from fetih_cli.models import _PROVIDER_MODELS
    ...
    # live only in the pool (e.g. after ``fetih auth add xai-oauth``).  Fall
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from fetih_cli.models import fetch_api_models
...
def _model_flow_custom(config):
    ...
    """
    from fetih_cli.auth import _save_model_choice, deactivate_provider
    from fetih_cli.config import get_env_value, load_config, save_config
    ...
    from fetih_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `fetih model` to set a model.")
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from fetih_cli.runtime_provider import _detect_api_mode_for_url
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use FETIH URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from fetih_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from fetih_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from fetih_cli import azure_detect
    import getpass
    ...
    print("Anthropic-style API endpoints.  FETIH will probe your")
    ...
    if use_entra:
        ...
        if not has_azure_identity_installed():
            ...
            print(
                "  FETIH will install it now (the preflight below "
...
def _remove_custom_provider(config):
    ...
    try:
        ...
        idx = menu.show()
        from fetih_cli.curses_ui import flush_stdin
...
def _model_flow_copilot(config, current_model=""):
    ...
    from fetih_cli.config import save_env_value, load_config, save_config
    from fetih_cli.models import (
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from fetih_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from fetih_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates FETIH turns to `copilot --acp`.")
    print("  FETIH currently starts its own ACP subprocess for each request.")
    print("  FETIH uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set FETIH_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``fetih setup`` / ``fetih model``.
    ...
    recover from a malformed paste without editing ``~/.fetih/.env`` by hand.
    ...
    from fetih_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from fetih_cli.config import save_env_value
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `fetih setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so fetih knows where to find it
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   FETIH typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with FETIH, enable billing on your "
    ...
    if provider_id == "lmstudio":
        from fetih_cli.auth import AuthError
        from fetih_cli.models import fetch_lmstudio_models
        ...
        from fetih_cli.models import fetch_ollama_cloud_models
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from fetih_constants import display_fetih_home as _dhh_fn

            print(
                f"    FETIH will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from fetih_cli.auth import get_anthropic_key
    ...
    if selected:
        ...
        # providers if the user switches without running 'fetih model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate FETIH CLI with a provider."""
    from fetih_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from fetih_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from fetih_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from fetih_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from fetih_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from fetih_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``fetih slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: fetih slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `fetih slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from fetih_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from fetih_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from fetih_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from fetih_cli.doctor import run_doctor
...
def cmd_dump(args):
    ...
    from fetih_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from fetih_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from fetih_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up FETIH home directory to a zip file."""
    if getattr(args, "quick", False):
        from fetih_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from fetih_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a FETIH backup from a zip file."""
    from fetih_cli.backup import run_import
...
def cmd_version(args):
    """Show version."""
    print(f"FETIH v{__version__} ({__release_date__})")
    ...
    try:
        from fetih_cli.banner import check_for_updates
        from fetih_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    ...
    from fetih_cli.uninstall import run_uninstall
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``fetih update --gateway`` so interactive prompts (stash restore,
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `fetih update`.
    ...
    print("  Preview now:  fetih curator run --dry-run")
    print("  Pause it:     fetih curator pause")
    print(
        "  Docs:         https://github.com/MustafaKemal0146/fetih/docs/user-guide/features/curator"
    )
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``fetih update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``fetih update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: fetih curator status)"
    )
...
def _update_via_zip(args):
    """Update FETIH Agent by downloading a ZIP archive.
    ...
    branch = "main"
    zip_url = (
        f"https://github.com/MustafaKemal0146/fetih/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    try:
        tmp_dir = tempfile.mkdtemp(prefix="fetih-update-")
        zip_path = os.path.join(tmp_dir, f"fetih-agent-{branch}.zip")
        ...
        # GitHub ZIPs extract to fetih-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"fetih-agent-{branch}")
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "fetih-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if FETIH behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes fetih completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but FETIH couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but FETIH couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if FETIH behaves unexpectedly.")
    return True


# =========================================================================
# Fork detection and upstream management for `fetih update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/MustafaKemal0146/fetih.git",
    "git@github.com:MustafaKemal0146/fetih.git",
    "https://github.com/MustafaKemal0146/fetih",
    "git@github.com:MustafaKemal0146/fetih",
}
OFFICIAL_REPO_URL = "https://github.com/MustafaKemal0146/fetih.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_fetih_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_fetih_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official FETIH repository.")
        print("  This means you may miss updates from MustafaKemal0146/fetih.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/MustafaKemal0146/fetih.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/MustafaKemal0146/fetih.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``fetih update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from fetih_constants import get_default_fetih_root

    default_home = get_default_fetih_root()
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``fetih update`` is still progressing even if pip/uv itself is silent.
    """
...
def _fetih_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    if not _is_windows():
        return []
    return [
        scripts_dir / "fetih.exe",
        scripts_dir / "fetih-gateway.exe",
    ]


def _quarantine_running_fetih_exe(scripts_dir: Path) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file lock on the running ``fetih.exe``.
    ...
    when ``fetih update`` runs, ``fetih.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``fetih.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next fetih invocation by ``_cleanup_quarantined_exes``.
    ...
    for shim in _fetih_exe_shims(scripts_dir):
        if not shim.exists():
            continue
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_fetih_exe`` if uv didn't write replacements."""
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``fetih.exe.old.*`` left by prior updates.

    Called early on every fetih invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``fetih update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `fetih update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``fetih.exe`` / ``fetih-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_fetih_exe`` for the rationale.
    """
    ...
    def _install(args: list[str]) -> None:
        ...
        if scripts_dir is not None:
            moved = _quarantine_running_fetih_exe(scripts_dir)
        try:
...
def _update_node_dependencies() -> None:
    ...
    for label, path in paths:
        ...
        # streamed output to ``~/.fetih/logs/update.log`` so nothing is lost.
...
class _UpdateOutputStream:
    """Stream wrapper used during ``fetih update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.fetih/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``fetih update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``fetih update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.fetih/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``fetih update --gateway``) the update is already
    ...
    try:
        ...
        # fetih_cli.config.get_fetih_home to simulate setup failure.
        from fetih_cli.config import get_fetih_home as _get_fetih_home

        logs_dir = _get_fetih_home() / "logs"
...
def _cmd_update_check():
    """Implement ``fetih update --check``: fetch and report without installing."""
    from fetih_cli.config import detect_install_method
    method = detect_install_method(PROJECT_ROOT)
    if method == "pip":
        from fetih_cli.config import recommended_update_command
        from fetih_cli.banner import check_via_pypi
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``fetih update`` without requiring a reinstall.  The
    ...
    ``fetih`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/fetih.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v fetih'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/fetih, code at /usr/local/lib/fetih-agent).
    fhs_link = Path("/usr/local/bin/fetih")
    ...
    path_comment = (
        "# FETIH Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of FETIH_HOME before running the update.
    ...
    FETIH_HOME directories.  The ``--backup`` flag on ``fetih update``
    ...
    try:
        from fetih_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_fetih_home so the user sees ~/.fetih/...
    try:
        from fetih_constants import get_fetih_home, display_fetih_home

        home = get_fetih_home()
        try:
            display_path = f"{display_fetih_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  fetih import {out_path}")
...
def cmd_update(args):
    """Update FETIH Agent to the latest version.

    Thin wrapper around ``_cmd_update_impl``: installs hangup protection,
    runs the update, then restores stdio on the way out (even on
    ``sys.exit`` or unhandled exceptions).
    """
    from fetih_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update FETIH Agent")
        return
...
def _cmd_update_pip(args):
    """Update FETIH via pip (for PyPI installs)."""
    ...
    uv = shutil.which("uv")
    if uv:
        cmd = [uv, "pip", "install", "--upgrade", "fetih-agent"]
    else:
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "fetih-agent"]
    ...
    print("✓ Update complete! Restart fetih to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        # the old bytecode (e.g. get_fetih_home added to fetih_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # modules in this process.  Reload fetih_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_fetih_home() added since the last release.
        try:
            import importlib
            import fetih_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit FETIH_HOME so
        # it is not affected by sync_skills()'s module-level FETIH_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's FETIH_HOME env var points at the default or a named profile.
        try:
            from fetih_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        if needs_migration:
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                # (gateway / non-interactive contexts) cannot prompt for API keys.
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: fetih config migrate")
            else:
                print()
                print("Skipped. Run 'fetih config migrate' later to configure.")
        else:
        ...
        # check `fetih curator status`. Self-stamps after printing so it
        ...
        # Tying the refresh to ``fetih update`` gives users a predictable
        ...
        try:
            if sys.platform == "darwin" and shutil.which("cua-driver"):
                from fetih_cli.tools_config import install_cua_driver
        ...
        # When running as ``fetih update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_fetih_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            # Discover all fetih-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. fetih-gateway.service or fetih-gateway-coder.service
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: fetih gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: fetih -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy FETIH gateway unit files are still installed.
        # When both fetih.service (from a pre-rename install) and the
        # current fetih-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `fetih update` surfaces the issue until the user migrates.
        try:
            from fetih_cli.gateway import (
                has_legacy_fetih_units,
                _find_legacy_fetih_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_fetih_units():
                print()
                print("⚠ Legacy FETIH gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_fetih_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (fetih.service) fight the current")
                print("  fetih-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    fetih gateway migrate-legacy")
                print()
        ...
        print("  fetih model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``fetih -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from fetih_constants import display_fetih_home

    action = getattr(args, "profile_action", None)

    if action is None:
        # Bare `fetih profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_fetih_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → fetih -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.fetih)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  fetih profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     fetih -p {name} chat")
                else:
        ...
        from fetih_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from fetih_constants import get_fetih_home as _hh
        ...
        from fetih_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `fetih profile info {name}` for full manifest)")
        if wrapper.exists():
        ...
        from fetih_cli.profiles import profile_exists
        ...
        from fetih_cli.profiles import rename_profile
        ...
        from fetih_cli.profiles import export_profile
        ...
        from fetih_cli.profiles import import_profile
        ...
        import tempfile
        from fetih_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="fetih_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  fetih -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      fetih -p {plan.manifest.name} chat")
        ...
        from fetih_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `fetih profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"fetih -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from fetih_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("fetih_requires"):
            print(f"Requires:     FETIH {data['fetih_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from fetih_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.fetih_requires:
        print(f"  Requires: FETIH {mf.fetih_requires}")
...
def cmd_completion(args, parser=None):
    ...
    from fetih_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_logs(args):
    """View and filter FETIH log files."""
    from fetih_cli.logs import tail_log, list_logs
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from fetih_cli.models import CANONICAL_PROVIDERS as _cp
...
# so that in ``fetih -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``fetih_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``fetih -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``fetih`` or only flags → defaults to ``chat``.
        return False
...
def main():
    """Main entry point for fetih CLI."""
    # Force UTF-8 stdio on Windows before anything prints.  No-op elsewhere.
    try:
        from fetih_cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``fetih.exe.old.*`` quarantine files left by previous
    # ``fetih update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_fetih_exe``.
    try:
        _cleanup_quarantined_exes()
    except Exception:
        pass

    from fetih_cli._parser import build_top_level_parser
    ...
    # =========================================================================
    from fetih_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://github.com/MustafaKemal0146/fetih/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `fetih model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy fetih.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy FETIH gateway unit files "
            "(e.g. fetih.service) left over from older installs. Profile "
            "units (fetih-gateway-<profile>.service) and unrelated "
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure FETIH Agent with an interactive wizard. "
        "Run a specific section: fetih setup model|tts|terminal|gateway|tools|agent",
    )
    ...
    setup_parser.add_argument(
        ...
        "compatibility — a bare 'fetih setup' now does this.",
    )
    setup_parser.add_argument(
        "--quick",
        action="store_true",
    ...
    slack_parser = subparsers.add_parser(
        ...
        description="Slack integration helpers for FETIH.",
    )
    ...
    slack_manifest.add_argument(
        ...
        "writes to $FETIH_HOME/slack-manifest.json.",
    )
    slack_manifest.add_argument(
        "--name",
        default=None,
        help='Bot display name (default: "FETIH")',
    )
    slack_manifest.add_argument(
        "--description",
        default=None,
    ...
    # =========================================================================
    from fetih_cli.send_cmd import register_send_subparser
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for FETIH CLI",
    )
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: fetih-cli)"
    )
    ...
    auth_spotify = auth_subparsers.add_parser(
        "spotify", help="Authenticate FETIH with Spotify via PKCE"
    )
    ...
    auth_spotify.add_argument(
        "--client-id", help="Spotify app client_id (or set FETIH_SPOTIFY_CLIENT_ID)"
    )
    auth_spotify.add_argument(
        "--redirect-uri",
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of FETIH Agent components",
    )
    ...
    cron_create.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.fetih/scripts/. Default mode: "
    ...
    cron_create.add_argument(
        "--profile",
        help="FETIH profile name to run the job under. Use 'default' for the root profile. Named profiles must already exist. Omit to preserve the scheduler's existing profile.",
    )
    ...
    cron_edit.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.fetih/scripts/. Pass empty string to clear. "
    ...
    cron_edit.add_argument(
        "--profile",
        help="FETIH profile name to run the job under. Use 'default' for the root profile. Pass empty string to clear.",
    )
    ...
    # =========================================================================
    from fetih_cli.kanban import build_parser as _build_kanban_parser
    ...
    hooks_parser = subparsers.add_parser(
        ...
        description=(
            "Inspect shell-script hooks declared in ~/.fetih/config.yaml, "
            "test them against synthetic payloads, and manage the first-use "
            "consent allowlist at ~/.fetih/shell-hooks-allowlist.json."
        ),
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with FETIH Agent setup",
    )
    ...
    doctor_parser.add_argument(
        ...
        help=(
            ...
            "advisory will no longer trigger startup banners. Run `fetih "
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your FETIH setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for FETIH Agent. Use 'fetih debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    fetih debug share              Upload debug report and print URL
    fetih debug share --lines 500  Include more log lines
    fetih debug share --expire 30  Keep paste for 30 days
    fetih debug share --local      Print report locally (no upload)
    fetih debug share --no-redact  Disable upload-time secret redaction
    fetih debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'fetih debug share'",
    )
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="FETIH yapılandırmasını zip dosyasına yedekle",
        description="Create a zip archive of your entire FETIH configuration, "
        "skills, sessions, and data (excludes the fetih-agent codebase). "
        "Use --quick for a fast snapshot of just critical state files.",
    )
    backup_parser.add_argument(
        "-o",
        "--output",
        help="Output path for the zip file (default: ~/fetih-backup-<timestamp>.zip)",
    )
    backup_parser.add_argument(
        "-q",
        "--quick",
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.fetih/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo fetih uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from fetih_cli.checkpoints import register_cli as _register_checkpoints_cli
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="FETIH yedeğini zip dosyasından geri yükle",
        description="Extract a previously created FETIH backup into your "
        "FETIH home directory, restoring configuration, skills, "
        "sessions, and data",
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage FETIH Agent configuration",
    )
    ...
    def cmd_pairing(args):
        from fetih_cli.pairing import pairing_command
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.fetih/skills/.bundled_manifest) "
            "so future 'fetih update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from fetih_cli.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from fetih_cli.skills_hub import skills_command
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/fetih-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
        "--force",
        "-f",
    ...
    _install_enable_group.add_argument(
        "--no-enable",
        action="store_true",
        help="Install disabled (skip confirmation prompt); enable later with `fetih plugins enable <name>`",
    )
    ...
    def cmd_plugins(args):
        from fetih_cli.plugins_cmd import plugins_command
    ...
    # subcommand — ``fetih --help``, ``fetih version``, ``fetih logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from fetih_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from fetih_cli.curator import register_cli as _register_curator_cli
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            mem_dir = get_fetih_home() / "memories"
            ...
            if not existing:
                print(
                    f"\n  Nothing to reset — no memory files found in {display_fetih_home()}/memories/\n"
                )
                return
            ...
            print(f"  Files were in: {display_fetih_home()}/memories/\n")
        else:
            from fetih_cli.memory_setup import memory_command
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'fetih tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # fetih tools disable <name...> [--platform cli]
    ...
    # fetih tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in {"list", "disable", "enable"}:
            from fetih_cli.tools_config import tools_disable_enable_command

            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from fetih_cli.tools_config import tools_command
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use `fetih computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `fetih tools` runs when you first\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            ...
            if path:
                ...
                print("  Refresh to latest: fetih computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: fetih computer-use install")
            return
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="MCP sunucularını yönet ve FETIH'i MCP sunucusu olarak çalıştır",
        description=(
            "Manage MCP server connections and run FETIH as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'fetih mcp add' to connect to a new server, or\n"
            "'fetih mcp serve' to expose FETIH conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="FETIH'i MCP sunucusu olarak çalıştır (konuşmaları ajanlara expose et)",
    )
    ...
    # flag is omitted, causing `fetih mcp add ...` to fall through to
    # interactive chat.
    ...
    def cmd_mcp(args):
        from fetih_cli.mcp_config import mcp_command
        ...
        if action == "list":
            ...
            if not args.yes:
                if not _confirm_prompt(
                    ...
                    print("Cancelled.")
                    return
            sessions_dir = get_fetih_home() / "sessions"
            ...
            # Launch fetih --resume <id> by replacing the current process
            print(f"Resuming session: {selected_id}")
            from fetih_cli.relaunch import relaunch
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to FETIH",
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to FETIH",
    ...
    claw_migrate.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip the pre-migration zip snapshot of ~/.fetih/ (by default a "
        "single restore-point archive is written to ~/.fetih/backups/ "
        "before apply; restorable with 'fetih import').",
    )
    claw_migrate.add_argument(
    ...
    def cmd_claw(args):
        from fetih_cli.claw import claw_command
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="FETIH'i en son sürüme güncelle",
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="FETIH'i kaldır",
        description="FETIH'i sistemden kaldır. Yeniden kurulum için config/data saklanabilir.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="FETIH'i ACP (Agent Client Protocol) sunucusu olarak çalıştır",
        description="FETIH'i ACP modunda başlat — editör entegrasyonu için (VS Code, Zed, JetBrains)",
    )
    ...
    acp_parser.add_argument(
        ...
        action="store_true",
        dest="acp_version",
        help="Print FETIH ACP version and exit",
    )
    acp_parser.add_argument(
        "--check",
        action="store_true",
    ...
    acp_parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive FETIH provider/model setup for ACP terminal auth",
    )
    acp_parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.fetih/node/ "
    ...
    def cmd_acp(args):
        """Launch FETIH Agent as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Profilleri yönet — birden fazla izole FETIH örneği",
    )
    ...
    profile_create.add_argument(
        "--no-skills",
        action="store_true",
        help="Create an empty profile with no bundled skills (opts out of `fetih update` skill sync)",
    )
    profile_create.add_argument(
        "--description",
        default=None,
        help="One- or two-sentence description of what this profile is good at. "
             ...
             "of profile name alone. Skip and add later via `fetih profile describe`.",
    )
    ...
    profile_install = profile_subparsers.add_parser(
        ...
        description=(
            "Install a FETIH profile distribution. SOURCE can be a git URL "
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter FETIH log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    fetih logs                    Show last 50 lines of agent.log
    fetih logs -f                 Follow agent.log in real time
    fetih logs errors             Show last 50 lines of errors.log
    fetih logs gateway -n 100     Show last 100 lines of gateway.log
    fetih logs --level WARNING    Only show WARNING and above
    fetih logs --session abc123   Filter by session ID
    fetih logs --component tools  Only show tool-related lines
    fetih logs --since 1h         Lines from the last hour
    fetih logs --since 30m -f     Follow, starting from 30 min ago
    fetih logs list               List available log files with sizes
""",
    )
    ...
    # e.g. ``fetih -c Pokemon Agent Dev`` → ``fetih -c 'Pokemon Agent Dev'``
    ...
    from fetih_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'fetih -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (fetih hooks list, cron
    ...
    if args.command in _AGENT_COMMANDS or (
        ...
        try:
            from fetih_cli.plugins import discover_plugins
    ...
    if getattr(args, "oneshot", None):
        from fetih_cli.oneshot import run_oneshot
