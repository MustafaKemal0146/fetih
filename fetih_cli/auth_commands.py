...
def auth_add_command(args) -> None:
    ...
    if provider == "nous":
        ...
        # <fetih-root>/shared/nous_auth.json (written by any previous
        # successful login), offer to import it instead of running the
        # full device-code flow. This makes `fetih --profile <name>
    ...
    if provider == "openai-codex":
        # Clear any existing suppression marker so a re-link after `fetih auth
    ...
    raise SystemExit(f"`fetih auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    # Unified removal dispatch.  Every credential source FETIH reads from
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `fetih auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `fetih auth` is called bare."""
