"""
Configuration management for FETIH.

Config files are stored in ~/.fetih/ for easy access:
- ~/.fetih/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.fetih/.env         - API keys and secrets

This module provides:
- fetih config          - Show current configuration
- fetih config edit     - Open config in editor
- fetih config set      - Set a specific value
- fetih config wizard   - Re-run setup wizard
"""
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.fetih/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``fetih logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  fetih config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `fetih plugins enable
    # observability/langfuse`); credentials gate the plugin at runtime.
    "FETIH_LANGFUSE_ENV",
    "FETIH_LANGFUSE_RELEASE",
    "FETIH_LANGFUSE_SAMPLE_RATE",
    "FETIH_LANGFUSE_MAX_CHARS",
    "FETIH_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
from fetih_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("FETIH_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_fetih_home() / ".managed"
...
def is_managed() -> bool:
    """Check if FETIH is running in package-manager-managed mode.

    Two signals: the FETIH_MANAGED env var (set by the systemd service),
    or a .managed marker file in FETIH_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade fetih-agent"
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how FETIH was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Stamped ``~/.fetih/.install_method`` file (written by installers)
    2. FETIH_MANAGED env / .managed marker (NixOS, Homebrew)
    3. Container detection (/.dockerenv, /run/.containerenv, cgroup)
    4. .git directory presence -> 'git'
    5. Fallback -> 'pip'
    """
    stamp = get_fetih_home() / ".install_method"
    try:
...
def stamp_install_method(method: str) -> None:
    """Write the install method to ~/.fetih/.install_method."""
    stamp = get_fetih_home() / ".install_method"
    try:
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade fetih-agent"
    if method == "docker":
        return "docker pull ghcr.io/MustafaKemal0146/fetih:latest"
    if method == "pip":
        import shutil
        uv = shutil.which("uv")
        if uv:
            return "uv pip install --upgrade fetih-agent"
        return "pip install --upgrade fetih-agent"
    return "fetih update"
...
def format_managed_message(action: str = "modify this FETIH installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("FETIH_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this FETIH installation is managed by NixOS "
            f"(FETIH_MANAGED={env_hint}).\n"
            "Edit services.fetih-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this FETIH installation is managed by Homebrew "
            f"(FETIH_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade fetih-agent"
        )

    return (
        f"Cannot {action}: this FETIH installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall FETIH."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from FETIH_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, fetih_bin
    or None if container mode is not active, we're already inside the
    container, or FETIH_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("FETIH_DEV") == "1":
        return None
    ...
    container_mode_file = get_fetih_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "fetih-agent")
    exec_user = info.get("exec_user", "fetih")
    fetih_bin = info.get("fetih_bin", "/data/current-package/bin/fetih")
...
# Re-export from fetih_constants — canonical definition lives there.
from fetih_constants import get_fetih_home  # noqa: F811,E402
from utils import atomic_replace

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_fetih_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_fetih_home() / ".env"
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the fetih group can
    share state with the gateway service.

    The mode can be overridden via the FETIH_HOME_MODE environment variable
    (e.g. FETIH_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse FETIH_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("FETIH_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When FETIH runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("FETIH_CONTAINER") or os.environ.get("FETIH_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set FETIH_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into FETIH_HOME if the user doesn't have one yet."""
...
def ensure_fetih_home():
    """Ensure ~/.fetih directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_fetih_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_fetih_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"FETIH_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the FETIH-level retry loop that wraps the whole call.  Lower
    ...
    "terminal": {
        ...
        # skipped. When empty, FETIH auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), FETIH sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when FETIH
        ...
        #  "/home/user/.fetih/cache/documents:/output"]
        ...
        # bundled FETIH image, which drops to the `fetih` user via gosu).
    ...
    "browser": {
        ...
        "camofox": {
            # When true, FETIH sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and FETIH should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``fetih chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.fetih/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: fetih sweeps the checkpoint base at startup
    ...
    # single read_file page exceeds these limits, FETIH truncates the
    ...
    "auxiliary": {
        ...
        # Invoked by ``fetih kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``fetih kanban decompose`` and the kanban
        ...
        # ``fetih profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `fetih model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # When true, `fetih --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `fetih -c` muscle memory.  Default off so existing
        # users aren't surprised.  FETIH_TUI_RESUME=<id> always wins.
        ...
        # e.g. `model · 68% · ~/projects/fetih`. Per-platform overrides go under
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.fetih/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.fetih/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # response. If not, FETIH feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before FETIH auto-pauses the goal and
    ...
    # always goes to ~/.fetih/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${FETIH_SKILL_DIR} and ${FETIH_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `fetih curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # skipped), snapshot ~/.fetih/skills/ into
        # ~/.fetih/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `fetih curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for fetih-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *FETIH*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (FETIH_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # stored approval from ~/.fetih/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or FETIH_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `fetih doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``fetih_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow FETIH to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # Also overridable via FETIH_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `fetih -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # falls back to the default profile (the one `fetih` launches with
        ...
        # decomposition is manual via `fetih kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     fetih-agent's own python (sys.executable). Maximum isolation
    ...
    # Logging — controls file logging to ~/.fetih/logs/.
    ...
    # curated model lists for OpenRouter and FETIH Portal from this URL,
    # falling back to the in-repo snapshot on network failure.  Lets us
    # update model picker lists without shipping a fetih-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://github.com/MustafaKemal0146/fetih/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `fetih model` invocation; network failures
    ...
    # Session storage — controls automatic cleanup of ~/.fetih/state.db.
    ...
    "sessions": {
        ...
        # default of ``fetih sessions prune``.
        "retention_days": 90,
    ...
    # ``fetih update`` behaviour.
    "updates": {
        # Run a full ``fetih backup``-style zip of FETIH_HOME before every
        # ``fetih update``.  Backups land in ``<FETIH_HOME>/backups/`` and
        # can be restored with ``fetih import <path>``.  Off by default —
        # on large FETIH_HOME directories the zip can add minutes to every
    ...
    "lsp": {
        ...
        #              ``<FETIH_HOME>/lsp/bin/`` on first use.
    ...
    # `fetih tools`. These settings tune the backing Responses API call.
    "x_search": {
...
# selection step (FETIH Portal / OpenRouter / Custom endpoint), so this
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'fetih model' for endpoint-specific config)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @fetih:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. FETIH_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #fetih)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: fetih-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'fetih-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote FETIH API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote FETIH API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote FETIH API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # FETIH_TOOL_PROGRESS and FETIH_TOOL_PROGRESS_MODE are deprecated —
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.fetih.config`` entries, then checks
    ...
    try:
        ...
        # should never break `fetih update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older FETIH versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``fetih_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'fetih setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — FETIH won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'fetih doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("FETIH_HOME", "~/.fetih")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("FETIH_TOOL_PROGRESS")
            old_mode = get_env_value("FETIH_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from FETIH_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from FETIH_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("FETIH_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from FETIH_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$FETIH_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_fetih_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `fetih plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `fetih curator status` has no
    ...
    #   3. Creates `~/.fetih/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_fetih_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_fetih_home() / "logs" / "curator"
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: fetih config set <key> <value>")
    ...
    # metadata.fetih.config in their SKILL.md frontmatter.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _fetih_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.fetih/config.yaml as-is, without merging defaults or migrating.
...
def _apply_session_model_override(expanded: Dict[str, Any]) -> None:
    """Apply the last-session model choice (~/.fetih/.last_model) to config.
    ...
    This is applied on BOTH cache-hit and cache-miss paths because
    .last_model changes independently of config.yaml mtime.
    """
    try:
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.fetih/config.yaml.
    ...
    (which change ``FETIH_HOME`` and therefore ``get_config_path()``)
    don't collide.
    """
    with _CONFIG_LOCK:
        ensure_fetih_home()
        ...
        if cached is not None and cache_key is not None and cached[:2] == cache_key:
            ...
            _apply_session_model_override(result)
            return result
        ...
        _apply_session_model_override(expanded)
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.fetih/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.fetih/.env.
    ...
    interactive menu render (`fetih tools`, `fetih setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `fetih tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real FETIH env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.fetih/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.fetih/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.fetih/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.fetih/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to FETIH — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known FETIH vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.fetih/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ FETIH Configuration                     │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  fetih config edit     # Edit config file", Colors.DIM))
    print(color("  fetih config set <key> <value>", Colors.DIM))
    print(color("  fetih setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: fetih config set <key> <value>")
            print()
            print("Examples:")
            print("  fetih config set model anthropic/claude-sonnet-4")
            print("  fetih config set terminal.backend docker")
            print("  fetih config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'fetih config migrate' to add them")
        ...
        print("  fetih config           Show current configuration")
        print("  fetih config edit      Open config in editor")
        print("  fetih config set <key> <value>   Set a config value")
        print("  fetih config check     Check for missing/outdated config")
        print("  fetih config migrate   Update config with new options")
        print("  fetih config path      Show config file path")
        print("  fetih config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``fetih config`` UI so users
