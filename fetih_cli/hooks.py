"""fetih hooks — inspect and manage shell-script hooks.

Usage::

    fetih hooks list
    fetih hooks test <event> [--for-tool X] [--payload-file F]
    fetih hooks revoke <command>
    fetih hooks doctor

Consent records live under ``~/.fetih/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.fetih/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``fetih hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: fetih hooks {list|test|revoke|doctor}")
        print("Run 'fetih hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.fetih/config.yaml.")
        print("See `fetih hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `fetih hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `fetih hooks test` and `fetih hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (FETIH wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `fetih hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `fetih hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `fetih hooks doctor`.")
