...
1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with fetih;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.fetih/plugins/<name>/``
3. **Project plugins** – ``./.fetih/plugins/<name>/`` (opt-in via
   ``FETIH_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``fetih_agent.plugins``
   entry-point group.
...
# ---------------------------------------------------------------------------
#
# Set ``FETIH_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
# stderr in addition to ~/.fetih/logs/agent.log. Aimed at plugin authors
...
_PLUGINS_DEBUG = os.getenv("FETIH_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When FETIH_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.

    Idempotent: only attaches the handler once per process unless ``force``
    is passed. Does not touch the root logger or other FETIH loggers.
    """
    ...
    logger.debug(
        "FETIH_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
ENTRY_POINTS_GROUP = "fetih_agent.plugins"

_NS_PARENT = "fetih_plugins"
...
class PluginManifest:
    ...
    #              in ~/.fetih/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``fetih plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``fetih honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``fetih <subcommand>``
    ...
    def register_skill(
        ...
        ``~/.fetih/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        #
        # Repo-shipped plugins live next to fetih_cli/. Two layouts are
        ...
        # 2. User plugins (~/.fetih/plugins/)
        user_dir = get_fetih_home() / "plugins"
        logger.debug("Scanning user plugins: %s", user_dir)
        user_manifests = self._scan_directory(user_dir, source="user")
        logger.debug("  user: %d manifest(s)", len(user_manifests))
        manifests.extend(user_manifests)

        # 3. Project plugins (./.fetih/plugins/)
        if _env_enabled("FETIH_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".fetih" / "plugins"
            ...
            logger.debug(
                "Project plugins disabled (set FETIH_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with fetih and must
            # just work. Selection among them (e.g. which image_gen backend
            # services calls) is driven by ``<category>.provider`` config,
            # enforced by the tool wrapper.
            #
            # Bundled platform plugins (gateway adapters like IRC) auto-load
            # for the same reason: every platform FETIH ships must be
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `fetih plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``fetih_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``fetih_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="fetih-plugin-command-await",
        daemon=True,
    )
    thread.start()
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``fetih tools`` TUI so plugin-provided toolsets appear
