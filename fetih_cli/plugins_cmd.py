"""``fetih plugins`` CLI subcommand — install, update, remove, and list plugins.

Plugins are installed from Git repositories into ``~/.fetih/plugins/``.
...
def _resolve_git_executable() -> Optional[str]:
    ...
    Matches other FETIH subprocess resolution: :func:`shutil.which` first,
...
def _plugins_dir() -> Path:
    ...
    plugins = get_fetih_home() / "plugins"
...
def _missing_requires_env_names(manifest: dict) -> list[str]:
    """Return declared ``requires_env`` names that are unset in ``~/.fetih/.env``."""
...
def _prompt_plugin_env_vars(manifest: dict, console) -> None:
    ...
    from fetih_cli.config import get_env_value, save_env_value  # noqa: F811
    ...
    for spec in missing:
        ...
        try:
            ...
            console.print(f"\n[dim]  Skipped (you can set these later in {display_fetih_home()}/.env)[/dim]")
            return

        if value:
            save_env_value(name, value)
            os.environ[name] = value
            console.print(f"  [green]✓[/green] Saved to {display_fetih_home()}/.env")
        else:
            console.print(f"  [dim]  Skipped (set {name} in {display_fetih_home()}/.env later)[/dim]")
...
def _install_plugin_core(identifier: str, *, force: bool) -> tuple[Path, dict, str]:
    """Clone Git plugin into ``~/.fetih/plugins``.
        ...
        if mv is not None:
            ...
            if mv_int > _SUPPORTED_MANIFEST_VERSION:
                ...
                raise PluginOperationError(
                    ...
                    f"Run {recommended_update_command()} to update FETIH.",
                ) from None
        ...
        if target.exists():
            if not force:
                raise PluginOperationError(
                    ...
                    f"or run `fetih plugins update {plugin_name}`.",
                )
...
def cmd_install(
    ...
    if not (target / "plugin.yaml").exists() and not (target / "plugin.yml").exists() and not (
        ...
        console.print(
            ...
            f"or __init__.py. It may not be a valid FETIH plugin.",
        )
    ...
    if should_enable:
        ...
        console.print(
            ...
            f"Run `fetih plugins enable {installed_name}` to activate.[/dim]",
        )

    console.print("[dim]Restart the gateway for the plugin to take effect:[/dim]")
    console.print("[dim]  fetih gateway restart[/dim]")
    console.print()
    ...
    if user_dir.is_dir():
        ...
        for child in user_dir.iterdir():
            ...
            if manifest.get("name") == name:
                return True
    # Bundled: <repo>/plugins/<name>/ (or FETIH_BUNDLED_PLUGINS on Nix).
    from fetih_cli.plugins import get_bundled_plugins_dir
...
def _discover_all_plugins() -> list:
    ...
    user types into ``fetih plugins enable <key>``. For category-namespaced
...
def cmd_list() -> None:
    ...
    if not entries:
        ...
        console.print("[dim]Install with:[/dim] fetih plugins install owner/repo")
        return
    ...
    console.print(table)
    console.print()
    console.print("[dim]Interactive toggle:[/dim] fetih plugins")
    console.print("[dim]Enable/disable:[/dim] fetih plugins enable/disable <name>")
...
def _user_installed_plugin_dir(name: str) -> Optional[Path]:
    """Resolved path under ``~/.fetih/plugins/<name>`` if it exists."""
    plugins_dir = _plugins_dir()
    try:
        target = _sanitize_plugin_name(name, plugins_dir)
    except ValueError:
...
def dashboard_update_user_plugin(name: str) -> dict[str, Any]:
    """``git pull`` inside ``~/.fetih/plugins/<name>``."""
...
def dashboard_remove_user_plugin(name: str) -> dict[str, Any]:
    """Delete a plugin tree under ``~/.fetih/plugins/`` only."""
    plugins_dir = _plugins_dir()
...
def plugins_command(args) -> None:
    """Dispatch fetih plugins subcommands."""
