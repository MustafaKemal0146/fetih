...
on Japanese locales, etc.).  FETIH's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``fetih_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. FETIH is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``FETIH_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("FETIH_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the FETIH-managed Git install directories so
    ...
    # already-running shells don't see the change — which means fetih
    ...
    # install without FETIH-managed PortableGit).
    ...
    # stdin is re-configured for completeness; FETIH's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and FETIH's
    ``fetih config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching FETIH (User env var in Windows Settings, or
    export in a PowerShell profile) and FETIH picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known FETIH-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\fetih\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``fetih`` from right after install) retain their old PATH.

    Any subprocess FETIH spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known FETIH-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "fetih", "git", "cmd"),
        os.path.join(local_appdata, "fetih", "git", "bin"),
        os.path.join(local_appdata, "fetih", "git", "usr", "bin"),
        # FETIH venv Scripts directory — host of the fetih.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes fetih, but harmless to include.
        os.path.join(local_appdata, "fetih", "fetih-agent", "venv", "Scripts"),
