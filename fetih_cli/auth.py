"""
Multi-provider authentication system for FETIH.

Supports OAuth device code flows (FETIH Portal, future: OpenAI Codex) and
traditional API key providers (OpenRouter, custom endpoints). Auth state
is persisted in ~/.fetih/auth.json with cross-process file locking.
...
- Legacy session key (fallback): mint an opaque 24h key when JWT auth is
  unavailable, or when FETIH_AGENT_USE_LEGACY_SESSION_KEYS is set for
...
from fetih_cli.config import get_fetih_home, get_config_path, read_raw_config
from fetih_constants import OPENROUTER_BASE_URL
...
# FETIH Portal defaults
DEFAULT_NOUS_PORTAL_URL = "https://github.com/MustafaKemal0146/fetih"
DEFAULT_NOUS_INFERENCE_URL = "https://inference-api.github.com/MustafaKemal0146/fetih/v1"
DEFAULT_NOUS_CLIENT_ID = "fetih-cli"
...
NOUS_LEGACY_SESSION_KEYS_ENV = "FETIH_AGENT_USE_LEGACY_SESSION_KEYS"
...
SPOTIFY_DOCS_URL = "https://github.com/MustafaKemal0146/fetih/docs/user-guide/features/spotify"
SPOTIFY_DASHBOARD_URL = "https://developer.spotify.com/dashboard"
SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120

XAI_OAUTH_DOCS_URL = "https://github.com/MustafaKemal0146/fetih/docs/guides/xai-grok-oauth"
OAUTH_OVER_SSH_DOCS_URL = "https://github.com/MustafaKemal0146/fetih/docs/guides/oauth-over-ssh"
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from fetih_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
    ...
    for env_var in pconfig.api_key_env_vars:
        # Check both os.environ and ~/.fetih/.env file
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `fetih model` to re-authenticate."

    if error.code == "subscription_required":
        return (
            "No active paid subscription found on FETIH Portal. "
    ...
    if error.code == "insufficient_credits":
        return (
            ...
            "Top up/renew credits in FETIH Portal, then retry."
        )
...
def _oauth_trace_enabled() -> bool:
    raw = os.getenv("FETIH_OAUTH_TRACE", "").strip().lower()
...
# =============================================================================
# Auth Store — persistence layer for ~/.fetih/auth.json
# =============================================================================

def _auth_file_path() -> Path:
    path = get_fetih_home() / "auth.json"
    # Seat belt: if pytest is running and FETIH_HOME resolves to the real
    # user's auth store, refuse rather than silently corrupt it. This catches
    # tests that forgot to monkeypatch FETIH_HOME, tests invoked without the
    # hermetic conftest, or sandbox escapes via threads/subprocesses. In
    # production (no PYTEST_CURRENT_TEST) this is a single dict lookup.
    if os.environ.get("PYTEST_CURRENT_TEST"):
        real_home_auth = (Path.home() / ".fetih" / "auth.json").resolve(strict=False)
        try:
        ...
        if resolved == real_home_auth:
            raise RuntimeError(
                ...
                "Set FETIH_HOME to a tmp_path in your test fixture, or run "
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    directory (classic mode, or custom FETIH_HOME that is not a profile).
    ...
    try:
        ...
        global_root = get_default_fetih_root()
    except Exception:
        return None
    profile_home = get_fetih_home()
    try:
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    ``~/.fetih/auth.json`` even when FETIH_HOME is set to a profile
    path. The hermetic conftest does not redirect ``HOME``, so
    ``get_default_fetih_root()`` for a profile-shaped FETIH_HOME can
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if real_home_env:
            real_root = Path(real_home_env) / ".fetih" / "auth.json"
            try:
...
def read_credential_pool(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    ``fetih auth add <provider>`` inside the profile, profile entries
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    # not by the user explicitly configuring anthropic in FETIH.
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `fetih logout`.
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    try:
        ...
        lines = ["Config issue detected — run 'fetih doctor' for full diagnostics:"]
        for ci in issues:
    ...
    *,
    ...
    if normalized != "auto":
        ...
        if _config_hint:
            ...
            msg += " Check 'fetih model' for available providers, or run 'fetih doctor' to diagnose config issues."
...
def resolve_qwen_runtime_credentials(
    *,
    ...
    base_url = os.getenv("FETIH_QWEN_BASE_URL", "").strip().rstrip("/") or DEFAULT_QWEN_BASE_URL
    return {
        "provider": "qwen-oauth",
        "base_url": base_url,
...
#
# Tokens live in ~/.fetih/auth/google_oauth.json (managed by agent.google_oauth).
...
def get_gemini_oauth_auth_status() -> Dict[str, Any]:
    """Return a status dict for `fetih auth list` / `fetih status`."""
    try:
...
# Spotify auth — PKCE tokens stored in ~/.fetih/auth.json
# =============================================================================
...
def _spotify_client_id(
    ...
    candidates = (
        explicit,
        get_env_value("FETIH_SPOTIFY_CLIENT_ID"),
    ...
    for candidate in candidates:
        ...
        if cleaned:
            return cleaned
    raise AuthError(
        "Spotify client_id is required. Set FETIH_SPOTIFY_CLIENT_ID or pass --client-id.",
        provider="spotify",
...
def _spotify_redirect_uri(
    ...
    candidates = (
        explicit,
        get_env_value("FETIH_SPOTIFY_REDIRECT_URI"),
...
def _spotify_api_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("FETIH_SPOTIFY_API_BASE_URL"),
...
def _spotify_accounts_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("FETIH_SPOTIFY_ACCOUNTS_BASE_URL"),
        ...
        def do_GET(self) -> None:  # noqa: N802
            ...
            if incoming["code"] is None and incoming["error"] is None:
                ...
                body = (
                    ...
                    "<code>fetih auth add xai-oauth</code> to retry.</p>"
                    "</body></html>"
                )
                self.wfile.write(body.encode("utf-8"))
                return
...
def _refresh_spotify_oauth_state(
    ...
    *,
    ...
    if not refresh_token:
        raise AuthError(
            "Spotify refresh token missing. Run `fetih auth spotify` again.",
            provider="spotify",
    ...
    if response.status_code >= 400:
        detail = response.text.strip()
        raise AuthError(
            "Spotify token refresh failed. Run `fetih auth spotify` again."
            + (f" Response: {detail}" if detail else ""),
            provider="spotify",
            code="spotify_refresh_failed",
            relogin_required=True,
...
def resolve_spotify_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Spotify is not authenticated. Run `fetih auth spotify` first.",
                provider="spotify",
    ...
    if not access_token:
        raise AuthError(
            "Spotify access token missing. Run `fetih auth spotify` again.",
            provider="spotify",
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    resulting client_id to ~/.fetih/.env, and return it.
    ...
    print("       App name:     anything (e.g. fetih-agent)")
    ...
    # Persist so subsequent `fetih auth spotify` runs skip the wizard.
    save_env_value("FETIH_SPOTIFY_CLIENT_ID", raw)
    ...
    if redirect_uri_hint and redirect_uri_hint != DEFAULT_SPOTIFY_REDIRECT_URI:
        save_env_value("FETIH_SPOTIFY_REDIRECT_URI", redirect_uri_hint)

    print()
    print("Saved FETIH_SPOTIFY_CLIENT_ID to ~/.fetih/.env")
    print()
    return raw
...
def login_spotify_command(args) -> None:
    ...
    # with "FETIH_SPOTIFY_CLIENT_ID is required".
    ...
    print()
    print("Open this URL to authorize FETIH:")
    print(authorize_url)
    print()
...
def _print_loopback_ssh_hint(redirect_uri: str, *, docs_url: str | None = None) -> None:
    ...
    print(divider)
    print(f"FETIH is waiting for the OAuth callback on {redirect_uri}")
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.fetih/auth.json (not ~/.codex/)
#
# FETIH maintains its own Codex OAuth session separate from the Codex CLI
# and VS Code extension. This prevents refresh token rotation conflicts
# where one app's refresh invalidates the other's session.
# =============================================================================

def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from FETIH auth store (~/.fetih/auth.json).
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `fetih auth` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Codex auth state is missing tokens. Run `fetih auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Codex auth is missing access_token. Run `fetih auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "Codex auth is missing refresh_token. Run `fetih auth` to re-authenticate.",
            provider="openai-codex",
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None) -> None:
    """Save Codex OAuth tokens to FETIH auth store (~/.fetih/auth.json)."""
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    """Refresh Codex OAuth tokens without mutating FETIH auth state."""
    ...
    if response.status_code != 200:
        ...
        if code == "refresh_token_reused":
            message = (
                ...
                "then run `fetih auth` to re-authenticate."
            )
            relogin_required = True
...
def _refresh_codex_auth_tokens(
    ...
    Saves the new tokens to FETIH auth store automatically.
    """
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from FETIH's own Codex token store."""
    data = _read_codex_tokens()
    tokens = dict(data["tokens"])
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = float(os.getenv("FETIH_CODEX_REFRESH_TIMEOUT_SECONDS", "20"))

    should_refresh = bool(force_refresh)
    if (not should_refresh) and refresh_if_expiring:
        should_refresh = _codex_access_token_is_expiring(access_token, refresh_skew_seconds)
    if should_refresh:
        # Re-read under lock to avoid racing with other FETIH processes
    ...
    base_url = (
        os.getenv("FETIH_CODEX_BASE_URL", "").strip().rstrip("/")
...
# =============================================================================
# xAI Grok OAuth — tokens stored in ~/.fetih/auth.json
# =============================================================================
...
def _read_xai_oauth_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    ...
    if not state:
        raise AuthError(
            "No xAI OAuth credentials stored. Select xAI Grok OAuth (SuperGrok Subscription) in `fetih model`.",
            provider="xai-oauth",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "xAI OAuth state is missing tokens. Re-authenticate with `fetih model`.",
            provider="xai-oauth",
    ...
    if not access_token:
        raise AuthError(
            "xAI OAuth state is missing access_token. Re-authenticate with `fetih model`.",
            provider="xai-oauth",
    ...
    if not refresh_token:
        raise AuthError(
            "xAI OAuth state is missing refresh_token. Re-authenticate with `fetih model`.",
            provider="xai-oauth",
...
def _xai_validate_oauth_endpoint(url: str, *, field: str) -> str:
    ...
    output is cached in ``~/.fetih/auth.json``. A single MITM during initial
    ...
    if host != "x.ai" and not host.endswith(".x.ai"):
        raise AuthError(
            ...
            f"discovery; re-authenticate with `fetih model` to re-fetch.",
            provider="xai-oauth",
            code="xai_discovery_invalid",
        )
    return url
...
def refresh_xai_oauth_pure(
    ...
    *,
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "xAI OAuth is missing refresh_token. Re-authenticate with `fetih model`.",
            provider="xai-oauth",
            code="xai_auth_missing_refresh_token",
            relogin_required=True,
        )
    ...
    # written by an older FETIH (or hand-edited) may carry a non-xAI
    ...
    # with a clear error so the user can re-run `fetih model` to refetch.
...
def resolve_xai_oauth_runtime_credentials(
    *,
    ...
    tokens = dict(data["tokens"])
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = float(os.getenv("FETIH_XAI_REFRESH_TIMEOUT_SECONDS", "20"))
    ...
    base_url = (
        os.getenv("FETIH_XAI_BASE_URL", "").strip().rstrip("/")
...
# =============================================================================
# FETIH Portal — token refresh, agent key minting, model discovery
# =============================================================================

# -----------------------------------------------------------------------------
# Shared Nous token store — lets OAuth credentials persist across profiles
# so a new `fetih --profile <name> auth add nous --type oauth` can one-tap
# import instead of running the full device-code flow every time.
#
# File lives at ${FETIH_SHARED_AUTH_DIR}/nous_auth.json, defaulting to
# ``<fetih-root>/shared/nous_auth.json`` where ``<fetih-root>`` is what
# ``get_default_fetih_root()`` returns — ``~/.fetih`` on Linux/macOS,
# ``%LOCALAPPDATA%\fetih`` on native Windows, or the Docker/custom root.
# It is OUTSIDE any named profile's FETIH_HOME so named profiles (which
# typically live under ``<fetih-root>/profiles/<name>/``) all see the
# same file.
#
...
def _nous_shared_auth_dir() -> Path:
    ...
    Honors ``FETIH_SHARED_AUTH_DIR`` so tests can redirect it to a tmp
    path without touching the real user's home. Defaults to
    ``<fetih-root>/shared/``, where ``<fetih-root>`` is what
    :func:`fetih_constants.get_default_fetih_root` returns — so
    Linux/macOS classic installs land at ``~/.fetih/shared/``, native
    Windows installs at ``%LOCALAPPDATA%\\fetih\\shared\\``, and
    Docker / custom ``FETIH_HOME`` deployments at
    ``<FETIH_HOME>/shared/``. Sits outside any named profile so all
    profiles under the same root share the store.
    """
    override = os.getenv("FETIH_SHARED_AUTH_DIR", "").strip()
    if override:
        return Path(override).expanduser()
    from fetih_constants import get_default_fetih_root
    return get_default_fetih_root() / "shared"


def _nous_shared_store_path() -> Path:
    path = _nous_shared_auth_dir() / NOUS_SHARED_STORE_FILENAME
    # Seat belt: if pytest is running and this resolves to a path under the
    # real user's FETIH root, refuse rather than silently corrupt cross-profile
    # state. Tests must set FETIH_SHARED_AUTH_DIR to a tmp_path (conftest
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        from fetih_constants import get_default_fetih_root
        real_home_shared = (
            get_default_fetih_root() / "shared" / NOUS_SHARED_STORE_FILENAME
        ...
        if resolved == real_home_shared:
            raise RuntimeError(
                ...
                f"{path}. Set FETIH_SHARED_AUTH_DIR to a tmp_path in your test fixture."
            )
    return path
...
def _nous_shared_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    ...
    try:
        ...
        # No FETIH_HOME yet (pre-setup): fall through without locking.
        yield
        return
...
def _refresh_access_token(
    *,
    ...
    # Detect the OAuth 2.1 "refresh token reuse" signal from the auth portal
    ...
    # called POST /api/oauth/token with FETIH's refresh_token without
    # persisting the rotated token back to auth.json — the server then
    # retires the original RT, FETIH's next refresh uses it, and the whole
    ...
    if code == "refresh_token_reused" or "reuse" in lowered or "reuse detected" in lowered:
        description = (
            "FETIH Portal detected refresh-token reuse and revoked this session.\n"
            "This usually means an external process (monitoring script, "
            "custom self-heal hook, or another FETIH install sharing "
            "~/.fetih/auth.json) called POST /api/oauth/token with FETIH's "
            "refresh token without persisting the rotated token back.\n"
            "Nous refresh tokens are single-use — only FETIH may call the "
            "refresh endpoint. For health checks, use `fetih auth status` "
            "instead.\n"
            "Re-authenticate with: fetih auth add nous"
        )
        relogin = True
    ...
    *,
    ...
    for item in data:
        ...
        if isinstance(model_id, str) and model_id.strip():
            mid = model_id.strip()
            # Skip FETIH models — they're not reliable for agentic tool-calling
            if "fetih" in mid.lower():
                continue
...
def resolve_nous_access_token(
    *,
    ...
    """Resolve a refresh-aware FETIH Portal access token for managed tool gateways."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
        ...
        if not state:
            raise AuthError(
                "FETIH is not logged into FETIH Portal.",
                provider="nous",
                relogin_required=True,
            )
        ...
        with _nous_shared_store_lock(timeout_seconds=max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)):
            ...
            if not isinstance(access_token, str) or not access_token:
                raise AuthError(
                    "No access token found for FETIH Portal login.",
                    provider="nous",
                    relogin_required=True,
                )
...
def persist_nous_credentials(
    ...
    *,
    ...
    Historically ``fetih auth add nous`` wrote a ``manual:device_code`` pool
    ...
    ``fetih auth add nous --label <name>``).  It gets embedded in the
    ...
    # these credentials via `fetih auth add nous --type oauth`. Best-
...
def resolve_nous_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError("FETIH is not logged into FETIH Portal.",
        ...
        with httpx.Client(timeout=timeout, headers={"Accept": "application/json"}, verify=verify) as client:
            ...
            if not isinstance(access_token, str) or not access_token:
                raise AuthError("No access token found for FETIH Portal login.",
                                provider="nous", relogin_required=True)
...
# which does a synchronous OAuth refresh POST to github.com/MustafaKemal0146/fetih. That can take
# ~350ms even on the failure path, and read-only UI surfaces (`fetih tools`, status panels,
# subscription-feature checks) call it many times per render — `fetih tools` → "All Platforms"
# was firing the refresh ~31× during one menu paint, racking up >13s of HTTP and burning
# single-use refresh tokens. Cache the snapshot for a few seconds, keyed on the auth.json
# mtime so that `fetih auth login/logout/add/remove` invalidate naturally on the next call.
...
def get_codex_auth_status() -> Dict[str, Any]:
    ...
    Checks the credential pool first (where `fetih auth` stores credentials),
    then falls back to the legacy provider state.
    """
    # Check credential pool first — this is where `fetih auth` and
    # `fetih model` store device_code tokens.
    try:
        from agent.credential_pool import load_pool
...
def get_external_process_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    command = (
        os.getenv("FETIH_COPILOT_ACP_COMMAND", "").strip()
        or os.getenv("COPILOT_CLI_PATH", "").strip()
        or "copilot"
    )
    raw_args = os.getenv("FETIH_COPILOT_ACP_ARGS", "").strip()
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    checks:
      ...
      * ``auth_mode == "entra_id"`` AND ``azure-identity`` is importable
        (we do NOT mint a token here; ``fetih doctor`` runs the live
    ...
    try:
        from fetih_cli.config import load_config, get_env_value
        cfg = load_config()
    except Exception:
        cfg = {}
        ...
        try:
            ...
            if not installed:
                info["hint"] = (
                    ...
                    "pip install azure-identity  (or rely on FETIH' "
                    "lazy-install at first use)."
                )
            else:
                info["hint"] = (
                    "azure-identity is installed; live credential validation "
                    "is skipped here. Run `fetih doctor` to verify token acquisition."
                )
            return info
        except Exception as exc:
...
def resolve_external_process_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if not resolved_command and not base_url.startswith("acp+tcp://"):
        raise AuthError(
            ...
            "Install GitHub Copilot CLI or set FETIH_COPILOT_ACP_COMMAND/COPILOT_CLI_PATH.",
            provider=provider_id,
...
def _logout_default_provider_from_config() -> Optional[str]:
    ...
    `fetih logout` historically keyed off auth.json.active_provider only.
...
def _prompt_model_selection(
    ...
    """
    from fetih_cli.models import _format_price_per_mtok
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from fetih_cli.config import save_config, load_config
...
def login_command(args) -> None:
    """Deprecated: use 'fetih model' or 'fetih setup' instead."""
    print("The 'fetih login' command has been removed.")
    print("Use 'fetih auth' to manage credentials,")
    print("'fetih model' to select a provider, or 'fetih setup' for full setup.")
    raise SystemExit(0)
...
def _login_openai_codex(
    ...
    *,
    ...
    """OpenAI Codex login via device code flow. Tokens stored in ~/.fetih/auth.json."""

    del args, pconfig  # kept for parity with other provider login helpers

    # Check for existing FETIH-owned credentials
    if not force_new_login:
        try:
            ...
            if isinstance(_resolved_key, str) and _resolved_key and not _codex_access_token_is_expiring(_resolved_key, 60):
                print("Existing Codex credentials found in FETIH auth store.")
                try:
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            print("FETIH will create its own session to avoid conflicts with Codex CLI / VS Code.")
            try:
            ...
            if do_import in {"y", "yes"}:
                ...
                base_url = os.getenv("FETIH_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
                config_path = _update_config_for_provider("openai-codex", base_url)
                print()
                print("Credentials imported. Note: if Codex CLI refreshes its token,")
                print("FETIH will keep working independently with its own session.")
                print(f"  Config updated: {config_path} (model.provider=openai-codex)")
                return

    # Run a fresh device code flow — FETIH gets its own OAuth session
    print()
    print("Signing in to OpenAI Codex...")
    print("(FETIH creates its own session — won't affect Codex CLI or VS Code)")
    print()

    creds = _codex_device_code_login()

    # Save tokens to FETIH auth store
...
def _login_xai_oauth(
    ...
    *,
    ...
    if not force_new_login:
        try:
            ...
            if isinstance(api_key, str) and api_key and not _xai_access_token_is_expiring(api_key, 60):
                print("Existing xAI OAuth credentials found in FETIH auth store.")
                try:
                    reuse = input("Use existing credentials? [Y/n]: ").strip().lower()
                except (EOFError, KeyboardInterrupt):
                    reuse = "y"
    ...
    print("(FETIH creates its own local OAuth session)")
    print()
...
def _xai_oauth_build_authorize_url(
    *,
    ...
    # `referrer=fetih-agent` lets xAI attribute FETIH-originated logins
    ...
    *,
    ...
    if not code_verifier:
        raise AuthError(
            ...
            "This is a bug in FETIH — please report at "
            "https://github.com/MustafaKemal0146/fetih/issues/26990.",
            provider="xai-oauth",
...
def _xai_oauth_loopback_login(
    *,
    ...
    try:
        ...
        print("Open this URL to authorize FETIH with xAI:")
        print(authorize_url)
        print()
...
def _minimax_save_auth_state(auth_state: Dict[str, Any]) -> None:
    """Persist MiniMax OAuth state to FETIH auth store (~/.fetih/auth.json)."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def _minimax_oauth_login(
    *, region: str = "global", open_browser: bool = True,
    ...
    print(f"Starting FETIH login via MiniMax ({region}) OAuth...")
...
def resolve_minimax_oauth_runtime_credentials(
    *, min_token_ttl_seconds: int = MINIMAX_OAUTH_REFRESH_SKEW_SECONDS,
    ...
    if not state or not state.get("access_token"):
        raise AuthError(
            "Not logged into MiniMax OAuth. Run `fetih model` and select "
            "MiniMax (OAuth).",
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target) or should_reset_config:
        ...
        if should_reset_config and os.getenv("OPENROUTER_API_KEY"):
            print("FETIH will use OpenRouter for inference.")
        elif should_reset_config:
            print("Run `fetih model` or configure an API key to use FETIH.")
        else:
