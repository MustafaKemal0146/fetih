"""``fetih slack ...`` CLI subcommands.

Today only ``fetih slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ fetih slack manifest > slack-manifest.json
    # or:
...
def _build_full_manifest(bot_name: str, bot_description: str) -> dict:
    ...
    it stays in sync with the rest of FETIH. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a FETIH deployment — users can tweak them in the Slack UI after
    pasting.
    """
    from fetih_cli.commands import slack_app_manifest
    ...
    return {
        ...
        "display_information": {
            "name": bot_name[:35],
            "description": (bot_description or "Your FETIH agent on Slack")[:140],
        ...
        "features": {
            ...
            "assistant_view": {
                "assistant_description": "Chat with FETIH in threads and DMs.",
            },
        },
        "oauth_config": {
            "scopes": {
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``fetih_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$FETIH_HOME/slack-manifest.json``)
      --name NAME     Override the bot display name (default: "FETIH")
      --description DESC  Override the bot description
      --slashes-only  Emit only the ``features.slash_commands`` array (for
                      merging into an existing manifest manually)
    """
    name = getattr(args, "name", None) or "FETIH"
    description = getattr(args, "description", None) or "Your FETIH agent on Slack"
    ...
    if write_target is not None:
        if isinstance(write_target, bool) and write_target:
            ...
            try:
                ...
                target = Path(os.environ.get("FETIH_HOME") or str(Path.home() / ".fetih")) / "slack-manifest.json"
        else:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your FETIH app\n"
