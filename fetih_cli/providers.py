"""
Single source of truth for provider identity in FETIH.
...
2. **FETIH overlays** — transport type, auth patterns, aggregator flags,
...
# -- FETIH overlay ----------------------------------------------------------
# FETIH-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class FETIHOverlay:
    """FETIH-specific provider metadata layered on top of models.dev."""
...
FETIH_OVERLAYS: Dict[str, FETIHOverlay] = {
    "openrouter": FETIHOverlay(
    ...
    "xai-oauth": FETIHOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": FETIHOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="FETIH_QWEN_BASE_URL",
    ),
    "google-gemini-cli": FETIHOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="cloudcode-pa://google",
    ),
    "lmstudio": FETIHOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": FETIHOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": FETIHOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": FETIHOverlay(
    ...
    "kimi-for-coding": FETIHOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "stepfun": FETIHOverlay(
        transport="openai_chat",
        extra_env_vars=("STEPFUN_API_KEY",),
        base_url_override="https://api.stepfun.ai/step_plan/v1",
        base_url_env_var="STEPFUN_BASE_URL",
    ),
    "minimax": FETIHOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-oauth": FETIHOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": FETIHOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": FETIHOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": FETIHOverlay(
        transport="openai_chat",
        base_url_env_var="DASHSCOPE_BASE_URL",
    ),
    "alibaba-coding-plan": FETIHOverlay(
        transport="openai_chat",
    ...
    "opencode": FETIHOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": FETIHOverlay(
        transport="openai_chat",
        is_aggregator=True,
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "fetih", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. FETIH overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + FETIH overlay
    ...
    overlay = FETIH_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + fetih extra
    ...
    if overlay is not None:
        # FETIH-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    # Direct provider checks for providers not in FETIH_OVERLAYS
    if provider == "bedrock":
