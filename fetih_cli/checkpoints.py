"""`fetih checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.fetih/checkpoints/``.  Actions:

    fetih checkpoints               # same as `status`
    fetih checkpoints status        # total size, project count, breakdown
    fetih checkpoints list          # per-project checkpoint counts + workdir
    fetih checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    fetih checkpoints clear [-f]    # nuke the entire base (asks first)
    fetih checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    fetih checkpoints
    fetih checkpoints prune --retention-days 3 --max-size-mb 200
    fetih checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: fetih checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``fetih checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `fetih checkpoints` → status
