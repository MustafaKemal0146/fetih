"""Shell completion script generation for fetih CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_fetih_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# FETIH bash completion
# Add to ~/.bashrc:
#   eval "$(fetih completion bash)"

_fetih_profiles() {{
    local profiles_dir="$HOME/.fetih/profiles"
...
_fetih_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_fetih_profiles)" -- "$cur"))
        return
    fi
...
complete -F _fetih_completion fetih
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef fetih
# FETIH zsh completion
# Add to ~/.zshrc:
#   eval "$(fetih completion zsh)"

_fetih_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.fetih/profiles" ]]; then
        profiles+=("${{(@f)$(ls $HOME/.fetih/profiles 2>/dev/null)}}")
    fi
...
_fetih() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_fetih_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'fetih command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# FETIH fish completion",
        "# Add to your config:",
        "#   fetih completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __fetih_profiles",
        "    echo default",
        "    if test -d $HOME/.fetih/profiles",
        "        ls $HOME/.fetih/profiles 2>/dev/null",
        "    end",
        "end",
        "",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c fetih -f -s p -l profile"
        " -d 'Profile name' -xa '(__fetih_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__fetih_profiles)' -d 'Profile name'"
                )
