"""
fetih fallback — manage the fallback provider chain.
...
Subcommands:
  fetih fallback [list]   Show the current fallback chain (default when no subcommand)
  fetih fallback add      Pick provider + model via the same picker as `fetih model`,
                           then append the selection to the chain
  fetih fallback remove   Pick an entry to delete from the chain
  fetih fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.fetih/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from fetih_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `fetih model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  fetih fallback add")
        print()
        return
    ...
    print("  Docs: https://github.com/MustafaKemal0146/fetih/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `fetih model`, then append the selection to the chain."""
    from fetih_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `fetih model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `fetih fallback list` to view, or `fetih fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``fetih fallback [subcommand]``."""
