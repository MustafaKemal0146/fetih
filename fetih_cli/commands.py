"""Slash command definitions and autocomplete for the FETIH CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    # Oturum
    CommandDef("new", "Yeni oturum başlat (temiz ID + geçmiş)", "Oturum",
               aliases=("reset",), args_hint="[isim]"),
    CommandDef("topic", "Telegram DM konu oturumlarını etkinleştir/incele", "Oturum",
               gateway_only=True, args_hint="[off|help|session-id]"),
    CommandDef("clear", "Ekranı temizle ve yeni oturum başlat", "Oturum",
               cli_only=True),
    CommandDef("redraw", "Arayüzü zorla yeniden çiz (kayma/drift durumunda)", "Oturum",
               cli_only=True),
    CommandDef("history", "Konuşma geçmişini göster", "Oturum",
               cli_only=True),
    CommandDef("save", "Mevcut konuşmayı kaydet", "Oturum",
               cli_only=True),
    CommandDef("retry", "Son mesajı tekrar dene (ajana yeniden gönder)", "Oturum"),
    CommandDef("undo", "Son kullanıcı/ajan mesaj çiftini sil", "Oturum"),
    CommandDef("title", "Mevcut oturum için başlık belirle", "Oturum",
               args_hint="[isim]"),
    CommandDef("handoff", "Bu oturumu mesajlaşma platformuna devret (Telegram, Discord, vs.)", "Oturum",
               args_hint="<platform>", cli_only=True),
    CommandDef("branch", "Oturumu dallandır (farklı bir yol dene)", "Oturum",
               aliases=("fork",), args_hint="[isim]"),
    CommandDef("compress", "Konuşma bağlamını elle sıkıştır", "Oturum",
               args_hint="[odak konu]"),
    CommandDef("rollback", "Dosya sistemi kontrol noktalarını listele/geri yükle", "Oturum",
               args_hint="[numara]"),
    CommandDef("snapshot", "FETIH yapılandırma/durum anlık görüntüsü al/geri yükle", "Oturum",
               cli_only=True, aliases=("snap",), args_hint="[create|restore <id>|prune]"),
    CommandDef("stop", "Çalışan tüm arka plan işlemlerini durdur", "Oturum"),
    CommandDef("approve", "Bekleyen tehlikeli komutu onayla", "Oturum",
               gateway_only=True, args_hint="[session|always]"),
    CommandDef("deny", "Bekleyen tehlikeli komutu reddet", "Oturum",
               gateway_only=True),
    CommandDef("background", "Komutu arka planda çalıştır", "Oturum",
               aliases=("bg", "btw"), args_hint="<komut>"),
    CommandDef("agents", "Aktif ajanları ve çalışan görevleri göster", "Oturum",
               aliases=("tasks",)),
    CommandDef("queue", "Komutu sıraya al (mevcut işlemi bölmez)", "Oturum",
               aliases=("q",), args_hint="<komut>"),
    CommandDef("steer", "Sonraki araç çağrısı sonrası mesaj enjekte et", "Oturum",
               args_hint="<komut>"),
    CommandDef("goal", "FETIH'in turlar boyunca çalışacağı kalıcı hedef belirle", "Oturum",
               args_hint="[metin | pause | resume | clear | status]"),
    CommandDef("subgoal", "Aktif hedefe ek kriter ekle/yönet", "Oturum",
               args_hint="[metin | remove N | clear]"),
    CommandDef("status", "Oturum bilgilerini göster", "Oturum"),
    CommandDef("whoami", "Slash komut yetki seviyeni göster (admin / user)", "Bilgi"),
    CommandDef("profile", "Aktif profil adını ve ana dizini göster", "Bilgi"),
    CommandDef("sethome", "Bu sohbeti ana kanal olarak ayarla", "Oturum",
               gateway_only=True, aliases=("set-home",)),
    CommandDef("resume", "Daha önce adlandırılmış oturumu devam ettir", "Oturum",
               args_hint="[isim]"),

    # Oturum Geçmişi
    CommandDef("sessions", "Önceki oturumları listele ve devam ettir", "Oturum"),

    # Yapılandırma
    CommandDef("config", "Mevcut yapılandırmayı göster", "Yapılandırma",
               cli_only=True),
    CommandDef("model", "Bu oturum için model değiştir", "Yapılandırma",
               aliases=("provider",), args_hint="[model] [--provider isim] [--global]"),
    CommandDef("codex-runtime", "OpenAI/Codex modelleri için codex app-server çalışma zamanını aç/kapat",
               "Yapılandırma", aliases=("codex_runtime",),
               args_hint="[auto|codex_app_server]"),
    CommandDef("gquota", "Google Gemini Code Assist kota kullanımını göster", "Bilgi",
               cli_only=True),

    CommandDef("personality", "Ön tanımlı kişilik seç", "Yapılandırma",
               args_hint="[isim]"),
    CommandDef("statusbar", "Bağlam/model durum çubuğunu aç/kapat", "Yapılandırma",
               cli_only=True, aliases=("sb",)),
    CommandDef("verbose", "Araç ilerleme gösterimini değiştir: off -> new -> all -> verbose",
               "Yapılandırma", cli_only=True,
               gateway_config_gate="display.tool_progress_command"),
    CommandDef("footer", "Son yanıtlarda gateway çalışma-zamanı alt bilgisini aç/kapat",
               "Yapılandırma", args_hint="[on|off|status]",
               subcommands=("on", "off", "status")),
    CommandDef("yolo", "YOLO modunu aç/kapat (tüm tehlikeli komut onaylarını atla)",
               "Yapılandırma"),
    CommandDef("reasoning", "Düşünme (reasoning) seviyesini ve gösterimini yönet", "Yapılandırma",
               args_hint="[level|show|hide]",
               subcommands=("none", "minimal", "low", "medium", "high", "xhigh", "show", "hide", "on", "off")),
    CommandDef("fast", "Hızlı modu aç/kapat — OpenAI Öncelikli İşleme / Anthropic Hızlı Mod (Normal/Hızlı)", "Yapılandırma",
    ...
    CommandDef("voice", "Ses modunu aç/kapat", "Yapılandırma",
               args_hint="[on|off|tts|status]", subcommands=("on", "off", "tts", "status")),
    CommandDef("busy", "FETIH çalışırken Enter tuşunun ne yapacağını belirle", "Yapılandırma",
               cli_only=True, args_hint="[queue|steer|interrupt|status]",
               subcommands=("queue", "steer", "interrupt", "status")),

    # Araçlar & Skill'ler
    CommandDef("tools", "Araçları yönet: /tools [list|disable|enable] [isim...]", "Araçlar & Skill",
               args_hint="[list|disable|enable] [isim...]", cli_only=True),
    CommandDef("toolsets", "Kullanılabilir araç setlerini listele", "Araçlar & Skill",
               cli_only=True),
    CommandDef("skills", "Skill ara, yükle, incele veya yönet",
               "Araçlar & Skill", cli_only=True,
               subcommands=("search", "browse", "inspect", "install")),
    CommandDef("cron", "Zamanlanmış görevleri yönet", "Araçlar & Skill",
               cli_only=True, args_hint="[alt-komut]",
               subcommands=("list", "add", "create", "edit", "pause", "resume", "run", "remove")),
    CommandDef("curator", "Arka plan skill bakımı (durum, çalıştır, sabitle, arşivle, arşiv-listesi)",
               "Araçlar & Skill", args_hint="[alt-komut]",
               subcommands=("status", "run", "pause", "resume", "pin", "unpin", "restore", "list-archived")),
    CommandDef("kanban", "Çoklu profil işbirliği panosu (görevler, bağlantılar, yorumlar)",
               "Araçlar & Skill", args_hint="[alt-komut]",
    ...
    CommandDef("reload", ".env değişkenlerini çalışan oturuma yeniden yükle", "Araçlar & Skill",
               cli_only=True),
    CommandDef("reload-mcp", "MCP sunucularını yapılandırmadan yeniden yükle", "Araçlar & Skill",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "~/.fetih/skills/ dizinini yeniden tara (yeni/silinen skill'ler)",
               "Araçlar & Skill", aliases=("reload_skills",)),
    CommandDef("browser", "Canlı Chrome tarayıcına CDP ile bağlan", "Araçlar & Skill",
               cli_only=True, args_hint="[connect|disconnect|status]",
               subcommands=("connect", "disconnect", "status")),
    CommandDef("plugins", "Yüklü eklentileri ve durumlarını listele",
               "Araçlar & Skill", cli_only=True),
    CommandDef("download-tools",
               "CTF/pentest araçlarını indir ve kur (86 araç, 9 kategori)",
               "Araçlar & Skill", cli_only=True,
               aliases=("tools-install",),
               args_hint="[all|basic|network|web|pentest|binary|crypto|forensics|stego|mobile|osint|status]",
               subcommands=("all", "basic", "status",
                            "network", "web", "pentest", "binary",
                            "crypto", "forensics", "stego", "mobile", "osint")),
    CommandDef("validate-skills",
               "Tüm skill'leri tara: YAML, MITRE ID, trigger çakışmaları, çeviri hataları",
               "Araçlar & Skill", cli_only=True,
               aliases=("skill-check",),
               args_hint="[--category isim] [--fix] [--report] [--json]",
               subcommands=("malware-analysis", "cloud-security", "threat-hunting",
                            "web-application-security", "network-security", "incident-response")),
    CommandDef("report",
               "Otomatik pentest raporu oluştur (yönetici özeti, CVSS, MITRE, NIST CSF)",
               "Araçlar & Skill", cli_only=True,
               aliases=("pentest-report",),
               args_hint="[summary|full|findings] [--format md|pdf]",
               subcommands=("summary", "full", "findings")),
    CommandDef("auto-solve",
               "CTF challenge'ını otomatik çöz: sınıflandır -> çöz -> flag topla",
               "Araçlar & Skill", cli_only=True,
               aliases=("autosolve", "solve-ctf"),
               args_hint="<challenge.zip|url> [--category web|crypto|reverse|forensics|pwn] [--resume]",
               subcommands=("--category", "--resume")),
    CommandDef("computer-use",
               "Masaüstü kontrolünü aç/kapat/durum (varsayılan: KAPALI)",
               "Araçlar & Skill", cli_only=True,
               aliases=("desktop", "cu"),
               args_hint="[on|off|status]",
               subcommands=("on", "off", "status")),
    CommandDef("hint",
               "CTF ipucu sistemi: seviye 1 (dürtme) -> 2 (yönlendirme) -> 3 (çözüm)",
               "Araçlar & Skill", cli_only=True,
               aliases=("ipucu", "clue"),
               args_hint="[more|solution|status|reset]",
               subcommands=("more", "solution", "status", "reset")),
    CommandDef("learnings",
               "Otomatik öğrenme veritabanı: kaydedilmiş teknikler, hatalar, çözümler",
               "Araçlar & Skill", cli_only=True,
               aliases=("learned", "learning"),
               args_hint="[list|search <sorgu>|stats]",
               subcommands=("list", "search", "stats")),
    CommandDef("dashboard",
               "FETIH durum panosu: oturum, araç, öğrenme ve hata istatistikleri",
               "Bilgi", cli_only=True,
               aliases=("stats", "metrics"),
               args_hint=""),

    # Bilgi
    CommandDef("commands", "Tüm komutları ve skill'leri sayfalı göster", "Bilgi",
               gateway_only=True, args_hint="[sayfa]"),
    CommandDef("help", "Kullanılabilir komutları göster", "Bilgi"),
    CommandDef("restart", "Aktif işlemleri boşalttıktan sonra gateway'i zarifçe yeniden başlat", "Oturum",
               gateway_only=True),
    CommandDef("usage", "Mevcut oturum için token kullanımı ve hız limitlerini göster", "Bilgi"),
    CommandDef("insights", "Kullanım analitiği ve içgörüleri göster", "Bilgi",
               args_hint="[gün]"),
    CommandDef("platforms", "Gateway/mesajlaşma platformu durumunu göster", "Bilgi",
               cli_only=True, aliases=("gateway",)),
    CommandDef("platform", "Hatalı gateway platformunu duraklat, devam ettir veya listele", "Bilgi",
               gateway_only=True, args_hint="<pause|resume|list> [isim]"),
    CommandDef("copy", "Son asistan yanıtını panoya kopyala", "Bilgi",
               cli_only=True, args_hint="[numarası]"),
    CommandDef("paste", "Panodaki görüntüyü ek olarak yükle", "Bilgi",
               cli_only=True),
    CommandDef("image", "Sonraki komutun için yerel bir görüntü dosyası ekle", "Bilgi",
               cli_only=True, args_hint="<dosya-yolu>"),
    CommandDef("update", "FETIH ve skill'leri en son sürüme güncelle", "Bilgi",
               aliases=("upgrade",),
               args_hint="[--check|--apply|--status|--rollback]",
               subcommands=("--check", "--apply", "--status", "--rollback")),
    CommandDef("debug", "Hata ayıklama raporu yükle (sistem bilgisi + loglar) ve paylaşılabilir link al", "Bilgi"),

    # Çıkış
    CommandDef("quit", "CLI'dan çık (--delete ile oturum geçmişini de sil)", "Çıkış",
...
def _build_description(cmd: CommandDef) -> str:
    ...
    if cmd.args_hint:
        return f"{cmd.description} (kullanım: /{cmd.name} {cmd.args_hint})"
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`fetih_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/fetih`` subcommand mapping, and
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``fetih skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``fetih skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``fetih skills list`` and
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/fetih <verb>`` subcommand.
    ...
    can still reach them via ``/fetih <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/fetih`` is always reserved as the first entry so the
    legacy ``/fetih <subcommand>`` form keeps working for anything that
    ...
    # Reserve /fetih as the catch-all top-level command.
    entries.append(("fetih", "Talk to FETIH or run a subcommand", "[subcommand] [args]"))
    seen.add("fetih")
...
def slack_app_manifest(request_url: str = "https://fetih-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /fetih handler.

    Maps both canonical names and aliases so /fetih bg do stuff works
    the same as /fetih background do stuff.

    Plugin-registered slash commands are included so ``/fetih <plugin-cmd>``
...
def _lmstudio_completion_models() -> list[str]:
    ...
    if not (os.environ.get("LM_API_KEY") or os.environ.get("LM_BASE_URL")):
        try:
            from fetih_cli.auth import _load_auth_store
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from fetih_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
