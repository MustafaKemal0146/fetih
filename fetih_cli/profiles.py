"""
Profile management for multiple isolated FETIH instances.

Each profile is a fully independent FETIH_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.fetih/profiles/<name>/`` by default.

The "default" profile is ``~/.fetih`` itself — backward compatible,
zero migration needed.

Usage::

    fetih profile create coder          # fresh profile + bundled skills
    fetih profile create coder --clone  # also copy config, .env, SOUL.md, skills
    fetih profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    fetih -p coder chat                 # or via flag
    fetih profile use coder             # set as sticky default
    fetih profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See fetih_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
# Marker file written by `fetih profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `fetih
...
# `fetih skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Two categories:
      1. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         FETIH infrastructure directories that only the default profile
         (``~/.fetih``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_fetih_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.fetih) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "fetih-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "fetih", "default", "test", "tmp", "root", "sudo",
})

# FETIH subcommands that cannot be used as profile names/aliases
_FETIH_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the fetih root, NOT to the current FETIH_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where FETIH_HOME points outside
    ``~/.fetih``, profiles live under ``FETIH_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_fetih_home() / "profiles"


def _get_default_fetih_home() -> Path:
    """Return the default (pre-profile) FETIH_HOME path.

    In standard deployments this is ``~/.fetih``.
    In Docker/custom deployments where FETIH_HOME is outside ``~/.fetih``
    (e.g. ``/opt/data``), returns FETIH_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_fetih_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the FETIH installation itself or a common system binary.  "
            f"Pick a different name."
        )


def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its FETIH_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_fetih_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, fetih subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
    if canon in _RESERVED_NAMES:
        return f"'{canon}' is a reserved name"
    if canon in _FETIH_SUBCOMMANDS:
        return f"'{canon}' conflicts with a fetih subcommand"
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``fetih profile list`` for an unrelated profile.
    """
...
# ``config.yaml``. ``config.yaml`` is the user-facing FETIH config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_fetih_home()
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``fetih update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.fetih)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.fetih/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Write the opt-out marker so seed_profile_skills() and `fetih update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`fetih profile create --no-skills`).\n"
                "Delete this file to re-enable sync on the next `fetih update`.\n",
                encoding="utf-8",
            )
        except OSError:
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `fetih profile describe`
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches FETIH_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``fetih profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "FETIH_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.fetih).\n"
            "To remove everything, use: fetih uninstall"
        )
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set FETIH_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("FETIH_HOME")
    try:
        os.environ["FETIH_HOME"] = str(profile_dir)
        from fetih_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["FETIH_HOME"] = old_home
        elif "FETIH_HOME" in os.environ:
            del os.environ["FETIH_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.fetih/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: fetih profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from FETIH_HOME.

    Returns ``"default"`` if FETIH_HOME is not set or points to ``~/.fetih``.
    Returns the profile name if FETIH_HOME points into ``~/.fetih/profiles/<name>``.
    Returns ``"custom"`` if FETIH_HOME is set to an unrecognized path.
    """
    from fetih_constants import get_fetih_home
    fetih_home = get_fetih_home()
    resolved = fetih_home.resolve()

    default_resolved = _get_default_fetih_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.fetih itself — its parent is ~/ and its
        # directory name is ".fetih", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: fetih profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.fetih itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.fetih). "
            "Specify a different name: fetih profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="fetih_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"fetih.{old_name}"
    new_host = f"fetih.{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_fetih_home() / "honcho.json",
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a FETIH_HOME path string.

    Called early in the CLI entry point, before any fetih modules
    are imported, to set the FETIH_HOME environment variable.
    """
