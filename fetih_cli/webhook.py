"""fetih webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    fetih webhook subscribe <name> [options]
    fetih webhook list
    fetih webhook remove <name>
    fetih webhook test <name> [--payload '{"key": "value"}']

Subscriptions persist to ~/.fetih/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _fetih_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: fetih gateway run
"""
...
def webhook_command(args):
    """Entry point for 'fetih webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: fetih webhook {subscribe|list|remove|test}")
        print("Run 'fetih webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (fetih gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: fetih webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from fetih webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (fetih gateway run)")
