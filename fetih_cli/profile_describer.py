...
Used by ``fetih profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``fetih_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the FETIH kanban board.
...
Rules:
  ...
  - Never write "FETIH profile" or other meta-narration.
    ...
    *,
    ...
    try:
        if canon == "default":
            ...
            profile_dir = Path(get_fetih_home())
        else:
