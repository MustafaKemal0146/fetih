...
The FETIH docs site hosts a JSON manifest of curated models for providers
we want to update without shipping a release (currently OpenRouter and
FETIH Portal). This module fetches, validates, and caches that manifest,
...
1. ``get_catalog()`` — returns a parsed manifest dict.
   ...
   - Reads disk cache at ``~/.fetih/cache/model_catalog.json``.
...
def _cache_path() -> Path:
    ...
    return get_fetih_home() / "cache" / "model_catalog.json"
...
def get_curated_nous_models() -> list[str] | None:
    """Return FETIH Portal's curated list of model ids from the manifest.
...
def reset_cache() -> None:
    """Clear the in-process cache. Used by tests and ``fetih model --refresh``."""
    global _catalog_cache, _catalog_cache_source_mtime
    _catalog_cache = None
