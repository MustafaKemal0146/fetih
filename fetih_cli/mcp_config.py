"""
MCP Server Management CLI — ``fetih mcp`` subcommand.

Implements ``fetih mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.fetih/config.yaml under the ``mcp_servers`` key.
"""
...
from fetih_cli.config import (
    ...
    get_env_value,
    save_env_value,
    get_fetih_home,  # noqa: F401 — used by test mocks
)
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from fetih_cli.cli_output import prompt as _shared_prompt
...
def cmd_mcp_add(args):
    ...
    # fetih_cli/main.py for why the dest is renamed.
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  fetih mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  fetih mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  fetih mcp add myserver --preset mypreset')
        return
            ...
            if auth_type == "header" or not auth_type:
                ...
                if existing_key:
                    ...
                    if api_key:
                        ...
                        _success(f"Saved to {display_fetih_home()}/.env as {env_key}")
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            _info("Fix the issue, then: fetih mcp test " + name)
        return
    ...
    print()
    _success(f"Saved '{name}' to {display_fetih_home()}/config.yaml ({tool_count}/{total} tools enabled)")
...
def cmd_mcp_remove(args):
    ...
    # earlier `fetih mcp test` in the same session) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  fetih mcp add <name> --url <endpoint>')
        _info('  fetih mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def cmd_mcp_login(args):
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `fetih mcp remove` + `fetih mcp add` to reconfigure auth.")
        return
...
def cmd_mcp_configure(args):
    ...
    if not _sys.stdin.isatty():
        print("Error: 'fetih mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``fetih mcp`` subcommands."""
    ...
    if handler:
        ...
        _info("fetih mcp serve                              Run as MCP server")
        _info("fetih mcp add <name> --url <endpoint>        Add an MCP server")
        _info("fetih mcp add <name> --command <cmd>         Add a stdio server")
        _info("fetih mcp add <name> --preset <preset>       Add from a known preset")
        _info("fetih mcp remove <name>                      Remove a server")
        _info("fetih mcp list                               List servers")
        _info("fetih mcp test <name>                        Test connection")
        _info("fetih mcp configure <name>                   Toggle tools")
        _info("fetih mcp login <name>                       Re-authenticate OAuth")
        print()
