"""Migrate FETIH' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads FETIH' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  FETIH mcp_servers.<n>.command/args/env  → codex stdio transport
  FETIH mcp_servers.<n>.url/headers       → codex streamable_http transport
  FETIH mcp_servers.<n>.timeout           → codex tool_timeout_sec
  FETIH mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  FETIH-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. FETIH' own
...
MIGRATION_MARKER = (
    "# managed by fetih-agent — `fetih codex-runtime migrate` regenerates this section"
)
MIGRATION_END_MARKER = (
    "# end fetih-agent managed section"
)
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in FETIH config.")
...
# FETIH keys that codex's MCP schema doesn't support — dropped during
...
_KEYS_DROPPED_WITH_WARNING = {
    # FETIH' sampling subsection — codex MCP has no equivalent
    "sampling",
}


def _translate_one_server(
    name: str, fetih_cfg: dict
) -> tuple[Optional[dict], list[str]]:
    """Translate one FETIH MCP server config to the codex inline-table dict
    ...
    if not isinstance(fetih_cfg, dict):
        return None, []

    skipped: list[str] = []
    out: dict[str, Any] = {}

    has_command = bool(fetih_cfg.get("command"))
    has_url = bool(fetih_cfg.get("url"))
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (FETIH_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by FETIH)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert FETIH' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before FETIH' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior FETIH
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="fetih-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a FETIH_HOME pointing
    at one of those paths is burned into ``~/.codex/config.toml``, every
    codex-routed fetih-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.fetih`` symlink that happens to live under ``/private/var/folders``
...
def _build_fetih_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches FETIH' own
    ...
    so a fetih installed under /opt/, /usr/local/, or a venv all work.
    FETIH_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # FETIH_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_fetih_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever FETIH_HOME its launcher sets
    ...
    # — that would override the launcher's FETIH_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("FETIH_HOME", tmp_path) would
    ...
    fetih_home = os.environ.get("FETIH_HOME") or ""
    if fetih_home and _looks_like_test_tempdir(fetih_home):
        fetih_home = ""
    if fetih_home:
        env["FETIH_HOME"] = fetih_home
    # PYTHONPATH passes through so a worktree-launched fetih finds the
    ...
    env["FETIH_QUIET"] = "1"
    env["FETIH_REDACT_SECRETS"] = env.get("FETIH_REDACT_SECRETS", "true")

    out: dict[str, Any] = {
        "command": sys.executable,
        "args": ["-m", "agent.transports.fetih_tools_mcp_server"],
    }
    if env:
        out["env"] = env
...
def migrate(
    ...
    *,
    ...
    expose_fetih_tools: bool = True,
) -> MigrationReport:
    """Translate FETIH mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        fetih_config: full ~/.fetih/config.yaml dict
        ...
        expose_fetih_tools: when True (default), register FETIH' own
            tool surface (web_search, browser_*, delegate_task, vision,
            memory, skills, etc.) as an MCP server in ~/.codex/config.toml
            so the codex subprocess can call back into FETIH for tools
    ...
    fetih_servers = (fetih_config or {}).get("mcp_servers") or {}
    if not isinstance(fetih_servers, dict):
        report.errors.append(
            "mcp_servers in FETIH config is not a dict; cannot migrate."
        )
        return report

    translated: dict[str, dict] = {}
    for name, cfg in fetih_servers.items():
    ...
    # Inject FETIH' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into FETIH for the tools codex
    # doesn't ship with — web_search, browser_*, delegate_task, vision,
    # memory, skills, session_search, image_generate, text_to_speech.
    # The server itself is agent/transports/fetih_tools_mcp_server.py
    # and is launched on demand by codex (stdio MCP).
    if expose_fetih_tools:
        translated["fetih-tools"] = _build_fetih_tools_mcp_entry()
        if "fetih-tools" not in report.migrated:
            report.migrated.append("fetih-tools")
