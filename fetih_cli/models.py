...
Add, remove, or reorder entries here — both `fetih setup` and
`fetih` provider-selection will pick up the change automatically.
"""
...
from fetih_cli import __version__ as _FETIH_VERSION

# Identify ourselves so endpoints fronted by Cloudflare's Browser Integrity
# Check (error 1010) don't reject the default ``Python-urllib/*`` signature.
_FETIH_USER_AGENT = f"fetih-cli/{_FETIH_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `fetih model`
    flow without maintaining a separate static list.
    """
    from fetih_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $FETIH_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $FETIH_HOME/models_dev_cache.json directly (no network) so this
    runs at import time without blocking. Falls back to ``_XAI_STATIC_FALLBACK``
    when the cache is empty or unreadable. FETIH refreshes the cache from
...
# ---------------------------------------------------------------------------
# FETIH Portal free-model helper
# ---------------------------------------------------------------------------
# The FETIH Portal models endpoint is the source of truth for which models
...
# ---------------------------------------------------------------------------
# FETIH Portal account tier detection
# ---------------------------------------------------------------------------

def fetch_nous_account_tier(access_token: str, portal_base_url: str = "") -> dict[str, Any]:
    """Fetch the user's FETIH Portal account/subscription info.
    ...
    """
    base = (portal_base_url or "https://github.com/MustafaKemal0146/fetih").rstrip("/")
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older FETIH that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older FETIH that doesn't ship
...
def check_nous_free_tier() -> bool:
    """Check if the current FETIH Portal user is on a free (unpaid) tier.
    ...
    try:
        from fetih_cli.auth import get_provider_auth_state, resolve_nous_runtime_credentials
...
# ---------------------------------------------------------------------------
# FETIH Portal recommended models
#
...
def fetch_nous_recommended_models(
    ...
    *,
    ...
    """Fetch the FETIH Portal's curated recommended-models payload.
...
def _resolve_nous_portal_url() -> str:
    ...
    try:
        ...
        return "https://github.com/MustafaKemal0146/fetih"
...
# this list:  fetih model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `fetih model` interactive picker
# ---------------------------------------------------------------------------
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model a user would be offered first in the ``fetih model`` picker.

    Used as a fallback when the user has configured a provider but never
    selected a model (e.g. ``fetih auth add openai-codex`` without
    ``fetih model``).
    """
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    fetih-agent is tool-calling-first — every provider path assumes the model
    ...
    **Permissive when the field is missing.** Some OpenRouter-compatible gateways
    (FETIH Portal, private mirrors, older catalog snapshots) don't populate
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from fetih_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — fetih-agent
...
def get_curated_nous_model_ids() -> list[str]:
    """Return the curated FETIH Portal model-id list.
    ...
    """
    try:
        from fetih_cli.model_catalog import get_curated_nous_models
...
def fetch_ai_gateway_models(
    ...
    *,
    ...
    from fetih_constants import AI_GATEWAY_BASE_URL
...
def fetch_models_with_pricing(
    ...
    *,
    ...
    Works with any OpenRouter-compatible endpoint (OpenRouter, FETIH Portal).
    """
...
def fetch_ai_gateway_pricing(
    ...
    *,
    ...
    """Fetch Vercel AI Gateway /v1/models and return fetih-shaped pricing.

    Vercel uses ``input`` / ``output`` field names; fetih's picker expects
...
_DEFAULT_NOUS_INFERENCE_BASE = "https://inference-api.github.com/MustafaKemal0146/fetih"


def _resolve_nous_pricing_credentials() -> tuple[str, str]:
    """Return ``(api_key, base_url)`` for FETIH Portal pricing.
    ...
    """
    try:
        from fetih_cli.auth import resolve_nous_runtime_credentials
...
def get_pricing_for_provider(provider: str, *, force_refresh: bool = False) -> dict[str, dict[str, str]]:
    ...
    if normalized == "nous":
        ...
        if base_url:
            # Nous base_url typically looks like https://inference-api.github.com/MustafaKemal0146/fetih/v1
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``fetih model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from fetih_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def _resolve_static_model_alias(
    ...
    try:
        from fetih_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to FETIH' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``fetih_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether FETIH should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``fetih auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.fetih/.env``.
    ...
    """
    try:
        from fetih_cli.auth import resolve_api_key_provider_credentials
    ...
    try:
        from fetih_cli.auth import read_credential_pool
        from fetih_cli.copilot_auth import (
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a FETIH release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a FETIH release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "nous":
        # Try live FETIH Portal /models endpoint
        try:
            from fetih_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
...
def copilot_default_headers() -> dict[str, str]:
    ...
    """
    try:
        from fetih_cli.copilot_auth import copilot_request_headers
        return copilot_request_headers(is_agent_turn=True)
    except ImportError:
        return {
            "Editor-Version": COPILOT_EDITOR_VERSION,
            "User-Agent": "FETIHAgent/1.0",
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _FETIH_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: FETIH' default Claude IDs elsewhere use
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _FETIH_USER_AGENT}
...
def _ollama_cloud_cache_path() -> Path:
    ...
    return get_fetih_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"FETIH will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so FETIH cannot verify the model name."
