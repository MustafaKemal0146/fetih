"""
Dump command for fetih CLI.

Outputs a compact, plain-text summary of the user's FETIH setup
...
from fetih_cli.config import get_fetih_home, get_env_path, get_project_root, load_config
...
def _redact(value: str) -> str:
    ...
    ``fetih dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from fetih_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(fetih_home: Path) -> int:
...
def _cron_summary(fetih_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = fetih_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"fetih_home:      {display_fetih_home()}")
    ...
    toolsets = config.get("toolsets", ["fetih-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(fetih_home)}")
    lines.append(f"  skills:             {_count_skills(fetih_home)}")
