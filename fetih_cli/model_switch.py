...
- ``fetih_cli.providers``        -- canonical provider identity + overlays
- ``fetih_cli.model_normalize``  -- per-provider name formatting
...
_FETIH_MODEL_WARNING = (
    "FETIH FETIH 3 & 4 models are NOT agentic and are not designed "
    "for use with FETIH. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real FETIH FETIH 3 / FETIH 4 chat families.
# The previous substring check (`"fetih" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``fetih-brain:qwen3-14b-ctx16k`` that just
# happen to carry "fetih" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   NousResearch/Hermes-3-Llama-3.1-70B, hermes-4-405b, openrouter/hermes3:70b
# Negative examples it must NOT match:
#   fetih-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])fetih[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)


def is_nous_fetih_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous FETIH 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_NON_AGENTIC_RE.search(model_name))


def _check_fetih_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous FETIH 3/4 chat model."""
    if is_nous_fetih_non_agentic(model_name):
        return _FETIH_MODEL_WARNING
    return ""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``fetih config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from fetih_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'fetih model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'fetih doctor' — config issues detected:"
    ...
    # Mirrors the same logic in fetih_cli.runtime_provider.resolve_runtime_provider;
    ...
    fetih_warn = _check_fetih_model_warning(new_model)
    if fetih_warn:
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from fetih_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from fetih_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by fetih provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "nous" pulls from the remote model-catalog manifest published at
    # https://github.com/MustafaKemal0146/fetih/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a FETIH release. Falls back to the in-repo
    ...
    # --- 1. Check FETIH-mapped providers ---
    for fetih_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        pconfig = PROVIDER_REGISTRY.get(fetih_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (FETIH_OVERLAYS) with proper auth store checking.
        ...
        if not has_creds:
            try:
                ...
                if store and fetih_id in store.get("credential_pool", {}):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
        ...
        # show up in the picker without requiring a FETIH release.
        model_ids = curated.get(fetih_id, [])
        if fetih_id in _MODELS_DEV_PREFERRED:
            model_ids = _merge_with_models_dev(fetih_id, model_ids)
        total = len(model_ids)
    ...
    # --- 2. Check FETIH-only providers (nous, openai-codex, copilot, opencode-go) ---
    from fetih_cli.providers import FETIH_OVERLAYS
    from fetih_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → FETIH provider ID.
    # FETIH_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use FETIH IDs ("copilot").
    _mdev_to_fetih = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in FETIH_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve FETIH slug — e.g. "github-copilot" → "copilot"
        fetih_slug = _mdev_to_fetih.get(pid, pid)
        if fetih_slug.lower() in seen_slugs:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(fetih_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, fetih_slug):
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or fetih_slug in providers_store):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(fetih_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", fetih_slug, exc)
        ...
        # configured.
        if not has_creds and fetih_slug == "anthropic":
            try:
                from agent.anthropic_adapter import (
                    read_claude_code_credentials,
                    read_fetih_oauth_credentials,
                )
                fetih_creds = read_fetih_oauth_credentials()
                cc_creds = read_claude_code_credentials()
                if (fetih_creds and fetih_creds.get("accessToken")) or \
        ...
        if fetih_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = provider_model_ids(fetih_slug)
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            try:
                ...
                model_ids = _ids if _ids is not None else (curated.get(fetih_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(fetih_slug, []) or curated.get(pid, [])
        else:
            # Use curated list — look up by FETIH slug, fall back to overlay key
            model_ids = curated.get(fetih_slug, []) or curated.get(pid, [])
            # Merge with models.dev for preferred providers (same rationale as above).
            if fetih_slug in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(fetih_slug, model_ids)
        total = len(model_ids)
        top = model_ids[:max_models]

        results.append({
            "slug": fetih_slug,
            "name": get_label(fetih_slug),
            "is_current": fetih_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "fetih",
        ...
        seen_slugs.add(fetih_slug.lower())
        _record_builtin_endpoint(fetih_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or FETIH_OVERLAYS (keeps /model in sync
    # with `fetih model`).
    try:
        from fetih_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is FETIH's canonical write key (matches
            ...
            # FETIH writes ``models:`` as a dict keyed by model id
            # (see fetih_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # FETIH's own writer uses; a hyphen variant is accepted
            ...
            # active model. FETIH's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, fetih_cli/config.py) already consume that dict.
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`fetih_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from fetih_cli.models import fetch_openrouter_models
