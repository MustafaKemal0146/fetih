"""
Backup and import commands for fetih CLI.

`fetih backup` creates a zip archive of the entire ~/.fetih/ directory
(excluding the fetih-agent repo and transient files).

`fetih import` restores from a backup zip, overlaying onto the current
FETIH_HOME root.
"""
...
from fetih_constants import get_default_fetih_root, get_fetih_home, display_fetih_home
...
_EXCLUDED_DIRS = {
    "fetih-agent",     # the codebase repo — re-clone instead
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to fetih root) should be skipped."""
...
def run_backup(args) -> None:
    """Create a zip backup of the FETIH home directory."""
    fetih_root = get_default_fetih_root()

    if not fetih_root.is_dir():
        print(f"Error: FETIH home directory not found at {fetih_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"fetih-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"fetih-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_fetih_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(fetih_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(fetih_root)
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(fetih_root)

            if _should_exclude(rel):
                continue

            # Skip the output zip itself if it happens to be inside fetih root
            try:
    ...
    print(f"\nRestore with: fetih import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a FETIH backup.
    ...
    # Look for telltale files that a fetih home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a FETIH backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.fetih/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a fetih dir name
        if prefix in {".fetih", "fetih"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_fetih_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (fetih_root / "config.yaml").exists()
        has_env = (fetih_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has FETIH configuration.")
        ...
        fetih_root.mkdir(parents=True, exist_ok=True)
        ...
        for member in members:
            ...
            target = fetih_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(fetih_root.resolve())
            except ValueError:
        ...
        profiles_dir = fetih_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # fetih_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: fetih profile list  (after installing fetih)")

        # Guidance
        print()
        if not (fetih_root / "fetih-agent").is_dir():
            print("Note: The fetih-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: fetih update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  fetih -p {pname} gateway install")

        print("Done. Your FETIH configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and fetih backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to FETIH_HOME).
...
# — `fetih update` snapshots this set before pulling so approved-user lists
...
def _quick_snapshot_root(fetih_home: Optional[Path] = None) -> Path:
    home = fetih_home or get_fetih_home()
    return home / _QUICK_SNAPSHOTS_DIR


def create_quick_snapshot(
    label: Optional[str] = None,
    fetih_home: Optional[Path] = None,
) -> Optional[str]:
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(fetih_home)
    if not root.exists():
        return []
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(fetih_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for fetih backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_fetih_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, fetih_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``fetih_root`` to ``out_path``.
...
def _pre_update_backup_dir(fetih_home: Optional[Path] = None) -> Path:
    home = fetih_home or get_fetih_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of FETIH_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<FETIH_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``fetih update``) should continue even if the backup fails.
    """
    fetih_root = fetih_home or get_default_fetih_root()
    if not fetih_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(fetih_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, fetih_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `fetih claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of FETIH_HOME under ``backups/`` before a
    ``fetih claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``fetih import <archive>``.  Writes to
    ``<FETIH_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `fetih import` and the
