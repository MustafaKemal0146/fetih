...
- Keep data: Remove code but keep ~/.fetih/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove FETIH PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing fetih-agent or fetih PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# FETIH" comment and following line
                if '# FETIH' in line or '# fetih-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('fetih' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing fetih
                if 'fetih' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the fetih wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "fetih",
        Path("/usr/local/bin/fetih"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains fetih_cli reference)
                content = wrapper.read_text()
                if 'fetih_cli' in content or 'fetih-agent' in content:
                    wrapper.unlink()
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``fetih gateway run`` processes
    ...
    try:
        from fetih_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from fetih_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``FETIH_HOME`` and ``FETIH_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\fetih\git\cmd``, ``%LOCALAPPDATA%\fetih\git\bin``,
#      ``%LOCALAPPDATA%\fetih\git\usr\bin``, ``%LOCALAPPDATA%\fetih\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\fetih\git\`` and Node to
#      ``%LOCALAPPDATA%\fetih\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``fetih dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\fetih\gateway-service\`` and sometimes
...
def _fetih_path_markers(fetih_home: Path) -> list[str]:
    """Path-entry substrings that identify FETIH-owned User-PATH entries."""
    root = str(fetih_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare fetih-agent install dir.
    markers = [root + "\\fetih-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if FETIH_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "fetih".  We don't want to catch
    # unrelated entries like "cfetih-foo" or "ephermeral", so we look for
    # backslash-fetih as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(fetih_home: Path) -> list[str]:
    """Strip FETIH-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _fetih_path_markers(fetih_home)
            kept: list[str] = []
...
def remove_fetih_env_vars_windows() -> list[str]:
    """Delete FETIH_HOME and FETIH_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("FETIH_HOME", "FETIH_GIT_BASH_PATH"):
                try:
...
def _is_default_fetih_home(fetih_home: Path) -> bool:
    """Return True when ``fetih_home`` points at the default (non-profile) root."""
    try:
        from fetih_constants import get_default_fetih_root
        return fetih_home.resolve() == get_default_fetih_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its FETIH_HOME directory.

    We shell out to ``fetih -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current FETIH_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m fetih_cli.main` so we don't depend on a `fetih`
    #    wrapper that may be half-removed mid-uninstall.
    fetih_invocation = [_sys.executable, "-m", "fetih_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                fetih_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's FETIH_HOME directory.
    try:
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.fetih/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.fetih/ for future reinstall
    """
    project_root = get_project_root()
    fetih_home = get_fetih_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie FETIH_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_fetih_home(fetih_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ FETIH Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {fetih_home / 'config.yaml'}")
    print(f"  Secrets: {fetih_home / '.env'}")
    print(f"  Data:    {fetih_home / 'cron/'}, {fetih_home / 'sessions/'}, {fetih_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their FETIH_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL FETIH data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the FETIH code but keep your configuration and data.")
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in fetih_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\fetih\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(fetih_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No FETIH-owned PATH entries in User environment")

        log_info("Removing FETIH_HOME / FETIH_GIT_BASH_PATH User env vars...")
        removed_env = remove_fetih_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No FETIH-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing fetih command...")
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.fetih/, just remove the fetih-agent subdir
            if fetih_home in project_root.parents or project_root.parent == fetih_home:
    ...
    #     under FETIH_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(fetih_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(fetih_home)
    ...
    # 5. Optionally remove ~/.fetih/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile FETIH_HOME dirs live under
        ...
        try:
            if fetih_home.exists():
                shutil.rmtree(fetih_home)
                log_success(f"Removed {fetih_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {fetih_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {fetih_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  irm https://raw.githubusercontent.com/MustafaKemal0146/fetih/main/scripts/install.ps1 | iex", Colors.DIM))
        else:
            print(color("  curl -fsSL https://raw.githubusercontent.com/MustafaKemal0146/fetih/main/scripts/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using FETIH! ⚕")
    print()
