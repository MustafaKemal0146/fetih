"""
Doctor command for fetih CLI.

Diagnoses issues with FETIH setup.
"""
...
from fetih_cli.config import get_project_root, get_fetih_home, get_env_path
...
_DHH = display_fetih_home()  # user-facing display path (e.g. ~/.fetih or ~/.fetih/profiles/coder)

# Load environment variables from ~/.fetih/.env so API key checks work
_env_path = get_env_path()
load_fetih_dotenv(fetih_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from fetih_cli.colors import Colors, color
from fetih_cli.models import _FETIH_USER_AGENT
from fetih_cli.vercel_auth import describe_vercel_auth
from fetih_constants import OPENROUTER_MODELS_URL
...
from fetih_constants import is_termux as _is_termux
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.fetih/.env contains provider auth/base URL settings."""
...
def _is_kanban_worker_env_gate(item: dict) -> bool:
    ...
    if item.get("name") != "kanban":
        return False
    if os.environ.get("FETIH_KANBAN_TASK"):
        return False
...
def _doctor_tool_availability_detail(toolset: str) -> str:
    ...
    if toolset == "kanban" and not os.environ.get("FETIH_KANBAN_TASK"):
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized in {"google / gemini", "gemini"}:
        try:
            from fetih_cli.auth import get_gemini_oauth_auth_status
            return bool((get_gemini_oauth_auth_status() or {}).get("logged_in"))
        except Exception:
            return False
    if normalized == "minimax":
        try:
            from fetih_cli.auth import get_minimax_oauth_auth_status
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from fetih_cli.providers import normalize_provider as _normalize_provider
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `fetih`.
    os.environ.setdefault("FETIH_INTERACTIVE", "1")

    # Handle `fetih doctor --ack <id>` as a fast path. Persist the ack and
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.fetih/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
    ...
    print(color("│                 🩺 FETIH Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`fetih doctor --ack {hit.advisory.id}`."
                )
    ...
    # Check ~/.fetih/.env (primary location for user config)
    env_path = FETIH_HOME / '.env'
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'fetih setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'fetih setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'fetih setup' to create one")
                issues.append("Run 'fetih setup' to create .env")
    
    # Check ~/.fetih/config.yaml (primary) or project cli-config.yaml (fallback)
    config_path = FETIH_HOME / 'config.yaml'
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from fetih_cli.config import get_compatible_custom_providers as _compatible_custom_providers
                from fetih_cli.providers import (
                ...
                if catalog_provider is None or (
                    ...
                    _fail_and_issue(
                        ...
                        (
                            ...
                            f"Fix: run 'fetih config set model.provider <valid_provider>'"
                        ),
                        issues,
                    )
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from fetih_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.fetih/.env or run 'fetih setup')",
                            (
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'fetih setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'fetih config set model.provider <name>'"
                            ),
                            issues,
                        )
                except Exception:
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from fetih_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
        try:
            from fetih_cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'fetih setup' to migrate config")
                else:
                    issues.append("Run 'fetih doctor --fix' or 'fetih setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'fetih doctor --fix'")
        except Exception:
            pass
    ...
    try:
        ...
        if nous_status.get("logged_in"):
            check_ok("FETIH Portal auth", "(logged in)")
        else:
            check_warn("FETIH Portal auth", "(not logged in)")
    ...
    if _safe_which("codex"):
        ...
        # Native OAuth uses FETIH' own device-code flow — the Codex CLI is
        # only needed if you want to import existing tokens from
        # ~/.codex/auth.json.  Downgrade to info so users running
        # `fetih auth openai-codex` aren't told they're missing something.
        check_info(
    ...
    fetih_home = FETIH_HOME
    if fetih_home.exists():
        check_ok(f"{_DHH} directory exists")
    elif should_fix:
        fetih_home.mkdir(parents=True, exist_ok=True)
    ...
    for subdir_name in expected_subdirs:
        subdir_path = fetih_home / subdir_name
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give FETIH a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# FETIH Kimlik Tanımı\n\n"
                "Sen FETIH'sin — CTF çözme, OSINT operasyonları ve siber güvenlik analizinde "
                "uzmanlaşmış otonom bir yapay zeka ajanısın. "
    ...
    memories_dir = fetih_home / "memories"
    ...
    state_db_path = fetih_home / "state.db"
    ...
    wal_path = fetih_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'fetih doctor --fix' to checkpoint")
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "fetih"
        ...
        _cmd_link = _cmd_link_dir / "fetih"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(fetih not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            manual_issues.append(
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/fetih → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/fetih points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/fetih → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/fetih — run 'fetih doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/fetih exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/fetih not found",
                    "(fetih command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/fetih → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/fetih symlink — run 'fetih doctor --fix'")
    ...
    if terminal_env == "vercel_sandbox":
        ...
        if importlib.util.find_spec("vercel") is not None:
            ...
            _fail_and_issue(
                ...
                "(pip install 'fetih-agent[vercel]')",
                "Install the Vercel optional dependency: pip install 'fetih-agent[vercel]'",
                issues,
            )
    ...
    if _safe_which("node"):
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `fetih doctor` invocation.
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     "Fix: run 'fetih config set model.provider <provider>' "
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            # GOOGLE_API_KEY/GEMINI_API_KEY always shows red in ``fetih doctor``.
    ...
    # never the legitimate source for `fetih doctor`.
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'fetih setup' to configure missing API keys for full tool access")
    except Exception as e:
        check_warn("Could not check tool availability", f"({e})")
    
    _section("Skills Hub")
    hub_dir = FETIH_HOME / "skills" / ".hub"
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: fetih skills list)")
    ...
    try:
        import yaml as _yaml
        _mem_cfg_path = FETIH_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                check_warn("Honcho config not found", "run: fetih memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                _fail_and_issue(
                    "Honcho API key or base URL not set",
                    "run: fetih memory setup",
                    "No Honcho API key — run 'fetih memory setup'",
                    issues,
                )
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                _fail_and_issue(
                    ...
                    "(set MEM0_API_KEY in .env or run fetih memory setup)",
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: fetih memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: fetih memory setup")
        except Exception as _e:
            check_warn(f"{_active_memory_provider} check failed", str(_e))

    try:
        from fetih_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
            ...
            if wrapper_dir.is_dir():
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "fetih -p" in content:
                            _m = _re.search(r"fetih -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'fetih doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
