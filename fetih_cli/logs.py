"""``fetih logs`` — view and filter FETIH log files.
...
Usage examples::
    ...
    fetih logs                    # last 50 lines of agent.log
    fetih logs -f                 # follow agent.log in real time
    fetih logs errors             # last 50 lines of errors.log
    fetih logs gateway -n 100    # last 100 lines of gateway.log
    fetih logs --level WARNING    # only WARNING+ lines
    fetih logs --session abc123   # filter by session ID substring
    fetih logs --component tools  # only tool-related lines
    fetih logs --since 1h         # lines from the last hour
    fetih logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_fetih_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when FETIH runs — try 'fetih chat' first)")
        sys.exit(1)
    ...
    if component:
        from fetih_logging import COMPONENT_PREFIXES
    ...
    if follow:
        print(f"--- {display_fetih_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_fetih_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_fetih_home()}/logs/")
        return

    print(f"Log files in {display_fetih_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'fetih chat' to generate logs)")
