"""
Unified tool configuration for FETIH.

`fetih tools` and `fetih setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.fetih/config.yaml under
...
from fetih_cli.nous_subscription import (
...
from fetih_cli.cli_output import (  # noqa: E402 — late import block
...
# They're still in _FETIH_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
#
# Video gen is off by default — it's a niche, paid, slow feature. Users
# who want it opt in via `fetih tools` → Video Generation, which walks
...
# `fetih tools` → X (Twitter) Search setup walks users through credential
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from fetih_cli.auth import _read_xai_oauth_tokens
...
# Platform-scoped toolsets: only appear in the `fetih tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``fetih tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from fetih_cli.plugins import discover_plugins, get_plugin_toolsets
...
from fetih_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
TOOL_CATEGORIES = {
    ...
    "x_search": {
        ...
        "setup_note": (
            "FETIH routes X searches through xAI's built-in x_search "
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_fetih_home()}/fetih-agent && npm install")
                if result.stderr:
                    _print_info(f"      {result.stderr.strip()[:200]}")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in fetih-agent directory)")
            return
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/MustafaKemal0146/fetih:latest"
            )
            return
        ...
        if not camofox_dir.exists() and _npm_bin:
            ...
            # package.json so that `fetih update` does not silently pull
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.fetih/config.yaml")
        ...
        # Run the full `fetih auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.fetih/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from fetih_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: fetih auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `fetih auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: fetih auth spotify")
        except Exception as exc:
        ...
        try:
            ...
            _print_info("    Run later: fetih auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `fetih auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: fetih auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: fetih auth add xai-oauth"
                )
        else:
...
def _get_platform_tools(
    ...
    *,
    ...
    # "fetih-cli" (which include all _FETIH_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[fetih-cli, spotify]`` after enabling Spotify via ``fetih
        ...
        # (e.g. "fetih-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
        ...
        # through ``fetih tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `fetih tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"fetih-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("fetih-")}
    ...
    # shipped as a bundled plugin but user must opt in via `fetih tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `fetih tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by fetih tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (fetih-cli, fetih-telegram, etc.)
    ...
    # Opening `fetih tools` is the user's opt-in to reconfigure tools, so treat
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `fetih tools` gets a silent no-op
...
def _configure_provider(provider: dict, config: dict):
    ...
    if provider.get("requires_nous_auth"):
        ...
        if not features.nous_auth_present:
            _print_warning("  Nous Subscription is only available after logging into FETIH Portal.")
            return
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `fetih tools` and `fetih setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ FETIH Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://github.com/MustafaKemal0146/fetih/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    print()
    from fetih_constants import display_fetih_home
    print(color(f"  Tool configuration saved to {display_fetih_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'fetih' or gateway restart.", Colors.DIM))
    print()
