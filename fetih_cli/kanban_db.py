...
``<root>`` is the **shared FETIH root** (the parent of any active
...
``fetih -p <profile>`` joins the same board as the dispatcher that
...
the profile's FETIH home but are otherwise isolated: a worker spawned
...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``FETIH_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``FETIH_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``fetih kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.fetih``. In Docker / custom
deployments where ``FETIH_HOME`` points outside ``~/.fetih`` (e.g.
``/opt/fetih``), ``<root>`` is ``FETIH_HOME``. Legacy env-var
overrides still work:

* ``FETIH_KANBAN_DB`` — pin the database file path directly.
* ``FETIH_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``FETIH_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``FETIH_KANBAN_DB``,
``FETIH_KANBAN_WORKSPACES_ROOT``, and ``FETIH_KANBAN_BOARD`` into
...
coding workloads.  See ``docs/fetih-kanban-v1-spec.pdf`` for the full
...
# enough that kebab-case names like ``atm10-server`` or ``fetih-agent``
...
def kanban_home() -> Path:
    """Return the shared FETIH root that anchors the kanban board.

    Resolution order:

    1. ``FETIH_KANBAN_HOME`` env var when set and non-empty (explicit
       override for tests and unusual deployments).
    2. ``get_default_fetih_root()``, which already returns ``<root>``
       when ``FETIH_HOME`` is ``<root>/profiles/<name>``, and returns
       ``FETIH_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``FETIH_HOME`` would silently fork the board per profile,
    which breaks the dispatcher / worker handoff.
    """
    override = os.environ.get("FETIH_KANBAN_HOME", "").strip()
    if override:
        return Path(override).expanduser()
    from fetih_constants import get_default_fetih_root
    return get_default_fetih_root()
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``fetih kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    1. ``FETIH_KANBAN_BOARD`` env var (set by the dispatcher on worker
       spawn, or manually for ad-hoc overrides).
    2. ``<root>/kanban/current`` on disk (set by ``fetih kanban boards
    ...
    """
    env = os.environ.get("FETIH_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``fetih kanban boards switch <typo>`` returns an error
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``FETIH_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    """
    override = os.environ.get("FETIH_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``FETIH_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
    ...
    """
    override = os.environ.get("FETIH_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``fetih kanban log`` unambiguous even when multiple
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``FETIH_KANBAN_DB`` env → ``FETIH_KANBAN_BOARD`` env →
    ...
    # See fetih_state._WAL_INCOMPAT_MARKERS for detection logic.
    from fetih_state import apply_wal_with_fallback
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``fetih kanban init`` and the
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from fetih_cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    each name to ``fetih --skills ...`` alongside the built-in
    ...
    # `fetih --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `fetih kanban recompute` (issue #22459).
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``fetih kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``fetih kanban complete <ready-task>
...
def complete_task(
    ...
    *,
    ...
    completion (``fetih kanban complete <id>``) works without requiring
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a FETIH
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``fetih kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real FETIH profile.
    ...
    if not rows:
        return False
    try:
        from fetih_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
    ...
    *,
    ...
    for row in ready_rows:
        ...
        if not row["assignee"]:
            ...
            continue
        # Skip ready tasks whose assignee is not a real FETIH profile.
        # `_default_spawn` invokes ``fetih -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a FETIH
...
def _resolve_fetih_argv() -> list[str]:
    """Resolve the ``fetih`` invocation as argv parts for ``Popen``.

    Tries in order:

    1. ``shutil.which("fetih")`` — the console-script shim, the same form
       that shows up in ``ps`` output and existing logs. Preferred so live
       systems' diagnostics stay familiar.
    2. ``sys.executable -m fetih_cli.main`` — fallback for setups where
       FETIH is launched from a venv and the ``fetih`` shim is not on
       the dispatcher's ``$PATH`` (cron, systemd ``User=`` services,
       launchd jobs, detached processes, etc.). Goes through the running
       interpreter so the result is independent of ``$PATH``.

    Mirrors ``gateway.run._resolve_fetih_bin`` for the same reason. Kept
    local (not imported from gateway) because ``fetih_cli`` sits below
    ...
    if fetih_bin:
        return [fetih_bin]
    # Fallback to the module form. ``fetih_cli.main`` is the actual
    # console-script target declared in pyproject.toml, NOT a top-level
    # ``fetih`` package — there is no ``fetih`` package to import.
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``fetih -p <profile> chat -q ...`` subprocess.
    ...
    ``FETIH_KANBAN_DB`` / ``FETIH_KANBAN_BOARD`` / workspaces_root env
    ...
    # Inject FETIH_HOME so the worker reads the profile-scoped config.yaml
    # (fallback_providers, toolsets, agent settings, etc.) instead of the root
    # config.  Without this, `env = dict(os.environ)` copies only the parent's
    # env, and when the child process starts `fetih -p <name>` the
    # _apply_profile_override() runs *before* fetih_constants is imported.
    # If FETIH_HOME is absent from the child's env, get_fetih_home() falls
    # back to Path.home() / ".fetih" (the DEFAULT profile root), ignoring the
    # profile-specific config entirely.  Fixes profile-scoped fallback_providers
    # being invisible to kanban workers.
    from fetih_cli.profiles import resolve_profile_env
    try:
        env["FETIH_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via FETIH_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # FETIH_HOME never had profiles created.
        pass
    if task.tenant:
        env["FETIH_TENANT"] = task.tenant
    env["FETIH_KANBAN_TASK"] = task.id
    env["FETIH_KANBAN_WORKSPACE"] = workspace
    if task.current_run_id is not None:
        env["FETIH_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["FETIH_KANBAN_CLAIM_LOCK"] = task.claim_lock
    ...
    # that even when the worker activates a profile (`fetih -p <name>`
    # rewrites FETIH_HOME), its kanban paths still match the
    # dispatcher's. Belt-and-braces with the `get_default_fetih_root()`
    # resolution in `kanban_home()` — symmetric resolution is the norm,
    # but unusual symlink / Docker layouts are caught here too.
    env["FETIH_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["FETIH_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    # Board slug — the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["FETIH_KANBAN_BOARD"] = resolved_board
    # FETIH_PROFILE is the author the kanban_comment tool defaults to.
    # `fetih -p <assignee>` activates the profile, but the env var is
    # what the tool reads — set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["FETIH_PROFILE"] = profile_arg
    ...
    # `fetih kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`fetih` executable not found on PATH. "
            "Install FETIH or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``fetih kanban daemon`` is systemd-friendly.
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled FETIH_PROFILE values like "fetih-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default FETIH root exists

    Reads profile paths directly so this module has no import dependency on
    ``fetih_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from fetih_constants import get_default_fetih_root
        default_root = get_default_fetih_root()
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``fetih kanban assignees`` for the terminal.
