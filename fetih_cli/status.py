"""
Status command for fetih CLI.

Shows the status of all FETIH components.
"""
...
from fetih_cli.config import get_env_path, get_env_value, get_fetih_home, load_config
from fetih_cli.models import provider_label
from fetih_cli.nous_subscription import get_nous_subscription_features
from fetih_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``fetih config``'s
...
def show_status(args):
    """Show status of all FETIH components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                    ⚔ FETIH Status                      │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    nous_label = "logged in" if nous_logged_in else "not logged in (run: fetih auth add nous --type oauth)"
    print(
        f"  {'FETIH Portal':<12}  {check_mark(nous_logged_in)} "
        f"{nous_label}"
    )
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: fetih model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: fetih auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: fetih auth add xai-oauth)'}"
    )
    ...
    if managed_nous_tools_enabled():
        ...
        if not features.nous_auth_present:
            print("  FETIH Portal   ✗ not logged in")
        else:
            print("  FETIH Portal   ✓ managed tools available")
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: fetih model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from fetih_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    if terminal_env == "ssh":
        ...
        sdk_label = "installed" if sdk_ok else "missing (install: pip install 'fetih[vercel]')"
    ...
    try:
        from fetih_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   fetih gateway")
    ...
    jobs_file = get_fetih_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_fetih_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'fetih doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'fetih setup' to configure", Colors.DIM))
    print()
