...
configured for "cli" in `fetih tools`.
Rules / memory / AGENTS.md / preloaded skills = same as a normal chat turn.
Approvals = auto-bypassed (FETIH_YOLO_MODE=1 is set for the call).
Working directory = the user's CWD (AGENTS.md etc. resolve from there as usual).

Model / provider selection mirrors `fetih chat`:
...
Env var fallbacks (used when the corresponding arg is not passed):
    ...
    - FETIH_INFERENCE_PROVIDER  (already read by resolve_runtime_provider)
"""
...
def _validate_explicit_toolsets(toolsets: object = None) -> tuple[list[str] | None, str | None]:
    ...
    try:
        ...
        return None, f"fetih -z: failed to validate --toolsets: {exc}\n"
    ...
    if any(name in {"all", "*"} for name in built_in):
        ...
        if ignored:
            sys.stderr.write(
                "fetih -z: --toolsets all enables every toolset; "
    ...
    if unresolved:
        try:
            ...
            from fetih_cli.tools_config import _parse_enabled_flag
    ...
    if unknown:
        sys.stderr.write(f"fetih -z: ignoring unknown --toolsets entries: {', '.join(unknown)}\n")
    if disabled:
        sys.stderr.write(
            "fetih -z: ignoring disabled MCP servers (set enabled: true in config.yaml to use): "
            f"{', '.join(disabled)}\n"
        )

    if not valid:
        return None, "fetih -z: --toolsets did not contain any valid toolsets.\n"
...
def run_oneshot(
    ...
    Args:
        ...
        model: Optional model override. Falls back to FETIH_INFERENCE_MODEL
            env var, then config.yaml's model.default / model.model.
        provider: Optional provider override. Falls back to
            FETIH_INFERENCE_PROVIDER env var, then config.yaml's model.provider,
            then "auto".
    ...
    env_model_early = os.getenv("FETIH_INFERENCE_MODEL", "").strip()
    if provider and not ((model or "").strip() or env_model_early):
        sys.stderr.write(
            "fetih -z: --provider requires --model (or FETIH_INFERENCE_MODEL). "
    ...
    os.environ["FETIH_ACCEPT_HOOKS"] = "1"
...
def _create_session_db_for_oneshot():
    """Best-effort SessionDB for ``fetih -z`` / oneshot mode.

    Oneshot bypasses ``FETIHCLI._init_agent()``, so it must wire the SQLite
...
def _run_agent(
    ...
    # Imports are local so they don't run when fetih is invoked for
    # other commands (keeps top-level CLI startup cheap).
    from fetih_cli.config import load_config
    from fetih_cli.models import detect_provider_for_model
    ...
    env_model = os.getenv("FETIH_INFERENCE_MODEL", "").strip()
    ...
    if effective_provider is None and (model or env_model):
        ...
        if explicit_model:
            ...
            try:
                from fetih_cli import model_switch as _ms
            ...
            if direct is not None:
                ...
                current_provider = (
                    cfg_provider
                    or os.getenv("FETIH_INFERENCE_PROVIDER", "").strip().lower()
                    or "auto"
                )
    ...
    agent = AIAgent(
        ...
        #                FETIH_INTERACTIVE which we never set
        #   - shell-hook approval → auto-approved via FETIH_ACCEPT_HOOKS=1
        #                (set above); also falls back to deny on non-tty
        #   - dangerous-command approval → bypassed via FETIH_YOLO_MODE=1
