"""
Interactive setup wizard for FETIH.
...
from fetih_cli.nous_subscription import get_nous_subscription_features
...
_DOCS_BASE = "https://github.com/MustafaKemal0146/fetih/docs"
...
# display_fetih_home imported lazily at call sites (stale-module safety during fetih update)
...
from fetih_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ FETIH Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure FETIH using environment variables or config commands:")
    print_info("  fetih config set model.provider custom")
    print_info("  fetih config set model.base_url http://localhost:8080/v1")
    print_info("  fetih config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'fetih setup' in an interactive terminal to use the full wizard.")
    print()
...
def _curses_prompt_choice(question: str, choices: list, default: int = 0, description: str | None = None) -> int:
    ...
    from fetih_cli.curses_ui import curses_radiolist
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from fetih_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'fetih setup')")


def _print_setup_summary(config: dict, fetih_home):
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        try:
            ...
            from fetih_cli.plugins import _ensure_plugins_discovered
    ...
    # Video generation — opt-in via `fetih tools` → Video Generation.
    # Only show the row when a plugin reports available so we don't badger
    # users who don't care about video gen with a "missing" status line.
    try:
        from agent.video_gen_registry import list_providers as _list_video_providers
        from fetih_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
        _ensure_plugins()
        _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'fetih setup tts'"))
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'fetih setup terminal'"))
    ...
    # Spotify (OAuth via fetih auth spotify — check auth.json, not env vars)
    try:
        from fetih_cli.auth import get_provider_auth_state
...
# between `fetih tools` and `fetih setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``fetih model``)
    ...
    provider added to ``fetih model`` is automatically available here.
    ...
    from fetih_cli.main import select_provider_and_model
    try:
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            print_info(
                "FETIH can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if _vision_needs_setup:
        ...
        print_info("doesn't currently provide one FETIH can auto-use for vision,")
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'fetih setup' or configure AUXILIARY_VISION_* settings")
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``fetih model`` -> xAI Grok OAuth (SuperGrok Subscription).
    """
    try:
        from fetih_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.fetih/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the FETIH auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via fetih setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'fetih setup tts')."""
    _setup_tts_provider(config)
...
def setup_terminal_backend(config: dict):
    ...
    if selected_backend == "local":
        ...
        print_info("Requires the optional SDK: pip install 'fetih-agent[vercel]'")
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                print_warning("Install failed — run manually: pip install 'fetih-agent[vercel]'")
                if result.stderr:
                    print_info(f"  Error: {result.stderr.strip().splitlines()[-1]}")
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into FETIH_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("FETIH_MAX_ITERATIONS")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # FETIH_MAX_ITERATIONS from agent.max_turns at startup.
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where FETIH delivers cron job results,")
...
def _setup_slack():
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `fetih update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://github.com/MustafaKemal0146/fetih/docs/user-guide/messaging/slack/")
    print()
...
def _write_slack_manifest_and_instruct():
    """Generate the Slack manifest, write it under FETIH_HOME, and print
    ...
    """
    try:
        from fetih_cli.slack_cli import _build_full_manifest
        from fetih_constants import get_fetih_home

        manifest = _build_full_manifest(
            bot_name="FETIH",
            bot_description="Your FETIH agent on Slack",
        )
        target = Path(get_fetih_home()) / "slack-manifest.json"
        ...
        print_info(
            "   Re-run `fetih slack manifest --write` anytime to refresh after "
            "FETIH adds new commands."
        )
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where FETIH delivers cron job results and notifications.")
...
def _setup_mattermost():
    ...
    print()
    print_info("📬 Home Channel: where FETIH delivers cron job results and notifications.")
    print_info("   To get a channel ID: click channel name → View Info → copy the ID")
    print_info("   You can also set this later by typing /set-home in a Mattermost channel.")
    home_channel = prompt("Home channel ID (leave empty to set later with /set-home)")
    if home_channel:
        save_env_value("MATTERMOST_HOME_CHANNEL", home_channel)
    print_info("   Open config in your editor:  fetih config edit")
...
def _setup_bluebubbles():
    ...
    print_info("Connects FETIH to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from fetih_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://github.com/MustafaKemal0146/fetih/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://github.com/MustafaKemal0146/fetih/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  fetih config edit")
    print_info("   Open config in your editor:  fetih config edit")
...
def setup_gateway(config: dict):
    ...
    from fetih_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with FETIH from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'fetih setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     fetih config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_fetih_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                print_info("  Or run in foreground:  fetih gateway")
        else:
            from fetih_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   fetih gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   fetih gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `fetih setup tools` and `fetih tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``fetih_cli.auth`` — lists every supported
    ...
    """
    try:
        from fetih_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from fetih_cli.config
    ...
    if section_key == "model":
        ...
        from fetih_cli.gateway import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_fetih.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_fetih migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_fetih", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and FETIH.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure FETIH to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point FETIH at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point FETIH at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point FETIH at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point FETIH at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to FETIH equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing FETIH config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in FETIH.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ FETIH's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(fetih_home: Path) -> bool:
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=fetih_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: fetih claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing FETIH configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing FETIH config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in FETIH (use fetih claw migrate --overwrite to force).")
    if skipped:
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      fetih setup           — full or quick (auto-detected)
      fetih setup model     — just model/provider
      fetih setup tts       — just text-to-speech
      fetih setup terminal  — just terminal backend
      fetih setup gateway   — just messaging platforms
      fetih setup tools     — just tool configuration
      fetih setup agent     — just agent settings
    """
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ FETIH Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│                 ⚔ FETIH Kurulum Sihirbazı              │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  FETIH kurulumunu yapılandıralım.                       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, fetih_home)
            return
        ...
        migration_ran = _offer_openclaw_migration(fetih_home)
        if migration_ran:
            config = load_config()
        ...
        if setup_mode == 0:
            _run_first_time_quick_setup(config, fetih_home, is_existing)
            return
...
def _run_first_time_quick_setup(config: dict, fetih_home, is_existing: bool):
    ...
    the user can customize later via ``fetih setup <section>``.
    """
...
def _run_quick_setup(config: dict, fetih_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'fetih setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect FETIH to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'fetih setup gateway'.")
