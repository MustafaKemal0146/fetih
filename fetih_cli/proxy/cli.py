"""CLI handlers for the ``fetih proxy`` subcommand."""
...
from fetih_cli.proxy.adapters import ADAPTERS, get_adapter
from fetih_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "fetih proxy requires aiohttp. Install one of:\n"
        "  pip install 'fetih-agent[messaging]'\n"
        "  pip install aiohttp",
        file=sys.stderr,
    )
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        print(
            ...
            f"Run `fetih login {adapter.name}` first.",
            file=sys.stderr,
        )
        return 2
    ...
    print(
        f"Starting FETIH proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("FETIH proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: fetih proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``fetih proxy <subcommand>``."""
    ...
    print(
        "fetih proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  fetih proxy start [--provider nous] [--host 127.0.0.1] [--port 8645]\n"
        "      Run the proxy in the foreground.\n"
        "  fetih proxy status\n"
        "      Show which upstream adapters are ready.\n"
        "  fetih proxy providers\n"
