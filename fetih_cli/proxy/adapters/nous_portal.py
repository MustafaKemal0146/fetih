"""FETIH Portal upstream adapter.

Reads the user's Nous OAuth state from ``~/.fetih/auth.json`` through the
...
:func:`fetih_cli.auth.resolve_nous_runtime_credentials`.
"""
...
from fetih_cli.proxy.adapters.base import UpstreamAdapter, UpstreamCredential

logger = logging.getLogger(__name__)

# Endpoints inference-api.github.com/MustafaKemal0146/fetih actually serves. Anything else
...
class NousPortalAdapter(UpstreamAdapter):
    """Proxy upstream for the FETIH Portal inference API."""
    ...
    def _get_credential(self, *, inference_auth_mode: str) -> UpstreamCredential:
        with self._lock:
            ...
            if state is None:
                raise RuntimeError(
                    "Not logged into FETIH Portal. Run `fetih login nous` first."
                )
            ...
            try:
                ...
                raise RuntimeError(
                    f"Failed to refresh FETIH Portal credentials: {exc}"
                ) from exc
            ...
            if not agent_key:
                raise RuntimeError(
                    "FETIH Portal refresh did not return a usable agent_key. "
                    "Try `fetih login nous` to re-authenticate."
                )
    ...
    # to fetih_cli.auth to avoid expanding that module's public surface.
    # ------------------------------------------------------------------
