"""Profile distributions — shareable, packaged FETIH profiles via git.

A distribution is a FETIH profile published as a git repository (or
...
* ``fetih profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``fetih skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``fetih profile``, not a parallel tree):

    fetih profile install <source> [--name N] [--alias] [--force] [--yes]
    fetih profile update  <name>  [--force-config] [--yes]
    fetih profile info    <name>
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "fetih-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            fetih_requires=str(data.get("fetih_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.fetih_requires:
            out["fetih_requires"] = self.fetih_requires
        if self.author:
...
def check_fetih_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires FETIH {op}{target}, "
...
def _env_template_from_manifest(manifest: DistributionManifest) -> str:
    ...
    lines = [
        "# Environment variables required by this FETIH distribution.",
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``fetih profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a FETIH profile distribution."
            )
...
def plan_install(
    ...
    from fetih_cli import __version__ as fetih_version
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a FETIH distribution."
        )

    # Version check up-front so we fail fast
    check_fetih_requires(manifest.fetih_requires, fetih_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.fetih).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="fetih_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `fetih profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`fetih profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="fetih_dist_update_") as tmp:
        plan = plan_install(
