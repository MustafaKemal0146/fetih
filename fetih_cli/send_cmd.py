"""CLI subcommand: ``fetih send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(f"fetih send: cannot read {file_path}: {exc}", file=sys.stderr)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"fetih send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"fetih send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"fetih send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"fetih send: no targets found for platform '{platform_filter}'. "
    ...
    if not any(platforms.values()):
        ...
        print("Set one up with `fetih gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.fetih/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_fetih_env() -> None:
    """Populate ``os.environ`` from ``~/.fetih/.env`` AND bridge top-level
    ...
    startup that ``fetih send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.fetih/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.fetih/config.yaml`` into
       ...
       via ``fetih config set``.

    See ``gateway/run.py`` for the canonical version of this bridge — we
    intentionally reimplement the minimum needed here so ``fetih send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.fetih/.env and ~/.fetih/config.yaml into os.environ so the
    ...
    if not target:
        print(
            "fetih send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  fetih send --to telegram \"hello\"\n"
            "  fetih send --to discord:#ops --file report.md\n"
            "  fetih send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "fetih send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `fetih send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform FETIH "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.fetih/.env + ~/.fetih/config.yaml) — no LLM, "
            "no agent loop, no running gateway required for bot-token "
            "platforms like Telegram/Discord/Slack/Signal."
        ),
        epilog=(
            "Examples:\n"
            "  fetih send --to telegram \"deploy finished\"\n"
            "  echo \"RAM 92%\" | fetih send --to telegram:-1001234567890\n"
            "  fetih send --to discord:#ops --file /tmp/report.md\n"
            "  fetih send --to slack:#eng --subject \"[CI]\" --file build.log\n"
            "  fetih send --list                  # all platforms\n"
            "  fetih send --list telegram         # filter by platform\n"
            "\n"
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `fetih send --list telegram`.",
    )
