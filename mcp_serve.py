"""
FETIH MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (FETIH-specific extra)

Usage:
    fetih mcp serve
    fetih mcp serve --verbose
...
logger = logging.getLogger("fetih.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using FETIH_HOME."""
    try:
        from fetih_constants import get_fetih_home
        return get_fetih_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("FETIH_HOME", Path.home() / ".fetih")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from fetih_constants import get_fetih_home
        directory_file = get_fetih_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("FETIH_HOME", Path.home() / ".fetih")
...
class EventBridge:
    ...
    This is the FETIH equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from fetih_constants import get_fetih_home
            db_file = get_fetih_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("FETIH_HOME", Path.home() / ".fetih")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the FETIH MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "fetih",
        instructions=(
            "FETIH messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the FETIH MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
