...
# 5. Symlinks the 'fetih' CLI command into a user-facing bin dir
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/fetih-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink fetih into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up fetih command..."

FETIH_BIN="$SCRIPT_DIR/venv/bin/fetih"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$FETIH_BIN" "$COMMAND_LINK_DIR/fetih"
echo -e "${GREEN}✓${NC} Symlinked fetih → $COMMAND_LINK_DISPLAY_DIR/fetih"
...
# ============================================================================
# Seed bundled skills into ~/.fetih/skills/
# ============================================================================

FETIH_SKILLS_DIR="${FETIH_HOME:-$HOME/.fetih}/skills"
mkdir -p "$FETIH_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.fetih/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$FETIH_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  fetih status        # Check configuration"
if is_termux; then
    echo "  fetih gateway       # Run gateway in foreground"
else
    echo "  fetih gateway install # Install gateway service (messaging + cron)"
fi
echo "  fetih cron list     # View scheduled jobs"
echo "  fetih doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m fetih_cli.main setup
fi
