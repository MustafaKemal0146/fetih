...
## 5. FETIH Configuration

### Config location

`$FETIH_HOME/config.yaml` (defaults to `~/.fetih/config.yaml` when `FETIH_HOME` is unset)
...
Restart the FETIH session for changes to take effect. The MCP connection is
