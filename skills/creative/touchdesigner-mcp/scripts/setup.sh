...
FETIH_HOME_DIR="${FETIH_HOME:-$HOME/.fetih}"
FETIH_CFG="${FETIH_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure FETIH config has twozero_td MCP entry ──
if [[ ! -f "$FETIH_CFG" ]]; then
    echo -e " ${FAIL} FETIH config not found at ${FETIH_CFG}"
    manual_steps+=("Create ${FETIH_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$FETIH_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in FETIH config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to FETIH config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${FETIH_CFG} manually"); }
    manual_steps+=("Restart FETIH session to pick up config change")
fi
