...
```
FETIH -> MCP (Streamable HTTP) -> twozero.tox (port 40404) -> TD Python
```
...
```bash
bash "${FETIH_HOME:-$HOME/.fetih}/skills/creative/touchdesigner-mcp/scripts/setup.sh"
```

The script will:
1. Check if TD is running
2. Download twozero.tox if not already cached
3. Add `twozero_td` MCP server to FETIH config (if missing)
...
3. **Restart FETIH session** to pick up the new MCP server
