...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if [ -f ~/.fetih/.env ] && grep -q "^GITHUB_TOKEN=" ~/.fetih/.env; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.fetih/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
...
```bash
source "${FETIH_HOME:-$HOME/.fetih}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by FETIH. Code looks clean — good test coverage, no security concerns."
