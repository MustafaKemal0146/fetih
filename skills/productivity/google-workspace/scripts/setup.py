#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for FETIH.
...
# Ensure sibling modules (_fetih_home) are importable when run standalone.
_SCRIPTS_DIR = str(Path(__file__).resolve().parent)
if _SCRIPTS_DIR not in sys.path:
    sys.path.insert(0, _SCRIPTS_DIR)

from _fetih_home import display_fetih_home, get_fetih_home

FETIH_HOME = get_fetih_home()
TOKEN_PATH = FETIH_HOME / "google_token.json"
CLIENT_SECRET_PATH = FETIH_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = FETIH_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same FETIH profile to refresh consent."
    )
...
def install_deps():
    ...
    try:
        ...
        print("  pip install 'fetih-agent[google]'")
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to FETIH home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_fetih_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for FETIH")
