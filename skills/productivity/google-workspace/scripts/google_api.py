#!/usr/bin/env python3
"""Google Workspace API CLI for FETIH.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing FETIH-facing JSON contract and falls back to the Python client
...
from _fetih_home import get_fetih_home
...
def _gws_binary() -> str | None:
    override = os.getenv("FETIH_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for FETIH")
