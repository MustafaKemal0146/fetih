...
description: "Operate the Teams meeting summary pipeline via FETIH CLI — summarize meetings, inspect pipeline status, replay jobs, manage Microsoft Graph subscriptions."
version: 1.1.0
...
Everything operator-facing is a `fetih teams-pipeline` subcommand run via the terminal tool. There are no new model tools for this pipeline — the CLI is the surface.
...
Before using the pipeline, verify these are set in `~/.fetih/.env`:
...
```bash
fetih teams-pipeline validate              # config snapshot — run first after any change
fetih teams-pipeline token-health          # Graph token status
fetih teams-pipeline token-health --force-refresh   # force a fresh token acquisition
fetih teams-pipeline list                  # recent meeting jobs
fetih teams-pipeline list --status failed  # only failed jobs
fetih teams-pipeline show <job-id>         # full detail of one job
fetih teams-pipeline subscriptions         # current Graph webhook subscriptions
```

### Re-running / debugging

```bash
fetih teams-pipeline run <job-id>          # replay a stored job (re-summarize, re-deliver)
fetih teams-pipeline fetch --meeting-id <id>   # dry-run: resolve meeting + transcript without persisting
fetih teams-pipeline fetch --join-web-url "<url>"   # dry-run by join URL
```

### Subscription management

```bash
fetih teams-pipeline subscribe \
  --resource communications/onlineMeetings/getAllTranscripts \
  --notification-url https://<your-public-host>/msgraph/webhook \
  --client-state "$MSGRAPH_WEBHOOK_CLIENT_STATE"

fetih teams-pipeline renew-subscription <sub-id> --expiration <iso-8601>
fetih teams-pipeline delete-subscription <sub-id>
fetih teams-pipeline maintain-subscriptions            # renew near-expiry ones
fetih teams-pipeline maintain-subscriptions --dry-run  # show what would be renewed
```
...
1. Run `fetih teams-pipeline subscriptions` — if it's empty or all entries show `expirationDateTime` in the past, that's the cause.
2. Recreate with `subscribe` as shown above.
3. **Set up automated renewal immediately** via `fetih cron add`, a systemd timer, or plain crontab. The operator runbook at `/docs/guides/operate-teams-meeting-pipeline#automating-subscription-renewal-required-for-production` has all three options. 12-hour interval is safe (6x headroom against the 72h limit).
