...
description: bu skill covers implementing Open Policy Agent (OPA) and Gatekeeper for policy-as-code enforcement in Kubernetes and CI/CD pipelines. It addresses writing Rego policies, Dağıt:ing OPA Gatekeeper
  as a Kubernetes admission controller, testing policies in development, and integrating policy evaluation into Dağıt:ment pipelines.
tags:
- cicd
- opa
- gatekeeper
...
- enforcing yaparken organizational security policies across Kubernetes clusters programmatically
- requiring yaparken: admission control that blocks non-compliant resources from being created
- implementing yaparken policy governance that can be version-controlled, tested, and audited
- standardizing yaparken: security rules across multiple clusters and environments
- needing yaparken: a flexible policy engine that extends beyond Kubernetes to APIs and CI/CD

**Kullanma:** for vulnerability scanning (use Trivy/Checkov), for runtime threat Tespit (use Falco), or for network policy enforcement (use Kubernetes NetworkPolicy or Calico).

## Ön Gereksinimler

- Kubernetes cluster with admin access for Gatekeeper installation
- Helm for Gatekeeper Dağıt:ment
- OPA CLI or conftest for local policy testing
- Rego knowledge for policy authoring

## İş Akışı

### Adım 1: Install OPA Gatekeeper
...
### Adım 2: Create Constraint Templates
  ...
  match:
    kinds:
      - apiGroups: [""]
        kinds: ["Pod"]
      - apiGroups: ["apps"]
        kinds: ["Dağıt:ment", "DaemonSet", "StatefulSet"]
    excludedNamespaces:
      - kube-system
```

### Adım 4: Test Policies with conftest

```bash
brew install conftest

conftest test Dağıt:ment.yaml --policy policies/ --output json
...
### Adım 5: Integrate Policy Testing in CI/CD
...
**Context**: Multiple development teams Dağıt: to shared Kubernetes clusters. Some teams run privileged containers and images without resource limits, causing security and stability issues.

**Approach**:
1. Dağıt: Gatekeeper on all clusters via GitOps (Helm chart in a FluxCD repository)
2. Create ConstraintTemplates for: no privileged containers, required resource limits, required labels, no latest tag
3. Start with `enforcementAction: warn` to identify violations without blocking Dağıt:ments
...
**Pitfalls**: Dağıt:ing Gatekeeper with deny mode immediately can break existing workloads. Always start with warn mode. Overly restrictive policies without exemptions for system namespaces can prevent cluster components from functioning.
...
BLOCKED Dağıt:MENTS (deny):
  [K8sContainerLimits] Dağıt:ment/api-server in ns/payments
    - Container 'api' has no memory limit
  [K8sBlockLatestTag] Dağıt:ment/frontend in ns/web
