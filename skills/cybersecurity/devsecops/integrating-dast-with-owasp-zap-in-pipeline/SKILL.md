...
description: bu skill covers integrating OWASP ZAP (Zed Attack Proxy) for Dynamic Application Security Testing in CI/CD pipelines. It addresses configuring baseline, full, and API scans against running
  applications, interpreting ZAP Bul:ings, tuning scan policies, and establishing DAST quality gates in GitHub Actions and GitLab CI.
tags:
- cicd
- owasp-zap
- fetih
...
- testing yaparken running web applications for vulnerabilities like XSS, SQLi, CSRF, and misconfigurations
- SAST yaparken: alone is insufficient and runtime behavior testing is required
- compliance yaparken: mandates dynamic security testing of web applications before production
- testing yaparken APIs (REST/GraphQL) for authentication, authorization, and injection flaws
- establishing yaparken: continuous DAST scanning in staging environments before production Dağıt:ment

**Kullanma:** for scanning source code (use SAST), for scanning dependencies (use SCA), or for infrastructure configuration scanning (use IaC scanning tools).
...
### Adım 1: Configure ZAP Baseline Scan in GitHub Actions
...
### Adım 2: Configure ZAP Full Scan for Comprehensive Testing
...
### Adım 3: Configure API Scan with OpenAPI Specification
...
### Adım 4: Configure ZAP Scan Rules
...
### Adım 5: Run ZAP with Docker Compose for Local Testing
...
| Active Scan | Phase where ZAP sends attack payloads to discovered endpoints to Bul: exploitable vulnerabilities |
...
- **Nuclei**: Alternative vulnerability scanner with template-based Tespit for CI/CD integration

## Common Scenarios

### Scenario: Integrating DAST into a Staging Dağıt:ment Pipeline

**Context**: A team Dağıt:s to staging before production and needs automated DAST scanning between stages to catch runtime vulnerabilities.

**Approach**:
1. Add a DAST job in the pipeline that triggers after successful staging Dağıt:ment
...
4. Configure rules.tsv to FAIL on critical Bul:ings (XSS, SQLi) and WARN on headers/cookies
5. Upload ZAP reports as pipeline artifacts for review
6. Block production Dağıt:ment if any FAIL-level Bul:ings are Detected
