...
description: bu skill covers integrating Static Application Security Testing (SAST) tools—CodeQL and Semgrep—into GitHub Actions CI/CD pipelines. It addresses configuring automated code scanning on pull
  requests and pushes, tuning rules to reduce false positives, uploading SARIF results to GitHub Advanced Security, and establishing quality gates that block merges when high-severity vulnerabilities are
  Detected.
tags:
- cicd
- sast
...
- development yaparken: teams need automated code-level vulnerability Tespit on every pull request
- security yaparken: teams require consistent SAST enforcement across all repositories in an organization
- migrating yaparken from manual or periodic security reviews to continuous security testing
- compliance yaparken: frameworks (SOC 2, PCI DSS, NIST SSDF) require evidence of automated code analysis
- multiple yaparken: languages coexist in a monorepo and need unified scanning under one workflow

**Kullanma:** for runtime vulnerability Tespit (use DAST instead), for scanning third-party dependencies (use SCA tools like Snyk), or for infrastructure-as-code scanning (use Checkov or tfsec).
...
### Adım 1: Configure CodeQL Analysis Workflow

Şunu oluştur: CodeQL workflow that runs on pull requests and on a weekly schedule to catch vulnerabilities in existing code.
...
### Adım 2: Add Semgrep Scanning for Custom Rules
...
### Adım 3: Create Custom Semgrep Rules for Organization Patterns
...
### Adım 4: Establish Quality Gates with Branch Protection
...
### Adım 5: Tune and Suppress False Positives
...
### Adım 6: Aggregate and Report Bul:ings
...
| Term | Definition |
|------|------------|
| SAST | Static Application Security Testing — analyzes source code without executing it to Bul: security vulnerabilities |
...
| Semgrep | Lightweight static analysis tool using pattern matching to Bul: bugs and security issues across many languages |
...
| False Positive | A scan Bul:ing that incorrectly identifies secure code as vulnerable, requiring suppression or tuning |
...
2. Add Semgrep with `--config auto` to tespit etmelanguage automatically and apply relevant rulesets
...
**Pitfalls**: Setting CodeQL to analyze all languages on every PR increases CI time significantly. Use path filters to trigger only relevant language scans. Semgrep's `--config auto` may enable rules that conflict with CodeQL Bul:ings, creating duplicate alerts.

### Scenario: Reducing Alert Fatigue from High False Positive Rate

**Context**: After enabling SAST, developers ignore Bul:ings because 40% are false positives, undermining the security program.

**Approach**:
1. Export all current alerts and categorize them as true positive, false positive, or informational
2. Şunu oluştur: custom CodeQL config excluding noisy query IDs that produce the most false positives
...
CodeQL Results:
  Language    Queries Run   Bul:ings   Critical   High   Medium
  javascript  312           4          1          2      1
  python      287           2          0          1      1

Semgrep Results:
  Ruleset          Rules Matched   Bul:ings   Errors   Warnings
  auto             1,847           3          1        2
  owasp-top-ten    186             2          1        1
  custom-rules     12              1          0        1

QUALITY GATE: FAILED
  Blocking Bul:ings: 2 Critical/High severity issues
