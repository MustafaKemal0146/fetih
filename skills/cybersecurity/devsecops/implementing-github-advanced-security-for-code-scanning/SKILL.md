...
description: Configure GitHub Advanced Security with CodeQL to perform automated static analysis and vulnerability Tespit across repositories at enterprise scale.
tags:
- sast
- devops-security
...
- Dağıt:ing yaparken or configuring implementing github advanced security for code scanning capabilities in your environment
...
CodeQL compiles source code into a queryable database, then executes security-focused queries against that database. The query suites ship with hundreds of checks mapped to CWE identifiers and cover OWASP Top 10, SANS Top 25, and language-specific vulnerability patterns. Custom queries can be authored using the CodeQL query language (QL) to tespit etmeorganization-specific anti-patterns.

### Default Setup vs. Advanced Setup

**Default Setup** enables code scanning with a single click from the repository's Code Security settings. GitHub automatically Belirle:s the languages present, selects appropriate query suites, and configures scanning triggers. This approach requires no workflow file and is ideal for rapid onboarding.
...
### Adım 1 --- Enable GHAS on the Organization

1. Şuraya git: Organization Settings > Code security and analysis
...
### Adım 2 --- Configure Default Setup for Quick Wins

1. Go to Repository Settings > Code security > Code scanning
2. Click "Kur:" in the CodeQL analysis row and select "Default"
3. Şunu incele: auto-Detected languages and query suite (default or extended)
4. Click "Enable CodeQL" to activate scanning on push and pull request events

### Adım 3 --- Advanced Setup with Custom Workflow
...
### Adım 5 --- Configure Branch Protection Rules

1. Şuraya git: Repository Settings > Branches > Branch protection rules
...
### Adım 6 --- Secret Scanning and Push Protection
...
### Adım 7 --- Dependency Review and Dependabot
...
| `security-extended` | Broader security queries including lower-severity Bul:ings | Comprehensive security coverage |
| `security-and-quality` | Security plus code quality queries | Teams wanting both security and maintainability checks |
| Custom packs | Organization-authored queries | Tespit etme internal anti-patterns and compliance violations |
