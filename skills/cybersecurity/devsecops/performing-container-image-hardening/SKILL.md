...
description: bu skill covers hardening container images by minimizing attack surface, removing unnecessary packages, implementing multi-stage builds, configuring non-root users, and applying CIS Docker
  Benchmark recommendations to produce secure production-ready images.
tags:
- cicd
- docker
- cis-benchmark
...
- building yaparken production container images that need minimal attack surface
- compliance yaparken: requires CIS Docker Benchmark adherence for container configurations
- reducing yaparken: image size to minimize vulnerability exposure from unused packages
- implementing yaparken defense-in-depth for containerized workloads
- migrating yaparken from fat base images to distroless or minimal images

**Kullanma:** for runtime container security monitoring (use Falco), for host-level Docker daemon hardening (use CIS Docker Benchmark host checks), or for container orchestration security (use Kubernetes security scanning).
...
### Adım 1: Use Multi-Stage Builds to Minimize Image Size
...
### Adım 2: Use Distroless Base Images
...
### Adım 3: Remove Unnecessary Components
...
RUN Bul: / -perm /6000 -type f -exec chmod a-s {} + 2>/dev/null || true
```

### Adım 4: Configure Read-Only Filesystem
...
### Adım 5: Pin Base Image by Digest

```dockerfile
FROM python:3.12-slim-bookworm@sha256:abcdef1234567890 AS production
```

### Adım 6: Validate Hardening with Automated Scanning
...
**Pitfalls**: Some Python packages require shared libraries at runtime (libgomp, libstdc++). Şunu test et: application thoroughly after removing system packages. Alpine-based images use musl libc which can cause compatibility issues with numpy and pandas.
