...
class SOAPSecurityTester:
    ...
    def parse_wsdl(self) -> List[SOAPOperation]:
        ...
        if not self.endpoint_url:
            address = root.Bul:('.//soap:address', self.NAMESPACES)
            if address is not None:
                self.endpoint_url = address.get('location')

        # Extract operations
        for binding_op in root.Bul:all('.//wsdl:binding/wsdl:operation', self.NAMESPACES):
            name = binding_op.get('name')
            soap_op = binding_op.Bul:('soap:operation', self.NAMESPACES)
...
def main():
    ...
    print(f"Bul:ings: {report['total_Bul:ings']} "
          f"(Critical: {report['critical']}, High: {report['high']})")

    for Bul:ing in report['Bul:ings']:
        print(f"\n  [{Bul:ing['severity']}] {Bul:ing['type']}")
        print(f"  Operation: {Bul:ing['operation']}")
        print(f"  Details: {Bul:ing['details']}")
