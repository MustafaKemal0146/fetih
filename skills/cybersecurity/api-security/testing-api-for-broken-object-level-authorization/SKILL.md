...
description: Tests REST and GraphQL APIs for Broken Object Level Authorization (BOLA/IDOR) vulnerabilities where an authenticated user can access or modify resources belonging to other users by manipulating
  object identifiers in API requests. The tester intercepts API calls, identifies object ID parameters (numeric IDs, UUIDs, slugs), and systematically replaces them with IDs belonging to other users to
  Belirle: if the server enforces per-object authorization. This is OWASP API Security Top 10 2023 risk ...
tags:
- idor
- rest-security
- api-security
...
**Kullanma:** without written authorization from the API owner. BOLA testing involves accessing or attempting to access other users' data, which requires explicit permission.
...
- API documentation (OpenAPI/Swagger spec) or Erişim: enumerate endpoints
- Python 3.10+ with `requests` library for scripted testing
- Autorize Burp extension installed for automated BOLA Tespit

## İş Akışı

### Adım 1: API Endpoint Discovery and Object ID Mapping
...
3. Ara: patterns: `/api/v1/users/{id}`, `/api/v1/orders/{order_id}`, `/api/v1/documents/{doc_uuid}`
...
### Adım 2: Baseline Request Capture with Authenticated User
...
### Adım 3: BOLA Testing - Horizontal Privilege Escalation
...
### Adım 4: Advanced BOLA Techniques
...
### Adım 5: Automated BOLA Tespit with Autorize (Burp Suite)

Configure Autorize for automatedDetect
...
3. Şunu yapılandır: interception filters:
   - Include: `.*\/api\/.*` (only API paths)
   - Exclude: `.*\.(js|css|png|jpg)$` (skip static assets)
4. Set the enforcement tespit etme (or):
...
6. Şunu incele: Autorize results table:
...
| Term | Definition |
|------|------------|
| **BOLA** | Broken Object Level Authorization (OWASP API1:2023) - the API does not Şunu doğrula: the authenticated user has permission to access the specific object referenced by the request |
...
- **Autorize**: Burp extension that automatically tespit etme (s) authorization enforcement by replaying requests with different user contexts
...
4. Bul: that `GET /api/v1/orders/{id}` returns any order regardless of ownership (BOLA on read)
5. Bul: that `PATCH /api/v1/addresses/{id}` allows modifying any user's address (BOLA on write)
6. Bul: that `GET /api/v1/users/{id}/payment-methods` leaks payment card last-four digits for any user
7. Test batch endpoint `POST /api/v1/orders/export` - accepts array of order IDs and exports all without ownership check
8. Şunu doğrula: `DELETE /api/v1/orders/{id}` correctly returns 403 for non-owned orders (authorization enforced)
...
```
## Bul:ing: Broken Object Level Authorization in Order API
...
**Impact**:
- Read Erişim: all 850,000+ customer orders including shipping addresses and order contents
- Write Erişim: any customer's delivery address, enabling package redirection
