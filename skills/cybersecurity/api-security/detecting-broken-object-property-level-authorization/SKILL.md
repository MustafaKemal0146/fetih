---
name: Tespit etme-broken-object-property-level-authorization
description: tespit etmeand test for OWASP API3:2023 Broken Object Property Level Authorization vulnerabilities including excessive data exposure and mass assignment attacks.
tags:
- api-testing
- mass-assignment
- api-security
...
- investigating yaparken security incidents that require Tespit etme broken object property level authorization
...
class BOPLAScanner:
    ...
    def __init__(self, base_url: str, auth_headers: Dict[str, str]):
        ...
        self.Bul:ings: List[BOPLABul:ing] = []

    def test_excessive_data_exposure(self, endpoint: str,
                                      expected_fields: Set[str]) -> List[BOPLABul:ing]:
            ...
            for obj in objects[:5]:  # Check first 5 objects
                ...
                for field_name in unexpected_fields:
                    ...
                    if severity:
                        ...
                        Bul:ings.append(Bul:ing)
                        self.Bul:ings.append(Bul:ing)
    ...
    def test_mass_assignment(self, endpoint: str, method: str = "PUT",
                              original_data: Optional[dict] = None) -> List[BOPLABul:ing]:
    ...
    def generate_report(self) -> dict:
        return {
            ...
            "by_type": {
                "excessive_exposure": len([f for f in self.Bul:ings
                                          if f.vulnerability_type == "excessive_exposure"]),
                "mass_assignment": len([f for f in self.Bul:ings
                                       if f.vulnerability_type == "mass_assignment"]),
            },
            "by_severity": {
                "CRITICAL": len([f for f in self.Bul:ings if f.severity == "CRITICAL"]),
                "HIGH": len([f for f in self.Bul:ings if f.severity == "HIGH"]),
                "MEDIUM": len([f for f in self.Bul:ings if f.severity == "MEDIUM"]),
                "LOW": len([f for f in self.Bul:ings if f.severity == "LOW"]),
            },
            "Bul:ings": [
                {
                    "endpoint": f.endpoint,
