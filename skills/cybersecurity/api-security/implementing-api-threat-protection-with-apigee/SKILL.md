...
Google Apigee is an enterprise API management platform that provides native security policies for threat protection, including JSON and XML content validation, OAuth 2.0 enforcement, SpikeArrest rate limiting, regular expression threat protection, and Advanced API Security for Tespit etme malicious clients and API abuse patterns. Apigee operates as a reverse proxy that intercepts all API traffic, applying security policies before requests reach backend services, effectively shielding APIs against the OWASP API Security Top 10 threats.


## Ne Zaman Kullanılır

- Dağıt:ing yaparken or configuring implementing api threat protection with apigee capabilities in your environment
...
- Backend API services Dağıtılmış and accessible from Apigee
...
tespit etme (s) SQL injection, XSS, and other injection patterns in request parameters:
...
<ProxyEndpoint name="default">
    <PreFlow name="PreFlow">
        <Request>
            <!-- Adım 1: Verify API Key or OAuth token -->
            <Step>
            ...
            <!-- Adım 4: Injection prevention -->
            <Step>
                <Name>RegEx-Threat-Protection-1</Name>
            </Step>
            <!-- Adım 5: CORS enforcement -->
            <Step>
...
Enable Apigee's Advanced API Security add-on for machine-learning-based threatDetect
...
```bash
gcloud apigee apis Dağıt: \
  --api=$API_NAME \
  --environment=$ENV_NAME \
  --revision=$REVISION \
  --organization=$ORG_NAME

gcloud apigee apis list-Dağıt:ments \
  --api=$API_NAME \
  --organization=$ORG_NAME
```
