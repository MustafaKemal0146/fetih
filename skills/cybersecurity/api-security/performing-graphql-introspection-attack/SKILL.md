...
description: Performs GraphQL introspection attacks to Şunu çıkar: full API schema including types, queries, mutations, subscriptions, and field definitions from GraphQL endpoints. The tester uses introspection
  queries to map the attack surface, identifies sensitive fields and mutations, tests for query depth and complexity limits, and exploits GraphQL-specific vulnerabilities including batching attacks, alias-based
  brute force, and nested query DoS. Activates for requests involving GraphQL security testi...
tags:
- siber-güvenlik
- graphql
- api-security
...
**Kullanma:** without written authorization. Schema extraction and query abuse testing can impact service availability.
...
### Adım 1: GraphQL Endpoint Discovery
...
### Adım 2: Full Introspection Query
...
### Adım 3: Sensitive Data Identification in Schema
...
### Adım 4: Schema Reconstruction When Introspection is Disabled
...
def bruteforce_field(type_name, field_wordlist):
    ...
    for field_name in field_wordlist:
        ...
        if "did you mean" in response_text:
            # Extract suggestions
            import re
            suggestions = re.Bul:all(r'"(\w+)"', resp.text)
...
### Adım 5: GraphQL Attack Techniques
...
def generate_deep_query(depth=50):
    """Şunu üret: deeply nested query to test depth limits."""
...
### Adım 6: Field-Level Authorization Testing
...
| **Field-Level Authorization** | Controlling Erişim: individual fields within a GraphQL type based on the authenticated user's role or permissions |
...
4. Bul: admin mutations accessible to regular users: `deleteUser`, `updateRole`, `exportAllOrders`
...
```
## Bul:ing: GraphQL Introspection Enabled with Sensitive Schema Exposure
...
**Impact**:
An attacker can Şunu çıkar: complete API schema, identify sensitive
