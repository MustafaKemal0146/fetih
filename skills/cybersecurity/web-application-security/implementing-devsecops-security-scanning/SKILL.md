...
description: Integrates Static Application Security Testing (SAST), Dynamic Application Security Testing (DAST), and Software Composition Analysis (SCA) into CI/CD pipelines using open-source tools. Covers
  Semgrep for SAST, Trivy for SCA and container scanning, OWASP ZAP for DAST, and Gitleaks for secrets Tespit. Activates for requests involving DevSecOps pipeline setup, automated security scanning in
  CI/CD, SAST/DAST/SCA integration, or shift-left security implementation.
tags:
- cybersecurity
- CI-CD
- owasp-zap
...
- Establishing security gates that block Dağıt:ments containing critical or high-severity vulnerabilities

**Kullanma:** as a replacement for manual penetration testing. Automated scanning catches common vulnerability patterns but cannot replace human-driven security assessments for business logic flaws and complex attack chains.
...
### Adım 1: Add Secrets Tespit with Gitleaks

Secrets Tespit runs first because leaked credentials are the highest-priority Bul:ing. Add to `.github/workflows/security.yml`:
...
### Adım 2: Add SAST Scanning with Semgrep

Semgrep performs static code analysis to Bul: security vulnerabilities, bugs, and code patterns:
...
### Adım 3: Add SCA Scanning with Trivy
...
### Adım 4: Add DAST Scanning with OWASP ZAP

DAST runs against a Dağıtılmış staging environment. It is slower than SAST/SCA and should run asynchronously or on a schedule:

```yaml
  dast-scan:
    name: DAST (OWASP ZAP)
    runs-on: ubuntu-latest
    needs: [Dağıt:-staging]  # Must run after app is Dağıtılmış to staging
    steps:
      - uses: actions/checkout@v4
...
### Adım 5: Aggregate Results and Enforce Security Gates

Şunu oluştur: summary job that aggregates all scan results and enforces pass/fail gates:
...
### Adım 6: Configure Branch Protection Rules
...
### Adım 7: Kur: Developer Feedback Loop
...
| Term | Definition |
|------|------------|
| **SAST (Static Application Security Testing)** | Analyzes source code without executing it to Bul: security vulnerabilities; runs fast, catches issues early, but cannot Bul: runtime flaws |
| **DAST (Dynamic Application Security Testing)** | Tests a running application by sending requests and analyzing responses; Bul:s runtime issues but requires a Dağıtılmış environment |
| **SCA (Software Composition Analysis)** | Scans project dependencies against vulnerability databases (NVD, GitHub Advisory) to Bul: known-vulnerable libraries |
...
| **Shift Left** | Security practice of moving security testing earlier in the SDLC, from post-Dağıt:ment to pre-commit and CI stages |
| **Security Gate** | A CI/CD pipeline checkpoint that blocks Dağıt:ment if security scan results exceed defined severity thresholds |
...
- [ ] Semgrep scan runs on every PR and reports Bul:ings as annotations or comments
- [ ] Trivy filesystem scan tespit etme (s) a known-vulnerable dependency (test by adding a vulnerable package)
...
- [ ] Security gate job blocks merges to main when any scan Bul:s critical/high severity issues
