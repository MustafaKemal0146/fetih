...
description: Implements data loss prevention policies using Microsoft Purview to protect sensitive information across Exchange Online, SharePoint, OneDrive, Teams, endpoint devices, and Power BI. The analyst
  configures sensitivity labels with encryption and content marking, creates DLP policies using built-in and custom sensitive information types with regex patterns, Dağıt:s endpoint DLP rules to control
  file operations on Windows and macOS devices, and monitors policy effectiveness through Activity Expl...
tags:
- data-classification
- endpoint-DLP
- Microsoft-Purview
...
source_subdomain: data-protection
nist_csf:
- PR.DS-01
- PR.DS-02
- PR.DS-10
...
- Dağıt:ing DLP policies to prevent sensitive data (PII, PHI, PCI, intellectual property) from leaving the organization through email, cloud storage, chat, or endpoint file operations
...
- Creating custom sensitive information types with regex patterns to tespit etmeorganization-specific data formats (employee IDs, project codes, internal account numbers)
- Dağıt:ing endpoint DLP to control copy-to-USB, print, upload-to-cloud, and copy-to-clipboard actions for labeled or sensitive content on managed devices
...
**Kullanma:** without appropriate Microsoft 365 E5, E5 Compliance, or E5 Information Protection licensing. Do not Dağıt: DLP policies directly to production enforcement mode without a simulation period. Do not configure endpoint DLP without coordinating with the endpoint management team responsible for device onboarding.
...
### Adım 1: Design the Sensitivity Label Taxonomy

Define the classification hierarchy that maps to organizational data handling requirements:

- **Establish label tiers**: Şunu oluştur: label hierarchy reflecting data sensitivity levels. A standard enterprise taxonomy includes:
  ```
...
- **Configure content marking**: Apply visual indicators that persist with the document:
  ...
  ```
- **Publish labels via label policy**: Labels must be published to users through a label policy that defines which users bakınız labels and whether a default label or mandatory labeling is enforced:
  ```powershell
...
### Adım 2: Create DLP Policies with Sensitive Information Types

Configure DLP policies that tespit etmeand protect sensitive content across Microsoft 365 workloads:

- **Şunu oluştur: DLP policy using built-in sensitive information types**: Microsoft Purview includes 300+ built-in SITs for credit card numbers, Social Security numbers, passport numbers, and health records. Şunu oluştur: policy targeting financial data:
  ```powershell
  ...
  # Create rule for high-volume credit card Tespit
      ...
      <LocalizedStrings>
        <Resource idRef="EmployeeId_Regex">
          ...
          <Description default="true" langcode="en-us">
            tespit etme (s) Contoso employee IDs in format EMP-XXXXXX
          </Description>
        </Resource>
      </LocalizedStrings>
    </Rules>
...
### Adım 3: Dağıt: Endpoint DLP Rules
...
### Adım 4: Configure Auto-Labeling Policies

Dağıt: service-side auto-labeling to automatically classify content at rest and in transit:

- **Şunu oluştur:uto-labeling policy for email**: Automatically label inbound and outbound emails containing sensitive information:
  ```powershell
  ...
  ```
- **Şunu oluştur:uto-labeling policy for SharePoint and OneDrive**: Label existing files at rest that match sensitive information patterns:
  ```powershell
  ...
  New-AutoSensitivityLabelRule -Name "Financial Docs SIT Match" `
    ...
    -ApplySensitivityLabel "Confidential-Finance"
  ```
- **Simulate before enforcing**: Always run auto-labeling in simulation mode first. Şunu incele: simulation results in the Microsoft Purview portal under Information Protection > Auto-labeling. The simulation shows estimated matches per location and sample content matches for validation. Only switch to enforcement mode after confirming accuracy:
  ```powershell
...
### Adım 5: Monitor with Activity Explorer and Manage DLP Alerts

Use Activity Explorer and the DLP alerts dashboard to monitor policy effectiveness and Araştır: incidents:

- **Access Activity Explorer**: Şuraya git: Microsoft Purview portal > Data Classification > Activity Explorer. Filter by activity type "DLPRuleMatch" to see all DLP policy matches. Key columns include:
...
- **Analyze false positive rates**: Export Activity Explorer data filtered by "Override" actions with justification text to identify rules that users frequently override. A high override rate (>20%) indicates the rule may be too aggressive or matching non-sensitive content:
  ...
  ```
- **Configure DLP alerts**: Kur: alert policies in Microsoft Purview > Data Loss Prevention > Alerts to receive notifications for high-severity matches:
  ```powershell
...
- **Retrieve DLP policy match details for compliance reporting**: Use the unified audit log to extract granular DLP match data including the matched content, SIT type, and confidence level:
  ...
  Search-UnifiedAuditLog -StartDate (Get-Date).AddDays(-7) `
    ...
    Select-Object CreationDate, UserIds, Operations,
      ...
      @{N='SITMatched';E={($_.AuditData | ConvertFrom-Json).SensitiveInfoTespits.SensitiveType}} |
...
| **Sensitive Information Type (SIT)** | A pattern-based classifier that tespit etme (s) specific data patterns (credit card numbers, SSNs, custom regex) in content. Each SIT has a confidence level (low/medium/high) Belirle:d by primary pattern match plus corroborating evidence (keywords, proximity). |
| **DLP Policy** | A set of rules that tespit etmesensitive information in Microsoft 365 locations (Exchange, SharePoint, OneDrive, Teams, Endpoints) and apply protective actions (audit, warn with override, block) based on the sensitivity of matched content and the sharing context. |
...
- **Microsoft Graph Security API**: REST API providing programmatic Erişim: DLP alerts (security/alerts_v2), data classification insights, and protection scope evaluation for integrating Purview DLP with custom applications and SIEM platforms.
- **Microsoft Intune**: Endpoint management platform used to onboard Windows and macOS devices to endpoint DLP, Dağıt: configuration profiles, and manage device compliance states.
...
2. Şunu oluştur: DLP policy "PCI-DSS Financial Data Protection" scoped to all Exchange, SharePoint, OneDrive, Teams, and Endpoint locations. Configure two rules: a warning rule for 1-4 credit card numbers (notify user, allow override with justification) and a blocking rule for 5+ credit card numbers (block external sharing, generate incident report to compliance team).
3. Dağıt: endpoint DLP with rules blocking copy-to-USB and upload-to-unapproved-cloud for any file containing credit card numbers or labeled "Confidential - Finance". Allow printing with audit logging. Configure approved USB device exceptions for encrypted corporate drives by vendor/product ID.
4. Şunu oluştur:uto-labeling policies that scan existing SharePoint finance sites and OneDrive locations, automatically applying "Confidential - Finance" to documents matching credit card number or bank account number SITs with confidence level 85+.
...
**Pitfalls**:
- Dağıt:ing DLP policies in enforcement mode without simulation, causing mass blocking of legitimate business communications and user productivity disruption
...
- Not coordinating auto-labeling Dağıt:ment with document library owners, leading to unexpected label changes on existing content that alter access permissions
...
**Approach**:
1. Şunu oluştur: custom sensitive information type using regex `RX-20[2-3][0-9]-[0-9]{4}` with corroborating keywords ("compound", "trial", "formulation", "assay", "efficacy") within 300-character proximity. Set primary pattern at 85% confidence and keyword-corroborated pattern at 95%.
2. Şunu oluştur: second custom SIT for compound identifiers using regex `CPD-[A-Z]{3}-[0-9]{5}` with keywords ("molecule", "synthesis", "pharmacokinetics") for higher confidence matching.
3. Dağıt: a DLP policy scoped to the Research department's Exchange distribution list, SharePoint research site collection, and research team OneDrive accounts. Block external sharing, block forwarding to non-research internal users, and Şunu üret:lerts for the research compliance officer.
...
5. Şunu oluştur: sensitivity label "Highly Confidential - Research" with encryption restricting Erişim: the Research security group. Configure auto-labeling to apply this label to documents matching either custom SIT.
6. Monitor Activity Explorer weekly for 30 days post-Dağıt:ment. The compliance team identifies that the RX-YYYY-NNNN regex matches historical project codes in archived documents. Refine the regex to `RX-202[4-6]-[0-9]{4}` to target only active project codes and reduce false positives by 60%.
...
- Not testing the custom SIT against a representative sample corpus before Dağıt:ing the DLP policy, missing edge cases in the regex pattern
...
```
## DLP Policy Dağıt:ment Report

**Policy Name**: PCI-DSS Financial Data Protection
**Dağıt:ment Date**: 2026-03-19
...
4. **Dağıt: endpoint DLP** in audit mode for 7 additional days before
