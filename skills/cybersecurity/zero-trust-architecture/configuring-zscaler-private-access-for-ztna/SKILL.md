...
description: Configuring Zscaler Private Access (ZPA) to replace traditional VPN with zero trust network access by Dağıt:ing App Connectors, defining application segments, configuring access policies based
  on user identity and device posture, and integrating with IdPs.
tags:
- sase
- siber-güvenlik
...
- replacing yaparken: traditional VPN concentrators with application-level zero trust access
- providing yaparken: remote users secure Erişim: internal applications without network-level connectivity
- implementing yaparken least-privilege access where users only see authorized applications
- needing yaparken: to make internal applications invisible to unauthorized users and the internet
- integrating yaparken ZTNA with existing SASE architecture using Zscaler Internet Access (ZIA)

**Kullanma:** for applications requiring raw UDP access (ZPA primarily supports TCP), for providing full network-level access equivalent to site-to-site VPN (use ZPA AppProtection or branch connector instead), or when the organization requires on-premises-only access control without cloud dependency.
...
- Zscaler Client Connector Dağıtılmış on user endpoints

## İş Akışı

### Adım 1: Dağıt: App Connectors in Application Network

App Connectors establish outbound-only tunnels to the ZPA cloud, providing Erişim: internal applications.
...
### Adım 2: Define Server Groups and Application Segments

Map internal applications to server groups and Şunu oluştur:pplication segments.

```text
ZPA Admin Portal Configuration:

1. Server Groups:
   Şuraya git:: Administration > App Connectors > Server Groups
...
2. Application Segments:
   Şuraya git:: Resources > Application Segments > Add Application Segment
...
### Adım 3: Configure Access Policies
...
### Adım 4: Configure Device Posture Profiles
...
### Adım 5: Enable Browser Access for Clientless ZTNA
...
### Adım 6: Configure Logging and Monitoring

Kur: log streaming for SIEM integration and continuous monitoring.
...
| Term | Definition |
|------|------------|
| App Connector | Lightweight Linux service that creates outbound-only encrypted tunnels from internal networks to ZPA cloud, providing Erişim: applications without inbound ports |
...
| Zscaler Client Connector | Endpoint agent kurulu: user devices that routes traffic to ZPA cloud for policy enforcement and application access |
...
- **ZPA App Connector**: Outbound-only tunnel endpoint Dağıtılmış in application networks
...
**Approach**:
1. Dağıt: 4 App Connectors (2 per data center) with HA configuration
