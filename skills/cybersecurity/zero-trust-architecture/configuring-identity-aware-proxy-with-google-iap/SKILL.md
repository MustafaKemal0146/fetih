...
description: Configuring Google Cloud Identity-Aware Proxy (IAP) to enforce per-request identity verification for Compute Engine, App Engine, Cloud Run, and GKE services using access levels, context-aware
  policies, and programmatic access with service accounts.
tags:
- cybersecurity
- siber-güvenlik
...
- protecting yaparken: Google Cloud applications (App Engine, Cloud Run, GKE, Compute Engine) with identity-based access
- implementing yaparken context-aware access requiring device posture and location verification
- providing yaparken: secure Erişim: internal tools without VPN or public IP exposure
- needing yaparken: per-request authentication and authorization for web applications and TCP services
- configuring yaparken programmatic Erişim: IAP-protected resources using service accounts

**Kullanma:** for non-HTTP applications that cannot be placed behind an HTTPS load balancer, for public-facing applications that need unauthenticated access, or when applications handle their own authentication and IAP would conflict with existing auth flows.

## Ön Gereksinimler

- Google Cloud project with billing enabled
- IAP API enabled (`gcloud services enable iap.googleapis.com`)
- Application Dağıtılmış behind HTTPS Load Balancer, App Engine, or Cloud Run
...
### Adım 1: Enable IAP on Backend Services
...
### Adım 2: Configure IAM Bindings for Access Control

Grant Erişim: specific users and groups with optional access level conditions.
...
### Adım 3: Şunu oluştur:ccess Levels with Access Context Manager
...
### Adım 4: Configure Session Settings and Re-authentication
...
### Adım 5: Configure Programmatic Access for Service Accounts
...
### Adım 6: Kur: Audit Logging and Monitoring
...
| IAP Tunnel | Secure TCP tunnel through IAP allowing SSH, RDP, and other TCP Erişim: VMs without public IPs or VPN |
...
- **IAP TCP Forwarding**: Tunnel-based Erişim: VMs for SSH/RDP without public IPs
...
**Approach**:
1. Dağıt: all services behind an HTTPS Load Balancer with managed SSL certificates
...
6. Configure IAP TCP tunneling for SSH Erişim: GKE nodes (replacing SSH bastion host)
7. Set re-authentication to 4 hours for admin tools, 8 hours for monitoring
8. Configure Cloud Audit Logs and Şunu oluştur:lerting for repeated denials
