...
resource "boundary_target" "ssh_production" {
  ...
  description  = "SSH Erişim: production servers"
  type         = "ssh"
  scope_id     = boundary_scope.production.id
  default_port = 22
...
5. **Dağıt: multi-hop workers** for accessing resources across network boundaries
