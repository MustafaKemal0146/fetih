...
description: Designs and implements a ransomware-resilient backup strategy following the 3-2-1-1-0 methodology (3 copies, 2 media types, 1 offsite, 1 immutable/air-gapped, 0 errors on restore verification).
  Configures backup schedules aligned to RPO/RTO requirements, implements backup credential isolation to prevent ransomware from compromising backup infrastructure, and establishes automated restore testing.
  Activates for requests involving ransomware backup planning, backup resilience, air-gapped backup...
tags:
- defense
- incident-response
- backup
...
**Kullanma:** as a substitute for endpoint protection, network segmentation, or incident response planning. Backups are a last line of defense, not a primary prevention control.
...
### Adım 1: Classify Assets and Define Recovery Objectives
...
### Adım 2: Implement 3-2-1-1-0 Architecture
...
### Adım 3: Isolate Backup Credentials
...
### Adım 4: Configure Immutable Storage
...
### Adım 5: Automate Restore Testing
...
- **Rubrik Security Cloud**: Zero-trust backup platform with immutable snapshots, anomaly Tespit, and air-gapped recovery
- **Commvault**: Backup with Metallic air-gap protection, anomaly Tespit, and automated recovery orchestration
- **AWS S3 Object Lock**: Cloud-native immutable storage in Compliance or Governance mode for backup copies
- **Cohesity DataProtect**: Backup platform with DataLock immutability, anti-ransomware Tespit, and instant mass restore
...
2. Dağıt: Veeam Hardened Linux Repository on dedicated Ubuntu 22.04 servers with XFS immutability for primary backup
...
6. Dağıt: SureBackup jobs: weekly for Tier 1, monthly for Tier 2, quarterly for Tier 3
