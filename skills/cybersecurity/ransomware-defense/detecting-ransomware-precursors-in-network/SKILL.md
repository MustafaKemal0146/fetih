---
name: Tespit etme-ransomware-precursors-in-network
description: tespit etme (s) early-stage ransomware indicators in network traffic before encryption begins, including initial access broker activity, command-and-control beaconing, credential harvesting, reconnaissance
  scanning, and staging behavior. Uses network Tespit tools (Zeek, Suricata, Arkime), SIEM correlation rules, and threat intelligence feeds to identify ransomware precursor patterns such as Cobalt Strike
  beacons, Mimikatz network signatures, and RDP brute-force attempts. Activates for requests inv...
tags:
- ransomware-defense
- defense
- incident-response
...
- Building Tespit rules for pre-ransomware network activity (the average time from Cobalt Strike Dağıt:ment to encryption is 17 minutes)
- Monitoring for initial access broker (IAB) indicators that precede ransomware Dağıt:ment
- Creating SIEM correlation rules that chain multiple precursor events into high-confidence alerts
- Tuning network Tespit systems to distinguish ransomware staging from normal administrative activity
- Investigating suspicious network patterns that may indicate ransomware operators have established a foothold

**Kullanma:** for post-encryption response (see recovering-from-ransomware-attack). bu skill focuses on the pre-encryption Tespit window where containment can prevent data loss.

## Ön Gereksinimler

- Network Tespit platform (Zeek/Bro, Suricata, or Arkime/Moloch) Dağıtılmış on network TAP or SPAN ports
...
### Adım 1: Identify Ransomware Kill Chain Phases in Network Traffic

Map network-observable indicators to each pre-encryption phase:

| Kill Chain Phase | Network Indicators | Tespit Source |
|------------------|--------------------|------------------|
...
### Adım 2: Dağıt: Network Detection Rules
...
**Zeek scripts for behavioralDetect**
...
### Adım 3: Create SIEM Correlation Rules
...
### Adım 4: Integrate Threat Intelligence
...
### Adım 5: Establish Alert Triage and Escalation
...
| Internal port scan + admin share access | Medium-High | 30 minutes | Araştır: source host, check EDR telemetry |
...
| **Dwell Time** | Time between initial compromise and ransomware Dağıt:ment, averaging 21 days but sometimes as short as 17 minutes |
| **Initial Access Broker (IAB)** | Threat actors who sell compromised network Erişim: ransomware operators on dark web markets |
...
| **Kerberoasting** | Credential harvesting technique requesting Kerberos service tickets for offline cracking, tespit etme (able) via unusual TGS-REQ patterns |
...
- **Suricata**: High-performance IDS/IPS with protocol analysis and multi-threading support for ransomware signature Tespit
- **Arkime (formerly Moloch)**: Full packet capture and search platform for deep forensic investigation of network events
- **RITA (Real Intelligence Threat Analytics)**: Open-source tool for Tespit etme beaconing, DNS tunneling, and long connections in Zeek logs
- **AC-Hunter**: Network threat hunting platform from Active Countermeasures for beacon Tespit and C2 identification

## Common Scenarios

### Scenario: Tespit etme LockBit Precursors in a Manufacturing Network
...
5. SMB tree_connect events show Erişim: ADMIN$ shares on 12 hosts (lateral movement staging)
6. Containment: Host isolated, credentials for engineering user reset, blocking rule for C2 IP Dağıtılmış
7. Full IR initiated before ransomware Dağıt:ment could begin
...
- Setting Tespit thresholds too high to avoid false positives, missing low-and-slow reconnaissance
- Ignoring encrypted traffic analysis (JA3/JA4 fingerprinting) that can identify Cobalt Strike even in TLS tunnels

## Output Format

```
## Ransomware Precursor Tespit Alert

**Alert ID**: [SIEM-generated ID]
**Tespit Time**: [Timestamp]
