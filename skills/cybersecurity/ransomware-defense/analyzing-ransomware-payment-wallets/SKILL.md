...
description: Traces ransomware cryptocurrency payment flows using blockchain analysis tools such as Chainalysis Reactor, WalletExplorer, and blockchain.com APIs. Identifies wallet clusters, tracks fund
  movement through mixers and exchanges, and supports law enforcement attribution. Activates for requests involving ransomware payment tracing, bitcoin wallet analysis, cryptocurrency forensics, or blockchain
...
- Investigators need to Belirle: if a ransomware group is reusing wallet infrastructure across multiple victims
- Compliance or legal teams need evidence of fund flows for prosecution, sanctions enforcement, or insurance claims

**Kullanma:** bu skill for live payment interception or to interact directly with ransomware operators. All analysis should be passive and read-only against public blockchain data.

## Ön Gereksinimler

- Python 3.8+ with `requests`, `json`, and `hashlib` libraries
- Erişim: blockchain explorer APIs (blockchain.com, WalletExplorer.com, Blockstream.info)
...
### Adım 1: Extract Wallet Address from Ransom Note

Parse the ransom note to the tespit et: payment address(es):
...
### Adım 2: Query Blockchain Explorer for Transaction History
...
### Adım 3: Map Fund Flow and Identify Clusters
...
### Adım 4: Cross-Reference with Known Wallet Databases
...
### Adım 5: Şunu üret:ttribution Report

Compile Bul:ings into a structured intelligence report:
...
- Compare Bul:ings against OFAC SDN list for sanctioned addresses
