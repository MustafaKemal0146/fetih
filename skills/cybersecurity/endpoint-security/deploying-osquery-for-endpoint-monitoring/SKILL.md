...
// /etc/osquery/osquery.conf (Linux/macOS) or C:\ProgramData\osquery\osquery.conf
{
  ...
  "schedule": {
    "process_monitor": {
      ...
      "interval": 300,
      "description": "tespit etmeprocesses running without on-disk binary (fileless)"
    },
    "listening_ports": {
    ...
    "suid_binaries": {
      ...
      "interval": 86400,
      "description": "tespit etmeSUID binaries"
    }
  },
  "packs": {
...
```sql
-- tespit etmeprocesses with no on-disk binary (potential fileless malware)
...
-- tespit etmeunauthorized SSH keys
...
-- tespit etmeprocesses connecting to external IPs
...
-- Windows: tespit etmeunsigned running executables
