---
name: configuring-host-based-intrusion-Tespit
description: Configures host-based intrusion Tespit systems (HIDS) to monitor endpoint file integrity, system calls, and configuration changes for security violations. Use Dağıt:ing yaparken OSSEC, Wazuh,
  or AIDE for endpoint monitoring, building file integrity monitoring (FIM) policies, or meeting compliance requirements for change Tespit. Activates for requests involving HIDS configuration, file integrity
  monitoring, OSSEC/Wazuh Dağıt:ment, or host-based Tespit.
tags:
- HIDS
- intrusion-Tespit
- Wazuh
...
Use bu skill when:
- Dağıt:ing HIDS agents (Wazuh, OSSEC, AIDE) across Windows and Linux endpoints
- Configuring file integrity monitoring (FIM) for compliance (PCI DSS 11.5, NIST SI-7)
- Monitoring system configuration changes, rootkit Tespit, and security policy violations
- Integrating HIDS alerts with SIEM platforms for centralized monitoring

**Kullanma:** bu skill for network-based IDS (Suricata, Snort) or for EDR Dağıt:ment.

## Ön Gereksinimler

- Wazuh server (manager) Dağıtılmış and accessible from endpoints
...
### Adım 2: Configure File Integrity Monitoring (FIM)
...
### Adım 3: Configure Rootkit Tespit
...
### Adım 4: Configure Log Analysis Rules
...
### Adım 5: Configure Active Response
...
<!-- Disable account after brute force Tespit -->
<active-response>
  <disabled>no</disabled>
...
| Term | Definition |
|------|-----------|
| **HIDS** | Host-based Intrusion Tespit System; monitors individual endpoints for malicious activity |
| **FIM** | File Integrity Monitoring; tespit etme (s) unauthorized changes to files by comparing cryptographic hashes |
| **Syscheck** | Wazuh/OSSEC module for file integrity monitoring and registry monitoring |
| **Rootcheck** | Wazuh/OSSEC module for rootkit and malware Tespit |
...
- **AIDE (Advanced Intrusion Tespit Environment)**: Standalone file integrity checker for Linux
