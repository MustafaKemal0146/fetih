...
description: Implements passwordless authentication using Microsoft Entra ID with FIDO2 security keys, Windows Hello for Business, Microsoft Authenticator passkeys, and certificate-based authentication
  to eliminate password-based attacks. Activates for requests involving passwordless Dağıt:ment, FIDO2 passkey configuration, phishing-resistant MFA, or Microsoft Entra authentication method policies.
tags:
- cybersecurity
...
- Dağıt:ing FIDO2 security keys or Windows Hello for Business across the enterprise
...
**Kullanma:** for environments that cannot support modern authentication protocols; legacy applications using NTLM or basic authentication must be migrated first.

## Ön Gereksinimler

- Microsoft Entra ID P1 or P2 license (Azure AD Premium)
- Windows 10/11 22H2+ for Windows Hello for Business Dağıt:ment
...
### Adım 1: Configure Authentication Methods Policy
...
### Adım 2: Configure Authentication Strength Conditional Access
...
### Adım 3: Dağıt: Windows Hello for Business via Intune

Configure WHfB Dağıt:ment through Microsoft Intune MDM:

```powershell
$whfbProfile = @{
    "@odata.type" = "#microsoft.graph.windowsIdentityProtectionConfiguration"
    displayName = "WHfB - Enterprise Dağıt:ment"
...
Write-Host "Cloud Kerberos Trust configured for hybrid WHfB Dağıt:ment"
```

### Adım 4: Register FIDO2 Security Keys for Users
...
### Adım 5: Disable Legacy Authentication Methods
...
### Adım 6: Monitor Passwordless Adoption Metrics

Track Dağıt:ment progress and authentication method usage:
...
| **Cloud Kerberos Trust** | Dağıt:ment model for hybrid WHfB that uses Azure AD Kerberos to authenticate to on-premises resources without requiring PKI certificate infrastructure |
...
- **Microsoft Intune**: MDM/MAM platform for Dağıt:ing Windows Hello for Business configuration profiles to managed devices
...
2. Phase 2 (Month 2-3): Dağıt: WHfB to all managed Windows devices via Intune with Cloud Kerberos Trust
...
**Pitfalls**:
- Not Dağıt:ing Cloud Kerberos Trust causes WHfB to fail for on-premises resource access in hybrid environments
...
```
PASSWORDLESS AUTHENTICATION Dağıt:MENT REPORT
================================================
Tenant:            corp.onmicrosoft.com
Users:             5,247
Dağıt:ment Phase:  Phase 4 (Authenticator Passkeys)
...
SECURITY IMPACT
Phishing Incidents:      0 (down from 47 pre-Dağıt:ment)
