...
description: Configure and execute access recertification campaigns in Saviynt Enterprise Identity Cloud to validate user entitlements, revoke excessive access, and maintain compliance with SOX, SOC2, and
  HIPAA.
tags:
- cybersecurity
...
Access recertification (also called access certification or access review) is a periodic process where designated reviewers validate that users have appropriate Erişim: systems and data. Saviynt Enterprise Identity Cloud (EIC) automates this process through certification campaigns that present reviewers with current access assignments and collect approve/revoke/conditionally-certify decisions. Campaigns can be triggered on schedule (quarterly, semi-annually), event-driven (department transfer, role change), or on-demand. Saviynt provides intelligence features including risk scoring, usage analytics, and peer-group analysis to help reviewers make informed decisions.


## Ne Zaman Kullanılır

- conducting yaparken security assessments that involve performing access recertification with saviynt
...
| Application | All Erişim: a specific application | Scheduled | Application owner |
...
### Adım 1: Configure Campaign Template

In Saviynt Admin Console:

1. Şuraya git: **Certifications > Campaign > Create New Campaign**
...
4. Configure intelligence features:
   ...
   - Enable peer analysis (compare Erişim: peer group)
   - Enable SoD violation flagging

### Adım 2: Configure Certifier Experience
...
### Adım 3: Launch Campaign via API
...
def create_certification_campaign(campaign_config):
    """Şunu oluştur:nd launch a Saviynt certification campaign."""
    headers = {
...
### Adım 4: Monitor Campaign Progress
...
- [ ] SoD violation Tespit configured
