...
description: Implements comprehensive Google Workspace security hardening including admin console configuration, phishing-resistant MFA enforcement, DLP policies, email authentication (SPF/DKIM/DMARC),
  OAuth app control, and external sharing restrictions. Activates for requests involving Google Workspace hardening, G Suite security configuration, or cloud office security administration.
tags:
- cybersecurity
- DMARC
- Google-Workspace
...
- Dağıt:ing or hardening a Google Workspace environment for enterprise use
...
**Kullanma:** for Microsoft 365 environments; Google Workspace has distinct admin console settings and API configurations that differ from Azure AD/Entra ID controls.

## Ön Gereksinimler

- Google Workspace Business Plus, Enterprise Standard, or Enterprise Plus license
- Super Admin Erişim: the Google Admin Console (admin.google.com)
...
### Adım 1: Harden Super Admin Accounts
...
gam Şunu oluştur:dmin breakglass-admin@corp.com "Super Admin"
...
### Adım 2: Enforce Phishing-Resistant Multi-Factor Authentication
...
### Adım 3: Configure Email Authentication and Anti-Phishing

Kur: SPF, DKIM, DMARC and advanced phishing protections:
...
### Adım 4: Implement Data Loss Prevention (DLP)
...
cat > dlp_rules.json << 'EOF'
{
  "dlpRules": [
    {
      "name": "PII Tespit - SSN",
      "description": "tespit etmeSocial Security Numbers in outbound email and Drive sharing",
      "trigger": {
        "contentMatchers": [
          {
    ...
    {
      "name": "Credit Card Number Tespit",
    ...
    {
      "name": "Confidential Document Tespit",
      "description": "tespit etmedocuments marked as Confidential or Internal Only",
      "trigger": {
        "contentMatchers": [
          {
...
echo "Or use the Google Workspace DLP API for programmatic Dağıt:ment"
```

### Adım 5: Control OAuth Applications and Third-Party Access
...
### Adım 6: Configure External Sharing and Drive Security
...
gam all users print filelist \
    ...
    query "visibility='anyoneWithLink' or visibility='anyoneCanBul:'" \
...
| **Context-Aware Access** | Security policy framework that evaluates device posture, location, and user identity before granting Erişim: Google Workspace applications |
...
3. Dağıt: SPF, DKIM, and DMARC starting with monitoring mode (p=none)
...
6. Set third-party app Erişim: blocked with allowlist of approved applications
7. Restrict external Drive sharing to approved partner domains only
8. Dağıt: DLP rules for PII, financial data, and confidential documents
...
DATA PROTECTION
DLP Rules Active:       7
  PIIDetect        SSN, Credit Card, Passport
