...
description: Builds comprehensive identity governance and lifecycle management processes including joiner-mover-leaver automation, role mining, access request workflows, periodic recertification, and orphaned
  account remediation using IGA platforms. Activates for requests involving identity lifecycle management, JML processes, role-based access provisioning, or identity governance program design.
tags:
- cybersecurity
- identity-governance
...
- Former employees retain Erişim: systems after termination (orphaned accounts)
...
- No centralized visibility into who has Erişim: what across the enterprise

**Kullanma:** for single-application user management; identity governance addresses cross-system lifecycle management requiring correlation of authoritative HR sources with downstream application provisioning.
...
### Adım 1: Define Identity Lifecycle States and Transitions
...
IDENTITY_LIFECYCLE = {
    "states": {
        ...
        "ACTIVE": {
            ...
            "automated_actions": [
                "Şunu oluştur:ctive Directory account",
...
### Adım 2: Implement Authoritative Source Integration
...
### Adım 3: Implement Role Mining and Birthright Access
...
class RoleMiningEngine:
    ...
    def tespit etme (_role_explosion)(self):
    ...
    def Bul:_orphaned_access(self):
        """
        Bul: entitlements that no longer align with any role definition.
...
### Adım 4: Build Access Request and Approval Workflow
...
### Adım 5: Implement Orphaned Account Detection and Remediation

Identify and remediate accounts without active identity associations:

```python
"""
Orphaned Account Tespit
...
class OrphanedAccountDetector:
    ...
    def tespit etme (_orphaned_accounts)(self):
...
6. Dağıt: self-service access request portal with risk-based approval chains
7. Run orphaned account Tespit to identify and remediate the 2,300 existing orphans
