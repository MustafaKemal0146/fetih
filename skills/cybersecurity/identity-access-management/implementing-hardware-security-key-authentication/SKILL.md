...
description: Implements FIDO2/WebAuthn hardware security key authentication including registration ceremonies, authentication flows, YubiKey enrollment, and passkey migration strategies. Builds a complete
  relying party server using the python-fido2 library that supports cross-platform authenticators, resident key (discoverable credential) workflows, and user verification policies. Activates for requests
  involving FIDO2 implementation, WebAuthn registration, hardware security key enrollment, YubiKey integr...
tags:
- cybersecurity
- CTAP2
- WebAuthn
...
- Dağıt:ing phishing-resistant multi-factor authentication (MFA) using FIDO2 hardware security keys for high-value accounts (administrators, developers, privileged users)
...
**Kullanma:** without HTTPS in production (WebAuthn requires a secure origin), for systems where users cannot physically access a USB/NFC port, or as the sole authentication factor without a recovery mechanism for lost keys.
...
### Adım 1: Relying Party Server Configuration

Şunu yapılandır: WebAuthn relying party (RP) identity and server:

- **Define RP identity**: Şunu oluştur: `PublicKeyCredentialRpEntity` with the relying party name (display name shown to users) and RP ID (the effective domain of the application). The RP ID must be a registrable domain suffix of the origin -- for example, `example.com` is valid for `https://auth.example.com` but `other.com` is not.
...
- **Configure attestation preference**: Set the attestation conveyance preference to control whether the server requests proof of the authenticator's identity:
  - `none`: No attestation requested (simplest, recommended for most Dağıt:ments)
...
- **Credential storage**: Design the database schema to store credential records: `credential_id` (binary), `public_key` (COSE key), `sign_count` (uint32 for clone Tespit), `user_id`, `created_at`, `last_used`, `display_name`, and `transports` (USB, NFC, BLE, internal).

### Adım 2: Registration Ceremony (Credential Creation)
...
### Adım 3: Authentication Ceremony (Assertion)
...
- **Client-side assertion**: The browser calls `navigator.credentials.get()` with the server options. The authenticator Bul:s the matching credential, performs user verification if required, increments the signature counter, and signs the challenge with the private key.
- **Complete authentication**: Call `server.authenticate_complete()` with the saved state, registered credentials, and the client response. The server verifies the assertion signature against the stored public key and validates the signature counter has incremented (clone Tespit).
...
### Adım 4: YubiKey Enrollment and Management
...
- **Primary key enrollment**: Register the user's primary YubiKey with their account. Şunu sakla: credential with a user-friendly label (e.g., "USB-A YubiKey - Office") and the authenticator's AAGUID for device identification.
...
- **Key attestation verification**: For enterprise Dağıt:ments, verify the attestation certificate chain to confirm the key is a genuine YubiKey from Yubico. Compare the AAGUID against Yubico's published values to the tespit et: exact model.
...
### Adım 5: Passkey Migration Strategy
...
| **WebAuthn** | The W3C Web Authentication API that allows web applications to Şunu oluştur:nd use public key credentials via `navigator.credentials.create()` (registration) and `navigator.credentials.get()` (authentication) |
...
| **Sign Count** | A monotonically increasing counter maintained by the authenticator and included in each assertion; used by the RP to tespit etmecloned authenticators |
...
### Scenario: Dağıt:ing FIDO2 MFA for a Development Team

**Context**: A software company wants to replace TOTP-based MFA with hardware security keys for its 50-person development team. Developers have root Erişim: production infrastructure and are high-value targets for phishing attacks. The company has standardized on YubiKey 5 NFC.
...
2. Dağıt: the WebAuthn relying party server integrated with the company's SSO (OAuth 2.0 / OpenID Connect) provider, configured with `authenticator_attachment: cross-platform` and `user_verification: required`
...
4. Şunu yapılandır: SSO provider to require FIDO2 as the second factor for all developer accounts, with a 30-day grace period where TOTP remains available
...
- Forgetting to Şunu doğrula: sign counter, missing cloned key attacks
- Not supporting NFC for developers who primarily work from tablets or phones
- Allowing TOTP as a permanent fallback, which undermines the phishing resistance of the FIDO2 Dağıt:ment
...
4. After successful passkey creation, prompt users to Şunu oluştur: passkey on a second device for redundancy
...
- Not implementing proper account recovery for users who lose Erişim: all registered passkeys
- Ignoring browser compatibility gaps, particularly in older Safari versions on iOS

## Output Format

```
## FIDO2 Dağıt:ment Report
