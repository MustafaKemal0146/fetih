...
- Dağıt:ing yaparken or configuring building role mining for rbac optimization capabilities in your environment
...
**3. Graph-Based Mining**: Model users and permissions as a bipartite graph, then Bul: dense subgraphs representing candidate roles.
...
### Adım 1: Extract User-Permission Data

Şunu topla: current access state from all identity sources:
...
### Adım 2: Bottom-Up Role Discovery Using Clustering

```python
from sklearn.cluster import AgglomerativeClustering
from sklearn.metrics import silhouette_score

def Bul:_optimal_clusters(matrix, max_k=50):
    """Bul: optimal number of roles using silhouette analysis."""
    scores = []
...
### Adım 3: Formal Concept Analysis
...
def mine_roles_fca(upa_matrix, min_support=3):
    ...
    # Bul: all maximal permission sets shared by at least min_support users
    for size in range(len(permissions), 0, -1):
        for perm_combo in combinations(permissions, size):
            perm_set = set(perm_combo)
            # Bul: users who have ALL permissions in this set
            matching_users = []
            for user in users:
...
### Adım 4: Evaluate and Select Roles
...
- [ ] Optimal role count Belirle:d via silhouette analysis or WSC
