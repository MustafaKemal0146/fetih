...
description: Performs OAuth 2.0 scope minimization review to identify over-permissioned third-party application integrations, excessive API scopes, unused token grants, and risky OAuth consent patterns
  across identity providers and SaaS platforms. Activates for requests involving OAuth scope audit, API permission review, third-party app risk assessment, or consent grant minimization.
tags:
- cybersecurity
- third-party-risk
- consent-review
...
**Kullanma:** for reviewing first-party application permissions within the same trust boundary; OAuth scope minimization focuses on third-party and cross-boundary consent grants.

## Ön Gereksinimler

- Admin Erişim: identity providers (Microsoft Entra ID, Okta, Google Workspace)
...
### Adım 1: Inventory All OAuth Grants and Consent Permissions
...
### Adım 2: Classify OAuth Scopes by Risk Level
...
MICROSOFT_GRAPH_SCOPE_RISK = {
    ...
    "high": {
        ...
        "risk_description": "Broad read Erişim: sensitive organizational data",
    ...
    "medium": {
        ...
        "risk_description": "Scoped Erişim: specific data types with write capability",
...
### Adım 3: Identify Over-Permissioned Applications

tespit etmeapps requesting more permissions than functionally needed:

```python
"""
Over-Permission Tespit
Identifies applications with excessive OAuth scopes relative to their function.
"""

def tespit etme (_over_permissions)(inventory, approved_apps_catalog):
    """
    Compare actual permissions against approved scope catalog
    to Bul: over-permissioned applications.
    """
    Bul:ings = []
    ...
    for app_name, permissions in app_permissions.items():
        ...
        if not approved:
            ...
            Bul:ings.append({
                "app_name": app_name,
                "Bul:ing_type": "UNAPPROVED_APPLICATION",
                "severity": "HIGH",
        ...
        # Bul: unused scopes (approved but activity logs show no API calls)
        # This requires API activity log correlation
        unused = approved_scopes - actual_scopes
        if unused:
            Bul:ings.append({
                "app_name": app_name,
                "Bul:ing_type": "UNUSED_APPROVED_SCOPES",
                "severity": "LOW",
        ...
        broad_patterns = [
            ("Mail.ReadWrite", "Mail.Read", "Write Erişim: mail when only read needed"),
            ("Files.ReadWrite.All", "Files.Read.All", "Write Erişim: all files when only read needed"),
            ("Directory.ReadWrite.All", "Directory.Read.All", "Write Erişim: directory when only read needed"),
            ("User.ReadWrite.All", "User.Read.All", "Write Erişim: users when only read needed"),
        ]
        ...
        for broad, narrow, description in broad_patterns:
            if broad in actual_scopes:
                Bul:ings.append({
                    "app_name": app_name,
                    "Bul:ing_type": "OVERLY_BROAD_SCOPE",
                    "severity": "MEDIUM",
                    "detail": description,
                    "current_scope": broad,
...
### Adım 4: Audit Token Usage and tespit etmeStale Grants
...
### Adım 5: Generate Remediation Plan and Execute Scope Reduction

Şunu oluştur:nd execute the scope minimization remediation plan:
...
def generate_remediation_plan(Bul:ings, stale_grants):
    """Create prioritized remediation plan."""
    plan = []

    # Priority 1: Revoke unapproved applications
    for f in Bul:ings:
        if f["Bul:ing_type"] == "UNAPPROVED_APPLICATION":
            plan.append({
                "priority": 1,
    ...
    for f in Bul:ings:
        if f["Bul:ing_type"] == "EXCESSIVE_SCOPES":
            plan.append({
                "priority": 2,
    ...
    for f in Bul:ings:
        if f["Bul:ing_type"] == "OVERLY_BROAD_SCOPE":
            plan.append({
                "priority": 3,
...
| **Application Permission** | OAuth scope granted directly to the application without user context, providing Erişim: all users' data (high risk) |
...
- Not implementing ongoing monitoring to tespit etmenew unauthorized OAuth grants after remediation
