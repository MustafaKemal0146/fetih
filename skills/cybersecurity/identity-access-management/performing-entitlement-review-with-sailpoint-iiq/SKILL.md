...
description: Performs entitlement review and access certification campaigns using SailPoint IdentityIQ including manager certifications, targeted entitlement reviews, role-based access validation, SOD violation
  remediation, and automated revocation workflows. Activates for requests involving access reviews, entitlement certifications, SailPoint IIQ governance, or periodic user access recertification.
tags:
...
- Audit Bul:ings require evidence of periodic access review and remediation tracking

**Kullanma:** for real-time access control decisions; IdentityIQ certifications are periodic review processes designed for governance and compliance validation.

## Ön Gereksinimler

- SailPoint IdentityIQ 8.2+ Dağıtılmış with database backend (Oracle, MySQL, or SQL Server)
...
### Adım 1: Define Certification Campaign Strategy
...
### Adım 2: Configure Targeted Entitlement Certification

Kur: focused reviews for high-risk applications and privileged entitlements:
...
### Adım 3: Implement SOD Policy Checks Within Certifications
...
### Adım 4: Configure Revocation and Remediation Workflows
...
// Adım 1: Create provisioning plan for revocation
...
// Adım 2: Execute provisioning with retry logic
...
// Adım 3: Send notification to user and manager
...
### Adım 5: Monitor Campaign Progress and Compliance Metrics
...
### Adım 6: Şunu üret:udit Evidence and Reports

Export certification results for auditor review:

```java
// Şunu üret:udit report for completed certifications
import sailpoint.object.*;
import sailpoint.api.*;
...
| **Access Review** | Individual review unit within a campaign where a certifier İncele:s and makes approve/revoke decisions on specific user entitlements |
...
| **SOD Violation** | Separation of Duties conflict where a user holds entitlements from two or more conflicting access groups that Şunu oluştur: segregation risk |
...
- Missing service accounts from review scope when they have Erişim: financial systems
