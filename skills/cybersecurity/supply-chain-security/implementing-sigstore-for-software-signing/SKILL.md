...
description: Implements Sigstore-based software signing and verification using Cosign keyless signing, Rekor transparency log verification, and Fulcio certificate authority integration to establish cryptographic
  provenance for container images, binaries, and software artifacts. The practitioner configures OIDC-based identity binding, verifies signing events against the Rekor transparency log, and integrates signing
  workflows into CI/CD pipelines. Activates for requests involving software supply chain sign...
tags:
- software-signing
- OIDC
- siber-güvenlik
...
source_subdomain: supply-chain-security
nist_csf:
- GV.SC-01
- GV.SC-03
- GV.SC-06
...
**Kullanma:** for signing artifacts that require air-gapped or offline signing workflows where OIDC authentication is unavailable, for environments that cannot reach the public Sigstore infrastructure (Fulcio, Rekor) and have no private instance Dağıtılmış, or as a replacement for traditional PGP/GPG signing where regulatory compliance mandates specific key management procedures.

## Ön Gereksinimler

- Cosign CLI v2.4+ kurulu (`go install github.com/sigstore/cosign/v2/cmd/cosign@latest` or binary release)
- Erişim: an OIDC identity provider supported by Fulcio (Google, GitHub, Microsoft, or a custom OIDC issuer)
- Container registry credentials (for signing container images) with push Erişim: store signature objects
- Python 3.9+ with `sigstore`, `requests`, and `cryptography` packages for the automation agent
- Network Erişim: `fulcio.sigstore.dev`, `rekor.sigstore.dev`, and `tuf-repo-cdn.sigstore.dev` (or private Sigstore instance URLs)

## İş Akışı

### Adım 1: Install and Configure Cosign
...
### Adım 2: Keyless Signing with Cosign and Fulcio
...
### Adım 3: Verify Signed Artifacts

Şunu doğrula: artifacts were signed by expected identities from expected OIDC issuers:
...
### Adım 4: Query the Rekor Transparency Log

Search and verify entries in the Rekor transparency log to audit signing events:

- **Search by email identity**: Use `rekor-cli search --email user@example.com` to Bul: all signing events for an identity
- **Search by artifact hash**: Use `rekor-cli search --sha sha256:<hash>` to Bul: signing events for a specific artifact
...
### Adım 5: Integrate into CI/CD Pipelines

Embed signing and verification into build and Dağıt:ment pipelines:
...
- **Kubernetes admission enforcement**: Dağıt: Sigstore Policy Controller or Kyverno with Cosign verification policies to reject unsigned or incorrectly signed images at admission time
...
**Context**: A DevOps team builds container images in GitHub Actions and Dağıt:s to a Kubernetes cluster. They need to ensure only images built by their CI pipeline can be Dağıtılmış, preventing supply chain attacks from compromised registries or unauthorized pushes.
...
3. Dağıt: Sigstore Policy Controller to the Kubernetes cluster with a ClusterImagePolicy requiring signatures from `--certificate-identity-regexp=https://github.com/myorg/myrepo/.*` and `--certificate-oidc-issuer=https://token.actions.githubusercontent.com`
...
5. Şunu test et: admission controller by attempting to Dağıt: an unsigned image and confirming it is rejected with a policy violation error
