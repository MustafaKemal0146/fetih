...
description: Parses Software Bill of Materials (SBOM) in CycloneDX and SPDX JSON formats to identify supply chain vulnerabilities by correlating components against the NVD CVE database via the NVD 2.0 API.
  Builds dependency graphs, calculates risk scores, identifies transitive vulnerability paths, and generates compliance reports. Activates for requests involving SBOM analysis, software composition analysis,
  supply chain security assessment, dependency vulnerability scanning, CycloneDX/SPDX parsing, or CV...
tags:
- dependency-analysis
- syft
- SBOM
...
- Incident response needs to Belirle: if a newly disclosed CVE affects Dağıtılmış software
- Procurement team requires supply chain risk assessment for a software acquisition

**Kullanma:** for runtime vulnerability scanning of live systems; use container scanning tools (Trivy, Grype CLI) or host-based vulnerability scanners (Nessus, Qualys) instead.
...
- Network Erişim: NVD API (https://services.nvd.nist.gov/rest/json/cves/2.0)
- Optionally: syft for SBOM generation, grype for cross-validation

## İş Akışı

### Adım 1: Generate SBOM (if not provided)

Use syft to Şunu oluştur:n SBOM from a container image or project directory:
...
### Adım 2: Parse SBOM and Extract Components
...
### Adım 3: Correlate Components with NVD CVE Database

Query the NVD 2.0 API to Bul: known vulnerabilities for each component:
...
### Adım 4: Build Dependency Graph and Identify Transitive Risks
...
### Adım 6: Cross-Validate with Grype

Use grype to independently scan the SBOM and compare Bul:ings:
...
### Adım 7: Generate Compliance Report
...
**Context**: After the Log4Shell (CVE-2021-44228) disclosure, the security team needs to Belirle: which vendor-supplied applications contain vulnerable versions of log4j. Several vendors have provided SBOMs per contractual requirements.

**Approach**:
1. Collect all vendor SBOMs (CycloneDX or SPDX JSON format)
2. Parse each SBOM and Ara: log4j-core components with versions < 2.17.1
...
7. Cross-validate Bul:ings with grype scan of the same SBOMs
