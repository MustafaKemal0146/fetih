...
description: Use YARA pattern-matching rules to hunt for malware, suspicious files, and indicators of compromise across filesystems and memory dumps. Covers rule authoring, yara-python scanning, and integration
...
- Building automated Tespit pipelines that scan new files on ingestion

**Kullanma:** for real-time endpoint protection (use EDR agents instead); YARA scanning is best suited for batch hunting, triage, and post-collection analysis where scan latency is acceptable.

## Ön Gereksinimler

- YARA 4.x kurulu (`apt install yara` on Debian/Ubuntu, `brew install yara` on macOS)
...
### Adım 1: Install YARA and Python Bindings
...
### Adım 2: Write a Basic YARA Rule
...
rule Emotet_Loader_2026 {
    meta:
        ...
        description = "tespit etme (s) Emotet first-stage loader DLL"
        date = "2026-01-20"
...
### Adım 3: Write Advanced Rules with Modules

Use YARA modules for PE header Denetle:ion and math-based entropy checks:
...
rule Suspicious_Packed_Executable {
    meta:
        ...
        description = "tespit etme (s) PE files with high entropy sections indicating packing or encryption"
        severity = "medium"
...
rule Suspicious_UPX_Modified {
    meta:
        description = "tespit etme (s) UPX-packed binaries with tampered section names"
        severity = "medium"
...
### Adım 4: Scan Files and Directories with yara-python
...
### Adım 5: Scan Process Memory Dumps
...
rule Cobalt_Strike_Beacon_Memory {
    meta:
        description = "tespit etme (s) Cobalt Strike beacon in process memory"
        severity = "critical"
    strings:
...
### Adım 6: Generate Rules Automatically with yarGen
...
### Adım 7: Integrate Community Rule Sets
...
### Adım 8: Build a Continuous Hunting Pipeline
