...
description: Hunt for adversary persistence through Windows Management Instrumentation event subscriptions by monitoring WMI consumer, filter, and binding creation events that execute malicious code triggered
  by system events.
tags:
- threat-hunting
- event-subscription
...
- proactively yaparken: searching for fileless persistence mechanisms in Windows environments
- threat intelligence sonrasında reports indicate WMI-based persistence by APT groups (APT29, APT32, FIN8)
- investigating yaparken systems where malware persists across reboots despite cleanup attempts
- incident response sırasında when standard persistence locations (Run keys, scheduled tasks) are clean
...
4. **Check Event Filter Triggers**: İncele: what triggers the subscription. Common malicious triggers include system startup (Win32_ProcessStartTrace), user logon, or timer-based execution intervals.
5. **Araştır: WmiPrvSe.exe Child Processes**: When a WMI subscription fires, the action is executed by WmiPrvSe.exe. Hunt for unusual child processes of WmiPrvSe.exe.
6. **Correlate with MOF Compilation**: tespit etme`mofcomp.exe` usage which compiles MOF files to create WMI subscriptions programmatically.
