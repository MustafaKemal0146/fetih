...
description: Identify command-and-control beaconing patterns in network traffic by applying statistical frequency analysis, jitter calculation, and coefficient of variation scoring to tespit etmeperiodic callbacks
...
- proactively yaparken: searching for compromised endpoints calling back to C2 infrastructure
- threat intelligence sonrasında reports indicate active C2 frameworks targeting your sector
- network yaparken: logs show periodic outbound connections to unfamiliar destinations
- During purple team exercises validating C2 Tespit capabilities
- investigating yaparken a potential breach and need to identify active C2 channels
...
1. **Define Beacon Parameters**: Establish Tespit thresholds -- coefficient of variation (CV) below 0.20 indicates strong periodicity, minimum 50 connections over 24 hours, average interval between 30 seconds and 24 hours.
...
4. **Apply Jitter Analysis**: Sophisticated C2 frameworks like Cobalt Strike add jitter (randomness) to beacon intervals. The Sunburst backdoor beaconed every 15 minutes plus/minus 90 seconds. Analyze jitter patterns to tespit etmeeven randomized beaconing.
...
8. **Correlate with Endpoint Telemetry**: Map beaconing source IPs to endpoint hostnames via DHCP logs, then correlate with process creation events (Sysmon Event ID 1, 3) to the tespit et: responsible process.
9. **Score and Prioritize**: Assign risk scores based on CV value, domain age, TI matches, data size consistency, and suspicious port usage. Escalate high-confidence Bul:ings.
...
| Jitter | Random variation added to beacon interval to evade Tespit |
...
### KQL -- Microsoft Sentinel Beacon Tespit
```kql
DeviceNetworkEvents
...
### Sigma Rule -- Beaconing Pattern Tespit
```yaml
...
3. **DNS Tunneling C2**: Encoded data exfiltration via DNS TXT/CNAME queries to attacker-controlled domains, tespit etme (able) via high subdomain entropy and query volume.
