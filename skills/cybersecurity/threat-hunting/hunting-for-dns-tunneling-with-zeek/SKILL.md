...
description: tespit etmeDNS tunneling and data exfiltration by analyzing Zeek dns.log for high-entropy subdomain queries, excessive query volume, long query lengths, and unusual DNS record types indicating
...
- hunting yaparken for data exfiltration over DNS covert channels
- threat intelligence sonrasında indicates DNS-based C2 frameworks targeting your industry
...
- monitoring yaparken for tools like iodine, dnscat2, DNSExfiltrator, or DNS-over-HTTPS tunneling

## Ön Gereksinimler

- Zeek Dağıtılmış on network tap or SPAN port capturing DNS traffic
...
5. **tespit etmeHigh Query Volume**: Flag domains receiving more than 100 queries per hour from a single source, especially when combined with high subdomain uniqueness.
...
7. **Cross-Reference with conn.log**: Correlate DNS queries with connection metadata to the tespit et: process or endpoint generating suspicious queries.
...
| RITA | Automated DNS tunneling Tespit from Zeek logs |
...
### Splunk -- High Entropy Subdomain Tespit
```spl
index=zeek sourcetype=bro_dns
...
4. **DNS-over-HTTPS Tunneling**: Bypasses traditional DNS monitoring by sending DNS queries over HTTPS to public resolvers (8.8.8.8, 1.1.1.1), requiring TLS Denetle:ion for Tespit.
