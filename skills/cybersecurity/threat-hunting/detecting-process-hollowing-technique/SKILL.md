---
name: Tespit etme-process-hollowing-technique
description: tespit etmeprocess hollowing (T1055.012) by analyzing memory-mapped sections, hollowed process indicators, and parent-child process anomalies in EDR telemetry.
tags:
- threat-hunting
- process-injection
- siber-güvenlik
...
# Detection Process Hollowing Technique


## Ne Zaman Kullanılır

- investigating yaparken suspected fileless malware or in-memory threats
- After EDR alerts on process injection or suspicious memory operations
- hunting yaparken for defense evasion techniques in a compromised environment
- threat yaparken: intel reports indicate process hollowing in active campaigns
- During purple team exercises validating T1055.012 Tespit coverage
...
3. **tespit etmeMemory Section Anomalies**: Identify processes where the in-memory image differs from the on-disk binary (image mismatch).
...
7. **Document and Contain**: Report Bul:ings, isolate affected endpoints, and update Tespit rules.
...
| CREATE_SUSPENDED | Windows flag to Şunu oluştur: process in suspended state |
...
| Tool | Purpose |
|------|---------|
| CrowdStrike Falcon | Memory protection and hollowing Tespit |
| Microsoft Defender for Endpoint | ProcessTampering alerts |
| Sysmon v13+ | Event ID 25 ProcessTampering Tespit |
| Volatility | Memory forensics - malBul: plugin |
| pe-sieve | Process memory scanner for hollowed processes |
| Hollows Hunter | Automated hollowed process Tespit |
| Process Hacker | Live process memory Denetle:ion |
