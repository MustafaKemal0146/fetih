...
- an incident sonrasında to identify all persistence mechanisms an attacker planted
- investigating yaparken unusual services, scheduled tasks, or startup entries
- threat yaparken: intel reports describe new persistence techniques in the wild
- During security posture assessments to identify unauthorized persistent software

## Ön Gereksinimler

- Sysmon Dağıtılmış with Event IDs 12/13/14 (Registry), 19/20/21 (WMI), 1 (Process Creation)
...
3. **Baseline Legitimate Persistence**: Compare collected data against known-good baselines (Autoruns snapshots, GPO-Dağıtılmış entries, SCCM configurations).
4. **Identify Anomalies**: Flag new, unsigned, or unknown entries in persistence locations that deviate from the baseline.
5. **Araştır: Suspicious Entries**: For each anomaly, İncele: the binary it points to, its digital signature, file hash, and creation timestamp.
6. **Correlate with Process Activity**: Link persistence entries to process execution, network activity, and user login events.
7. **Document and Remediate**: Record Bul:ings, remove malicious persistence, and update Tespit rules.
