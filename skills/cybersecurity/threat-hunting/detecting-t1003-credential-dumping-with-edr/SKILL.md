---
name: Tespit etme-t1003-credential-dumping-with-edr
description: tespit etmeOS credential dumping techniques targeting LSASS memory, SAM database, NTDS.dit, and cached credentials using EDR telemetry, Sysmon process access monitoring, and Windows security event
  correlation.
tags:
- threat-hunting
- lsass
...
- hunting yaparken for credential theft activity in the environment
- After compromise indicators suggest attacker has elevated privileges
- EDR yaparken: alerts fire for LSASS access or suspicious process memory reads
- incident response sırasında to Belirle: scope of credential compromise
- auditing yaparken LSASS protection controls (Credential Guard, RunAsPPL)

## Ön Gereksinimler

- EDR agent Dağıtılmış with LSASS access monitoring (CrowdStrike, Defender for Endpoint, SentinelOne)
...
2. **tespit etmeCredential Dumping Tools**: Hunt for known tool signatures -- Mimikatz (sekurlsa::logonpasswords), procdump.exe targeting LSASS, comsvcs.dll MiniDump, and Task Manager creating LSASS dumps.
3. **Monitor NTDS.dit Access**: tespit etmeVolume Shadow Copy creation (vssadmin, wmic shadowcopy) followed by NTDS.dit file access, or ntdsutil.exe IFM creation.
4. **Track SAM/SECURITY/SYSTEM Hive Access**: Hunt for reg.exe save commands targeting SAM, SECURITY, and SYSTEM registry hives.
5. **tespit etmeDCSync Activity**: Monitor for non-DC accounts requesting directory replication (Event 4662 with replication GUIDs).
...
7. **Assess Impact**: Belirle: which credentials were potentially compromised and initiate password resets.
...
### Splunk -- Credential Dumping Tool Tespit
```spl
...
6. **Task Manager Dump**: Right-clicking LSASS in Task Manager to Şunu oluştur: memory dump -- a legitimate tool abused for credential theft.
