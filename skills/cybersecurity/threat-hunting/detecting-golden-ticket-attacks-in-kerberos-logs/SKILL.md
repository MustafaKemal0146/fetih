---
name: Tespit etme-golden-ticket-attacks-in-kerberos-logs
description: tespit etmeGolden Ticket attacks in Active Directory by analyzing Kerberos TGT anomalies including mismatched encryption types, impossible ticket lifetimes, non-existent accounts, and forged PAC
  signatures in domain controller event logs.
tags:
- threat-hunting
- golden-ticket
- kerberos
...
- KRBTGT yaparken: account hash may have been compromised via DCSync or NTDS.dit extraction
- hunting yaparken for forged Kerberos tickets used for persistent domain access
- After incident response reveals credential theft at the domain level
- investigating yaparken impossible logon patterns (users logging in from multiple locations simultaneously)
- During post-breach assessment to Belirle: if Golden Tickets are in use
...
2. **tespit etmeEncryption Type Anomalies**: Golden Tickets often use RC4 (0x17) encryption. If your domain enforces AES (0x12), any RC4 TGT is a red flag. Monitor TicketEncryptionType in Event 4769.
3. **Check Ticket Lifetime Anomalies**: Default Kerberos TGT lifetime is 10 hours with 7-day renewal. Golden Tickets can be forged with 10-year lifetimes. tespit etmetickets with durations exceeding policy.
...
5. **tespit etmeTGS Without Prior TGT**: When a service ticket (4769) appears without a preceding TGT request (4768) from the same IP/account, this may indicate a pre-existing Golden Ticket.
...
2. **RC4 Downgrade**: Golden Ticket forged with RC4 encryption in an AES-only environment, tespit etme (able) by encryption type mismatch.
