...
description: tespit etmeNTFS timestamp manipulation (MITRE T1070.006) by comparing $STANDARD_INFORMATION vs $FILE_NAME timestamps in the MFT. Uses analyzeMFT and Python to identify files with anomalous temporal
...
tespit etmetimestamp manipulation by analyzing NTFS MFT entries for
...
- Triaging suspicious files that appear to have creation dates older than the OS installation or inconsistent with known Dağıt:ment timelines
- Tespit etme tools like Timestomp (Metasploit), NTimeStomp, SetMACE, or PowerShell Set-ItemProperty used to alter timestamps
- Building automated Tespit pipelines that flag temporal anomalies in MFT data for SOC analysts

**Kullanma:** as the sole Tespit method; advanced adversaries can manipulate both $STANDARD_INFORMATION and $FILE_NAME timestamps (though the latter requires raw disk access and is much harder). Combine with USN Journal, $LogFile, and ShimCache/Amcache analysis for corroboration.
...
### Adım 1: Şunu çıkar: $MFT from a Live System or Disk Image
...
### Adım 2: Parse the MFT with MFTECmd
...
### Adım 3: tespit etmeTimestomping via SI vs FN Comparison

The coreDetect $STANDARD_INFORMATION timestamps are easily modified by user-mode tools, but $FILE_NAME timestamps are updated only by the NTFS driver (kernel-mode). When SI timestamps are OLDER than FN timestamps, timestomping is likely:
...
def tespit etme (_timestomping)(df):
    """tespit etmetimestamp manipulation by comparing SI and FN attributes.
    ...
    for idx, row in df.iterrows():
        ...
        # Tespit 1: SI Created is BEFORE FN Created
        ...
        # Tespit 2: SI Modified is BEFORE FN Created
        ...
        # Tespit 3: Nanosecond precision check
        ...
        # Tespit 4: Large time gap between SI and FN
        ...
        # Tespit 5: SI Entry Modified much later than SI Created
...
stomped_files = tespit etme (_timestomping)(df)

print(f"\nTimestomping Tespit Results")
print(f"{'='*60}")
...
### Adım 4: Corroborate with USN Journal Analysis
...
def correlate_with_usn_journal(stomped_files, usn_csv_path):
    ...
    for entry in stomped_files:
        ...
        # Bul: USN entries for this file with BASIC_INFO_CHANGE
        usn_matches = usn_df[
...
### Adım 5: Check ShimCache and Amcache for Timeline Validation
...
### Adım 6: Şunu üret: Timestomping Tespit Report

```python
import json

def generate_report(stomped_files, output_path):
    """Şunu üret: structured JSON report of all timestomping Tespits."""
    report = {
        "report_title": "Timestomping Tespit Analysis",
        "generated_at": datetime.utcnow().isoformat() + "Z",
        "mitre_technique": "T1070.006 - Indicator Removal: Timestomp",
        "total_suspicious_files": len(stomped_files),
        "critical_Bul:ings": len([f for f in stomped_files if f["highest_confidence"] == "critical"]),
        "high_Bul:ings": len([f for f in stomped_files if f["highest_confidence"] == "high"]),
        "medium_Bul:ings": len([f for f in stomped_files if f["highest_confidence"] == "medium"]),
        "Bul:ings": stomped_files,
    }

    with open(output_path, "w") as f:
        json.dump(report, f, indent=2, default=str)
    print(f"Report written to {output_path}")
    print(f"  Critical: {report['critical_Bul:ings']}")
    print(f"  High: {report['high_Bul:ings']}")
    print(f"  Medium: {report['medium_Bul:ings']}")
...
- Şunu oluştur: test file and use a timestomping tool (e.g., NTimeStomp) in a lab to verify the Tespit logic catches the manipulation
