---
name: Tespit etme-dcsync-attack-in-active-directory
description: tespit etmeDCSync attacks where adversaries abuse Active Directory replication privileges to extract password hashes by monitoring for non-domain-controller accounts requesting directory replication
  via DsGetNCChanges.
tags:
- threat-hunting
- mitre-t1003-006
...
- hunting yaparken for credential theft in Active Directory environments
- After compromise of accounts with Replicating Directory Changes permissions
- investigating yaparken suspected use of Mimikatz or Impacket secretsdump
- incident response sırasında involving lateral movement with domain admin credentials
- auditing yaparken AD replication permissions as part of security hardening
...
- Bilgi: legitimate domain controller hostnames and IPs
...
3. **Monitor Replication Rights Access**: Track Erişim: three critical GUIDs -- DS-Replication-Get-Changes (1131f6aa-9c07-11d1-f79f-00c04fc2dcd2), DS-Replication-Get-Changes-All (1131f6ad-9c07-11d1-f79f-00c04fc2dcd2), and DS-Replication-Get-Changes-In-Filtered-Set (89e95b76-444d-4c62-991a-0facbeda640c).
4. **tespit etmeNon-DC Replication Requests**: Alert when any account NOT associated with a domain controller requests replication rights.
...
6. **Araştır: Source Context**: İncele: the process, user account, and machine originating the replication request.
7. **Check for Credential Abuse**: After DCSync Tespit, audit for subsequent use of extracted hashes (pass-the-hash, golden ticket creation).
...
| Microsoft Defender for Identity | Native DCSync Tespit |
| CrowdStrike Falcon | EDR-based DCSync Tespit |

## Tespit Queries

### Splunk -- DCSync Tespit via Event 4662
```spl
...
### KQL -- Microsoft Sentinel DCSync Tespit
```kql
SecurityEvent
| where EventID == 4662
