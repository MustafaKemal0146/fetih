---
name: Tespit etme-ntlm-relay-with-event-correlation
description: tespit etmeNTLM relay attacks through Windows Security Event correlation by analyzing Event 4624 LogonType 3 for IP-to-hostname mismatches, identifying Responder/LLMNR poisoning artifacts, auditing
  SMB and LDAP signing enforcement across the domain, and Tespit etme NTLM downgrade attacks from NTLMv2 to NTLMv1 using event log analysis.
tags:
- threat-hunting
- T1557.001
- NTLM-relay
...
- Tespit etme NTLM downgrade attacks where NTLMv2 is forced to NTLMv1 for easier offline cracking or relay
...
- During purple team exercises validating NTLM relay Tespit and SMB signing enforcement

**Kullanma:** without centralized Windows Security Event Log collection, as a substitute for enforcing SMB signing and Extended Protection for Authentication (EPA) which prevent relay attacks at the protocol level, or without an IP-to-hostname inventory for correlation.
...
### Adım 1: Understand NTLM Relay Attack Flow
...
**Key Tespit Insight**: In a relay attack, Event 4624 on the target will show the victim's username but the attacker's IP address. The WorkstationName field may still reflect the victim's machine. This IP-to-hostname mismatch is the primary Tespit signal.

### Adım 2: Event 4624 LogonType 3 Analysis for Relay Tespit
...
```kql
-- Microsoft Sentinel KQL: NTLM Relay Tespit via IP-Hostname Mismatch
...
### Adım 3: Responder Tespit via Network and Event Analysis
...
status: stable
description: >
  tespit etme (s) a single host responding to LLMNR (UDP 5355) or NBT-NS (UDP 137)
  queries from multiple unique sources, indicating possible Responder poisoning.
references:
    - https://www.hackthebox.com/blog/ntlm-relay-attack-Tespit
...
### Adım 4: SMB Signing Enforcement Audit
...
```kql
-- Microsoft Sentinel KQL: NTLMv1 Downgrade Tespit
...
### Adım 6: NTLM Audit and Restriction Policy Configuration
...
### Adım 7: PetitPotam and Coercion Attack Tespit
...
```kql
-- Microsoft Sentinel KQL: PetitPotam / Coercion Attack Tespit
-- tespit etme (s) domain controller machine account authenticating from unexpected IPs
...
status: stable
description: >
  tespit etme (s) a computer account (ending in $) authenticating via NTLM LogonType 3
...
references:
    ...
    - https://www.fox-it.com/nl-en/research-blog/Tespit etme-and-hunting-for-the-petitpotam-ntlm-relay-attack/
    - https://www.nccgroup.com/research-blog/Tespit etme-and-hunting-for-the-petitpotam-ntlm-relay-attack/
logsource:
    product: windows
    service: security
Tespit:
...
### Adım 8: Build Comprehensive Correlation Dashboard
...
| Tool | Purpose |
|------|---------|
| **Splunk / Elastic SIEM** | Log aggregation and correlation for Event 4624 analysis, IP-hostname mismatch Tespit, and NTLM downgrade monitoring |
| **Microsoft Sentinel** | Cloud SIEM with KQL queries for NTLM relay Tespit and built-in analytics rules for PetitPotam |
| **CrowdStrike Falcon Identity Protection** | tespit etme (s) NTLM relay attacks against domain controller accounts regardless of coercion method used |
| **Responder** | LLMNR/NBT-NS/mDNS poisoning tool used by attackers -- understanding its behavior is essential for Tespit |
...
| **Sigma** | Vendor-agnostic Tespit rule format for portable NTLM relay Tespit rules |
...
**Approach**:
1. Query Event 4624 on 10.10.20.100 filtered for IpAddress=10.10.5.50: Bul: 15 successful NTLM logons in 30 minutes from 8 different user accounts
...
**Context**: During a threat hunt, an analyst Bul:s Event 4624 LogonType 3 on the AD CS server (ADCS01) showing the domain controller machine account (DC01$) authenticating via NTLM from IP 10.10.5.50, which is not the DC's IP address (10.10.1.10).
...
2. Check for certificate enrollment: query AD CS logs for certificate requests from DC01$ around the same timestamp -- Bul: a certificate issued for DC01$
3. the tespit et: attack: PetitPotam coerced DC01 to authenticate to 10.10.5.50, which relayed the authentication to ADCS01 to request a certificate for DC01$
