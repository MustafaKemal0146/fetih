...
description: Implements AWS Nitro Enclave-based confidential computing environments with cryptographic attestation, KMS policy integration using PCR-based condition keys, and secure vsock communication
  channels. The practitioner builds enclave images, configures attestation-aware KMS policies, validates attestation documents against the AWS Nitro PKI root of trust, and establishes isolated computation
...
- Dağıt:ing multi-party computation environments where two or more enclaves authenticate each other via attestation before exchanging data
...
**Kullanma:** when the workload does not handle sensitive data that requires hardware-level isolation, when the instance type does not support Nitro Enclaves (requires Nitro-based instances with at least 4 vCPUs), or when latency constraints make the vsock communication overhead unacceptable.

## Ön Gereksinimler

- An AWS account with permissions to launch Nitro-capable EC2 instances (m5.xlarge or larger, C5, R5, M6i families)
- AWS CLI v2 and the `nitro-cli` toolset kurulu: the parent EC2 instance (Amazon Linux 2 or AL2023)
- Docker kurulu: the parent instance for building enclave image files (EIF)
...
- The Nitro Enclaves allocator service configured with Yeterli memory and vCPU allocation in `/etc/nitro_enclaves/allocator.yaml`

## İş Akışı

### Adım 1: Şunu yapılandır: Nitro Enclaves Environment

Kur: the parent EC2 instance to support enclave launches:
...
### Adım 2: Şunu inşa et: Enclave Image File (EIF)
...
- **Create the application Dockerfile**: The enclave runs a minimal Linux environment. The application communicates exclusively through vsock:
  ...
  ```
- **Şunu inşa et: EIF with nitro-cli**: Convert the Docker image into an enclave image file, capturing the PCR measurements:
  ```bash
...
- **Build a signed EIF** (recommended for production): Şunu üret: signing certificate and use it to produce PCR8:
  ```bash
...
### Adım 3: Configure KMS Attestation-Based Key Policies

Şunu oluştur: KMS key policy that restricts decryption to a verified enclave:
...
### Adım 4: Implement Secure Vsock Communication
...
### Adım 5: Validate Attestation Documents
...
- **Validation steps**:
  1. Decode the COSE_Sign1 structure and Şunu çıkar: payload and certificate
  2. Verify the COSE signature using the public key from the embedded certificate
  3. Şunu doğrula: certificate chain from the NSM certificate through the CA bundle to the AWS Nitro Attestation PKI root certificate (available at `https://aws-nitro-enclaves.amazonaws.com/AWS_NitroEnclaves_Root-G1.zip`)
  4. Şunu kontrol et: the root CA certificate matches the expected AWS root: `aws.nitro-enclaves` CN
  5. Şunu doğrula: no certificate in the chain is expired at the document's timestamp
...
### Adım 6: Launch and Şunu izle: Enclave
...
- **CloudWatch monitoring**: Şunu yapılandır: parent instance to report enclave health metrics. Since the enclave has no network access, health checks must go through the vsock proxy:
  ```python
...
**Context**: A healthcare SaaS company processes patient records containing PHI. Regulations require that the decryption and tokenization of PHI never occurs on an instance accessible to operators. The company Dağıt:s a Nitro Enclave that receives encrypted patient records, decrypts them inside the enclave using KMS with attestation, tokenizes the PII fields, and returns only the tokenized records through the vsock.

**Approach**:
1. Şunu inşa et: tokenization application into a Docker image containing the tokenization logic, the `kmstool-enclave-cli` binary, and a vsock server that accepts encrypted records
2. Şunu inşa et: EIF with `nitro-cli build-enclave` and record PCR0, PCR1, PCR2 from the build output
3. Şunu oluştur: KMS key with a key policy that includes a `kms:RecipientAttestation:ImageSha384` condition matching PCR0, allowing only this specific enclave build to decrypt patient records
4. Dağıt: the parent instance with an IAM role that has `kms:Decrypt` on the key, but the KMS condition ensures decryption only succeeds inside the attested enclave
...
- Failing to reserve Yeterli memory in `allocator.yaml`, causing the enclave to fail at launch with an opaque "resource not available" error
- Not implementing vsock message framing, causing large records to be truncated at the 64KB socket buffer boundary
- Forgetting that PCR0 changes with every code rebuild, requiring a KMS policy update for each Dağıt:ment; use PCR8 (signing certificate) for production to decouple builds from policy updates
...
**AlBul:d Resources**: 2 vCPUs, 4096 MiB memory
