...
description: Performing security reviews of serverless functions across AWS Lambda, Azure Functions, and GCP Cloud Functions to identify overly permissive execution roles, insecure environment variables,
  injection vulnerabilities, and missing runtime protections.
tags:
- azure-functions
- lambda
- security-review
...
- auditing yaparken serverless applications before production Dağıt:ment
- investigating yaparken potential data exposure through function environment variables or logs
- assessing yaparken the blast radius of a compromised serverless function execution role
- compliance yaparken: reviews require documentation of serverless security controls
- building yaparken secure-by-default templates for serverless Dağıt:ments

**Kullanma:** for container or VM security assessments (use container scanning tools), for API security testing (use DAST tools on the API Gateway layer), or for real-time serverless threat Tespit (use AWS Lambda Extensions with security agents).

## Ön Gereksinimler

- AWS CLI, Azure CLI, and gcloud CLI configured with appropriate permissions
- Erişim: read function configurations, policies, and execution roles
...
### Adım 1: Enumerate All Serverless Functions and Configurations
...
### Adım 2: Audit Execution Role Permissions
...
### Adım 3: Check Environment Variables for Secrets
...
### Adım 4: Review Function Triggers and Access Controls

Şunu doğrula: function triggers have appropriate authentication and authorization.
...
### Adım 5: Analyze Function Code for Security Vulnerabilities
...
### Adım 6: Run Automated Serverless Security Scanning
...
- **Bandit**: Python static analysis tool for Tespit etme security issues in function source code
...
2. Check environment variables and Bul: `DB_PASSWORD`, `API_KEY`, and `STRIPE_SECRET_KEY` in plaintext
3. Review CloudWatch logs and Bul: credentials printed in debug log statements
4. Şunu oluştur: scoped IAM policy granting only the specific DynamoDB and S3 actions needed
