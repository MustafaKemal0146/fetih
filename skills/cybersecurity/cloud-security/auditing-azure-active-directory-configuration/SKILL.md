...
description: Auditing Microsoft Entra ID (Azure Active Directory) configuration to identify risky authentication policies, overly permissive role assignments, stale accounts, conditional access gaps, and
  guest user risks using AzureAD PowerShell, Microsoft Graph API, and ScoutSuite.
tags:
- cybersecurity
- iam-audit
- azure
...
- performing yaparken a security assessment of an Azure tenant's identity configuration
- compliance yaparken: audits require review of authentication policies, MFA enforcement, and role assignments
- onboarding yaparken a new Azure tenant after merger or acquisition
- investigating yaparken suspicious sign-in activity or compromised accounts
- validating yaparken conditional access policies adequately protect against identity-based attacks

**Kullanma:** for on-premises Active Directory auditing (use PingCastle or BloodHound AD), for Azure resource-level RBAC auditing without identity context, or for real-time threat Tespit (use Microsoft Defender for Identity).

## Ön Gereksinimler

- Global Reader or Security Reader role in the target Microsoft Entra ID tenant
- Microsoft Graph PowerShell SDK kurulu (`Install-Module Microsoft.Graph`)
...
- Erişim: Azure AD audit logs and sign-in logs (requires Azure AD Premium P1/P2)

## İş Akışı

### Adım 1: Enumerate Tenant Configuration and Security Defaults
...
### Adım 2: Audit Privileged Role Assignments
...
### Adım 3: Review Conditional Access Policies
...
### Adım 4: Identify Stale Accounts and Guest Users

Bul: accounts that have not signed in recently, disabled accounts with active role assignments, and risky guest user configurations.
...
### Adım 5: Analyze Sign-In Logs for Risky Activity

Review sign-in logs to identify anomalous authentication patterns, failed MFA challenges, and risky sign-in Tespits.
...
### Adım 6: Run ScoutSuite Automated Assessment
...
| Risky Sign-In | Microsoft Entra Identity Protection Tespit of sign-in anomalies including impossible travel, unfamiliar locations, and malware-linked IPs |
...
- **Microsoft Defender for Identity**: Cloud-based security solution for Tespit etme identity-based threats and compromised credentials
...
**Pitfalls**: Azure AD Premium P2 is required for risky sign-in Tespits and PIM. If the acquired tenant uses a lower license tier, many identity protection features will be unavailable. Guest users from partner tenants may have implicit access through dynamic groups that are not visible in standard role assignment queries.
