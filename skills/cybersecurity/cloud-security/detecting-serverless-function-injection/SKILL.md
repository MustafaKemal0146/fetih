---
name: Tespit etme-serverless-function-injection
description: tespit etme (s) and prevents code injection attacks targeting serverless functions (AWS Lambda, Azure Functions, Google Cloud Functions) through event source poisoning, malicious layer injection, runtime
  command execution, and IAM privilege escalation via function modification. The analyst combines static analysis of function code, CloudTrail event correlation, runtime behavior monitoring, and IAM policy
  auditing to identify injection vectors across the expanded serverless attack surface including AP...
tags:
- siber-güvenlik
...
- Tespit etme privilege escalation paths where an adversary with `lambda:UpdateFunctionCode` and `iam:PassRole` can assume higher-privilege execution roles
...
- Building Tespit rules for SOC teams monitoring serverless workloads for unauthorized function modifications, layer additions, and suspicious invocation patterns

**Kullanma:** for load testing or denial-of-service simulation against serverless functions, for testing against production functions processing live customer data without explicit authorization, or for modifying IAM policies in shared accounts without change management approval.
...
- Erişim: function source code or Dağıt:ment packages for static analysis
- CloudWatch Logs Insights access for querying Lambda execution logs

## İş Akışı

### Adım 1: Enumerate the Serverless Attack Surface
...
- **Download function Dağıt:ment packages**:
  ...
  ```
- **Python injection sinks** (Lambda Python runtimes): Ara: functions that execute strings as code:
  ```python
...
- **Node.js injection sinks** (Lambda Node.js runtimes):
  ...
  ```
- **Run Semgrep with serverless rules**: Use purpose-built rules that tespit etmeevent data flowing into injection sinks:
  ```bash
...
- **Custom pattern Tespit**: Ara: indirect injection patterns where event data is concatenated into strings that are later executed:
  ```python
...
### Adım 3: tespit etmeEvent Source Poisoning
...
- **Tespit via CloudWatch Logs Insights**: Query for evidence of injection attempts in function execution logs:
  ```
...
### Adım 4: tespit etmeMalicious Lambda Layer Injection
...
- **Audit current layer attachments**: List all functions and their layer versions to identify unexpected additions:
  ...
  ```
- **tespit etmelayer modification events in CloudTrail**: Query for `UpdateFunctionConfiguration` events that add or change layers:
  ```bash
...
- **Analyze layer contents**: Download and Denetle: layer packages for malicious code:
  ```bash
...
- **CloudTrail Tespit query for layer changes**:
  ```json
  {
    "source": ["aws.lambda"],
...
### Adım 5: tespit etmeIAM Privilege Escalation via Lambda
...
- **tespit etmeUpdateFunctionCode events**: Monitor CloudTrail for function code modifications:
  ```bash
  aws cloudtrail lookup-events \
...
- **tespit etmePassRole to Lambda**: `iam:PassRole` is required to attach a different execution role to a function. Monitor for this:
  ```
...
- **tespit etmecredential exfiltration from Lambda**: A compromised function may call STS or create new IAM entities:
  ```
...
- **EventBridge rule for real-time alerting**: Şunu oluştur:n EventBridge rule to trigger an SNS alert whenever function code is modified:
  ```json
  {
    "source": ["aws.lambda"],
    "detail-type": ["AWS API Call via CloudTrail"],
...
### Adım 6: Implement Runtime Injection Prevention

Dağıt: runtime protection controls to prevent injection at execution time:
    ...
    "Statement": [
      {
        ...
        "Condition": {
          "StringNotLike": {
            "aws:PrincipalArn": "arn:aws:iam::*:role/CICD-Dağıt:mentRole"
          }
        }
      }
    ]
...
- **Semgrep**: Static analysis tool with serverless-specific rule packs that tespit etmeevent data flowing into injection sinks across Python, Node.js, Java, and Go Lambda runtimes
...
- **AWS CloudTrail**: Logs Lambda management events (UpdateFunctionCode, CreateFunction) and data events (Invoke) for Tespit etme unauthorized modifications and anomalous invocation patterns
...
### Scenario: Tespit etme and Responding to a Lambda-Based Privilege Escalation Attack
...
1. Query CloudTrail for `UpdateFunctionCode` events in the past 7 days to identify when the function was modified and by which principal:
   ...
   | sort eventTime desc
   ```
2. Discover that the function was modified from an IP address in an unexpected geographic location at 02:47 UTC, outside of normal Dağıt:ment windows
3. Download the modified function code and Bul: an injected snippet that POSTs `os.environ['AWS_ACCESS_KEY_ID']`, `AWS_SECRET_ACCESS_KEY`, and `AWS_SESSION_TOKEN` to an external endpoint on each invocation
...
6. Search CloudTrail for API calls made by the exfiltrated credentials from outside AWS, Bul:ing `sts:GetCallerIdentity`, `s3:ListBuckets`, `dynamodb:Scan` on the customer table, and `iam:CreateUser` attempts
7. Respond by reverting the function code from the last known-good Dağıt:ment package in the CI/CD artifact store, rotating the execution role's session tokens, and adding an SCP that restricts `lambda:UpdateFunctionCode` to the CI/CD role only
...
#### Bul:ING-001: OS Command Injection in S3 Event Handler
...
**Remediation**: Replace os.system() with subprocess.run() with shell=False
  and Şunu doğrula: S3 key against an allowlist pattern.

#### Bul:ING-002: IAM Privilege Escalation Path
...
#### Bul:ING-003: Unauthorized Layer Attached
...
**Remediation**: Remove the external layer, vendor the dependency into the
  function's Dağıt:ment package, add AWS Config rule to block external layers.

### Tespit Rules Dağıtılmış
