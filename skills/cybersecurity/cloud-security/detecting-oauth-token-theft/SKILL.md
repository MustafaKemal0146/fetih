---
name: Tespit etme-oauth-token-theft
description: tespit etme (s) and responds to OAuth token theft and replay attacks in cloud environments, focusing on Microsoft Entra ID (Azure AD) token protection, conditional access policies, and sign-in anomaly
  Tespit. Covers access token theft, refresh token replay, Primary Refresh Token (PRT) abuse, and pass-the-cookie attacks. Activates for requests involving OAuth token theft Tespit, token replay prevention,
  Azure AD conditional access token protection, or cloud identity attack investigation.
tags:
- PRT
- cybersecurity
- token-theft
...
- Tespit etme OAuth device code phishing campaigns that bypass MFA
...
**Kullanma:** for on-premises Kerberos ticket attacks (pass-the-ticket, golden ticket); use Active Directory-specific investigation techniques for those scenarios.

## Ön Gereksinimler

- Microsoft Entra ID P2 license (required for Identity Protection risk Tespits and conditional access)
- Global Administrator or Security Administrator role in the Entra admin center
- Microsoft Defender for Cloud Apps (MDCA) license for session anomaly Tespit
- Erişim: Entra ID Sign-in Logs and Audit Logs (requires Diagnostic Settings configured to Log Analytics or Sentinel)
...
### Adım 1: Understand the Token Theft Attack Surface
...
### Adım 2: Configure Entra ID Sign-in Risk Tespit

Enable Identity Protection to flag anomalous token usage:

```
Entra Admin Center > Protection > Identity Protection > Risk Tespits

Key risk Tespits for token theft:
...
### Adım 3: Enable Token Protection (Preview)
...
### Adım 4: tespit etmeToken Replay in Sign-in Logs

Query Entra sign-in logs for indicators of token theft:

```kusto
// KQL query for Microsoft Sentinel or Log Analytics
// tespit etmesign-ins where the token was issued in one location and used in another
SigninLogs
...
// tespit etmeimpossible travel with token reuse
SigninLogs
| where TimeGenerated > ago(7d)
...
// tespit etmedevice code flow abuse (often used in phishing)
SigninLogs
| where TimeGenerated > ago(7d)
...
// tespit etmetoken replay: same token used from multiple IPs
...
### Adım 5: Araştır: and Respond to Token Theft
...
### Adım 6: Implement Continuous Access Evaluation (CAE)
...
- Azure AD Identity Protection tespit etme (s) elevated user risk
- Network location change violates conditional access policy
```

### Adım 7: Configure Defender for Cloud Apps Session Policies

Kur: real-time session monitoring to tespit etmeand block suspicious token usage:
...
- [ ] Identity Protection risk Tespits are enabled and generating alerts for anomalous token activity
