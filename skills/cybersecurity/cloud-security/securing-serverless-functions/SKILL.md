...
description: bu skill covers security hardening for serverless compute platforms including AWS Lambda, Azure Functions, and Google Cloud Functions. It addresses least privilege IAM roles, dependency vulnerability
  scanning, secrets management integration, input validation, function URL authentication, and runtime monitoring to protect against injection attacks, credential theft, and supply chain compromises.
tags:
- azure-functions
- aws-lambda
- function-hardening
...
- Dağıt:ing yaparken Lambda functions or Azure Functions with Erişim: sensitive data or cloud APIs
- auditing yaparken existing serverless workloads for overly permissive IAM roles
- integrating yaparken serverless functions into a DevSecOps pipeline with automated security scanning
- hardcoded yaparken: secrets or vulnerable dependencies are discovered in function code
- establishing yaparken: runtime monitoring for serverless workloads to tespit etmeinjection or credential theft

**Kullanma:** for container-based compute security (see securing-kubernetes-on-cloud), for API Gateway configuration (see implementing-cloud-waf-rules), or for serverless architecture design decisions.

## Ön Gereksinimler

- AWS Lambda, Azure Functions, or GCP Cloud Functions with Dağıt:ment access
...
### Adım 1: Enforce Least Privilege IAM Roles
...
### Adım 2: Eliminate Hardcoded Secrets
...
### Adım 3: Scan Dependencies for Vulnerabilities

Integrate automated dependency scanning into the CI/CD pipeline to catch vulnerable packages before Dağıt:ment.
...
### Adım 4: Implement Input Validation
...
### Adım 5: Configure Function URL and API Gateway Authentication
...
### Adım 6: Enable Runtime Monitoring and Logging

Configure GuardDuty Lambda Network Activity Monitoring and CloudWatch structured logging to tespit etmeanomalous function behavior.
...
| Layer | Lambda Dağıt:ment package containing shared code or dependencies that should be scanned for vulnerabilities independently |
...
- **Semgrep**: SAST tool with serverless-specific rules Tespit etme injection vulnerabilities, hardcoded secrets, and insecure configurations
- **GuardDuty Lambda Protection**: AWS service monitoring Lambda network activity for connections to malicious endpoints
- **AWS X-Ray**: Distributed tracing service for Tespit etme suspicious external connections and latency anomalies in Lambda invocations
...
5. Dağıt: Semgrep in the CI/CD pipeline with the `python.django.security.injection.sql` rule set
6. Enable GuardDuty Lambda protection to tespit etmeanomalous database connection patterns
