...
description: Securing AWS Lambda execution roles by implementing least-privilege IAM policies, applying permission boundaries, restricting resource-based policies, using IAM Access Analyzer to validate
  permissions, and enforcing role scoping through SCPs.
tags:
- least-privilege
- execution-roles
- lambda
...
- Dağıt:ing yaparken new Lambda functions and defining their IAM execution roles
- remediating yaparken overly permissive Lambda roles discovered during security audits
- implementing yaparken least-privilege access patterns for serverless architectures
- building yaparken reusable IAM templates for Lambda functions across teams
- Security yaparken: Hub or Prowler reports Lambda functions with excessive permissions

**Kullanma:** for securing Lambda function invocation (use resource-based policies and API Gateway authorizers), for Lambda code security (use SAST tools), or for Lambda network security (use VPC configuration and security groups).
...
### Adım 1: Audit Current Lambda Execution Role Permissions
...
### Adım 2: Analyze Actual API Usage with CloudTrail

Use CloudTrail and IAM Access Analyzer to Belirle: which API actions the function actually uses.
...
### Adım 3: Create Least-Privilege Execution Policies
...
### Adım 4: Apply Permission Boundaries
...
### Adım 5: Validate Policies with IAM Access Analyzer
...
aws accessanalyzer validate-policy \
  ...
  --query 'Bul:ings[*].[Bul:ingType,issueCode,learnMoreLink]' --output table
...
### Adım 6: Enforce Role Standards with SCPs
...
| Resource-Scoped Policy | IAM policy that specifies exact resource ARNs rather than wildcards, limiting Erişim: only the specific resources needed |
...
- **IAM Policy Simulator**: Tests effective permissions for a role against specific API actions before Dağıt:ment
- **CloudTrail**: Audit log of all API calls used to Belirle: actual function permission usage
...
### Scenario: Reducing a Lambda Function from AdministratorErişim: Least Privilege

**Context**: A security audit Bul:s 12 Lambda functions using a shared execution role with `AdministratorAccess`. The team needs to scope each function to minimum required permissions without breaking production.
