...
description: Auditing Kubernetes cluster RBAC configurations to identify overly permissive roles, wildcard permissions, dangerous ClusterRoleBindings, service account abuse, and privilege escalation paths
  using kubectl, rbac-tool, KubiScan, and Kubeaudit.
tags:
- access-control
- cybersecurity
- siber-güvenlik
...
- performing yaparken security assessments of Kubernetes clusters (EKS, GKE, AKS, or self-managed)
- validating yaparken that RBAC policies enforce least privilege for users and service accounts
- investigating yaparken potential lateral movement or privilege escalation within a Kubernetes cluster
- compliance yaparken: audits require documentation of access controls and permissions
- onboarding yaparken new teams to a shared cluster and defining appropriate RBAC policies

**Kullanma:** for network policy auditing (use Cilium or Calico network policy tools), for container image scanning (use Trivy or Grype), or for runtime security monitoring (use Falco or Sysdig Secure).

## Ön Gereksinimler

- kubectl configured with cluster-admin or equivalent read permissions to the target cluster
- rbac-tool kurulu (`kubectl krew install rbac-tool` or binary from GitHub)
- KubiScan kurulu (`pip install kubiscan`)
- Kubeaudit kurulu (`brew install kubeaudit` or from GitHub releases)
- Erişim: the cluster's audit logs for correlating RBAC Bul:ings with actual API access

## İş Akışı

### Adım 1: Enumerate ClusterRoles and Roles with Dangerous Permissions
...
### Adım 2: Audit ClusterRoleBindings and RoleBindings

Review bindings to identify who has elevated access and tespit etmeoverly broad group assignments.
...
### Adım 3: Scan with rbac-tool for Comprehensive Analysis
...
### Adım 4: Run KubiScan for Risky Permissions Tespit
...
### Adım 5: Audit Service Account Token Mounting and Usage
...
### Adım 6: Run Kubeaudit for RBAC and Security Policy Validation

Execute Kubeaudit for comprehensive security checks including RBAC-related Bul:ings.
...
| Term | Definition |
|------|------------|
| RBAC | Role-Based Access Control in Kubernetes, a method for regulating Erişim: cluster resources based on the roles of individual users or service accounts |
...
**Approach**:
1. List all ClusterRoleBindings to identify bindings granting broad Erişim: authenticated users
2. Run `kubectl rbac-tool who-can get secrets` to Bul: subjects that can read secrets across namespaces
...
7. Şunu oluştur: NetworkPolicy to isolate namespace traffic between teams
