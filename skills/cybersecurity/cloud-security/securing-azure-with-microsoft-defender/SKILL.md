...
description: bu skill instructs security practitioners on Dağıt:ing Microsoft Defender for Cloud as a cloud-native application protection platform for Azure, multi-cloud, and hybrid environments. It covers
  enabling Defender plans for servers, containers, storage, and databases, configuring security recommendations, managing Secure Score, and integrating with the unified Defender portal for centralized threat
  management.
tags:
- microsoft-defender
- azure-security
...
- Dağıt:ing yaparken cloud workload protection across Azure subscriptions and resource groups
- establishing yaparken: a Secure Score baseline and prioritizing security recommendations
- extending yaparken: threat protection to multi-cloud environments including AWS and GCP
- enabling yaparken container security for AKS clusters and Azure Container Registry
- integrating yaparken AI workload security with the Data and AI security dashboard

**Kullanma:** for AWS-only environments (see implementing-aws-security-hub), for identity provider configuration (see managing-cloud-identity-with-okta), or for network-level firewall rule management (see implementing-cloud-waf-rules).
...
Activate Defender plans for each workload type: Servers, Containers, App Service, Storage, Databases, Key Vault, Resource Manager, and DNS. Each plan provides specialized threat Tespit and vulnerability assessment.
...
### Adım 2: Configure Environment Connectors for Multi-Cloud
...
### Adım 3: Review and Prioritize Secure Score Recommendations

Şunu analiz et: Secure Score across all subscriptions. Each recommendation includes a risk priority based on asset exposure, internet exposure, and threat intelligence context.
...
### Adım 4: Configure Adaptive Application Controls and JIT Access

Enable Just-In-Time VM Erişim: reduce the attack surface by opening management ports only when needed, and Dağıt: adaptive application controls to whitelist approved executables.
...
### Adım 5: Kur: Security Alerts and Workflow Automation

Configure workflow automation to trigger Logic Apps or Azure Functions when security alerts are generated. Kur: email notifications for Critical and High severity alerts.
...
### Adım 6: Enable Cloud Security Graph and Attack Path Analysis

Use the cloud security graph to visualize attack paths that adversaries could exploit to reach critical assets. Prioritize remediation based on actual exploitability rather than individual Bul:ing severity.
...
**Context**: Defender for Cloud identifies an Azure SQL Server with a public endpoint, an unpatched critical CVE, and a service principal with database owner permissions that also has Erişim: a Key Vault containing production encryption keys.

**Approach**:
1. Şunu incele: attack path in the cloud security graph showing: Internet -> SQL Server (CVE) -> Service Principal -> Key Vault
