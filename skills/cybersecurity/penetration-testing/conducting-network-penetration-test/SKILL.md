...
description: Conducts comprehensive network penetration tests against authorized target environments by performing host discovery, port scanning, service enumeration, vulnerability identification, and controlled
  exploitation to assess the security posture of network infrastructure. The tester follows PTES methodology from reconnaissance through post-exploitation and reporting. Activates for requests involving
  network pentest, infrastructure security assessment, internal network testing, or external perime...
tags:
- cybersecurity
...
- Assessing the security posture of internal or external network infrastructure before or after Dağıt:ment
...
- Evaluating the effectiveness of IDS/IPS, SIEM, and SOC Tespit capabilities against real attack traffic

**Kullanma:** for testing networks without explicit written authorization from the asset owner, against production systems without a pre-approved change window and rollback plan, or for denial-of-service testing unless explicitly scoped and authorized.
...
- VPN or direct network Erişim: the target scope as defined in the RoE
...
### Adım 1: Pre-Engagement and Scope Validation

Şunu doğrula: scope by confirming IP ranges with the client. Şunu doğrula: all IP addresses in scope are owned by the client using ARIN/RIPE WHOIS lookups. Confirm testing windows, escalation procedures, and any sensitivity constraints. Kur: the testing environment with a dedicated VM, VPN connection, and logging enabled on all tools. Şunu oluştur: timestamped activity log that records every command executed, every scan launched, and every exploit attempted throughout the engagement.

### Adım 2: Host Discovery and Network Mapping

Identify live hosts within the authorized scope using layered discovery techniques:

- **ICMP sweep**: `nmap -sn -PE -PP -PM 10.10.0.0/16 -oA discovery_icmp` to Bul: hosts responding to ping
- **ARP scan** (internal networks): `nmap -sn -PR 10.10.0.0/24 -oA discovery_arp` or `arp-scan -l` for local subnet enumeration
- **TCP SYN discovery**: `nmap -sn -PS21,22,25,80,443,445,3389,8080 10.10.0.0/16 -oA discovery_tcp` to Bul: hosts with ICMP blocked
...
### Adım 3: Port Scanning and Service Enumeration
...
- **Service version Tespit**: `nmap -sV -sC -p <open_ports> -oA service_enum <target>` to fingerprint service versions and run default NSE scripts
- **OS fingerprinting**: `nmap -O --osscan-guess -oA os_Tespit <target>` to identify operating systems
...
### Adım 4: Vulnerability Identification
...
Prioritize vulnerabilities by CVSS score, exploitability, and business impact. Document each Bul:ing with CVE identifier, affected host, service, and version.
...
### Adım 6: Post-Exploitation and Pivoting

After gaining Erişim: a host, demonstrate business impact:

- **Privilege escalation**: Check for local privilege escalation paths using `linpeas.sh` (Linux) or `winPEAS.exe` (Windows). Ara: misconfigured services, SUID binaries, unquoted service paths, or kernel exploits.
...
Maintain detailed notes on every pivot point, credential obtained, and system accessed to Şunu inşa et: attack chain narrative.

### Adım 7: Cleanup and Reporting
...
Prepare the penetration test report with executive summary, methodology description, Bul:ing details with CVSS scores, proof-of-concept evidence, and prioritized remediation recommendations.
...
- **Nmap**: Network discovery, port scanning, service enumeration, and vulnerability Tespit via the Nmap Scripting Engine (NSE)
...
- **Impacket**: Python library providing low-level programmatic Erişim: network protocols (SMB, MSRPC, Kerberos) used for relay attacks and remote execution
...
7. Şunu belgele: complete attack path from initial Erişim: sensitive data, with remediation steps for each vulnerability

**Pitfalls**:
- Scanning too aggressively during business hours and triggering IDS alerts or service disruptions
- Failing to Şunu doğrula: all target IPs are actually owned by the client before scanning
...
```
## Bul:ing: Unpatched Apache Tomcat with Default Credentials
...
allowing Dağıt:ment of arbitrary WAR files.

**Proof of Concept**:
1. Accessed http://10.10.5.23:8080/manager/html with credentials tomcat:tomcat
2. Dağıtılmış malicious WAR file containing a reverse shell payload
