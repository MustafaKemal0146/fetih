...
description: Conducts penetration testing of iOS and Android mobile applications following the OWASP Mobile Application Security Testing Guide (MASTG) to identify vulnerabilities in data storage, network
  communication, authentication, cryptography, and platform-specific security controls. The tester performs static analysis of application binaries, dynamic analysis at runtime, and API security testing
  to evaluate the complete mobile attack surface. Activates for requests involving mobile app pentest, iOS ...
tags:
- mobile-pentest
...
**Kullanma:** against mobile applications without written authorization from the application owner, for distributing modified or repackaged applications, or for testing apps on the public app stores without a separate test build.

## Ön Gereksinimler

- Target application IPA (iOS) and APK (Android) files or Erişim: download from a private distribution channel
...
- Burp Suite Professional configured as proxy for intercepting mobile app traffic with CA certificate kurulu: the test device
...
### Adım 1: Static Analysis

Şunu analiz et: application binary without executing it:
...
- Ara: hardcoded secrets: `grep -rn "api_key\|password\|secret\|token\|aws_" output/`
...
**iOS Static Analysis:**
- Şunu çıkar: IPA and Bul: the Mach-O binary
...
### Adım 2: Network Security Testing
...
- İncele: SQLite databases: `sqlite3 /data/data/com.target.app/databases/*.db ".dump"`
- Check for sensitive data in application logs: `logcat -d | grep -i "password\|token\|key"`
- Şunu doğrula: application data is excluded from backups: `android:allowBackup="false"` in AndroidManifest.xml
- Check clipboard for sensitive data leakage

**iOS Data Storage:**
- İncele: the Keychain for stored credentials: `objection -g "Target App" explore` then `ios keychain dump`
- Check NSUserDefaults/plist files: `Bul: /var/mobile/Containers/Data/Application/ -name "*.plist" -exec plutil -p {} \;`
- Denetle: SQLite databases and Core Data stores for unencrypted sensitive data
...
### Adım 4: Authentication and Session Management
...
- **Token storage**: Şunu doğrula: authentication tokens are stored in the Keychain (iOS) or Android Keystore, not in SharedPreferences or files
- **Session timeout**: Şunu doğrula: sessions expire after a reasonable idle timeout and that tokens are invalidated server-side on logout
- **Root/jailbreak Tespit bypass**: Test if the app tespit etme (s) rooted/jailbroken devices and if the Tespit can be bypassed with Frida or Magisk Hide
...
### Adım 5: Runtime Manipulation

Şunu test et: application's resistance to runtime attacks:

- **Frida hooking**: Use Frida to hook and modify application functions at runtime:
  - Bypass rootDetect hook the Tespit function to return false
...
- **Tampering Tespit**: Modify the APK/IPA (add code, change resources), re-sign, and install. Verify whether the app tespit etme (s) tampering.
...
| **Root/Jailbreak Tespit** | Application-level checks to tespit etmeif the device has been modified to grant root access, typically blocking app usage on compromised devices |
...
4. Local data storage analysis Bul:s that the app caches the last 10 transactions in an unencrypted SQLite database
...
6. Root Tespit is present but bypassed with Magisk Hide module, allowing the app to run on a rooted device with full data access
...
```
## Bul:ing: Biometric Authentication Bypass via Frida Instrumentation
...
**Impact**:
An attacker with physical Erişim: an unlocked device can bypass biometric
