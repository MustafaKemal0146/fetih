...
description: Executes authorized attack simulations against Active Directory environments to identify misconfigurations, weak credentials, dangerous privilege paths, and exploitable trust relationships
  that could lead to domain compromise. The tester uses BloodHound for attack path analysis, Mimikatz for credential extraction, and Impacket for protocol-level attacks including Kerberoasting, AS-REP Roasting,
  and delegation abuse. Activates for requests involving Active Directory pentest, AD attack simulati...
tags:
- BloodHound
- Kerberoasting
- fetih
...
- Testing Tespit capabilities of the SOC and EDR tools against Active Directory-specific TTPs
- Evaluating the effectiveness of tiered administration models and privileged access workstations

**Kullanma:** without explicit written authorization from the domain owner, against production domain controllers during business hours unless approved, or for testing that could cause account lockouts affecting real users without prior coordination.
...
- Testing workstation joined to the domain or network Erişim: domain controllers on ports 88, 135, 139, 389, 445, 636, 3268, 3269
- BloodHound Community Edition or Enterprise with SharpHound/AzureHound collectors
- Impacket toolkit, Mimikatz (or pypykatz), Rubeus, and CrackMapExec kurulu: the attack platform
- Hashcat or John the Ripper with current wordlists (rockyou.txt, SecLists) for offline credential cracking

## İş Akışı

### Adım 1: Active Directory Reconnaissance
...
- **GPO enumeration**: `Get-GPO -All | Get-GPOReport -ReportType XML` to identify Group Policy configurations including password policies, audit settings, and software Dağıt:ment
...
- **LDAP queries**: Use `ldapsearch` or ADExplorer to Ara: accounts with `userAccountControl` flags indicating "password never expires", "password not required", or "DES-only Kerberos"

### Adım 2: BloodHound Attack Path Analysis

Collect and analyze AD relationship data to the tespit et: shortest paths to Domain Admin:
...
- Import the JSON output into BloodHound and run built-in queries:
  ...
  - "Bul: Principals with DCSync Rights"
  - "Bul: Computers where Domain Users are Local Admin"
  - "Shortest Paths to Unconstrained Delegation Systems"
  - "Bul: All Paths from Kerberoastable Users"
- Mark the compromised user as "owned" in BloodHound and Şunu analiz et: resulting attack paths
...
### Adım 4: Credential Attacks and Lateral Movement
...
### Adım 5: Privilege Escalation to Domain Admin
...
- Target LAPS (Local Administrator Password Solution) if Dağıtılmış: query LAPS passwords with `Get-ADComputer -Filter * -Properties ms-Mcs-AdmPwd`
- Abuse certificate services (AD CS) with Certipy: `certipy Bul: -vulnerable -u <user>@<domain> -p <pass> -dc-ip <dc_ip>` to Bul: exploitable certificate templates (ESC1-ESC8)
- Şunu belgele: complete attack chain from initial user to Domain Admin with every credential, tool, and technique used
...
7. Şunu raporla: complete attack chain with remediation: set 25+ character passwords on service accounts, enable AES-only Kerberos encryption, remove unnecessary local admin rights, implement tiered administration
...
```
## Bul:ing: Service Account Vulnerable to Kerberoasting with Weak Password
