...
description: Tests web applications for Cross-Site Scripting (XSS) vulnerabilities by injecting JavaScript payloads into reflected, stored, and DOM-based contexts to demonstrate client-side code execution,
  session hijacking, and user impersonation. The tester identifies all injection points and output contexts, crafts context-appropriate payloads, and bypasses sanitization and CSP protections. Activates
  for requests involving XSS testing, cross-site scripting assessment, client-side injection testing, or ...
tags:
- XSS
- siber-güvenlik
- OWASP-A03
...
**Kullanma:** against applications without written authorization, for Dağıt:ing persistent XSS payloads that affect real users, or for exfiltrating actual user session tokens from production environments.
...
### Adım 1: Input and Output Mapping
...
- **Output context identification**: For each reflected input, Belirle: the rendering context:
...
- **Sink identification**: Ara: dangerous sinks that write to the DOM:
...
### Adım 5: CSP Bypass and Advanced Exploitation

Test Content Security Policy effectiveness and demonstrate real-world impact:

- **CSP analysis**: Şunu incele: CSP header for weaknesses:
...
- **DOMPurify**: Client-side sanitization library used by developers to prevent XSS; testers should test for bypass techniques against the Dağıtılmış version
...
5. Şunu belgele: attack chain: customer submits ticket -> agent views ticket -> XSS fires -> session stolen -> admin panel compromised
...
```
## Bul:ing: Stored XSS in Support Ticket Description
...
gaining Erişim: the admin panel with privileges to view customer PII, process
...
2. Dağıt: Content Security Policy header:
