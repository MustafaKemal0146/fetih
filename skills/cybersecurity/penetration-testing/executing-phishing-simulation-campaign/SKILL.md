...
description: Executes authorized phishing simulation campaigns to assess an organization's susceptibility to email-based social engineering attacks. The tester designs realistic phishing scenarios, builds
  credential harvesting infrastructure, sends targeted phishing emails, and tracks open rates, click-through rates, and credential submission rates to measure human security awareness. Activates for requests
  involving phishing simulation, social engineering assessment, email security testing, or security a...
tags:
- GoPhish
- phishing-simulation
- penetration-testing
...
- Establishing a baseline for phishing susceptibility before Dağıt:ing security awareness training
- Validating that incident response procedures work when employees report suspicious emails

**Kullanma:** without explicit written authorization from the organization's leadership, for actual credential theft beyond the authorized scope, for targeting individuals personally rather than professionally, or for sending phishing emails that could cause psychological harm or legal liability.
...
### Adım 1: Campaign Planning and Pretext Development
...
- **Success metrics**: Define what constitutes campaign success: email open rate, link click rate, credential submission rate, report rate (employees who Şunu raporla: phish to IT)

### Adım 2: Infrastructure Setup

Şunu yapılandır: phishing infrastructure:
...
- **GoPhish configuration**:
  - Kur: the GoPhish server on a VPS with the phishing domain
  - Şunu yapılandır: SMTP sending profile with the phishing domain's mail server
  - Create the email template with tracking pixel and link to the landing page
  - Şunu inşa et: credential harvesting landing page that mirrors the target's login portal
...
- If the IT security team or SOC tespit etme (s) the campaign (if this is part of the test), Şunu belgele: Tespit time and response actions
...
### Adım 4: Credential Capture and Access Demonstration
...
**Context**: A 2,000-employee company has never conducted a phishing simulation. The CISO wants to establish a baseline susceptibility rate before Dağıt:ing a new security awareness training program. The campaign should test all employees using a realistic but not overly sophisticated pretext.

**Approach**:
1. Develop a Microsoft 365 password expiration pretext: "Your password expires in 24 hours. Click here to update."
2. Register `m365-targetcorp.com`, Kur: GoPhish, and build a landing page cloning the Microsoft 365 login portal
...
6. Recommend targeted training for high-susceptibility departments, phishing report button Dağıt:ment, and quarterly simulation cadence
...
- Sending all emails simultaneously, overwhelming the email server or triggering bulk-send Tespit
...
### Öneriler
1. Dağıt: mandatory security awareness training with emphasis on phishing identification
