...
description: Conducts security testing of REST, GraphQL, and gRPC APIs to identify vulnerabilities in authentication, authorization, rate limiting, input validation, and business logic. The tester uses
  the OWASP API Security Top 10 as the testing framework, combining Burp Suite interception with Postman collections and custom scripts to test endpoint security at every privilege level. Activates for
  requests involving API security testing, REST API pentest, GraphQL security assessment, or API vulnerability...
tags:
...
**Kullanma:** against APIs without written authorization, for load testing or denial-of-service testing unless explicitly scoped, or for testing production APIs that process real financial transactions without safeguards.

## Ön Gereksinimler

- API documentation (OpenAPI/Swagger, GraphQL schema, Postman collection) or application Erişim: reverse-engineer the API
...
### Adım 1: API Discovery and Documentation
...
### Adım 2: Authentication and Token Testing

Test authentication mechanisms for weaknesses:

- **JWT analysis**: Decode the JWT and Denetle: claims (sub, exp, iss, aud, role). Test:
...
### Adım 3: Authorization Testing (BOLA/BFLA)
...
### Adım 5: Data Exposure and Response Analysis
...
| Term | Definition |
|------|------------|
| **BOLA** | Broken Object Level Authorization (OWASP API #1); failure to Şunu doğrula: the requesting user is authorized to access a specific object, enabling IDOR attacks |
...
| **Mass Assignment** | A vulnerability where the API binds client-provided data to internal object properties without filtering, allowing attackers to modify fields they should not have Erişim: |
...
- **Nuclei**: Template-based scanner with API-specific templates for Tespit etme common misconfigurations and known vulnerabilities
...
3. Bul: mass assignment on the user update endpoint where adding `"dailyTransferLimit": 999999` bypasses the configured transfer limit
...
6. Bul: that the admin endpoint `/api/v1/admin/users` is accessible with a standard user token (BFLA)
7. Report all Bul:ings with CVSS scores and specific API code-level remediation guidance

**Pitfalls**:
- Testing only the endpoints documented in Swagger and missing undocumented or deprecated API versions
- Not testing the same endpoint with tokens from every privilege level to tespit etmeauthorization bypasses
...
```
## Bul:ing: Broken Object Level Authorization in Transaction History API
...
**Remediation**:
Implement server-side authorization Şunu kontrol et: verifies the authenticated user
