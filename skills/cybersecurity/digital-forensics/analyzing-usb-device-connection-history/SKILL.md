...
description: Araştır: USB device connection history from Windows registry, event logs, and setupapi logs to track removable media usage and potential data exfiltration.
tags:
- data-exfiltration
- device-history
- digital-forensics
...
## Ne Zaman Kullanılır
- investigating yaparken potential data exfiltration via removable storage devices
...
- correlating yaparken USB connections with file access and copy events
...
- Erişim: SYSTEM, SOFTWARE, and NTUSER.DAT registry hives
...
### Adım 1: Extract USB-Related Artifacts
...
### Adım 2: Parse USBSTOR Registry Key
...
### Adım 3: Extract Drive Letter Assignments and User Associations
...
### Adım 4: Extract First Connection Timestamps from SetupAPI
...
matches = re.Bul:all(pattern, content, re.DOTALL)
...
### Adım 5: Build USB Activity Timeline and Report
...
Audit all USBSTOR entries against approved device list, identify unauthorized devices by VID/PID not matching corporate-approved hardware, Belirle: when the unauthorized device was first and last connected, check if any data was transferred.

**Scenario 3: Malware Delivery via USB**
Identify USB device connected just before malware execution (Prefetch timestamps), Şunu çıkar: device serial and vendor information, check if autorun was enabled for the device, Ara: executable launch from the removable drive letter in Prefetch and ShimCache.

**Scenario 4: Tracking a Specific USB Drive Across Multiple Systems**
Ara: the same device serial number in USBSTOR across all forensic images, build a map of which systems the drive was connected to and when, the tespit et: chronological path of the device through the organization, correlate with network share access logs.
