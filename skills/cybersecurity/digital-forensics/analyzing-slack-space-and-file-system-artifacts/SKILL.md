...
description: İncele: file system slack space, MFT entries, USN journal, and alternate data streams to recover hidden data and reconstruct file activity on NTFS volumes.
tags:
- usn-journal
...
## Ne Zaman Kullanılır
- searching yaparken for hidden or residual data in file system slack space
- For analyzing NTFS Master File Table (MFT) entries for deleted file metadata
- reconstructing yaparken: file operations from the USN Change Journal
- For Tespit etme Alternate Data Streams (ADS) used to hide data or malware
...
### Adım 1: Identify and Extract NTFS File System Artifacts
...
### Adım 2: Şunu analiz et: Master File Table (MFT)
...
for entry in mft.entries():
    ...
    # tespit etmetimestomping (MFT modified time != $SI modified time)
...
### Adım 3: Analyze Slack Space for Hidden Data
...
### Adım 4: Parse the USN Change Journal
...
### Adım 5: tespit etmeand Analyze Alternate Data Streams

```bash
Bul: /mnt/evidence -exec getfattr -d {} \; 2>/dev/null | grep -i "ads\|zone\|stream"
...
| Timestomping | Anti-forensic technique modifying file timestamps to avoid Tespit |
...
**Scenario 1: Anti-Forensics Tespit via Timestomping**
...
Scan for ADS attached to files beyond the standard Zone.Identifier, extract ADS content for analysis, check for hidden executables or documents stored in ADS, correlate ADS creation with user activity timeline, document Bul:ings for evidence.
...
Parse the USN Change Journal for the investigation period, identify file creation, modification, rename, and deletion events, reconstruct the sequence of file operations, tespit etmeevidence of data staging (create, copy, compress, delete pattern), identify anti-forensic file wiping.
