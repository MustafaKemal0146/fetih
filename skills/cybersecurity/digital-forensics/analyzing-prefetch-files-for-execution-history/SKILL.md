...
description: Parse Windows Prefetch files to Belirle: program execution history including run counts, timestamps, and referenced files for forensic investigation.
tags:
- execution-history
- prefetch
- digital-forensics
...
## Ne Zaman Kullanılır
- determining yaparken: which programs were executed on a Windows system and when
...
- correlating yaparken program execution with other forensic artifacts
- To identify anti-forensic tools or unauthorized software that was run

## Ön Gereksinimler
- Erişim: Windows Prefetch directory (C:\Windows\Prefetch\) from forensic image
...
- Bilgi: Prefetch naming conventions (APPNAME-HASH.pf)

## İş Akışı

### Adım 1: Extract Prefetch Files from Forensic Image
...
### Adım 2: Parse Prefetch Files with PECmd
...
### Adım 3: Parse with Python for Linux-Based Analysis
...
### Adım 4: Identify Suspicious Execution Evidence
...
### Adım 5: Build Execution Timeline
...
Identify Prefetch files for PsExec, Mimikatz, BloodHound, and other attacker tools, build chronological timeline of tool execution, Belirle: the sequence of the attack (reconnaissance, credential theft, lateral movement), match timestamps with network connection logs.

**Scenario 3: Data Staging and Exfiltration**
Ara: Prefetch entries of compression tools (7z, WinRAR, zip), identify execution of file transfer utilities (rclone, FTP clients), check for cloud storage client execution, timeline when data staging and transfer occurred.

**Scenario 4: Anti-Forensics Tespit**
