...
## Ne Zaman Kullanılır
- reconstructing yaparken: user file access history from Windows shortcut files
...
Bul: /mnt/evidence/ -name "*.lnk" -type f 2>/dev/null > /cases/case-2024-001/lnk/all_lnk_locations.txt
...
### Adım 2: Parse LNK Files with LECmd
...
### Adım 3: Parse LNK Files with Python
...
### Adım 4: Analyze for Investigative Value
...
İncele: Startup folder LNK files for malicious targets, check target path and arguments for encoded commands or suspicious executables, verify target file exists and İncele: it, correlate creation timestamp with initial compromise time.
