...
Bul: /cases/case-2024-001/browser/ -type f -exec sha256sum {} \; \
   > /cases/case-2024-001/browser/artifact_hashes.txt
```

### Adım 2: Extract Chrome Browsing History and Downloads
...
### Adım 3: Extract Firefox Browsing History
...
### Adım 4: Extract Cookies and Stored Credentials
...
### Adım 5: Use Hindsight for Comprehensive Chrome Analysis
...
Extract browser history around the reported phishing timeframe, the tespit et: phishing URL that was visited, check downloads for malicious attachments, İncele: cookies for session tokens that may have been stolen, correlate with email header analysis.

**Scenario 2: Data Exfiltration via Cloud Services**
Search history for cloud storage URLs (Dropbox, Google Drive, OneDrive, Mega), İncele: downloads and uploads, check form history for file names entered, review cookies for active cloud service sessions during the investigation period.

**Scenario 3: Policy Violation Investigation**
Extract complete browsing history for the investigation period, categorize sites visited, identify Erişim: prohibited content categories, document timestamps and visit duration, correlate with network proxy logs for verification.

**Scenario 4: Malware Delivery Vector Analysis**
Trace the chain of redirects leading to a drive-by download, İncele: the downloads database for the malware payload, check cache for exploit kit landing pages, the tespit et: initial referrer URL that started the infection chain.
