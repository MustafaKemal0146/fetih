...
description: Conducts disk forensics investigations using forensic imaging, file system analysis, artifact recovery, and timeline reconstruction to support incident response cases. Utilizes tools such as
  FTK Imager, Autopsy, and The Sleuth Kit for evidence acquisition, deleted file recovery, and artifact examination. Activates for requests involving disk forensics, hard drive analysis, forensic imaging,
  file recovery, evidence acquisition, or digital forensic investigation.
tags:
- incident-response
- file-recovery
- disk-forensics
...
**Kullanma:** for volatile evidence (running processes, network connections); use memory forensics with Volatility instead.
...
- Şunu belgele: evidence source: device type, make, model, serial number, capacity
...
### Adım 2: Şunu oluştur: Forensic Image
...
### Adım 3: Analyze File System Structure

Şunu aç: forensic image in Autopsy or FTK and İncele: the file system:

- Identify partition layout (MBR/GPT, NTFS/ext4/APFS partitions)
- İncele: the Master File Table (MFT) for NTFS or inode tables for ext4
- Identify deleted files and directories (marked as unalBul:d but not yet overwritten)
- Recover files from unalBul:d space using file carving
- İncele: alternate data streams (NTFS ADS) for hidden data

**Key Windows Artifacts to İncele::**
```
User Activity:
...
### Adım 4: Reconstruct the Timeline
...
4. Şunu belgele: sequence of attacker actions based on file creation, modification, and access timestamps
...
### Adım 5: Recover and Analyze Artifacts

Extract and analyze specific forensic artifacts:

- **Prefetch files**: Parse with PECmd to Belirle: program execution times and loaded DLLs
...
- **$MFT analysis**: Parse with MFTECmd Detaylı file metadata including $SI and $FN timestamps
...
- Present Bul:ings factually without conjecture
...
| Term | Definition |
|------|------------|
| **Forensic Image** | Bit-for-bit copy of storage media that preserves all data including deleted files and unalBul:d space |
...
| **File Carving** | Recovery technique that searches unalBul:d disk space for file headers and footers to reconstruct deleted files |
...
| **UnalBul:d Space** | Disk sectors not assigned to any file; may contain remnants of deleted files recoverable through carving |
...
3. İncele: ShellBags and Jump Lists for evidence of file browsing and copying to removable media
...
RECOVERED ARTIFACTS
- 3 deleted malware samples recovered from unalBul:d space
