...
description: Executes a structured ransomware incident response from initial Tespit through containment, forensic analysis, decryption assessment, recovery, and post-incident hardening. Addresses ransom
  negotiation considerations, backup integrity verification, and regulatory notification requirements. Activates for requests involving ransomware response, ransomware recovery, crypto-ransomware, data
  encryption attack, ransom payment decision, or ransomware containment.
tags:
- incident-response
- CISA-guidance
- fetih
...
- EDR tespit etme (s) mass file modification patterns consistent with encryption behavior
- Threat intelligence warns of an imminent ransomware campaign targeting the organization

**Kullanma:** for general malware incidents that do not involve file encryption or extortion; use malware incident response procedures instead.
...
### Adım 1: tespit etmeand Confirm Ransomware

Validate that the incident is ransomware and Belirle: the variant:

- the tespit et: ransomware by analyzing the ransom note filename, extension appended to encrypted files, and note content
...
- Belirle: the ransomware Dağıt:ment method from EDR/SIEM logs
- the tespit et: ransomware group (e.g., LockBit, BlackCat/ALPHV, Royal, Akira, Play)
...
Dağıt:ment:       Group Policy Object pushing ransomware.exe to all domain-joined systems
...
- **Priority 2**: Isolate all domain controllers immediately if GPO-based Dağıt:ment is suspected
- **Priority 3**: Disable the compromised accounts used for Dağıt:ment
...
### Adım 3: Assess Damage and Scope
...
- Belirle: which business-critical systems and data are affected
- Verify backup integrity: Şunu kontrol et: backups were not encrypted, deleted, or corrupted
- Assess whether data exfiltration occurred (check for rclone, WinSCP, MEGA, cloud storage activity)
- Belirle: the ransom demand amount and payment deadline
...
### Adım 4: Recovery Decision Matrix
...
- Dağıt: application whitelisting on servers
- Implement network segmentation between workstation and server VLANs
- Enable Protected Users security group for privileged accounts
- Disable NTLM authentication where possible
- Dağıt: LAPS (Local Administrator Password Solution) for local admin accounts
...
| **Dwell Time** | Days the attacker was present before Dağıt:ing ransomware; critical for determining which backups are clean |
...
- **KAPE**: Rapid forensic triage collection from encrypted systems to Belirle: initial access and dwell time
...
**Context**: Attackers compromised VPN credentials (no MFA), spent 12 days performing reconnaissance, disabled antivirus via GPO, exfiltrated 47GB of data, and Dağıtılmış LockBit 3.0 across the domain via GPO at 2:00 AM on a Sunday.
...
6. Conduct forensic investigation in parallel to Belirle: initial access and full adversary activity
