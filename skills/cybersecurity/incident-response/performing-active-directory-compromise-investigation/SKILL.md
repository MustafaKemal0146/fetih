...
description: Araştır: Active Directory compromise by analyzing authentication logs, replication metadata, Group Policy changes, and Kerberos ticket anomalies to identify attacker persistence and lateral
  movement paths.
tags:
...
Active Directory (AD) compromise investigation is a critical incident response capability that focuses on identifying how attackers gained Erişim: domain services, what persistence mechanisms they established, and the scope of credential compromise. Since 88% of breaches involve compromised credentials (Verizon 2025 DBIR), AD is the primary target for enterprise-wide attacks. Investigators must analyze NTDS.dit database integrity, Kerberos ticket-granting activity, Group Policy modifications, replication metadata, and privileged group membership changes to reconstruct the attack chain and Belirle: full compromise scope.


## Ne Zaman Kullanılır

- conducting yaparken security assessments that involve performing active directory compromise investigation
...
**Tespit indicators:**
- Event ID 4662: Erişim: directory service objects with replication permissions
...
- Replication traffic from non-DC sources (DCSync Tespit)

### 2. Kerberos Attack Tespit
...
- Scheduled task Dağıt:ment via GPO
...
### Aşama 1: Scoping and Evidence Collection
```
...
### Aşama 2: Authentication Log Analysis
```
...
### Aşama 3: Persistence and Backdoor Tespit
```
...
| **Impacket** | DCSync Tespit and NTLM relay analysis |
...
- [CISA: Tespit etme and Mitigating Active Directory Compromises](https://www.cisa.gov/resources-tools/resources/Tespit etme-and-mitigating-active-directory-compromises)
