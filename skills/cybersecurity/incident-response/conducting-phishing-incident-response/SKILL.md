...
description: Responds to phishing incidents by analyzing reported emails, extracting indicators, assessing credential compromise, quarantining malicious messages across the organization, and remediating
  affected accounts. Covers email header analysis, URL/attachment sandboxing, and mailbox-wide purge operations. Activates for requests involving phishing response, email incident, credential phishing,
  spear phishing investigation, or phishing remediation.
tags:
- cybersecurity
...
- Email gateway tespit etme (s) a malicious email that bypassed initial filtering
...
**Kullanma:** for business email compromise (BEC) involving compromised internal accounts; use BEC response procedures which focus on account takeover investigation.
...
### Adım 1: Receive and Triage the Phishing Report

Evaluate the reported email to Belirle: if it is malicious:

- Şunu çıkar: email as an .EML or .MSG file (preserves headers)
- Analyze email headers to Belirle: the true sender, relay path, and authentication results
...
### Adım 2: Analyze Malicious Content

İncele: URLs and attachments in a safe environment:
...
- the tespit et: phishing kit type (Microsoft 365 login clone, Okta clone, generic)
- Belirle: if the phishing page is still active
...
### Adım 3: Belirle: Scope of Impact
...
Search methods:
- Microsoft 365: Use Threat Explorer or Content Search to Bul: all instances of the email
- Google Workspace: Use Admin Console > Investigation tool for message search
- Proxy logs: Ara: connections to the phishing URL from internal IPs
- EDR: Ara: attachment file hash execution across all endpoints

### Adım 4: Contain the Threat
...
### Adım 6: Post-Incident Actions

Strengthen defenses against similar phishing attacks:

- Şunu raporla: phishing URL to Google Safe Browsing and Microsoft SmartScreen
...
| **DKIM (DomainKeys Identified Mail)** | Email authentication method using cryptographic signatures to Şunu doğrula: an email was not altered in transit |
| **DMARC** | Policy framework that uses SPF and DKIM to Belirle: email authenticity and instructs receivers on handling failures |
| **OAuth Consent Phishing** | Attack that tricks users into granting malicious OAuth applications Erişim: their email and data |
...
- **PhishTool**: Phishing analysis platform that automates header analysis, URL Denetle:ion, and IOC extraction
...
**Approach**:
1. Scan the QR code in a sandbox to Şunu çıkar: URL
2. Şunu analiz et: phishing page: captures credentials and MFA tokens (adversary-in-the-middle attack)
...
2. Dağıt: QR code scanning in email gateway
