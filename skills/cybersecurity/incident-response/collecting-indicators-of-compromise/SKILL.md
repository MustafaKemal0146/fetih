...
description: Systematically collects, categorizes, and distributes indicators of compromise (IOCs) during and after security incidents to enable Tespit, blocking, and threat intelligence sharing. Covers
  network, host, email, and behavioral indicators using STIX/TAXII formats and threat intelligence platforms. Activates for requests involving IOC collection, indicator extraction, threat indicator sharing,
  compromise indicators, STIX export, or IOC enrichment.
tags:
...
- Post-incident to document all observed adversary artifacts for future Tespit
- sharing yaparken threat intelligence with ISACs, sector partners, or law enforcement
- building yaparken Tespit rules in SIEM, EDR, or network security tools
- enriching yaparken IOCs with threat intelligence context for risk scoring

**Kullanma:** for behavioral TTP analysis without accompanying technical indicators; use MITRE ATT&CK mapping for behavioral characterization.

## Ön Gereksinimler

- Erişim: incident evidence sources: SIEM logs, EDR telemetry, memory dumps, disk images, network captures
...
### Adım 1: Identify IOC Categories
...
### Adım 2: Extract IOCs from Evidence Sources
...
### Adım 3: Enrich IOCs with Context
...
### Adım 4: Score and Prioritize IOCs
...
### Adım 5: Distribute IOCs for Tespit and Blocking
...
### Adım 6: Share IOCs with Partners
...
**Context**: After responding to a Qakbot infection that led to Cobalt Strike Dağıt:ment, the IR team must package all IOCs for sharing with the Financial Services ISAC (FS-ISAC).
...
6. Şunu oluştur: human-readable IOC summary report for email distribution
