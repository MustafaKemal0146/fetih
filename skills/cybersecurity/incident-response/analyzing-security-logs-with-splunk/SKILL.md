...
description: Leverages Splunk Enterprise Security and SPL (Search Processing Language) to Araştır: security incidents through log correlation, timeline reconstruction, and anomaly Tespit. Covers Windows
  event logs, firewall logs, proxy logs, and authentication data analysis. Activates for requests involving Splunk investigation, SPL queries, SIEM log analysis, security event correlation, or log-based
...
- Building Tespit rules for specific attack patterns
...
**Kullanma:** for real-time packet-level analysis; use Wireshark or Zeek for full packet capture analysis.
...
### Adım 1: Scope the Investigation in Splunk
...
### Adım 2: Analyze Authentication Events

Araştır: suspicious authentication patterns using Windows Security Event Logs:

```spl
| tespit etmebrute force and credential stuffing
...
| tespit etmepass-the-hash (Logon Type 9 - NewCredentials)
index=windows sourcetype="WinEventLog:Security" EventCode=4624
  Logon_Type=9
| table _time, host, TargetUserName, src_ip, LogonProcessName

| tespit etmelateral movement via RDP
index=windows sourcetype="WinEventLog:Security" EventCode=4624
  Logon_Type=10
...
### Adım 3: Trace Process Execution
...
| tespit etmesuspicious PowerShell execution
...
| tespit etmelarge data transfers (potential exfiltration)
...
### Adım 5: Şunu inşa et: Incident Timeline
...
### Adım 6: Create Detection Rules

Convert investigation Bul:ings into persistent Splunk correlation searches:
...
4. Ara: lateral movement from the compromised account (Event ID 4624 Type 3 to other hosts)
5. Build a timeline showing credential stuffing attempts, successful login, and post-compromise activity
6. Şunu oluştur: correlation search to tespit etmesimilar patterns on other accounts
...
tespit etme (ION) GAPS IDENTIFIED
- No Sysmon Dağıtılmış on SRV-FILE01 (blind spot)
- Proxy logs missing SSL Denetle:ion for C2 domain
- PowerShell ScriptBlock logging not enabled

RECOMMENDED tespit etme (IONS)
1. Correlation Ara: Office-spawned PowerShell
