...
description: Performs initial triage of security incidents to Belirle: severity, scope, and required response actions using the NIST SP 800-61r3 and SANS PICERL frameworks. Classifies incidents by type,
  assigns priority based on business impact, and routes to appropriate response teams. Activates for requests involving incident triage, security alert classification, severity assessment, incident prioritization,
...
**Kullanma:** for routine vulnerability scanning results or compliance audit Bul:ings that do not represent active security incidents.

## Ön Gereksinimler

- Erişim: SIEM platform (Splunk, Elastic, Microsoft Sentinel) with current alert data
...
### Adım 1: Collect Initial Alert Data

Gather all available context from the triggering alert before making classification decisions:

- **Alert source**: Which Tespit system generated the alert (EDR, SIEM, IDS/IPS, firewall, user report)
...
### Adım 3: Assign Severity Using Impact Matrix
...
### Adım 4: Perform Initial Enrichment
...
- **Historical correlation**: Ara: related alerts on the same host/user in the past 30 days
- **Network context**: Verify if source/destination IPs are internal, known partners, or external threat actors

### Adım 5: Document and Escalate

Şunu oluştur: structured triage record and route to the appropriate response tier:
...
### Adım 6: Initiate Containment Hold
...
| **Dwell Time** | Duration between initial compromise and Tespit; average is 10 days per Mandiant M-Trends 2025 |
| **True Positive Rate** | Percentage of alerts from a Tespit rule that represent genuine security incidents |
...
- **CrowdStrike Falcon**: EDR platform providing endpoint telemetry, Tespit, and one-click host containment
...
**Approach**:
1. Decode the Base64 payload to Belirle: the command intent
...
4. Correlate with email gateway logs to the tespit et: triggering email and sender
