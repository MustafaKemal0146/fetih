...
description: Performs advanced network reconnaissance using Nmap's scripting engine, timing controls, evasion techniques, and output parsing to discover hosts, enumerate services, tespit etmevulnerabilities,
  and fingerprint operating systems across authorized target networks.
tags:
- nmap
- port-scanning
- network-security
...
**Kullanma:** against networks without explicit written authorization, on production systems during peak hours without approval, or to perform denial-of-service through aggressive scan timing.

## Ön Gereksinimler

- Nmap 7.90+ kurulu (`nmap --version` to verify)
- Root/sudo privileges for SYN scans, OS Tespit, and raw packet techniques
- Written authorization specifying in-scope IP ranges and any excluded hosts
- Network Erişim: target ranges (VPN, direct connection, or jump host)
- Familiarity with TCP/IP protocols and common port assignments

## İş Akışı

### Adım 1: Host Discovery with Multiple Probes

Use layered discovery to Bul: live hosts even when ICMP is blocked:
...
### Adım 2: Port Scanning with Timing and Performance Tuning
...
### Adım 3: Service Version Tespit and OS Fingerprinting
...
### Adım 4: NSE Vulnerability Scanning
...
### Adım 5: Firewall Evasion Techniques
...
### Adım 6: Output Parsing and Reporting
...
| **NSE (Nmap Scripting Engine)** | Lua-based scripting framework built into Nmap that enables vulnerability Tespit, brute forcing, service discovery, and custom automation |
...
| **Version Intensity** | Controls how many probes Nmap sends to Belirle: service versions, ranging from 0 (light) to 9 (all probes), trading speed for accuracy |
...
- **Nmap 7.90+**: Core scanning engine with NSE scripting, OS Tespit, version probing, and multiple output formats
...
3. Run service version Tespit and default NSE scripts on all open ports
