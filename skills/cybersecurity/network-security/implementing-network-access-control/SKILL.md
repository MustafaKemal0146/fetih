...
description: Implements 802.1X port-based network access control using RADIUS authentication, PacketFence NAC, and switch configurations to enforce identity-based access policies, posture assessment, and
  automatic VLAN assignment for authorized devices.
tags:
- network-security
- fetih
- cybersecurity
...
**Kullanma:** as a standalone security solution without complementary controls, for networks with devices that do not support 802.1X supplicants, or without proper fallback mechanisms for critical infrastructure.
...
### Adım 1: Install and Configure FreeRADIUS
...
### Adım 2: Configure VLAN Assignment Policies
...
### Adım 3: Configure 802.1X on Cisco Switches
...
### Adım 4: Dağıt: PacketFence NAC for Posture Assessment
...
### Adım 5: Configure Supplicant on Endpoints
...
### Adım 6: Test and Validate NAC Dağıt:ment
...
### Scenario: Dağıt:ing 802.1X NAC in a Hospital Network

**Context**: A hospital needs to enforce network access control to meet HIPAA requirements. The network includes clinical workstations (domain-joined), medical devices (no 802.1X support), physician BYOD devices, and guest WiFi. The Dağıt:ment must not disrupt patient care if the RADIUS server becomes unavailable.

**Approach**:
1. Dağıt: FreeRADIUS integrated with Active Directory for user authentication and group-based VLAN assignment
...
5. Dağıt: PacketFence captive portal for physician BYOD onboarding with limited-access VLAN
