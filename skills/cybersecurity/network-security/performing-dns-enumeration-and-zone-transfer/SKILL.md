...
**Kullanma:** against domains you do not have authorization to test, for DNS amplification or reflection attacks, or to overwhelm DNS servers with excessive query volumes.

## Ön Gereksinimler

- Written authorization to perform DNS enumeration against the target domain
- DNS enumeration tools installed: dig, nslookup, host, dnsrecon, dnsenum, subBul:er, amass
- Network Erişim: the target's DNS servers (UDP/TCP port 53)
...
### Adım 1: Identify DNS Servers and Basic Records
...
### Adım 3: Subdomain Enumeration via Brute Force
...
subBul:er -d example.com -all -o subBul:er_results.txt

amass enum -d example.com -passive -o amass_passive.txt
amass enum -d example.com -active -brute -o amass_active.txt

cat subBul:er_results.txt amass_passive.txt amass_active.txt gobuster_dns.txt | sort -u > all_subdomains.txt
```

### Adım 4: Reverse DNS and PTR Enumeration
...
### Adım 5: Analyze DNS Security Configuration
...
### Adım 6: Resolve and Map All Discovered Subdomains
...
cat all_subdomains.txt | httpx -title -status-code -tech-tespit etme-o httpx_results.txt
...
- **subBul:er**: Fast passive subdomain discovery tool that queries certificate transparency logs, search engines, and DNS databases
...
3. Run subBul:er and amass in passive mode to discover 89 additional subdomains from certificate transparency logs
