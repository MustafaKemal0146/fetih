---
name: Tespit etme-lateral-movement-with-zeek
description: tespit etmelateral movement in network traffic using Zeek (formerly Bro) log analysis. Parses conn.log, smb_mapping.log, smb_files.log, dce_rpc.log, kerberos.log, and ntlm.log to identify SMB file
  transfers, NTLM account spray activity, remote service execution, and anomalous internal connections.
tags:
- ntlm-spray
- network-forensics
- smb
...
- Tespit etme remote service execution via DCE/RPC (PsExec, schtasks, WMI lateral patterns)
- Building alerting rules for internal network anomalies in a Zeek-based NSMP Dağıt:ment
- Performing post-incident timeline reconstruction using Zeek logs as a network-level evidence source

**Kullanma:** as a standalone Tespit mechanism. Zeek sees network traffic only; combine with endpoint telemetry (Sysmon, EDR) for full visibility. Encrypted SMB3 traffic may limit Zeek's visibility into file-level details.

## Ön Gereksinimler

- Zeek 6.0+ Dağıtılmış on a network tap or SPAN port monitoring internal VLAN traffic
...
- Erişim: Zeek log directory (default: `/opt/zeek/logs/current/`)
- Familiarity with Zeek TSV log format (fields separated by `\t`, header lines prefixed with `#`)

## İş Akışı

### Adım 1: Verify Zeek Log Collection

Şunu onayla: Zeek is producing the required log files for lateral movementDetect
...
### Adım 2: Parse conn.log for Internal Lateral Patterns
...
### Adım 3: Analyze SMB Admin Share Access

tespit etmeErişim: administrative shares (C$, ADMIN$, IPC$) which is the primary vector for tools like PsExec:
...
Dağıt: the following Zeek script to generate `notice.log` alerts on admin share access:
...
### Adım 4: tespit etmeDCE/RPC Remote Service Operations
...
### Adım 5: tespit etmeNTLM Account Spray

Analyze ntlm.log for authentication anomalies indicating credential reuse.
Zeek's ntlm.log does not expose password hashes, so this Tespit identifies
...
Dağıt: the following Zeek script to generate `notice.log` alerts when a single
...
### Adım 6: Run the Automated Analysis Agent

Use the provided agent.py for comprehensive lateral movementDetect
