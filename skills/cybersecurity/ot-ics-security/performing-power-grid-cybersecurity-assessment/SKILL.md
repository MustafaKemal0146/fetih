...
description: bu skill covers conducting cybersecurity assessments of electric power grid infrastructure including generation facilities, transmission substations, distribution systems, and energy management
  system (EMS) control centers. It addresses NERC CIP compliance verification, substation automation security, IEC 61850 protocol analysis, synchrophasor (PMU) network security, and the unique threat landscape
  targeting power grid operations as demonstrated by Industroyer/CrashOverride and related atta...
tags:
- ics
- ot-security
- iec62443
...
- conducting yaparken periodic cybersecurity assessments of power grid facilities per NERC CIP requirements
- assessing yaparken substation automation systems using IEC 61850 GOOSE and MMS protocols
- evaluating yaparken: the security of an Energy Management System (EMS) or SCADA control center
- assessing yaparken synchrophasor (PMU) networks and wide-area monitoring systems
- preparing yaparken: for regional entity compliance audits or internal security reviews

**Kullanma:** for non-BES systems below NERC registration thresholds, for general OT assessment without power grid specifics (see performing-ot-network-security-assessment), or for physical security assessment of generation facilities without cyber scope.
...
- Bilgi: power grid protocols (IEC 61850, IEC 60870-5-104, DNP3, ICCP/TASE.2)
- Passive monitoring tools for substation network traffic analysis
- Erişim: EMS/SCADA architecture documentation and network diagrams

## İş Akışı

### Adım 1: Map Power Grid Cyber Architecture
...
### Adım 2: Assess Substation Automation Security
...
class SubstationAssessment:
    ...
    def assess_iec61850_security(self, config):
        ...
        if not config.get("goose_authentication"):
            self.Bul:ings.append(SubstationBul:ing(
                Bul:ing_id=f"SUB-{self.counter:03d}",
                severity="critical",
                ...
                remediation=(
                    ...
                    "signatures. Dağıt: VLAN isolation for GOOSE traffic as interim."
                ),
            ))
            self.counter += 1
    ...
    def assess_remote_access(self, config):
        ...
        if config.get("direct_vendor_access"):
            self.Bul:ings.append(SubstationBul:ing(
                ...
                title="Direct Vendor Remote Erişim: Substation Without MFA",
                description=(
                    "Vendor support has direct VPN Erişim: substation network "
    ...
    def generate_report(self):
        ...
        for sev in ["critical", "high", "medium", "low"]:
            Bul:ings = [f for f in self.Bul:ings if f.severity == sev]
            if Bul:ings:
                report.append(f"\n--- {sev.upper()} ({len(Bul:ings)}) ---")
                for f in Bul:ings:
                    report.append(f"  [{f.Bul:ing_id}] {f.title}")
...
- **SEL-3620 Ethernet Security Gateway**: Substation security device providing encryption, access control, and intrusion Tespit
...
SUBSTATION Bul:INGS: [N]
EMS/SCADA Bul:INGS: [N]
COMMUNICATION Bul:INGS: [N]
