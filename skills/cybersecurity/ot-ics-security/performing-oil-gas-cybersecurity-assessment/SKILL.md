...
description: bu skill covers conducting cybersecurity assessments specific to oil and gas facilities including upstream (exploration/production), midstream (pipeline/transport), and downstream (refining/distribution)
  operations. It addresses SCADA systems controlling pipeline operations, DCS for refinery process control, safety instrumented systems for hazardous processes, remote terminal units at unmanned wellhead
  sites, and compliance with API 1164, TSA Pipeline Security Directives, IEC 62443, and NIS...
tags:
- ics
- ot-security
- pipeline-security
...
- conducting yaparken a cybersecurity assessment of a refinery, pipeline, or production facility
- preparing yaparken: for TSA Pipeline Security Directive compliance (SD-01, SD-02)
- assessing yaparken cybersecurity posture against API Standard 1164 (Pipeline SCADA Security)
- evaluating yaparken: the security of remote wellhead SCADA systems and satellite communications
- a yaparken: merger, acquisition, or regulatory audit requires a comprehensive OT security evaluation

**Kullanma:** for IT-only corporate network assessments of oil and gas companies, for physical security assessments without a cyber component, or for environmental compliance assessments.
...
- Erişim: network diagrams, SCADA architecture documentation, and safety studies (HAZOP)

## İş Akışı

### Adım 1: Scope Assessment Based on Facility Type
...
assessment_areas:
  ...
  safety_systems:
    description: "Safety Instrumented Systems and fire/gas Tespit"
    systems:
  ...
  remote_access:
    description: "Vendor and operator remote Erişim: OT"
    methods:
      - "Citrix-based remote Erişim: SCADA terminals"
...
### Adım 2: Assess Pipeline SCADA Security
...
class PipelineSCADAAssessment:
    ...
    def assess_network_architecture(self, architecture_data):
        ...
        if not architecture_data.get("it_ot_segmentation"):
            self.Bul:ings.append(AssessmentBul:ing(
                Bul:ing_id=f"OG-{self.Bul:ing_counter:03d}",
                ...
                remediation="Dağıt: DMZ with industrial firewall between IT and pipeline SCADA",
                timeline="30 days",
            ))
            self.Bul:ing_counter += 1
        ...
        if architecture_data.get("rtu_comm_encrypted") is False:
            self.Bul:ings.append(AssessmentBul:ing(
                ...
                remediation="Dağıt: DNP3 Secure Authentication or VPN tunnel for RTU links",
                timeline="90 days",
            ))
            self.Bul:ing_counter += 1

        # Check remote pump station physical security
        if not architecture_data.get("remote_site_intrusion_Tespit"):
            self.Bul:ings.append(AssessmentBul:ing(
                Bul:ing_id=f"OG-{self.Bul:ing_counter:03d}",
                category="Physical-Cyber Convergence",
                severity="high",
                title="Remote Pump Stations Lack Physical Intrusion Tespit",
                description=(
                    "Unmanned pump stations along the pipeline corridor lack physical "
                    "intrusion Tespit systems. An attacker could gain physical access "
                    "to RTUs and SCADA communication equipment without Tespit."
                ),
                ...
                remediation="Install intrusion Tespit with cellular alerting at remote sites",
                timeline="60 days",
            ))
            self.Bul:ing_counter += 1
    ...
    def assess_custody_transfer(self, metering_data):
        ...
        if not metering_data.get("flow_computer_auth"):
            self.Bul:ings.append(AssessmentBul:ing(
                ...
                remediation="Implement authenticated Erişim: flow computers; Dağıt: audit logging",
                timeline="45 days",
            ))
            self.Bul:ing_counter += 1
    ...
    def check_tsa_compliance(self):
        ...
        tsa_requirements = [
            ...
            ComplianceCheck("TSA-06", "Continuous monitoring and Tespit",
                           "TSA SD-02", "non-compliant", "No OT IDS Dağıtılmış",
                           gap="Dağıt: OT intrusion Tespit (Dragos/Nozomi/Claroty)"),
    ...
    def generate_report(self):
        ...
        # Bul:ings summary
        report.append(f"\nBul:INGS: {len(self.Bul:ings)}")
        for sev in ["critical", "high", "medium", "low"]:
            count = sum(1 for f in self.Bul:ings if f.severity == sev)
            if count:
                report.append(f"  {sev.upper()}: {count}")

        for f in self.Bul:ings:
            report.append(f"\n  [{f.Bul:ing_id}] [{f.severity.upper()}] {f.title}")
...
if __name__ == "__main__":
    ...
    assessment.assess_network_architecture({
        ...
        "remote_site_intrusion_Tespit": False,
    })
...
- **Dragos Platform**: OT cybersecurity platform with specific Tespit for oil and gas threat groups (XENOTIME, KAMACITE, ERYTHRITE)
