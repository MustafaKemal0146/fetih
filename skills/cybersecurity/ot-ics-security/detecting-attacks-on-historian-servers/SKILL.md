---
name: Tespit etme-attacks-on-historian-servers
description: tespit etmecyber attacks targeting OT historian servers (OSIsoft PI, Ignition, Wonderware) that sit at the IT/OT boundary and serve as pivot points for lateral movement between enterprise and control
  networks, including data manipulation, unauthorized queries, and exploitation of historian-specific vulnerabilities.
tags:
- historian
- ics
- ot-security
...
- monitoring yaparken historian servers that bridge IT and OT networks for compromise indicators
- Tespit yaparken: etme unauthorized queries or data manipulation in process historian databases
- investigating yaparken lateral movement through historian servers between IT and OT zones
- responding yaparken to alerts about exploitation of historian-specific vulnerabilities (CVE-2025-0921)
- validating yaparken historian data integrity after a suspected OT security incident

**Kullanma:** for general database security monitoring (see database security skills), for historian Dağıt:ment and configuration, or for IT-only data warehouse security.
...
### Adım 1: Monitor Historian for Attack Indicators

```python
#!/usr/bin/env python3
"""OT Historian Attack tespit etme (or).
...
class HistorianAttackDetector:
    """tespit etme (s) attacks targeting OT historian servers."""
    ...
    def check_lateral_movement_indicators(self):
        ...
        indicators.append({
            ...
            "Tespit": "Monitor firewall logs for historian IP connecting to PLC ports (502, 102, 44818)",
        })
        ...
        indicators.append({
            ...
            "Tespit": "Monitor process creation events (Sysmon EventID 1) on historian",
        })
        ...
        indicators.append({
            ...
            "Tespit": "Monitor Windows Security Event 4624 for logons from non-baseline sources",
        })

        return indicators

    def generate_report(self):
        """Generate historian attack Tespit report."""
        print(f"\n{'='*70}")
        print("HISTORIAN ATTACK tespit etme (ION) REPORT")
        print(f"{'='*70}")
        ...
        for indicator in self.check_lateral_movement_indicators():
            ...
            print(f"   Detect {indicator['Tespit']}")


if __name__ == "__main__":
    tespit etme (or) = HistorianAttackDetector(
    ...
    tespit etme (or).set_baseline(
        authorized_clients=["10.10.2.10", "10.10.2.20", "10.10.3.50", "10.10.150.10"],
        authorized_query_patterns={},
    )

    tespit etme (or).check_active_connections()
    tespit etme (or).check_data_integrity(tags=["REACTOR_01.TEMP", "PUMP_03.FLOW"], hours_back=24)
...
| OSIsoft PI | Most widely Dağıtılmış OT historian, used by 65% of Global 500 process companies |
...
```
HISTORIAN ATTACK tespit etme (ION) REPORT
====================================
