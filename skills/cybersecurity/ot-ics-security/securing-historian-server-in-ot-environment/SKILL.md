...
description: bu skill covers hardening and securing process historian servers (OSIsoft PI, Honeywell PHD, GE Proficy, AVEVA Historian) in OT environments. It addresses network placement across Purdue
  levels, access control for historian interfaces, data replication through DMZ using data diodes or PI-to-PI connectors, SQL injection prevention in historian queries, and integrity protection of process
...
- Dağıt:ing yaparken a new historian server in an OT environment and configuring it securely from the start
- hardening yaparken an existing historian after a security assessment identified it as a high-risk target
- designing yaparken historian data replication architecture through a DMZ for IT Erişim: process data
- implementing yaparken access controls to prevent unauthorized modification of historical process data
- investigating yaparken suspected historian compromise or data integrity issues

**Kullanma:** for IT-only database security without OT data (see general database hardening), for real-time SCADA data transmission security (see Tespit etme-attacks-on-scada-systems), or for historian selection and sizing decisions.
...
- Erişim: historian administration credentials
- DMZ infrastructure for IT-facing data replication

## İş Akışı

### Adım 1: Audit Current Historian Security Configuration
...
class HistorianSecurityAudit:
    ...
    def check_network_exposure(self):
        ...
        for svc in exposed:
            if svc["port"] in (80, 135, 445, 3389):
                self.Bul:ings.append(AuditBul:ing(
                    Bul:ing_id=f"HIST-{self.counter:03d}",
                    severity="high",
    ...
    def check_data_integrity(self):
        ...
        integrity_checks = [
            ...
            AuditBul:ing(
                Bul:ing_id=f"HIST-{self.counter + 1:03d}",
                severity="medium",
                category="Data Integrity",
        ...
        self.Bul:ings.extend(integrity_checks)
...
### Adım 2: Harden Historian Server
...
### Adım 3: Implement Secure Data Replication to DMZ

Configure historian data replication through the DMZ using PI-to-PI interfaces or data diodes to provide IT Erişim: process data without exposing the OT historian.
...
- **PI Vision**: Web-based visualization tool for PI data, Dağıtılmış in DMZ for enterprise access
