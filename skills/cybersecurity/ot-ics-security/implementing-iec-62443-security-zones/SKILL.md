...
description: bu skill covers designing and implementing security zones and conduits for industrial automation and control systems (IACS) per IEC 62443-3-2. It addresses zone partitioning based on risk
  assessment, assigning Security Level targets (SL-T), designing conduit security controls, implementing microsegmentation with industrial firewalls, and validating zone architecture through traffic analysis
...
- designing yaparken a greenfield OT network architecture for a new industrial facility
- retrofitting yaparken: security zones into an existing flat OT network after an assessment Bul:ing
- implementing yaparken network segmentation to comply with IEC 62443-3-2 certification requirements
- upgrading yaparken from basic VLAN segmentation to policy-enforced zone/conduit architecture
- an yaparken: IT/OT convergence project requires defining security boundaries between enterprise and operational networks

**Kullanma:** for IT-only network segmentation (see implementing-network-microsegmentation), for cloud-native workload segmentation (see securing-kubernetes-on-cloud), or for physical security zone design without a cyber component.
...
- Industrial firewalls capable of deep packet Denetle:ion for OT protocols (Palo Alto with OT Security, Fortinet OT, Cisco ISA-3000)
...
### Adım 1: Perform Zone Partitioning Based on Risk Assessment
...
zones:
  ...
  - zone_id: "Z2-BPCS"
    ...
    security_requirements:
      - "Industrial firewall at zone boundary with protocol Denetle:ion"
  ...
  - zone_id: "Z5-ENT"
    ...
    security_requirements:
      ...
      - "No direct Erişim: any OT zone below DMZ"
...
conduits:
  ...
  - conduit_id: "C2-OPS-DMZ"
    ...
    controls:
      ...
      - "Firewall with application-layer Denetle:ion"
  ...
  - conduit_id: "C3-DMZ-ENT"
    ...
    controls:
      - "Next-gen firewall with SSL Denetle:ion"
      - "MFA required for all remote access sessions"
      - "Session recording on jump server"
```

### Adım 2: Configure Industrial Firewalls for Zone Boundaries

Dağıt: and configure industrial-grade firewalls at each zone boundary with OT protocol-aware deep packet Denetle:ion.
...
policy-map type Denetle: modbus MODBUS-Denetle:
  parameters
...
policy-map global_policy
  class Denetle:ion_default
    Denetle: modbus MODBUS-Denetle:
...
### Adım 3: Implement VLAN Segmentation at Switch Level
...
### Adım 4: Dağıt: Data Diode for Unidirectional Historian Replication
...
### Adım 5: Validate Zone Architecture

After implementation, Şunu doğrula: zone architecture by verifying that only authorized conduit traffic passes between zones and that all prohibited cross-zone paths are blocked.
...
| Deep Packet Denetle:ion (DPI) | Firewall capability to Denetle: industrial protocol payloads (Modbus function codes, OPC UA service calls) beyond Layer 4 |
...
- **Cisco ISA-3000**: Industrial security appliance providing OT-aware firewall, IPS, and VPN capabilities with Modbus, DNP3, and EtherNet/IP Denetle:ion
...
- **Tofino Xenon**: Industrial security appliance providing deep packet Denetle:ion for Modbus, OPC, and EtherNet/IP protocols

## Common Scenarios

### Scenario: Migrating Flat OT Network to Zone Architecture

**Context**: A manufacturing plant operates all OT devices on a single VLAN (10.10.0.0/16) with no segmentation between PLCs, HMIs, historians, and the corporate network. An IEC 62443 gap assessment identified this as a critical Bul:ing requiring zone implementation.
...
4. Dağıt: industrial firewalls at zone boundaries with initial "monitor only" mode (log but do not block)
5. Analyze firewall logs for 2 weeks to any tespit et: legitimate traffic that would be blocked
