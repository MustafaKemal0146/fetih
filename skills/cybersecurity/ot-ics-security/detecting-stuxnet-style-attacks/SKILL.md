---
name: Tespit etme-stuxnet-style-attacks
description: bu skill covers Tespit etme sophisticated cyber-physical attacks that follow the Stuxnet attack pattern of modifying PLC logic while spoofing sensor readings to hide the manipulation from operators.
  It addresses PLC logic integrity monitoring, physics-based process anomaly Tespit, engineering workstation compromise indicators, USB-borne attack vectors, and multi-stage attack chain Tespit spanning
...
- implementing yaparken advanced threat Tespit for high-value OT targets (nuclear, chemical, critical infrastructure)
- building yaparken Tespit for APT-style attacks targeting PLC logic and process manipulation
- establishing yaparken: PLC logic integrity monitoring to tespit etmeunauthorized modifications
- investigating yaparken suspected process anomalies that may indicate cyber-physical attacks
- designing yaparken defense-in-depth strategies against nation-state level OT threats

**Kullanma:** for basic OT intrusion Tespit (see Tespit etme-attacks-on-scada-systems), for malware analysis of Stuxnet samples (see malware reverse engineering skills), or for PLC programming and logic development.
...
### Adım 1: Understand the Stuxnet Attack Chain

Map Tespit opportunities across the multi-stage Stuxnet-style attack chain.
...
attack_chain:
  ...
  stage_2_lateral_movement:
   ...
   Detect
      - "Network IDS Tespit etme exploit traffic (MS08-067, MS10-061)"
  ...
  stage_3_ews_compromise:
   ...
   Detect
      ...
      - "DLL injection Tespit in PLC programming software"
  ...
  stage_5_process_manipulation:
   ...
   Detect
      - "Physics-based anomaly Tespit (process model deviation)"
...
### Adım 2: Implement PLC Logic Integrity Monitoring
...
against known-good baselines to tespit etmeunauthorized modifications
...
### Adım 3: Dağıt: Physics-Based Process Anomaly Tespit
...
```python
#!/usr/bin/env python3
"""Physics-Based Cyber-Physical Attack tespit etme (or).

Uses simplified physics models to tespit etmeprocess manipulation
...
class CentrifugePhysicsModel:
    ...
    tespit etme (s) manipulation by cross-correlating:
    ...
    def report(self):
        if self.alerts:
            print(f"\n{'='*60}")
            print("PHYSICS-BASED ANOMALY tespit etme (ION) ALERTS")
            print(f"{'='*60}")
            for a in self.alerts:
                print(f"\n  [{a.severity.upper()}] {a.alert_type}")
                print(f"    {a.description}")
...
| Physics-Based Tespit | Using mathematical models of physical processes to tespit etmewhen reported sensor values are inconsistent with actual physics |
...
- **Claroty xDome**: Continuous PLC logic monitoring with baseline comparison and change Tespit
- **SIGA OT Solutions**: Physical signal monitoring at the electrical level for Tespit etme process manipulation
- **Nozomi Guardian**: OT monitoring with PLC program change Tespit capabilities
- **Siemens SINEMA Remote Connect**: Secure remote access with PLC project version tracking

## Output Format

```
Stuxnet-Style Attack Tespit Report
========================================
Monitored PLCs: [N]
