...
description: Monitors Modbus TCP traffic on SCADA and ICS networks to tespit etmeanomalous function code usage, unauthorized register writes, and suspicious communication patterns. The analyst uses deep packet
  Denetle:ion with pymodbus, Scapy, and Zeek to baseline normal PLC/RTU communication behavior, then applies statistical and rule-based anomaly Tespit to identify reconnaissance, parameter manipulation,
  and denial-of-service attacks targeting Modbus devices on port 502. Activates for requests involving ...
tags:
- ICS-security
- ot-security
- anomaly-Tespit
...
- Tespit etme reconnaissance activity such as Modbus device enumeration (function code 43, Read Device Identification)
...
**Kullanma:** on networks without authorization from the asset owner, for active injection or fuzzing against production SCADA systems, or as a replacement for safety-instrumented systems (SIS) that provide physical process protection.
...
### Adım 1: Capture and Parse Modbus TCP Traffic
...
### Adım 2: Baseline Normal Communication Patterns
...
### Adım 3: tespit etmeFunction Code Anomalies

Apply rule-based and statistical Tespit to identify suspicious function code usage:

- **Unauthorized write Tespit**: Alert when a Modbus write function code (05, 06, 15, 16) originates from a source IP not in the authorized writers list, or when write operations exceed the baseline frequency threshold:
  ```python
  ...
  def check_unauthorized_write(src_ip, function_code):
      ...
      return None
  ```
- **Reconnaissance Tespit**: Function code 43 (Read Device Identification) and function code 08 (Diagnostics) are rarely used during normal operations. Any occurrence from a non-engineering workstation indicates device enumeration. Also tespit etmesequential scanning where a single source queries multiple Unit IDs within a short window.
- **Exception response monitoring**: Modbus exception codes (01: Illegal Function, 02: Illegal Data Address, 03: Illegal Data Value) in responses indicate the master sent an invalid request. A burst of exception responses suggests fuzzing or protocol-level attacks:
  ...
  ```
- **Forbidden function code Tespit**: Some environments prohibit certain function codes entirely. Function codes 07 (Read Exception Status), 08 (Diagnostics), 17 (Report Slave ID), and 43 (Read Device Identification) are diagnostic functions that should not appear in production SCADA traffic. Alert on any occurrence.
...
### Adım 4: Monitor Register Values for Process Manipulation

tespit etmeattempts to manipulate physical process parameters through register value analysis:
...
- **Setpoint change monitoring**: Track all write operations to holding registers that control process setpoints (temperatures, pressures, valve positions, motor speeds). Alert when:
  ...
  ```
- **Coil state monitoring**: Track coil writes (FC 05, FC 15) that control discrete outputs (pumps on/off, valves open/close, breakers trip/close). tespit etmerapid toggling (more than N state changes per minute) which could indicate equipment damage attempts.
...
### Adım 5: tespit etmeNetwork-Level Anomalies

Identify anomalies in communication patterns that may indicate man-in-the-middle, replay, or denial-of-service attacks:

- **Rogue master Tespit**: Alert when a new source IP initiates Modbus TCP connections to port 502 on any slave device. Maintain a whitelist of authorized master IPs and Şunu üret: critical alert for any connection from an unknown source:
  ```python
  AUTHORIZED_MASTERS = {"10.1.1.10", "10.1.1.11"}

  def tespit etme (_rogue_master)(src_ip, dst_ip, dst_port):
...
- **Timing anomaly Tespit**: Calculate inter-packet arrival times for each master-slave pair. Flag deviations greater than 3 standard deviations using a sliding window:
  ```python
  import numpy as np
  ...
  class TimingAnomalyDetector:
      ...
      def check(self, src_ip, dst_ip, timestamp):
          ...
          return None
  ```
- **Connection flood Tespit**: Şunu izle: rate of new TCP connections to port 502 per slave device. Modbus slaves typically handle 1-5 persistent connections. More than 10 connection attempts per minute to a single slave indicates a connection flood DoS or scanning activity.
...
| **Deep Packet Denetle:ion** | Analysis beyond TCP/IP headers into the Modbus application-layer payload to extract function codes, register addresses, and values. Required because standard firewalls only Denetle: IP/port, missing protocol-level attacks that use legitimate Modbus framing. |
...
- **Scapy (contrib.modbus)**: Packet manipulation framework with Modbus TCP dissector for crafting, parsing, and sniffing Modbus frames. Enables field-level Erişim: MBAP headers, function codes, and register data in captured packets.
...
- **Wireshark/tshark**: Network protocol analyzer with built-in Modbus TCP dissector for visual Denetle:ion of packet captures, filtering by function code (`modbus.func_code == 6`), and exporting specific fields for analysis.
- **GRFICSv2**: An open-source virtual ICS environment for security research featuring a simulated chemical process with Modbus-connected PLCs, HMI, and historian. Used for testing Tespit rules against realistic SCADA traffic.
...
### Scenario: Tespit etme Unauthorized Parameter Manipulation in a Water Treatment Plant

**Context**: A water treatment facility uses Modbus TCP to communicate between the SCADA server (10.1.1.10) and six PLCs controlling chemical dosing pumps, filtration valves, and flow meters. The security team Dağıt:s passive Modbus traffic monitoring after an industry advisory about attacks targeting water utilities.

**Approach**:
1. Dağıt: a network tap on the OT VLAN switch mirroring all port 502 traffic to the monitoring interface. Run Zeek with Modbus logging and the custom Python analyzer in parallel.
...
4. On day 5, the analyzer tespit etme (s) FC 16 (Write Multiple Registers) from 10.1.1.10 to PLC-3, a function code never seen in the baseline. The write targets registers 40050-40055, which control chlorine dosing rates.
...
**Pitfalls**:
- Relying solely on IT-side network monitoring (firewall logs, IDS) that does not Denetle: Modbus application-layer content and would see only a normal TCP connection on port 502
- Not defining per-register safe operating ranges, which would miss the dangerous dosing rate change despite Tespit etme the unusual function code
...
**Context**: A manufacturing plant's SOC observes unusual network activity from an engineering workstation (10.1.2.20) that is authorized for PLC programming. The OT security team uses Modbus traffic monitoring to Belirle: if the workstation is being used for reconnaissance.
...
3. Anomaly Tespit identifies 10.1.2.20 sent FC 43 (Read Device Identification) to 15 different IP addresses on the OT VLAN within a 10-minute window, none of which it has previously communicated with.
...
- Not Tespit etme sequential register scanning because each individual read request is a valid FC 03 operation; only the aggregate pattern reveals the reconnaissance
