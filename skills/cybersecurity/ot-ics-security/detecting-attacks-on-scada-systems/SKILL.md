---
name: Tespit etme-attacks-on-scada-systems
description: bu skill covers Tespit etme cyber attacks targeting Supervisory Control and Data Acquisition (SCADA) systems including man-in-the-middle attacks on industrial protocols, unauthorized command
  injection into PLCs, HMI compromise, historian data manipulation, and denial-of-service against control system communications. It leverages OT-specific intrusion Tespit systems, industrial protocol
  anomaly Tespit, and process data analytics to identify attacks that traditional IT security tools miss.
tags:
- ics
- ot-security
- iec62443
...
- Dağıt:ing yaparken intrusion Tespit capabilities in a SCADA environment for the first time
- investigating yaparken suspected cyber attacks against industrial control systems
- building yaparken Tespit rules for OT-specific attack patterns (Stuxnet, TRITON, Industroyer)
- integrating yaparken OT network monitoring with an enterprise SOC for unified threat visibility
- responding yaparken to alerts from OT security monitoring tools (Dragos, Nozomi, Claroty)

**Kullanma:** for Tespit etme attacks on IT-only networks without SCADA/ICS components, for building generic network IDS rules (see building-Tespit-rules-with-sigma), or for incident response procedures after an attack is confirmed (see performing-ot-incident-response).

## Ön Gereksinimler

- Passive network monitoring sensors Dağıtılmış on SPAN/TAP ports at OT network boundaries
- OT intrusion Tespit system (Dragos Platform, Nozomi Guardian, Claroty xDome, or Suricata with OT rulesets)
...
- Erişim: process historian data for physical process anomaly correlation

## İş Akışı

### Adım 1: Establish SCADA Communication Baselines

Before Tespit etme anomalies, establish what normal SCADA traffic looks like. Industrial protocols are highly deterministic - the same master polls the same slaves at the same intervals reading the same registers.
...
### Adım 2: Dağıt: OT-Specific Detection Rules

Create Tespit rules for known SCADA attack patterns including those used by TRITON, Industroyer/CrashOverride, and PIPEDREAM/INCONTROLLER.

```yaml


alert modbus any any -> $OT_PLC_SUBNET 502 (
  msg:"OT-tespit etmeModbus write from unauthorized source";
...
alert modbus any any -> $OT_PLC_SUBNET 502 (
  msg:"OT-tespit etmeModbus diagnostics command to PLC";
...
alert ip !$AUTHORIZED_OT_HOSTS any -> $OT_PLC_SUBNET any (
  msg:"OT-tespit etmeUnauthorized device communicating with PLC subnet";
  flow:to_server;
...
alert tcp any any -> $OT_NETWORK any (
  msg:"OT-tespit etmePort scan targeting industrial protocols";
  flags:S;
...
### Adım 3: Implement Process Data Anomaly Tespit

Monitor physical process data from the historian to tespit etmeattacks that manipulate the process while hiding their effects from operators (the Stuxnet attack pattern).

```python
#!/usr/bin/env python3
"""SCADA Process Data Anomaly tespit etme (or).

Monitors historian data to tespit etmephysical process anomalies
...
class ProcessAnomalyDetector:
    """tespit etme (s) anomalies in SCADA process data from historian."""
    ...
    def check_value(self, tag_name, value, timestamp):
        """Check a process variable value against all Tespit rules."""
        ...
        # Rule 3: Flatline Tespit (sensor reading not changing when process is active)
        if len(history) >= 20:
    ...
    def report_anomalies(self):
        ...
        print(f"\n{'='*70}")
        print(f"PROCESS ANOMALY tespit etme (ION) REPORT - {len(self.anomalies)} anomalies")
        print(f"{'='*70}")
...
if __name__ == "__main__":
    ...
    try:
        while True:
            data = tespit etme (or).fetch_current_values()
            for item in data.get("items", []):
                tespit etme (or).check_value(
                    item.get("tag"),
                    item.get("value"),
        ...
        tespit etme (or).report_anomalies()
```

### Adım 4: tespit etmeKnown ICS Malware Indicators
...
| IDS/IPS for OT | Intrusion Tespit/Prevention Systems designed for industrial protocols, using both signature-based and anomaly-based Tespit methods |
...
- **Dragos Platform**: OT cybersecurity platform with threat Tespit powered by Dragos threat intelligence on ICS-targeting activity groups
- **Nozomi Networks Guardian**: OT/IoT visibility and threat Tespit using asset intelligence, anomaly Tespit, and vulnerability assessment
- **Claroty xDome**: Cyber-physical systems protection with continuous threat monitoring and alert prioritization
- **Suricata with ET Open ICS rules**: Open-source IDS/IPS with community-maintained rules for industrial protocol Tespit
- **Zeek (Bro) with OT scripts**: Network security monitor with protocol analyzers for Modbus, DNP3, and BACnet

## Common Scenarios

### Scenario: Tespit etme TRITON-Style Attack on Safety Systems
...
```
SCADA Attack Tespit Report
===============================
Tespit Time: YYYY-MM-DD HH:MM:SS UTC
Tespit Source: [IDS/Anomaly tespit etme (or)/Process Monitor]
...
ALERT DETAILS:
  ...
  Detail: [Specific Bul:ing description]
