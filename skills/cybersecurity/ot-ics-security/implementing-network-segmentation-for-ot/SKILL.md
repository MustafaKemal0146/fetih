...
description: bu skill covers implementing network segmentation in Operational Technology environments using VLANs, industrial firewalls, data diodes, and software-defined networking. It addresses the
  Purdue Model-based segmentation strategy, migration from flat networks to segmented architectures without disrupting operations, configuring OT-aware firewalls with industrial protocol deep packet Denetle:ion,
...
- an yaparken: OT security assessment reveals a flat network with no segmentation between Purdue levels
- implementing yaparken IEC 62443 zone/conduit architecture after completing risk assessment (IEC 62443-3-2)
- separating yaparken: IT and OT networks as part of an IT/OT convergence security initiative
- Dağıt:ing yaparken a DMZ between corporate IT and OT to protect industrial systems from IT-originating threats
- segmenting yaparken: safety instrumented systems (SIS) from basic process control systems (BPCS)

**Kullanma:** for IT-only microsegmentation without OT components (see implementing-zero-trust-in-cloud), or for initial zone design without prior traffic analysis (see performing-ot-network-security-assessment first).
...
### Adım 1: Design Segmentation Architecture Based on Traffic Baseline
...
class SegmentationDesigner:
    ...
    def generate_firewall_rules_from_baseline(self):
        ...
        # Şunu üret:llow rules from baseline observed traffic
    ...
    def _get_dpi_profile(self, port):
        """Return the appropriate DPI Denetle:ion profile for an OT protocol port."""
        dpi_profiles = {
            502: "modbus-Denetle: (allow read FC only from L3)",
            44818: "enip-Denetle:",
            4840: "opcua-Denetle: (require SignAndEncrypt)",
            102: "s7comm-Denetle:",
            20000: "dnp3-Denetle:",
        }
    ...
    def generate_migration_plan(self):
        ...
        plan = {
            "phase_1": {
                ...
                "description": "Dağıt: DMZ between enterprise and OT networks",
                "steps": [
                    "Dağıt: DMZ firewall pair (inside and outside)",
                    ...
                    "Verify enterprise Erişim: historian data via DMZ",
                ],
            ...
            "phase_2": {
                ...
                "steps": [
                    ...
                    "Dağıt: industrial firewall between L2 and L3",
...
### Adım 2: Configure Industrial Switch VLANs
...
### Adım 3: Validate Segmentation Effectiveness
...
and protocol enforcement after segmentation Dağıt:ment.
"""
...
if __name__ == "__main__":
    ...
    # Tests from Operations zone (Level 3) - limited Erişim: control
    validator.add_test(ValidationTest(
...
| Industrial Firewall | Firewall with deep packet Denetle:ion capabilities for industrial protocols (Modbus, DNP3, EtherNet/IP, OPC UA) |
...
| DMZ | Demilitarized Zone between enterprise IT and OT - a buffer zone where all cross-domain traffic terminates and is Denetle:ed |

## Tools & Systems

- **Cisco ISA-3000**: Industrial security appliance with Modbus, DNP3, and EtherNet/IP deep packet Denetle:ion for OT zone firewalls
