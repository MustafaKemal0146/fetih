...
description: bu skill covers conducting comprehensive security assessments of Operational Technology (OT) networks including SCADA systems, DCS architectures, and industrial control system communication
  paths. It addresses the Purdue Reference Model layers, identifies IT/OT convergence risks, evaluates firewall rules between zones, and maps industrial protocol traffic (Modbus, DNP3, OPC UA, EtherNet/IP)
  to tespit etmemisconfigurations, unauthorized connections, and attack surfaces in critical infrastructure.
tags:
- ics
- ot-security
- iec62443
...
- conducting yaparken an initial security baseline of an OT/ICS environment for a new client
- evaluating yaparken: the security posture of a facility after an IT/OT convergence initiative
- preparing yaparken: for IEC 62443 or NERC CIP compliance audits
- assessing yaparken risk following a merger or acquisition involving industrial facilities
- investigating yaparken whether an OT network has been compromised or has unmonitored pathways to corporate IT

**Kullanma:** for IT-only network assessments without OT components, for application-layer vulnerability scanning of IT web applications (see performing-web-app-penetration-test), or for active exploitation of live OT systems without explicit authorization and safety controls in place.
...
- Erişim: network diagrams, firewall rule sets, and asset inventories (or the ability to perform passive discovery)
- Safety briefing on the physical processes controlled by the OT systems under assessment

## İş Akışı

### Adım 1: Establish Assessment Scope and Safety Boundaries
...
### Adım 2: Perform Passive Network Discovery and Asset Inventory

Dağıt: passive monitoring to map all devices, communication flows, and protocols on the OT network without sending any traffic that could disrupt operations.
...
def analyze_ot_pcap(pcap_file):
    ...
    for pkt in packets:
        ...
        # tespit etmecross-zone communication
...
def generate_assessment_report(results):
    """Generate the OT network assessment Bul:ings report."""
    report = []
    report.append("=" * 70)
    ...
    report.append("\n--- Bul:INGS ---")
    # Check for Level 4 to Level 0-1 direct connections (critical Bul:ing)
...
### Adım 3: Evaluate Firewall Rules Between Purdue Zones
...
def analyze_rules(rules):
    ...
    Bul:ings = {"critical": [], "high": [], "medium": [], "low": [], "info": []}
    ...
    for rule in rules:
        ...
        if zone_pair in PROHIBITED_CONDUITS:
            Bul:ings["critical"].append({
                "rule_id": rule.rule_id,
                "Bul:ing": f"Prohibited direct connection: {rule.source_zone} -> {rule.dest_zone}",
                "detail": f"Rule allows {rule.source_ip} to reach {rule.dest_ip}:{rule.port} ({rule.service})",
                "remediation": "Remove rule. Route traffic through DMZ (Level 3.5) with application-layer Denetle:ion.",
            })

        # Check for overly broad rules (any/any)
        elif rule.source_ip in ("any", "0.0.0.0/0") or rule.dest_ip in ("any", "0.0.0.0/0"):
            Bul:ings["high"].append({
                "rule_id": rule.rule_id,
                "Bul:ing": f"Overly permissive rule with 'any' address",
        ...
        # Check allowed conduits for port violations
            ...
            if port and port not in conduit["allowed_ports"]:
                Bul:ings["medium"].append({
                    "rule_id": rule.rule_id,
                    "Bul:ing": f"Unauthorized port in conduit {zone_pair}",
...
if __name__ == "__main__":
    ...
    Bul:ings = analyze_rules(rules)

    print("=" * 70)
    print("OT ZONE FIREWALL RULE ANALYSIS")
    print("=" * 70)
    for severity in ["critical", "high", "medium", "low"]:
        if Bul:ings[severity]:
            print(f"\n--- {severity.upper()} Bul:INGS ({len(Bul:ings[severity])}) ---")
            for f in Bul:ings[severity]:
                print(f"  [{f['rule_id']}] {f['Bul:ing']}")
                print(f"    Detail: {f['detail']}")
                print(f"    Fix: {f['remediation']}")
```

### Adım 4: Assess Industrial Protocol Security
...
### Adım 5: Şunu üret:ssessment Report

Compile Bul:ings into a structured report aligned with IEC 62443 and NIST SP 800-82 Rev.3.
...
EXECUTIVE SUMMARY:
  ...
  12 critical and 23 high-severity Bul:ings were identified, primarily
...
CRITICAL Bul:INGS:
  ...
  [OT-002] Modbus/TCP write commands without authentication
    ...
    Remediation: Dağıt: Modbus-aware firewall; restrict write-capable sources

  [OT-003] Flat network - no segmentation between Purdue levels
    Detail: All OT devices share VLAN 100 (10.10.0.0/16)
    Impact: Compromised HMI has direct Erişim: all PLCs and SIS
    Remediation: Implement zone-based segmentation per IEC 62443-3-2

RISK MATRIX:
  Critical: 12 Bul:ings (immediate remediation required)
  High:     23 Bul:ings (remediate within 30 days)
  Medium:   15 Bul:ings (remediate within 90 days)
  Low:       8 Bul:ings (remediate in next maintenance cycle)
```
...
- **Nozomi Networks Guardian**: Passive OT network monitoring platform providing asset discovery, vulnerability assessment, and anomaly Tespit for industrial environments
- **Dragos Platform**: OT cybersecurity platform with asset visibility, threat Tespit, and vulnerability management designed for critical infrastructure
...
**Approach**:
1. Dağıt: passive monitoring on the SPAN port to capture a complete communication baseline over 2-4 weeks
...
EXECUTIVE SUMMARY:
  [2-3 sentence overview of Bul:ings and risk level]
