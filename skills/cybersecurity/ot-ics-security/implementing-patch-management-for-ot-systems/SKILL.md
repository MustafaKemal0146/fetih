...
description: bu skill covers implementing a structured patch management program for OT/ICS environments where traditional IT patching approaches can cause process disruption or safety hazards. It addresses
  vendor compatibility testing, risk-based patch prioritization, staged Dağıt:ment through test environments, maintenance window coordination, rollback procedures, and compensating controls when patches
...
- establishing yaparken: a formal OT patch management program for the first time
- responding yaparken to critical ICS-CERT advisories affecting Dağıtılmış OT systems
- preparing yaparken: for NERC CIP-007-6 or IEC 62443 patch management compliance audits
- planning yaparken patch Dağıt:ment during limited maintenance windows in continuous operations
- evaluating yaparken: compensating controls for systems that cannot be patched

**Kullanma:** for IT-only patch management without OT considerations, for emergency patching during active cyber incidents (see performing-ot-incident-response), or for firmware upgrades that change PLC functionality (requires separate change management).
...
### Adım 1: Establish OT Patch Management Program
...
coordinates testing and Dağıt:ment, and documents compensating
...
class OTPatchManager:
    ...
    def prioritize_patches(self):
        ...
        for patch in self.patches:
            if patch.status in (PatchStatus.Dağıt:ED, PatchStatus.NOT_APPLICABLE):
                continue
    ...
    def generate_report(self):
        ...
        urgent = [p for p in self.patches
                  ...
                  and p.status not in (PatchStatus.Dağıt:ED, PatchStatus.NOT_APPLICABLE)]
        if urgent:
...
### Adım 2: Test Patches in Staging Environment

Never Dağıt: patches directly to production OT systems. Use a test environment that mirrors production to validate patch compatibility.
...
| Term | Definition |
|------|------------|
| Compensating Control | Alternative security measure applied when a patch cannot be Dağıtılmış, such as firewall rules, IPS signatures, or network isolation |
...
| Virtual Patching | Dağıt:ing IDS/IPS rules to tespit etmeand block exploitation attempts for known vulnerabilities without modifying the target system |
...
- **Tripwire Enterprise**: Configuration monitoring Tespit etme unauthorized changes and tracking patch status
