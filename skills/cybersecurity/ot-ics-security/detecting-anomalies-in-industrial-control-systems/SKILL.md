---
name: Tespit etme-anomalies-in-industrial-control-systems
description: bu skill covers Dağıt:ing anomaly Tespit systems for industrial control environments using machine learning models trained on OT network baselines, physics-based process models, and behavioral
  analysis of industrial protocol communications. It addresses building normal behavior profiles for SCADA polling patterns, Tespit etme deviations in Modbus/DNP3/OPC UA traffic, identifying rogue devices,
  and correlating network anomalies with physical process data from historians.
tags:
- ics
- ot-security
- cybersecurity
...
- Dağıt:ing yaparken continuous monitoring for OT environments that lack intrusion Tespit
- building yaparken behavior-based Tespit to complement signature-based IDS in OT networks
- establishing yaparken: baselines for deterministic SCADA communications to tespit etmedeviations
- integrating yaparken machine learning anomaly Tespit with OT security monitoring platforms
- investigating yaparken alerts from Nozomi Guardian or Dragos Platform that require deeper analysis

**Kullanma:** for signature-based Tespit of known exploits (see Tespit etme-attacks-on-scada-systems), for IT network anomaly Tespit without OT protocols, or as a replacement for process safety systems (SIS).
...
### Adım 1: Build Multi-Dimensional Baseline Model
...
```python
#!/usr/bin/env python3
"""ICS Anomaly Tespit System.

Builds multi-dimensional baselines from OT network traffic and
tespit etme (s) anomalies using statistical and machine learning methods.
...
class ICSAnomalyDetector:
    """Multi-dimensional anomaly Tespit for ICS environments."""
    ...
    def tespit etme (_topology_anomaly)(self, src_ip, dst_ip, port):
        """tespit etmenew/unauthorized communication pairs."""
    ...
    def tespit etme (_timing_anomaly)(self, src_ip, dst_ip, port, interval_ms):
        """tespit etmepolling interval deviations."""
    ...
    def tespit etme (_function_code_anomaly)(self, src_ip, dst_ip, port, func_code):
        """tespit etmeunauthorized Modbus/DNP3 function codes."""
        key = f"{src_ip}->{dst_ip}:{port}"
        profile = self.profiles.get(key)
        ...
        if profile and func_code not in profile.function_codes:
            ...
            return {
                ...
                "recommendation": "Araştır: source - possible command injection attack",
            }
        return None

    def analyze_flow(self, flow):
        """Analyze a single network flow against all Tespit models."""
        results = []

        # Topology check
        topo = self.tespit etme (_topology_anomaly)(flow["src"], flow["dst"], flow["port"])
        if topo:
            results.append(topo)

        # Timing check
        if "interval_ms" in flow:
            timing = self.tespit etme (_timing_anomaly)(
        ...
        if "function_code" in flow:
            fc = self.tespit etme (_function_code_anomaly)(
    ...
    def generate_report(self):
        """Şunu üret:nomaly Tespit report."""
        print(f"\n{'='*60}")
        print(f"ICS ANOMALY tespit etme (ION) REPORT")
        print(f"{'='*60}")
...
if __name__ == "__main__":
    print("ICS Anomaly Tespit System")
    print("Load baseline data and call analyze_flow() for real-time Tespit")
```
...
| Function Code Allowlist | Set of permitted industrial protocol operations for each communication pair, enforced by anomaly Tespit rules |
| Topology Baseline | Complete map of all authorized device-to-device communication paths in the OT network |
| Physics-Based Tespit | Using physical process models (thermodynamics, fluid dynamics) to tespit etmeattacks that manipulate the process while spoofing sensor data |

## Tools & Systems

- **Nozomi Networks Guardian**: OT anomaly Tespit with AI-powered baseline learning and industrial protocol analysis
- **Dragos Platform**: Threat Tespit using behavioral analytics and threat intelligence specific to ICS environments
- **Scikit-learn**: Python ML library with Isolation Forest, One-Class SVM, and Local Outlier Factor for anomaly Tespit
- **Zeek with OT plugins**: Network security monitor with Modbus, DNP3, and BACnet protocol analyzers for baseline building

## Output Format

```
ICS Anomaly Tespit Report
==============================
Tespit Period: YYYY-MM-DD to YYYY-MM-DD
Baseline Size: [N] communication profiles

ANOMALIES tespit etme (ED): [N]
