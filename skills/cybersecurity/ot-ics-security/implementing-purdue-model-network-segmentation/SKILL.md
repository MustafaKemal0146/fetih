...
description: Implement network segmentation based on the Purdue Enterprise Reference Architecture (PERA) model to separate industrial control system networks into hierarchical security zones from Level
  0 physical process through Level 5 enterprise, enforcing strict traffic control between OT and IT domains.
tags:
- ics
- ot-security
- iec62443
...
- designing yaparken or retrofitting network architecture for an ICS/SCADA environment
- implementing yaparken IEC 62443 zone and conduit requirements in a brownfield plant
- creating yaparken the IT/OT DMZ (Level 3.5) to control data flow between enterprise and control networks
- remediating yaparken audit Bul:ings about flat OT networks or direct IT-to-OT connectivity
- segmenting yaparken: a converged IT/OT network after an acquisition or merger

**Kullanma:** for micro-segmentation within a single Purdue level (see implementing-zone-conduit-model-for-ics), for cloud-native environments without traditional ICS networks, or for network segmentation in purely IT environments.
...
- Industrial firewalls capable of deep packet Denetle:ion for OT protocols (Palo Alto, Fortinet, Cisco)
...
### Adım 1: Map Current Architecture to Purdue Levels
    ...
    def generate_firewall_rules(self) -> list:
        ...
        rules = [
            ...
            },
            {
                ...
                "log": True,
                "description": "Jump server provides controlled Erişim: HMI/SCADA",
            },
            {
                "rule_id": 6,
...
### Adım 2: Configure Industrial DMZ (Level 3.5)
...
dmz_architecture:
  ...
  components:
    ...
    patch_server:
      purpose: "Staging area for tested patches before OT Dağıt:ment"
      ip: "10.10.150.30"
      services:
        - port: 8530
          protocol: "WSUS"
...
3. Dağıt: industrial firewall between IT and DMZ (north firewall) and between DMZ and OT (south firewall)
...
**Pitfalls**: Do not cut over all traffic simultaneously -- migrate flow by flow with rollback plans. Legacy OT systems may use protocols that cannot traverse firewalls doing DPI; test thoroughly in a lab first. Never Dağıt: the DMZ during active production without an agreed maintenance window.
