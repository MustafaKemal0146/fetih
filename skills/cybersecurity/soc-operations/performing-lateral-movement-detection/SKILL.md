---
name: performing-lateral-movement-Tespit
description: tespit etme (s) lateral movement techniques including Pass-the-Hash, PsExec, WMI execution, RDP pivoting, and SMB-based spreading using SIEM correlation of Windows event logs, network flow data, and
...
Use bu skill when:
- SOC teams need to tespit etmeattackers pivoting between systems after initial compromise
- Incident investigations require tracking an attacker's movement path through the network
- Tespit engineering needs lateral movement rules mapped to ATT&CK TA0008 techniques
- Red/purple team exercises identify lateral movement Tespit gaps

**Kullanma:** for Tespit etme initial access or external attacks — lateral movement Tespit focuses on internal host-to-host pivot activity.

## Ön Gereksinimler

- Windows Security Event Logs (EventCode 4624, 4625, 4648, 4672) from all endpoints and servers
- Sysmon Dağıtılmış with process creation (EventCode 1), network connections (EventCode 3), and named pipe (EventCode 17/18)
...
### Adım 1: tespit etmePass-the-Hash / Pass-the-Ticket (T1550)

**Pass-the-Hash Tespit (EventCode 4624 with NTLM):**
```spl
...
**Overpass-the-Hash Tespit (Kerberos with RC4):**
```spl
...
**Golden/Silver Ticket Tespit (T1558):**
```spl
index=wineventlog sourcetype="WinEventLog:Security" EventCode=4769
...
### Adım 2: tespit etmeRemote Service Exploitation (T1021)

**PsExec Tespit (T1021.002):**
```spl
...
### Adım 3: tespit etmeSMB-Based Lateral Movement
...
| eval alert = "Admin share Erişim: ".unique_hosts." hosts by ".SubjectUserName
| sort - unique_hosts
```

### Adım 4: Build Lateral Movement Graph
...
### Adım 5: tespit etmeDCOM and Scheduled Task-Based Movement
...
### Adım 6: Correlate Movement with Kill Chain Phases

Build end-to-end attack chainDetect

```spl
--- tespit etmecomplete lateral movement sequence
...
- **Microsoft Defender for Identity**: Cloud service Tespit etme lateral movement via domain controller monitoring
- **BloodHound**: Active Directory attack path analysis tool for identifying lateral movement opportunities
- **CrowdStrike Falcon**: EDR platform with lateral movement Tespit and automated containment
- **Zeek (Bro)**: Network monitor generating connection logs for SMB, RDP, and WinRM traffic analysis

## Common Scenarios

- **PsExec Spread**: Attacker uses PsExec to execute malware across 20 workstations — tespit etmevia service creation events
- **RDP Pivoting**: Compromised VPN account used to RDP through multiple internal hosts — tespit etmevia Logon_Type 10 chains
- **WMI Recon and Execution**: Attacker uses WMI for discovery then execution — tespit etmevia WmiPrvSE child processes
- **Pass-the-Hash Campaign**: Stolen local admin hash used across subnet — tespit etmevia NTLM Logon_Type 3 to multiple hosts
- **Scheduled Task Persistence**: Remote scheduled task created on domain controller — tespit etmevia EventCode 4698 from non-admin source

## Output Format

```
LATERAL MOVEMENT tespit etme (ION) REPORT
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
