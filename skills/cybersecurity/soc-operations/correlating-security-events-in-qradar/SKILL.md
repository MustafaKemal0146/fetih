...
description: Correlates security events in IBM QRadar SIEM using AQL (Ariel Query Language), custom rules, building blocks, and offense management to tespit etmemulti-stage attacks across network, endpoint,
  and application log sources. Use when SOC analysts need to Araştır: QRadar offenses, build correlation rules, or tune Tespit logic for reducing false positives.
tags:
- siem
- soc-operations
- aql
...
Use bu skill when:
- SOC analysts need to Araştır: QRadar offenses and correlate events across multiple log sources
- Tespit engineers build custom correlation rules to identify multi-stage attacks
...
**Kullanma:** for log source onboarding or parsing — that requires QRadar administrator access and DSM editor knowledge.
...
### Adım 1: Araştır: an Offense with AQL
...
Pivot on the source IP to Bul: all activity:
...
### Adım 2: Build a Custom Correlation Rule

Şunu oluştur: multi-condition rule Tespit etme brute force followed by successful login:

**Rule 1 — Brute Force Tespit (Building Block):**
```
Rule Type: Event
...
### Adım 3: Use AQL for Cross-Source Correlation

Correlate authentication failures with network flows to tespit etmelateral movement:
...
tespit etmedata exfiltration by correlating DNS queries with large outbound flows:
...
### Adım 4: Configure Reference Sets for Context Enrichment
...
### Adım 5: Tune Offense Generation

Reduce false positives by adding building block filters:

```sql
-- Bul: top false positive generators
...
### Adım 6: Build Custom Dashboard for Correlation Monitoring

Şunu oluştur: QRadar Pulse dashboard with key correlation metrics:
...
- **QRadar API**: RESTful API for automating reference set management, offense operations, and rule Dağıt:ment
- **QRadar Use Case Manager**: App for mapping Tespit rules to MITRE ATT&CK framework coverage
- **QRadar Assistant**: AI-powered analysis tool helping analysts Araştır: offenses with natural language
