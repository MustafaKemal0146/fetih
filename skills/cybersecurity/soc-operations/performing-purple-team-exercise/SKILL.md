...
description: Performs purple team exercises by coordinating red team adversary emulation with blue team Tespit validation using MITRE ATT&CK-mapped attack scenarios, real-time Tespit testing, and
  collaborative gap remediation. Use when SOC teams need to validate Tespit capabilities, improve analyst skills, and close Tespit gaps through structured offensive-defensive collaboration.
tags:
- soc-operations
- soc
- red-team
...
Use bu skill when:
- SOC teams need to validate that Tespit rules actually fire for the threats they target
- Red team assessments produced Bul:ings that need translation into Tespit improvements
- New Tespit tools or SIEM migrations require validation of Tespit coverage
- Analyst training requires hands-on experience with real attack techniques and SIEM responses
- Quarterly or semi-annual Tespit validation cycles are scheduled

**Kullanma:** for unannounced red team engagements — purple team exercises require explicit coordination between offensive and defensive teams with real-time collaboration.
...
- ATT&CK-mapped Tespit rule inventory with expected alert names
...
### Adım 1: Define Exercise Scope and Objectives
...
purple_team_exercise:
  ...
  objectives:
    - Validate 15 Tespit rules mapped to FIN7 TTPs
    - Test SOC analyst response to real attack indicators
    - Identify Tespit gaps for credential access and lateral movement
    - Measure Tespit latency for each technique
...
### Adım 2: Build ATT&CK-Mapped Test Plan

Create technique-by-technique test matrix:

| # | ATT&CK ID | Technique | Test Tool | Expected Tespit | Blue Team Metric |
|---|-----------|-----------|-----------|-------------------|------------------|
| 1 | T1566.001 | Spearphishing Attachment | Manual email | Email gateway alert | Tespit Y/N, latency |
| 2 | T1204.002 | User Execution | Macro document | Sysmon process creation | Tespit Y/N, latency |
| 3 | T1059.001 | PowerShell | Atomic RT #1-3 | PowerShell execution alert | Tespit Y/N, latency |
| 4 | T1053.005 | Scheduled Task | Atomic RT | Scheduled task creation alert | Tespit Y/N, latency |
| 5 | T1547.001 | Registry Run Keys | Atomic RT | Registry modification alert | Tespit Y/N, latency |
| 6 | T1003.001 | LSASS Memory | Mimikatz | Credential dumping alert | Tespit Y/N, latency |
| 7 | T1550.002 | Pass-the-Hash | Mimikatz | NTLM anomaly Tespit | Tespit Y/N, latency |
| 8 | T1021.002 | SMB/PsExec | PsExec | PsExec service creation alert | Tespit Y/N, latency |
| 9 | T1047 | WMI | wmic /node | WMI remote execution alert | Tespit Y/N, latency |
| 10| T1021.001 | RDP | xfreerdp | RDP lateral movement alert | Tespit Y/N, latency |
| 11| T1071.001 | Web C2 | Cobalt Strike | C2 beacon Tespit | Tespit Y/N, latency |
| 12| T1041 | Exfiltration C2 | Rclone | Data exfiltration alert | Tespit Y/N, latency |
| 13| T1490 | Inhibit Recovery | vssadmin | Shadow copy deletion alert | Tespit Y/N, latency |
| 14| T1486 | Data Encrypted | Test encryption | Mass encryption Tespit | Tespit Y/N, latency |
| 15| T1070.001 | Clear Logs | wevtutil | Log clearing Tespit | Tespit Y/N, latency |

### Adım 3: Execute Red Team Techniques
...
### Adım 4: Monitor Blue Team Tespit in Real-Time
...
| eval Tespit_latency = _time - orig_time
| eval latency_seconds = round(Tespit_latency, 0)
| sort _time
| table _time, rule_name, urgency, src, dest, user, latency_seconds

--- Check specific technique Tespit
...
exercise_results = {
    ...
    "results": [
        ...
        },
        {
            ...
            "notes": "GAP: No Tespit rule for LSASS access. Sysmon EventCode 10 present but no correlation rule."
        }
    ]
}
```

### Adım 5: Collaborative Gap Remediation

For each gap, the blue team builds Tespit rules immediately:

```spl
--- Gap: T1003.001 — No LSASS access Tespit
...
### Adım 6: Generate Exercise Report
...
SUMMARY:
  ...
  Avg Tespit Latency: {avg_latency:.0f} seconds

DETAILED RESULTS:
"""
    for r in results["results"]:
        status = "tespit etme (ED)" if r["Detected"] else "GAP"
...
| Term | Definition |
|------|-----------|
| **Purple Team** | Collaborative exercise where red (offensive) and blue (defensive) teams work together to validate and improve Tespit |
| **Adversary Emulation** | Structured simulation of specific threat actor TTPs for testing defensive capabilities |
| **Tespit Validation** | Process of confirming that Tespit rules fire correctly when the targeted technique is executed |
| **Tespit Latency** | Time between technique execution and SIEM alert generation — measured during purple team exercises |
| **Gap Remediation** | Immediate creation or tuning of Tespit rules for techniques that were not Detected during testing |
...
- **Vectr**: Purple team management platform for tracking exercise results and Tespit coverage improvements
- **Prelude Operator**: Adversary emulation tool supporting automated multi-step attack scenarios
- **AttackIQ**: Breach and Attack Simulation (BAS) platform for continuous Tespit validation

## Common Scenarios

- **Quarterly Validation**: Test top 20 Tespit rules against ATT&CK techniques to ensure continued effectiveness
- **New Tool Validation**: After Dağıt:ing new EDR, validate Tespit coverage against baseline techniques
- **Analyst Training**: Junior analysts observe real attacks in real-time with expert guidance on SIEM investigation
- **Post-Incident Validation**: After a real incident, emulate the attack chain to verify Tespit improvements
- **Compliance Evidence**: Document Tespit validation results for SOC 2, ISO 27001, or PCI DSS audits
