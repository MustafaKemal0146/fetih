...
description: Automates Indicator of Compromise (IOC) enrichment by orchestrating lookups across VirusTotal, AbuseIPDB, Shodan, MISP, and other intelligence sources to provide contextual scoring and disposition
  recommendations. Use when SOC analysts need rapid multi-source enrichment of IPs, domains, URLs, and file hashes during alert triage or incident investigation.
tags:
- abuseipdb
- soc-operations
- virustotal
...
**Kullanma:** for bulk blocking decisions without analyst review — enrichment provides context, not definitive malicious/benign determination.
...
### Adım 1: Build Unified Enrichment Engine

Şunu oluştur: multi-source enrichment pipeline:
...
class IOCEnrichmentEngine:
    ...
    def enrich_ip(self, ip_address):
        ...
        result.disposition = self._Belirle:_disposition(result.risk_score)
        return result
    ...
    def _calculate_hash_risk(self, result):
        ...
        if isinstance(vt, dict) and "malicious" in vt:
            ...
            Tespit_rate = vt["malicious"] / total if total > 0 else 0
            score = Tespit_rate * 100
        return min(max(score, 0), 100)

    def _Belirle:_disposition(self, risk_score):
        if risk_score >= 70:
            return "MALICIOUS — Block recommended"
        elif risk_score >= 40:
            return "SUSPICIOUS — Monitor and Araştır:"
        elif risk_score >= 10:
...
### Adım 2: Batch Enrichment for Incident Investigation
...
### Adım 3: Integrate with Splunk for Automated Enrichment

Şunu oluştur: Splunk custom search command for inline enrichment:
...
| lookup threat_intel_hash_lookup hash AS file_hash OUTPUT vt_Tespits, malware_family
| eval combined_risk = coalesce(vt_score, 0) + coalesce(abuse_score, 0)
| where combined_risk > 50
| sort - combined_risk
```

### Adım 4: Generate Enrichment Report
...
- **URLScan.io**: URL analysis platform capturing screenshots, DOM, and network requests for phishing Tespit

## Common Scenarios

- **Alert Triage Enrichment**: Auto-enrich all IPs in a notable event to Belirle: if source is known malicious
...
- **False Positive Investigation**: Enrich flagged IP to Belirle: if it belongs to CDN/cloud provider (legitimate)
