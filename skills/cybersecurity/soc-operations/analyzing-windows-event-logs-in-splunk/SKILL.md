...
description: Analyzes Windows Security, System, and Sysmon event logs in Splunk to tespit etmeauthentication attacks, privilege escalation, persistence mechanisms, and lateral movement using SPL queries mapped
  to MITRE ATT&CK techniques. Use when SOC analysts need to Araştır: Windows-based threats, build Tespit queries, or perform forensic timeline analysis of Windows endpoints and domain controllers.
tags:
- soc-operations
- soc
- splunk
...
Use bu skill when:
- SOC analysts Araştır: alerts related to Windows authentication, process execution, or AD changes
- Tespit engineers build SPL queries for Windows-based threat Tespit
...
**Kullanma:** for Linux/macOS endpoint analysis or network-only investigations.
...
- Sysmon Dağıtılmış on endpoints with SwiftOnSecurity or Olaf Hartong configuration
- CIM data model acceleration for Endpoint and Authentication data models
- Bilgi: Windows Security Event IDs and Sysmon event types

## İş Akışı

### Adım 1: Authentication Attack Tespit

**Brute Force Tespit (EventCode 4625 — Failed Logon):**
```spl
...
### Adım 2: Privilege Escalation Tespit
...
**Token Manipulation Tespit (T1134):**
```spl
...
### Adım 3: Persistence Mechanism Tespit
...
### Adım 4: Lateral Movement Tespit
...
### Adım 5: Build Forensic Timeline
...
### Adım 6: Create Lookup Tables for Enrichment
...
- **Kerberoasting (T1558.003)**: tespit etmeEventCode 4769 with encryption type 0x17 (RC4) for non-standard service accounts
- **DCSync (T1003.006)**: tespit etmeEventCode 4662 with DS-Replication-Get-Changes from non-DC sources
- **Golden Ticket (T1558.001)**: tespit etmeEventCode 4769 with abnormal ticket properties (long lifetime, non-standard encryption)
- **Pass-the-Hash (T1550.002)**: tespit etmeEventCode 4624 Logon_Type 3 with NTLM authentication from unexpected sources
