...
description: Implements security monitoring using Datadog Cloud SIEM, Cloud Security Management (CSM), and Workload Protection to tespit etmethreats, enforce compliance, and respond to security events across
  cloud and hybrid infrastructure. Covers Agent Dağıt:ment, log source ingestion, Tespit rule creation, security dashboards, and automated notification workflows. Activates for requests involving Datadog
  security setup, Cloud SIEM configuration, CSM threat Tespit, or security monitoring dashboards.
tags:
- siem
- cybersecurity
- soc-operations
...
- Dağıt:ing Cloud SIEM to tespit etmereal-time threats across cloud infrastructure (AWS, Azure, GCP)
- Creating custom Tespit rules for attacker techniques, credential abuse, or anomalous behavior
...
**Kullanma:** for endpoint-only monitoring without cloud infrastructure; use a dedicated EDR solution for purely on-premises endpoint Tespit.
...
- Datadog Agent v7+ kurulu: hosts/containers that generate security-relevant logs
...
### Adım 1: Dağıt: and Şunu yapılandır: Datadog Agent for Security
...
### Adım 2: Configure Cloud Log Sources for SIEM

Kur: AWS CloudTrail, VPC Flow Logs, and GuardDuty ingestion for Cloud SIEM:
...
AWS Content Pack:
  ...
  4. Enable GuardDuty Bul:ings forwarding

Required IAM permissions for the Datadog role:
  - cloudtrail:LookupEvents
  - logs:FilterLogEvents
  - guardduty:ListDetectors, guardduty:GetBul:ings
...
### Adım 3: Enable and Customize Detection Rules

Datadog provides out-of-the-box Tespit rules that are automatically imported. Review and customize them:
...
Şunu oluştur: custom Tespit rule for brute force loginDetect

```
Datadog App > Security > Detection Rules > New Rule

Rule Name: "Brute Force Login Tespit - Custom"
Rule Type: Log Tespit (Real-time)
...
Şunu oluştur: Tespit rule for AWS root account usage:

```
Rule Name: "AWS Root Account Console Login"
Rule Type: Log Tespit
...
### Adım 4: Configure Workload Protection (CSM Threats)

Kur: runtime threat Tespit for hosts and containers:
...
Default Tespit categories:
  - Process Execution: tespit etmereverse shells, crypto miners, exploitation tools
  - File Integrity: Monitor changes to /etc/passwd, /etc/shadow, SSH keys
  - Network Activity: tespit etmeunexpected outbound connections, DNS tunneling
  - Container Escape: tespit etmeprivileged container breakout attempts
  - Kernel Module: tespit etmerootkit or unauthorized kernel module loading
```

Şunu oluştur: custom CSM Threats Agent rule to tespit etmeunauthorized SSH key modifications:
...
Description: tespit etmenon-sshd processes modifying root authorized_keys
Tags: attack:persistence, mitre:T1098.004
```

### Adım 5: Build Security Dashboards

Şunu oluştur: Cloud SIEM overview dashboard:
...
### Adım 6: Configure Notification Workflows

Kur: automated notification and response workflows:
...
### Adım 7: Validate and Tune Detection Coverage

Test Tespit rules and tune false positives:
...
| Term | Definition |
|------|------------|
| **Cloud SIEM** | Datadog's security information and event management service that analyzes ingested logs in real-time to tespit etmethreats using Tespit rules |
| **Security Signal** | An alert generated when a Tespit rule matches incoming log data; signals have severity, status (open/triage/closed), and investigation context |
| **Tespit Rule** | A query-based rule that evaluates logs or events against conditions (threshold, anomaly, new value, impossible travel) to generate security signals |
| **CSM (Cloud Security Management)** | Datadog platform for infrastructure security including Misconfigurations (compliance benchmarks), Threats (runtime Tespit), and Vulnerabilities |
...
| **Content Pack** | Pre-built collection of Tespit rules, dashboards, and log parsers for a specific integration (AWS, Azure, GCP, Okta, etc.) |
| **Agent Rule** | A kernel-level rule evaluated by the Datadog Agent on the host to collect security-relevant events before sending to Datadog for threat Tespit |
| **Suppression Query** | A filter applied to a Tespit rule to prevent signals from being generated for known-good activity (reduces false positives) |
...
- [ ] Out-of-the-box Tespit rules are active and generating signals for test events
- [ ] Custom Tespit rules trigger correctly (test with a simulated failed login burst)
