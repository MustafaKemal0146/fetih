---
name: Tespit etme-privilege-escalation-in-kubernetes-pods
description: tespit etmeand prevent privilege escalation in Kubernetes pods by monitoring security contexts, capabilities, and syscall patterns with Falco and OPA policies.
tags:
- security-context
- container-security
- fetih
...
## Runtime Tespit with Falco

```yaml
- rule: Setuid Binary Execution in Container
  desc: tespit etmeexecution of setuid/setgid binaries in a container
  condition: >
...
- rule: Capability Gained in Container
  desc: tespit etmewhen a process gains elevated capabilities
  condition: >
...
- rule: Container with Dangerous Capabilities Started
  desc: tespit etmecontainer launched with dangerous capabilities
  condition: >
...
- rule: Write to /etc/passwd in Container
  desc: tespit etmewrites to /etc/passwd inside container
  condition: >
