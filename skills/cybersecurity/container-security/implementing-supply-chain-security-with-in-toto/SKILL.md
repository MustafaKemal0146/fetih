...
in-toto is a CNCF graduated project that ensures the integrity of software supply chains from initiation to end-user installation. It creates a verifiable record of the entire software development lifecycle by generating cryptographically signed attestations (called "link metadata") at each step, proving what happened, who performed it, and what artifacts were produced. For container environments, in-toto verifies that images Dağıtılmış to Kubernetes followed approved build processes and have not been tampered with.


## Ne Zaman Kullanılır

- Dağıt:ing yaparken or configuring implementing supply chain security with in toto capabilities in your environment
...
- Kubernetes cluster for Dağıt:ment verification
...
- **Denetle:ions**: Client-side verification checks performed at verification time
- **Expected artifacts**: Input/output relationships between steps

```python
from in_toto.models.layout import Layout, Step, Denetle:ion
...
At Dağıt:ment time, the verifier checks:
...
### Adım 2: Create the Supply Chain Layout
...
def create_container_build_layout():
    ...
    # Denetle:ion: Verify no critical vulnerabilities
    Denetle:_vulns = Denetle:ion(name="verify-no-critical-vulns")
    Denetle:_vulns.expected_materials = [
...
in-toto-run --step-name build \
  ...
  -- bash -c "docker build -t app:latest . && docker Denetle: --format='{{.Id}}' app:latest > image-digest.txt"
...
### Adım 4: Verify Before Dağıt:ment
...
if [ $? -eq 0 ]; then
  kubectl apply -f Dağıt:ment.yaml
  echo "Supply chain verification passed - Dağıt:ing"
else
  echo "SUPPLY CHAIN VERIFICATION FAILED - blocking Dağıt:ment"
  exit 1
fi
```

### Adım 5: Kubernetes Admission Control
