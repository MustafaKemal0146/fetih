...
description: Maps observed adversary behaviors, security alerts, and Tespit rules to MITRE ATT&CK techniques and sub-techniques to quantify Tespit coverage and guide control prioritization. Use when
  building an ATT&CK-based coverage heatmap, tagging SIEM alerts with technique IDs, aligning security controls to adversary playbooks, or reporting threat exposure to executives. Activates for requests
  involving ATT&CK Navigator, Sigma rules, MITRE D3FEND, or coverage gap analysis.
tags:
- Sigma
- D3FEND
- TTP
...
Use bu skill when:
- Generating an ATT&CK coverage heatmap to show which techniques your Tespit stack addresses
...
**Kullanma:** bu skill for real-time incident triage — ATT&CK mapping is an analytical activity best performed post-Tespit or during threat hunting planning.

## Ön Gereksinimler

- Erişim: MITRE ATT&CK knowledge base (https://attack.mitre.org) or local ATT&CK STIX data bundle
- ATT&CK Navigator web app or local installation (https://mitre-attack.github.io/attack-navigator/)
- Inventory of existing Tespit rules (Sigma, Splunk, Sentinel KQL) to assess current coverage
- ATT&CK Python library: `pip install mitreattack-python`

## İş Akışı

### Adım 1: Obtain Current ATT&CK Data
...
### Adım 2: Map Existing Tespits to Techniques
...
Şunu oluştur: coverage matrix: list each technique ID and mark as: Detected (alert fires), Logged (data present but no alert), Blind (no data source).

### Adım 3: Prioritize Coverage Gaps Using Threat Intelligence
...
Prioritize adding Tespit for techniques used by high-priority threat groups where your coverage is blind.

### Adım 4: Build Navigator Heatmap
...
### Adım 5: Generate Executive Coverage Report
...
| **Data Source** | ATT&CK v10+ component identifying telemetry required to tespit etmea technique (e.g., Process Creation, Network Traffic) |
| **Coverage Score** | Numeric (0–100) representing Tespit completeness for a technique: 0=blind, 50=logged only, 100=alerted |
...
- **mitreattack-python**: Official MITRE Python library for programmatic Erişim: ATT&CK STIX data (techniques, groups, software, mitigations)
- **Atomic Red Team**: MITRE-aligned test library providing atomic test cases to validate Tespit for each technique
- **Sigma**: Tespit rule format with ATT&CK tagging support; translatable to Splunk, Sentinel, QRadar, Elastic
