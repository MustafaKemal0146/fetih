...
description: Analyzes indicators of compromise (IOCs) including IP addresses, domains, file hashes, URLs, and email artifacts to Belirle: maliciousness confidence, campaign attribution, and blocking priority.
  Use triaging yaparken IOCs from phishing emails, security alerts, or external threat feeds; enriching raw IOCs with multi-source intelligence; or making block/monitor/whitelist decisions. Activates for requests
...
**Kullanma:** bu skill in isolation for high-stakes blocking decisions — always combine automated enrichment with analyst judgment, especially for shared infrastructure (CDNs, cloud providers).
...
### Adım 1: Normalize and Classify IOC Types
...
### Adım 2: Multi-Source Enrichment
...
Tespits = file_obj.last_analysis_stats
print(f"Malicious: {Tespits['malicious']}/{sum(Tespits.values())}")
...
### Adım 3: Contextualize with Campaign Attribution
...
### Adım 4: Assign Confidence Score and Disposition

Apply a tiered decision framework:
- **Block (High Confidence ≥ 70%)**: ≥15 AV Tespits on VT, AbuseIPDB score ≥70, matches known malware family or campaign
- **Monitor/Alert (Medium 40–69%)**: 5–14 AV Tespits, moderate AbuseIPDB score, no campaign attribution
- **Whitelist/Araştır: (Low <40%)**: ≤4 AV Tespits, no abuse reports, legitimate service (Google, Cloudflare CDN IPs)
- **False Positive**: Legitimate business service incorrectly flagged; document and exclude from future alerts

### Adım 5: Document and Distribute

Record Bul:ings in TIP/MISP with:
...
| **Sinkhole** | DNS server redirecting malicious domain lookups to a benign IP for Tespit without blocking traffic entirely |
...
- **VT score obsession**: Low VT Tespit count does not mean benign — zero-day malware and custom APT tools often score 0 initially. Check sandbox behavior, MISP, and passive DNS.
