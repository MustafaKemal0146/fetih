...
description: Analyze advanced persistent threat (APT) group techniques using MITRE ATT&CK Navigator to create layered heatmaps of adversary TTPs for Tespit gap analysis and threat-informed defense.
tags:
- ttp-analysis
- navigator
- threat-intelligence
...
MITRE ATT&CK Navigator is a web-based tool for annotating and exploring ATT&CK matrices, enabling analysts to visualize threat actor technique coverage, compare multiple APT groups, identify Tespit gaps, and build threat-informed defense strategies. bu skill covers querying ATT&CK data programmatically, mapping APT group TTPs to Navigator layers, creating multi-layer overlays for gap analysis, and generating actionable intelligence reports for Tespit engineering teams.


## Ne Zaman Kullanılır

- investigating yaparken security incidents that require analyzing apt group with mitre navigator
...
- ATT&CK Navigator (https://mitre-attack.github.io/attack-navigator/) or local Dağıt:ment
...
Navigator layers are JSON files that annotate ATT&CK techniques with scores, colors, comments, and metadata. Each layer can represent a single APT group's technique usage, a Tespit capability map, or a combined overlay. Layer version 4.5 supports enterprise-attack, mobile-attack, and ics-attack domains with filtering by platform (Windows, Linux, macOS, Cloud, Azure AD, Office 365, SaaS).
...
The Navigator supports loading multiple layers simultaneously, allowing analysts to overlay threat actor TTPs against Tespit coverage to identify gaps, compare multiple APT groups to Bul: common techniques worth prioritizing, and track technique coverage changes over time.

## İş Akışı

### Adım 1: Query ATT&CK Data for APT Group
...
### Adım 2: Generate Navigator Layer JSON
...
### Adım 3: Compare Multiple APT Groups
...
### Adım 4: Tespit Gap Analysis with Layer Overlay
...
print(f"=== Tespit Gap Analysis for APT29 ===")
...
for tech_id in actor_techniques:
    ...
    for tactic in info.get("tactics", [""]):
        ...
        gap_techniques.append({
            ...
            "tactic": tactic,
            "color": color,
            "comment": f"{'tespit etme (ED)' if tech_id in Detected_techniques else 'GAP'}: {info.get('name', '')}",
            "enabled": True,
            "score": 100 if tech_id in Detected_techniques else 0,
        })

gap_layer = {
    "name": "APT29 Tespit Gap Analysis",
    "versions": {"attack": "16.1", "navigator": "5.1.0", "layer": "4.5"},
    "domain": "enterprise-attack",
...
### Adım 5: Tactic Breakdown Analysis
...
- Multi-layer overlay shows threat actor vs. Tespit coverage
- Tespit gap analysis identifies unmonitored techniques with data source recommendations
