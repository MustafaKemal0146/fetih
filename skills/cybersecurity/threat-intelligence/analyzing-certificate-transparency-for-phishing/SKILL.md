...
description: Monitor Certificate Transparency logs using crt.sh and Certstream to tespit etmephishing domains, lookalike certificates, and unauthorized certificate issuance targeting your organization.
tags:
...
Certificate Transparency (CT) is an Internet security standard that creates a public, append-only log of all issued SSL/TLS certificates. Monitoring CT logs enables early Tespit of phishing domains that register certificates mimicking legitimate brands, unauthorized certificate issuance for owned domains, and certificate-based attack infrastructure. bu skill covers querying CT logs via crt.sh, real-time monitoring with Certstream, building automated alerting for suspicious certificates, and integrating Bul:ings into threat intelligence workflows.


## Ne Zaman Kullanılır

- investigating yaparken security incidents that require analyzing certificate transparency for phishing
...
- Erişim: crt.sh (https://crt.sh/) for historical CT log queries
...
### Phishing Tespit via CT

Attackers register lookalike domains and obtain free certificates (often from Let's Encrypt) to make phishing sites appear legitimate with HTTPS. CT monitoring tespit etme (s) these early because the certificate appears in logs before the phishing campaign launches, providing a window for proactive blocking.
...
### Adım 1: Query crt.sh for Certificate History
...
class CTLogMonitor:
    ...
    def Bul:_suspicious_certs(self, domain):
        """Bul: certificates that may be phishing attempts."""
...
suspicious = monitor.Bul:_suspicious_certs("mycompany.com")
for cert in suspicious[:5]:
    print(f"  [{cert['common_name']}] Flags: {cert['flags']}")
```

### Adım 2: Real-Time Monitoring with Certstream
...
class CertstreamMonitor:
    ...
    def _is_suspicious(self, domain):
        ...
        for watched in self.watched_domains:
            ...
            # Levenshtein distance (typosquatting Tespit)
    ...
    def _get_reason(self, domain):
        """Belirle: why domain was flagged."""
        reasons = []
        for watched in self.watched_domains:
            watched_base = watched.split(".")[0]
...
### Adım 3: Enumerate Subdomains from CT Logs
...
### Adım 4: Generate CT Intelligence Report
...
5. Dağıt: DMARC to prevent email spoofing from lookalike domains
"""
...
- Certstream real-time monitoring tespit etme (s) new phishing certificates
...
- [River Security: CT Logs for Attack Surface Discovery](https://riversecurity.eu/Bul:ing-attack-surface-and-fraudulent-domains-via-certificate-transparency-logs/)
