...
description: Develops comprehensive threat actor profiles for APT groups, criminal organizations, and hacktivist collectives by aggregating TTP documentation, historical campaign data, tooling fingerprints,
  and attribution indicators from multiple intelligence sources. Use when briefing executives on sector-specific threats, updating threat model assumptions, or prioritizing defensive controls against specific
  adversaries. Activates for requests involving MITRE ATT&CK Groups, Mandiant APT profiles, CrowdS...
tags:
- attribution
- NIST-CSF
- siber-güvenlik
...
- Enabling SOC analysts to understand attacker objectives and TTPs to improve Tespit tuning

**Kullanma:** bu skill for real-time incident attribution — attribution during active incidents should be deprioritized in favor of containment. Profile refinement occurs post-incident.

## Ön Gereksinimler

- Erişim: MITRE ATT&CK Groups database (https://attack.mitre.org/groups/)
...
### Adım 4: Assess Detection Coverage Against Profile

Compare the adversary's technique list against your Tespit coverage matrix (from ATT&CK Navigator layer). Identify:
- Techniques used by this group where you have no Tespit (critical gaps)
- Techniques where you have partial coverage (logging but no alerting)
- Compensating controls where Tespit is not feasible (network segmentation as mitigation for lateral movement)

### Adım 5: Package Profile for Distribution
...
- **SOC analyst brief** (3–5 pages): Full TTP list with Tespit status, IOC list, hunt hypotheses
- **Technical appendix**: YARA rules, Sigma Tespits, STIX JSON object for TIP import
