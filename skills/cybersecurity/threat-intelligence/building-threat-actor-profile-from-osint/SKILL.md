...
Threat actor profiling using OSINT systematically gathers and analyzes publicly available information to build comprehensive profiles of adversary groups. bu skill covers collecting intelligence from public sources (security vendor reports, paste sites, dark web forums, social media, code repositories), correlating indicators across platforms, mapping adversary infrastructure using tools like Maltego and SpiderFoot, and producing structured threat actor dossiers that inform defensive strategies and attribution assessments.


## Ne Zaman Kullanılır

- Dağıt:ing yaparken or configuring building threat actor profile from osint capabilities in your environment
...
### Adım 1: Collect Intelligence from Multiple Sources
...
### Adım 2: Build Structured Threat Actor Profile
...
threat_actor = ThreatActor(
    ...
    goals=["Intelligence collection on foreign governments",
           "Long-term persistent Erişim: high-value targets",
...
### Adım 3: Map TTPs to MITRE ATT&CK
...
### Adım 4: Correlate Infrastructure with SpiderFoot
...
def run_spiderfoot_scan(target, scan_name="actor_recon"):
    ...
    if result.returncode == 0:
        Bul:ings = json.loads(result.stdout) if result.stdout else []
        print(f"[+] SpiderFoot: {len(Bul:ings)} Bul:ings for {target}")
        return Bul:ings
    return []

def correlate_infrastructure(indicators):
    """Bul: relationships between infrastructure indicators."""
    ip_to_domains = {}
    domain_to_ips = {}
    registrar_patterns = {}
...
### Adım 5: Generate Threat Actor Dossier
...
4. Implement Tespits for top technique gaps
"""
...
- Profile is actionable for Tespit engineering and threat hunting
