...
Adversary infrastructure tracking uses passive DNS records, certificate transparency logs, WHOIS registration data, and IP enrichment to discover, map, and monitor threat actor command-and-control (C2) networks. Attackers frequently reuse hosting providers, registrars, SSL certificates, and naming patterns across campaigns, enabling analysts to pivot from known indicators to discover new infrastructure. bu skill covers building an automated tracking system that identifies infrastructure relationships, tespit etme (s) newly registered domains matching adversary patterns, and maintains a continuously updated map of threat actor networks.


## Ne Zaman Kullanılır

- Dağıt:ing yaparken or configuring building adversary infrastructure tracking system capabilities in your environment
...
- Erişim: passive DNS data sources
...
Pivoting identifies related infrastructure by following connections: IP pivot (Bul: all domains on an IP), domain pivot (Bul: all IPs a domain resolved to), WHOIS pivot (Bul: domains with same registrant), certificate pivot (Bul: hosts sharing SSL certificates), and NS/MX pivot (Bul: domains using same name servers or mail servers).
...
### Adım 1: Passive DNS Infrastructure Discovery
...
class InfrastructureTracker:
    ...
    def reverse_ip_lookup(self, ip_address):
        """Bul: all domains hosted on an IP address."""
        headers = {"apikey": self.st_key}
...
### Adım 2: Build Infrastructure Graph
...
class InfrastructureGraph:
    ...
    def Bul:_hub_nodes(self, top_n=10):
        """Bul: high-centrality nodes (shared infrastructure)."""
...
clusters = infra_graph.Bul:_clusters()
hubs = infra_graph.Bul:_hub_nodes()
infra_graph.export_graph()
```

### Adım 3: Monitor for New Infrastructure
