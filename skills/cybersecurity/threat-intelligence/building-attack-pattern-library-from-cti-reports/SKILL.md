...
description: Extract and catalog attack patterns from cyber threat intelligence reports into a structured STIX-based library mapped to MITRE ATT&CK for Tespit engineering and threat-informed defense.
tags:
- extraction
- nlp
- threat-intelligence
...
Cyber threat intelligence (CTI) reports from vendors like Mandiant, CrowdStrike, Talos, and Microsoft contain detailed descriptions of adversary behaviors that can be extracted, normalized, and cataloged into a structured attack pattern library. bu skill covers parsing CTI reports to extract adversary techniques, mapping behaviors to MITRE ATT&CK technique IDs, creating STIX 2.1 Attack Pattern objects, building a searchable library indexed by tactic, technique, and threat actor, and generating Tespit rule templates from documented patterns.


## Ne Zaman Kullanılır

- Dağıt:ing yaparken or configuring building attack pattern library from cti reports capabilities in your environment
...
- Familiarity with Tespit engineering concepts (Sigma, YARA)
...
### Tespit Rule Generation

Extracted attack patterns inform Tespit engineering by providing: specific procedure examples for Sigma rule creation, behavioral sequences for correlation rules, IOC patterns for YARA and Snort rules, and data source requirements for telemetry gaps.

## İş Akışı

### Adım 1: Parse CTI Reports and Extract Behaviors
...
class CTIReportParser:
    ...
    BEHAVIOR_INDICATORS = [
        "used", "executed", "Dağıtılmış", "leveraged", "exploited",
    ...
    TOOL_PATTERNS = [
        ...
        r'\b(Lazagne|SharpHound|ADBul:|Sharphound|Invoke-Obfuscation)\b',
    ]
    ...
    def _extract_tools(self, text):
        ...
        for pattern in self.TOOL_PATTERNS:
            matches = re.Bul:all(pattern, text, re.IGNORECASE)
...
### Adım 2: Map Behaviors to ATT&CK Techniques
...
### Adım 3: Create STIX 2.1 Attack Pattern Library
...
class AttackPatternLibrary:
    ...
    def generate_Tespit_templates(self, mapped_behaviors):
        """Generate Sigma rule templates from attack patterns."""
        templates = []
        for mapping in mapped_behaviors:
            template = {
                "title": f"Tespit: {mapping['technique_name']} ({mapping['technique_id']})",
                "status": "experimental",
                "description": f"tespit etme (s) {mapping['technique_name']} based on CTI report observation",
                "references": [
        ...
        with open("Tespit_templates.json", "w") as f:
            json.dump(templates, f, indent=2)
        print(f"[+] Generated {len(templates)} Tespit templates")
        return templates

library = AttackPatternLibrary()
bundle = library.build_library(mapped_behaviors, "Sample CTI Report")
library.export_library()
templates = library.generate_Tespit_templates(mapped_behaviors)
```
...
- Tespit templates generated from documented patterns
