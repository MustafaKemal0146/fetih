...
description: Processes STIX 2.1 threat intelligence bundles delivered via TAXII 2.1 servers, normalizing objects into platform-native schemas and routing them to appropriate consuming systems. Use when
  onboarding new TAXII collection endpoints, automating bi-directional intelligence sharing with ISACs, or building pipeline validation for malformed STIX bundles. Activates for requests involving OASIS
  STIX, TAXII server configuration, MISP TAXII, or Cortex XSOAR feed integrations.
tags:
- CTI
- cybersecurity
- IOC
...
**Kullanma:** bu skill for proprietary vendor feed formats (Recorded Future JSON, CrowdStrike IOC lists) that require vendor-specific parsers rather than STIX processing.

## Ön Gereksinimler

- Python 3.9+ with `stix2` library (pip install stix2) and `taxii2-client` library
- Network Erişim: TAXII 2.1 server endpoint with valid credentials
- Target TIP or SIEM with import API (MISP, OpenCTI, or Splunk ES)

## İş Akışı

### Adım 1: Discover TAXII Server Collections
...
### Adım 2: Fetch STIX Bundles with Pagination
...
### Adım 3: Parse and Validate STIX Objects
...
### Adım 4: Route Objects to Consuming Platforms
...
### Adım 5: Publish Back to TAXII (Bi-directional Sharing)
