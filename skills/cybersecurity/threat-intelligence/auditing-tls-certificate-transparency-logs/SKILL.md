...
description: Monitors Certificate Transparency (CT) logs to tespit etmeunauthorized certificate issuance, discover subdomains via CT data, and alert on suspicious certificate activity for owned domains. Uses
  the crt.sh API and direct CT log querying based on RFC 6962 to build continuous monitoring pipelines that catch rogue certificates, track CA behavior, and map the external attack surface. Activates for
  requests involving certificate transparency monitoring, CT log auditing, subdomain discovery via certifi...
tags:
...
- Tespit etme phishing infrastructure that uses look-alike domain certificates (typosquatting, homograph attacks)
...
**Kullanma:** for attacking or disrupting Certificate Authorities, for scraping CT logs in violation of rate limits or terms of service, or as the sole method of subdomain enumeration without corroborating results through DNS verification.

## Ön Gereksinimler

- Python 3.10+ with `requests`, `cryptography`, and `pyOpenSSL` libraries installed
- Network Erişim: crt.sh (HTTPS) and public CT log servers
...
### Adım 1: Domain Inventory and Baseline

Şunu inşa et: initial certificate inventory for monitored domains:
...
- **Build baseline database**: Şunu sakla: initial certificate set in a local SQLite database with columns for certificate ID, domain, issuer, validity dates, SANs (Subject Alternative Names), and first-seen timestamp. This baseline prevents alerting on already-known certificates.
- **Identify authorized CAs**: From the baseline, Şunu çıkar: set of Certificate Authorities that have legitimately issued certificates for your domains. Any future issuance from a CA not in this set triggers a high-priority alert.
...
### Adım 2: Continuous CT Log Monitoring

Kur: ongoing monitoring for new certificate issuances:
...
- **Parse certificate details**: For each new certificate, Şunu çıkar: full SAN list, issuer chain, validity period, key type and size, CT log SCT (Signed Certificate Timestamp) details, and certificate fingerprint (SHA-256).
- **tespit etmeprecertificates**: CT logs include precertificates (poisoned certificates submitted before final issuance). Şunu takip et:se as early warnings since they indicate a certificate is about to be issued but may not yet be active.
...
### Adım 3: Subdomain Discovery via CT Data
...
- **Historical certificate mining**: Query crt.sh without the `exclude=expired` parameter to Bul: subdomains from expired certificates that may still resolve in DNS. These represent historical infrastructure that could be vulnerable to subdomain takeover.
- **DNS validation**: For each discovered subdomain, perform DNS resolution (A, AAAA, CNAME records) to Belirle: if the subdomain is currently active. Cross-reference with known IP ranges to identify shadow IT or unauthorized services.
- **Typosquat Tespit**: Generate permutations of the monitored domain (bitsquatting, homograph, insertion, omission, transposition, keyboard-adjacent replacement) and query CT logs for certificates issued to these variations. Certificates for typosquat domains strongly indicate phishing infrastructure.
...
### Adım 4: Certificate Issuance Alerting
...
- **New subdomain alert**: Trigger when a certificate contains a SAN with a previously unseen subdomain. This catches shadow IT Dağıt:ments and unauthorized services.
...
### Adım 5: CT Log Integrity Verification and Reporting
...
- **Compliance evidence**: For organizations subject to PCI-DSS, SOC 2, or similar frameworks, CT monitoring logs provide evidence of certificate lifecycle management and unauthorized issuance Tespit capabilities.

## Key Concepts

| Term | Definition |
|------|------------|
| **Certificate Transparency (CT)** | An open framework (RFC 6962) requiring Certificate Authorities to log all issued certificates in publicly auditable append-only logs, enabling domain owners to tespit etmeunauthorized issuance |
...
| **CAA Record** | DNS Certification Authority Authorization record that specifies which CAs are permitted to issue certificates for a domain; CT monitoring tespit etme (s) violations of CAA policy |
...
### Scenario: Tespit etme Unauthorized Certificate Issuance for a Financial Services Company
...
**Approach**:
1. CT monitoring agent tespit etme (s) a new certificate from "FreeSSL CA" for `secure-login.bank.example.com` in crt.sh results, which is not in the authorized CA list (DigiCert, Sectigo)
2. Alert fires as unauthorized CA + new subdomain, escalating to the security team within 15 minutes of CT log entry
3. Araştır: the certificate: Şunu çıkar: public key, check if the domain validated via HTTP-01 or DNS-01 challenge, query WHOIS for the issuing organization
...
- Failing to Şunu doğrula: CAA records are properly configured on all domains after an incident
...
3. DNS-resolve all 347 subdomains, Bul:ing 189 currently active with A/AAAA records
...
6. Present Bul:ings to the target organization showing the gap between their known asset inventory and the CT-derived attack surface
...
- **Action Required**: Araştır: immediately, request revocation
