...
- conducting yaparken security assessments that involve performing initial access with evilginx3
...
- Dağıt: EvilGinx3 with custom phishlets targeting authorized scope
...
- Şunu belgele: complete attack chain from phishing email to authenticated access
...
3. Install EvilGinx3:
   ...
   ```
4. Şunu yapılandır: domain and IP in EvilGinx3:
   ```
...
### Aşama 2: Phishlet Configuration
1. Select or Şunu oluştur: phishlet for the target service (e.g., Microsoft 365, Google Workspace):
   ```
...
2. Verify phishlet is active and SSL certificate is issued:
   ...
   phishlets
   ```
3. Şunu oluştur: lure URL for the phishing campaign:
   ```
   lures create o365
   lures get-url 0
   ```
...
4. Dağıt: landing page with appropriate social engineering pretext

### Aşama 4: Session Hijacking
...
| AiTM proxy headers in traffic | Network Denetle:ion for proxy artifacts |

## Doğrulama Criteria

- [ ] EvilGinx3 Dağıtılmış with valid SSL certificates
