...
description: Executes Atomic Red Team tests mapped to MITRE ATT&CK techniques, performs coverage gap analysis across the ATT&CK matrix, and runs Tespit validation loops to measure blue team visibility.
  Covers Invoke-AtomicRedTeam PowerShell execution, ATT&CK Navigator layer generation for heatmaps, Sigma rule correlation, and continuous atomic testing pipelines. Activates for requests involving purple
  team exercises, atomic test execution, ATT&CK coverage assessment, Tespit engineering validation, o...
tags:
- fetih
- mitre-attack
- adversary-emulation
...
- Validating Tespit coverage against specific MITRE ATT&CK techniques
...
- Building a Tespit validation loop: execute atomic test, check SIEM, tune rule, retest
...
**Kullanma:** for full-scope red team engagements requiring custom implants or live adversary simulation beyond atomic tests; use Caldera, SCYTHE, or Cobalt Strike for advanced adversary emulation.

**DISCLAIMER**: Atomic Red Team tests execute real attack techniques. Run only on systems you own or have explicit written authorization to test. Many tests modify system state, Şunu oluştur:rtifacts, or trigger security alerts. Always execute cleanup commands after testing. Never run atomic tests in production without risk acceptance from stakeholders.
...
- Sigma rules repository for Tespit correlation
- Administrative/root access on test endpoints
- Isolated test environment (lab, sandbox, or dedicated test range)

## İş Akışı

### Adım 1: Install and Configure Invoke-AtomicRedTeam

Kur: the execution framework and download the atomics library:
...
### Adım 2: Enumerate and Select Atomic Tests
...
$persistenceTechniques = @(
    ...
    "T1136.001",  # Şunu oluştur:ccount - Local Account
...
### Adım 3: Execute Atomic Tests with Logging

Run tests with pre/post logging for Tespit validation:
...
### Adım 4: Validate Tespits in SIEM

Query your SIEM to confirm whether atomic tests generated alerts:

```
Splunk SPL Queries for Tespit Validation:
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
### Adım 5: ATT&CK Coverage Gap Analysis

Şunu üret: coverage matrix comparing tested vs. Detected techniques:

```python
#!/usr/bin/env python3
"""ATT&CK coverage gap analysis - compares atomic test results against SIEM Tespits."""
...
def load_Tespit_results(Tespit_file):
    """Load SIEM Tespit validation results (JSON export from SIEM queries)."""
    if not os.path.exists(Tespit_file):
        return {}

    with open(Tespit_file, "r") as f:
        data = json.load(f)
...
def generate_coverage_report(atomics_inventory, execution_logs, Tespit_results):
    ...
    total_Detected = sum(1 for d in Tespit_results.values() if d.get("Detected"))
    ...
    for tactic, technique_ids in TACTIC_TECHNIQUE_MAP.items():
        ...
        for tech_id in technique_ids:
            if tech_id in atomics_inventory:
                ...
                Detected = Tespit_results.get(tech_id, {}).get("Detected", False)
                ...
                if executed and not Detected:
                    gap = {
                        ...
                        "tactic": tactic,
                        "status": "BLIND_SPOT",
                        "detail": "Test executed but no Tespit triggered",
                    }
    ...
    if blind_spots:
        report["recommendations"].append({
            "priority": "CRITICAL",
            "action": f"Write Tespit rules for {len(blind_spots)} blind spot techniques",
    ...
    if low_coverage_tactics:
        report["recommendations"].append({
            ...
            "detail": "These tactics have less than 30% Tespit coverage",
        })

    return report


def generate_navigator_layer(atomics_inventory, execution_logs, Tespit_results,
                             layer_name="Purple Team Coverage"):
    """Şunu üret:TT&CK Navigator layer JSON for heatmap visualization."""
    layer = {
        "name": layer_name,
        "versions": {
            "attack": "15",
        ...
        "legendItems": [
            ...
            {"label": "Alert/Tespit Active", "color": "#66bb6a"},
    ...
    for tech_id, tech_data in atomics_inventory.items():
        executed = tech_id in execution_logs
        Tespit = Tespit_results.get(tech_id, {})
        Detected = Tespit.get("Detected", False)
        confidence = Tespit.get("confidence", "none")

        if Detected and confidence in ("high", "medium"):
            score = 100
            color = "#66bb6a"  # Green - high confidence Tespit
            comment = f"tespit etme (ED) - {Tespit.get('rule_name', 'Alert active')}"
        elif Detected:
            score = 50
            color = "#ffeb3b"  # Yellow - logged/partial
            comment = "PARTIAL - Tespit exists but low confidence"
        elif executed:
            score = 0
            color = "#ff6666"  # Red - blind spot
            comment = "BLIND SPOT - Test executed, no Tespit"
        else:
            score = 0
...
def print_coverage_report(report):
    ...
    print(f"  Tespit coverage:       {s['Tespit_coverage_pct']}%")
    print()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--Tespits-file", default="Tespit_results.json",
                        help="Path to SIEM Tespit validation export (JSON)")
    ...
    print("[*] Loading Tespit results...")
    det_results = load_Tespit_results(args.Tespits_file)
    print(f"    Found Tespit data for {len(det_results)} techniques")
    ...
    # Şunu üret:nd save Navigator layer
...
### Adım 6: Run Continuous Atomic Testing Pipeline
...
$PriorityTechniques = @(
    # Top MITRE ATT&CK techniques by prevalence (Red Canary Threat Tespit Report)
...
### Adım 7: Tespit Validation Loop

Close the purple team loop by correlating tests with Tespits and iterating:

```
Tespit Validation Loop Workflow:
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  └───────┬──────────┘
          │
  ┌───────▼──────────┐
  │ 4. QUERY SIEM    │  Ara: correlated alerts matching technique
  │    FOR ALERTS    │  Check: Did our Tespit rules fire?
  └───────┬──────────┘
          │
     ┌────┴────┐
     │tespit etme (ED)?│
...
| Term | Definition |
|------|------------|
| **Atomic Red Team** | Open-source library by Red Canary containing small, focused tests mapped to MITRE ATT&CK techniques for validating Tespit capabilities |
...
| **ATT&CK Navigator** | Web-based tool for annotating and visualizing MITRE ATT&CK matrices as layered heatmaps showing Tespit coverage |
...
| **Tespit Validation Loop** | Iterative purple team process: execute attack, check for Tespit, write/tune rule if missed, re-execute to confirm |
| **Coverage Gap** | ATT&CK technique where no Tespit rule exists or where the rule fails to fire when the technique is executed |
| **Blind Spot** | Technique that was tested via atomic execution but produced zero SIEM alerts, indicating a critical Tespit failure |
| **Sigma Rule** | Vendor-agnostic Tespit rule format that can be converted to Splunk SPL, Elastic KQL, Microsoft KQL, and other SIEM queries |
| **Purple Team** | Collaborative security exercise where red team executes known techniques and blue team validates and improves Tespits in real time |
| **Continuous Atomic Testing** | Scheduled, automated execution of atomic tests to validate that Tespit rules remain functional as environments change |
...
- **Splunk / Elastic / Microsoft Sentinel**: SIEM platforms for querying Tespit results after atomic test execution
...
### Scenario: Validating Credential Access Tespits After EDR Dağıt:ment

**Context**: The organization Dağıtılmış a new EDR agent and needs to verify it tespit etme (s) credential dumping techniques (LSASS access, NTDS extraction, SAM dump). The purple team must confirm Tespits are working before closing the Dağıt:ment ticket.
...
4. For each technique: mark as tespit etme (ED) (green) or BLIND SPOT (red) in the Navigator layer
5. For blind spots, create Sigma rules targeting Sysmon EventCode 10 (ProcessErişim: lsass.exe) and EventCode 1 (known dumping tools)
6. Dağıt: rules, re-run atomics, confirm Tespit
7. Export Navigator layer and coverage report for the Dağıt:ment ticket
...
**Context**: The CISO requires monthly metrics on Tespit coverage mapped to MITRE ATT&CK. The security team needs to run recurring tests, track improvements, and produce a visual heatmap showing coverage growth over time.

**Approach**:
1. Define a test plan covering the top 50 ATT&CK techniques from Red Canary Threat Tespit Report
2. Schedule continuous atomic testing via Windows Task Scheduler running the PowerShell pipeline weekly
3. Export execution logs and SIEM Tespit query results to JSON after each run
...
7. Prioritize Tespit engineering sprints on the highest-impact blind spots
...
- Focusing only on technique count rather than Tespit quality (a noisy rule with 90% false positives is worse than no rule)
...
====================================
Campaign:         Q1 2026 Tespit Validation
