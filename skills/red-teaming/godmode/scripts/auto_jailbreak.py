...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("FETIH_HOME", os.path.expanduser("~/.fetih")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("FETIH_HOME", Path.home() / ".fetih")) / "skills" / "red-teaming" / "godmode"
...
FETIH_HOME = Path(os.getenv("FETIH_HOME", Path.home() / ".fetih"))
CONFIG_PATH = FETIH_HOME / "config.yaml"
PREFILL_PATH = FETIH_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/FETIH models — already uncensored, just needs clean prompt
    "fetih": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "fetih" in model_lower or "nous" in model_lower:
        return "fetih"
...
def _get_current_model() -> tuple:
    """Read current model and provider from FETIH config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.fetih/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart FETIH for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart FETIH for changes to take effect.")
