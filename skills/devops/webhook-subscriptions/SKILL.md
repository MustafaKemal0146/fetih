...
Create dynamic webhook subscriptions so external services (GitHub, GitLab, Stripe, CI/CD, IoT sensors, monitoring tools) can trigger FETIH agent runs by POSTing events to a URL.
...
All management is via the `fetih webhook` CLI command:

### Create a subscription
```bash
fetih webhook subscribe <name> \
...
```bash
fetih webhook remove <name>
```

### Test a subscription
```bash
fetih webhook test <name>
fetih webhook test <name> --payload '{"key": "value"}'
```
...
### GitHub: new issues
```bash
fetih webhook subscribe github-issues \
  --events "issues" \
...
### GitHub: PR reviews
```bash
fetih webhook subscribe github-prs \
  --events "pull_request" \
...
```bash
fetih webhook subscribe stripe-payments \
...
```bash
fetih webhook subscribe ci-builds \
  --events "pipeline" \
...
```bash
fetih webhook subscribe alerts \
...
```bash
fetih webhook subscribe antenna-matches \
  --deliver telegram \
...
- Subscriptions persist to `~/.fetih/webhook_subscriptions.json`

## How It Works

1. `fetih webhook subscribe` writes to `~/.fetih/webhook_subscriptions.json`
...
1. **Is the gateway running?** Check with `systemctl --user status fetih-gateway` or `ps aux | grep gateway`
2. **Is the webhook server listening?** `curl http://localhost:8644/health` should return `{"status": "ok"}`
3. **Check gateway logs:** `grep webhook ~/.fetih/logs/gateway.log | tail -20`
