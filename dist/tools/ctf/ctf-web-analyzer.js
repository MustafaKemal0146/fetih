...
function detectXSS(url, input, headers, params) {
    const vulns = [];
    const targetUrl = url || 'TARGET_URL';
...
function detectLFI(url, params) {
    ...
    for (const [key, val] of Object.entries(params)) {
        if (LFI_PARAMS.includes(key.toLowerCase())) {
            ...
            vulns[vulns.length - 1].payloads.push(...(isLinux ? LFI_LINUX_FILES : LFI_WINDOWS_FILES));
        }
    }
...
// ─── 1.4 IDOR Tespiti ────────────────────────────────────────────────────────
function detectIDOR(url, params) {
    const vulns = [];
    // URL'de numeric ID
...
function directoryDiscovery(baseUrl) {
...
function parseUrlAndParams(input) {
    const params = {};
    const headers = {};
    let url = '';
    // URL tespiti
...
export function analyzeWeb(input) {
    ...
    if (headers['server'])
        recommendations.push(`⚠️  Server header versiyon açıklıyor: ${headers['server']}`);
    if (headers['x-powered-by'])
        recommendations.push(`⚠️  X-Powered-By teknoloji açıklıyor: ${headers['x-powered-by']}`);
    // Tool önerileri
    const criticalVulns = vulns.filter(v => v.severity === 'critical' || v.severity === 'high');
    for (const v of criticalVulns) {
        if (v.tool_command)
            tools.push(v.tool_command);
    }
    if (vulns.some(v => v.type.includes('SQL')))
        tools.push('sqlmap', 'ghauri');
    if (vulns.some(v => v.type.includes('XSS')))
        tools.push('dalfox', 'xsser');
    if (vulns.some(v => v.type.includes('LFI')))
        tools.push('ffuf', 'dotdotpwn');
    return {
...
// ─── Tool Tanımı ─────────────────────────────────────────────────────────────
export const ctfWebAnalyzerTool = {
    name: 'ctf_web_analyzer',
    description: 'CTF web güvenliği analizi: SQLi, XSS, LFI/Path Traversal, IDOR tespiti. ' +
    ...
    async execute(raw) {
        ...
        if (result.vulnerabilities_found.length > 0) {
            ...
            for (const v of result.vulnerabilities_found) {
                ...
                if (v.tool_command)
                    lines.push(`    Komut: ${v.tool_command}`);
            }
        }
