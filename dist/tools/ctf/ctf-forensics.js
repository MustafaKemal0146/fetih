...
export function parsePcap(buf, maxPackets = 100) {
    const result = {
    ...
    if (magic === 0xA1B2C3D4) {
        ...
        result.notes.push('PCAPNG formatı — basit parser sadece pcap destekler. tshark öneriliyor.');
        return result;
    }
    else {
        result.notes.push(`Bilinmeyen magic: 0x${magic.toString(16)}`);
        return result;
    }
    ...
    while (offset < buf.length - 16 && pktIdx < maxPackets) {
        ...
        if (capLen <= 0 || capLen > 65535 || dataOffset + capLen > buf.length)
            break;
        const data = buf.slice(dataOffset, dataOffset + capLen);
        const pkt = { index: pktIdx, ts, capLen, origLen };
        ...
        if (data.length >= 14) {
            ...
            if (ethertype === 0x0800 && data.length >= 14 + 20) {
                ...
                ipsSeen.add(pkt.srcIp);
                ipsSeen.add(pkt.dstIp);
                ...
                if ((proto === 6 || proto === 17) && ipHeader.length >= ihl + 8) {
                    ...
                    if (proto === 6 && transport.length >= 20) {
                        ...
                        if (payload.length > 0) {
                            ...
                            if (httpMatch)
                                result.httpRequests.push(`${httpMatch[1]} ${httpMatch[2]}`);
                        }
                    }
                }
            }
    ...
    while (countOffset < buf.length - 16) {
        ...
        if (capLen <= 0 || capLen > 65535 || countOffset + 16 + capLen > buf.length)
            break;
...
export function volatilityRun(filePath, plugin) {
...
export function foremostRun(filePath, outputDir) {
...
export function largeFileStringsSweep(filePath, chunkSize = 1 * 1024 * 1024, maxFlags = 50) {
    if (!existsSync(filePath))
        return { flags: [], bytesScanned: 0, notes: ['Dosya yok'] };
    ...
    try {
        ...
        while (pos < fileSize && flags.length < maxFlags) {
            ...
            for (const f of found) {
                if (!flags.includes(f) && flags.length < maxFlags)
                    flags.push(f);
            }
...
// ─── Tool Tanımı ─────────────────────────────────────────────────────────────
export const ctfForensicsTool = {
    name: 'ctf_forensics',
    description: 'CTF Forensics: pcap_parse (binary PCAP parse — paket sayım, IP/port, HTTP, flag sweep), ' +
    ...
    async execute(input) {
        ...
        if (!existsSync(filePath))
            return { output: `Dosya bulunamadı: ${filePath}`, isError: true };
        switch (action) {
            case 'pcap_parse': {
