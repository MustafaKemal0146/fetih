...
import { solve } from './ctf-solver.js';
import { analyzeWeb } from './ctf-web-analyzer.js';
import { analyzeNetwork } from './ctf-network-analyzer.js';
import { analyzeFile } from './ctf-file-analyzer.js';
import { analyzeSteganography } from './ctf-stego.js';
import { analyzeAudio } from './ctf-audio-analyzer.js';
import { analyzeBinary } from './ctf-binary-analyzer.js';
function detectInputType(input) {
    ...
    if ((trimmed.startsWith('/') || /^[A-Za-z]:\\/.test(trimmed)) && existsSync(trimmed)) {
        ...
        if (magic4[0] === 0xFF && magic4[1] === 0xD8)
            return 'file_image';
        if (magic4[0] === 0x89 && magic4[1] === 0x50)
            return 'file_image'; // PNG
        if (magic4[0] === 0x47 && magic4[1] === 0x49)
            return 'file_image'; // GIF
        if (magic4[0] === 0xA1 || magic4[0] === 0xD4)
            return 'file_pcap';
        if (magic4[0] === 0x0A && magic4[1] === 0x0D)
            return 'file_pcap'; // pcapng
        // Audio: WAV/OGG/FLAC/MP3/M4A
        if (head4 === 'RIFF' && head4to8 === 'WAVE')
            return 'file_audio';
        if (head4 === 'OggS' || head4 === 'fLaC')
            return 'file_audio';
        if (head4to8 === 'ftyp')
            return 'file_audio';
        if (head4.startsWith('ID3') || (magic4[0] === 0xFF && (magic4[1] & 0xE0) === 0xE0))
            return 'file_audio';
        // Binary: ELF / PE
        if (magic4[0] === 0x7F && magic4[1] === 0x45 && magic4[2] === 0x4C && magic4[3] === 0x46)
            return 'file_binary';
        if (magic4[0] === 0x4D && magic4[1] === 0x5A)
            return 'file_binary'; // PE/EXE
        if (magic4[0] === 0x50 && magic4[1] === 0x4B)
            return 'file_zip';
        if (magic4[0] === 0x25 && magic4[1] === 0x50)
            return 'file_pdf';
        return 'file_other';
    }
    // Hash formatı (length-based hex veya $-prefix)
    if (/^\$[0-9aybs]/i.test(trimmed) && trimmed.length > 10)
        return 'hash';
    if (/^[0-9a-fA-F]{32,128}$/.test(trimmed) && [32, 40, 56, 64, 96, 128].includes(trimmed.length))
        return 'hash';
    // RSA parametreleri
    if (/p\s*=\s*\d+/i.test(trimmed) && /q\s*=\s*\d+/i.test(trimmed))
        return 'rsa_params';
    // URL veya HTTP içeriği
    if (/^https?:\/\//i.test(trimmed) || /^(GET|POST|PUT|DELETE)\s+/m.test(trimmed))
        return 'url_or_http';
    if (/^[A-Za-z-]+:\s/m.test(trimmed) && trimmed.includes('HTTP/'))
        return 'url_or_http';
    // Nmap çıktısı
    if (/\d+\/(tcp|udp)\s+open/i.test(trimmed))
        return 'nmap_output';
    // PCAP metin çıktısı
    if (/Ethernet|IP \d+\.\d+|TCP \d+\.\d+/.test(trimmed))
        return 'pcap_text';
    // Encoding/kripto
...
// ─── 3.2 Paralel Analiz ──────────────────────────────────────────────────────
async function runModules(input, inputType) {
    const modules = [];
    const results = {};
    switch (inputType) {
        case 'file_image': {
...
function extractBestResult(results) {
    const candidates = [];
...
function buildMarkdown(report) {
    ...
    if (report.file_result) {
        ...
        if (report.file_result.carvedFiles.length > 0)
            lines.push(`  - ${report.file_result.carvedFiles.length} gizli dosya tespit edildi`);
    }
...
// ─── Ana Fonksiyon ───────────────────────────────────────────────────────────
export async function autoAnalyze(input) {
...
// ─── Tool Tanımı ─────────────────────────────────────────────────────────────
export const ctfAutoTool = {
    name: 'ctf_auto',
    description: 'Fetih CTF otomatik analiz motoru. Input türünü otomatik tespit eder ve doğru modülü çalıştırır. ' +
