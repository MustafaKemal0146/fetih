...
export function detectEcb(ciphertext) {
    const blockSize = 16;
...
export function cbcBitFlipGuide() {
    return [
...
// ─── Padding Oracle Attack Template ──────────────────────────────────────────
export function paddingOracleTemplate(targetUrl) {
    return [
...
export function aesEcbDecrypt(ciphertext, key) {
...
// ─── Tool Tanımı ─────────────────────────────────────────────────────────────
export const ctfAesHelperTool = {
    name: 'ctf_aes_helper',
    description: 'CTF AES analiz: ecb_detect (16-byte block tekrar pattern\'i), ecb_decrypt (key ile çöz), ' +
    ...
    async execute(input) {
        ...
        switch (action) {
            case 'ecb_detect': {
                ...
                if (!ct)
                    return { output: 'ciphertext gerekli', isError: true };
                const enc = String(input['ciphertextEncoding'] ?? 'hex');
            ...
            case 'ecb_decrypt': {
                ...
                if (!ct || !key)
                    return { output: 'ciphertext ve key gerekli', isError: true };
                const enc = String(input['ciphertextEncoding'] ?? 'hex');
