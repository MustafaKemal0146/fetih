...
async function startSession(command, args, cwd) {
    ensureCleanup();
    const id = `s${nextId++}`;
...
async function recvUntil(session, pattern, timeout) {
...
async function recvAll(session, idleMs) {
...
function send(session, text) {
    if (session.closed)
        throw new Error('Session kapalı');
    session.proc.stdin.write(text);
}
// ─── Tool Tanımı ─────────────────────────────────────────────────────────────
export const interactiveSessionTool = {
    name: 'interactive_session',
    description: 'Uzun süreli interaktif process yönetimi (gdb, msfconsole, nc, telnet vb.). ' +
        ...
        try {
            switch (action) {
                case 'start': {
                    ...
                    if (!command)
                        return { output: 'command gerekli', isError: true };
                ...
                case 'sendline': {
                    if (!sid)
                        return { output: 'session ID gerekli', isError: true };
                    const session = sessions.get(sid);
                    if (!session)
                        return { output: `Session yok: ${sid}`, isError: true };
                ...
                case 'recv_until': {
                    ...
                    if (!pattern)
                        return { output: 'pattern gerekli', isError: true };
                ...
                case 'kill': {
                    ...
                    const session = sessions.get(sid);
                    if (!session)
                        return { output: `Session yok: ${sid}`, isError: false };
                    try {
                ...
                case 'list': {
                    if (sessions.size === 0)
                        return { output: 'Aktif session yok', isError: false };
