...
export function cyclicPattern(length, alphabet = 'abcdefghijklmnopqrstuvwxyz') {
...
export function cyclicFind(value, length = 4096) {
    let target;
...
export function formatStringWrite(addr, value, offset, archBits = 64) {
...
export function formatStringOffsetTemplate() {
    return [
...
export function getShellcode(name) {
    return SHELLCODES[name] ?? null;
}
export function listShellcodes() {
...
export function findRopGadgets(filePath, maxGadgets = 50) {
    if (!existsSync(filePath))
        return { ok: false, output: 'Dosya bulunamadı', source: 'none' };
    // ROPgadget kuruluysa
...
export function checksec(filePath) {
    const result = {
        ok: false, arch: 'unknown',
        nx: 'unknown', pie: 'unknown', relro: 'unknown', canary: 'unknown',
        notes: [],
    };
    if (!existsSync(filePath)) {
        result.notes.push('Dosya bulunamadı');
        return result;
    }
    if (!commandExists('readelf')) {
        result.notes.push('readelf yok');
        return result;
    }
    try {
...
// ─── Tool Tanımı ─────────────────────────────────────────────────────────────
export const ctfPwnTool = {
    name: 'ctf_pwn',
    description: 'CTF PWN/Exploit toolkit. Eylemler: cyclic (pattern üret), cyclic_find (offset bul), ' +
    ...
    async execute(input) {
        ...
        switch (action) {
            ...
            case 'cyclic_find': {
                ...
                const offset = cyclicFind(numVal, len);
                return {
                    output: offset >= 0
            ...
            case 'fmt_string': {
                ...
                const arch = (Number(input['arch'] ?? 64) === 32 ? 32 : 64);
                if (addr === 0 || off === 0) {
            ...
            case 'shellcode': {
                ...
                if (!sc)
                    return { output: `Bilinmeyen shellcode: ${name}. Liste için action=shellcode_list`, isError: true };
                return {
                    output: [
            ...
            case 'rop': {
                ...
                if (!path)
                    return { output: 'path gerekli', isError: true };
