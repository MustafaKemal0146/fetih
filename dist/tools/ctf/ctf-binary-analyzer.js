...
function safeRun(cmd, args, maxBytes = 16_000) {
    try {
...
// ─── Modüller ────────────────────────────────────────────────────────────────
function fileTypeSection(filePath) {
...
function stringsSection(filePath) {
    ...
    // İlginç kelime arama
    const interesting = r.output.split('\n').filter(line => /flag|ctf|secret|password|key|admin|backdoor|TODO|FIXME|http:\/\/|https:\/\//i.test(line)).slice(0, 30);
...
function readelfSection(filePath) {
    ...
    // Stripped/PIE
    if (/Type:\s+EXEC/.test(r.output))
        security.push('Type: EXEC — non-PIE (statik adresler)');
    if (/Type:\s+DYN/.test(r.output))
        security.push('Type: DYN — PIE veya shared lib (ASLR)');
    return { section: { module: 'readelf -h -l', ok: r.ok, output: r.output, notes: [] }, security };
}
function nmSection(filePath) {
    if (!commandExists('nm')) {
        return { module: 'nm', ok: false, output: '', notes: ['nm yok'] };
    }
    const r = safeRun('nm', ['--demangle', filePath], 8000);
    // İlginç sembol arama
    const interesting = r.output.split('\n').filter(line => /\b(main|flag|secret|password|admin|backdoor|debug|hidden|key)\b/i.test(line)).slice(0, 30);
...
function objdumpSection(filePath) {
    ...
    for (const fn of dangerous) {
        ...
        if (re.test(r.output))
            security.push(`⚠️  ${fn}@plt — buffer overflow / komut enjeksiyonu vektörü`);
    }
...
function binwalkSection(filePath) {
...
// ─── Ana Analiz ──────────────────────────────────────────────────────────────
export async function analyzeBinary(filePath) {
    ...
    if (isELF) {
        ...
        suggestedNext.push('gdb dynamic analysis: gdb -q ' + filePath, 'pwntools template: from pwn import *; ELF("' + filePath + '")', 'radare2: r2 -A ' + filePath, 'Ghidra: import dosya, decompile main');
    }
    else if (isPE) {
        suggestedNext.push('PE analiz için: pefile (Python), CFF Explorer, Ghidra', 'IDA Pro veya Binary Ninja ile decompile');
    }
...
// ─── Tool Tanımı ─────────────────────────────────────────────────────────────
export const ctfBinaryAnalyzerTool = {
    name: 'ctf_binary_analyzer',
    description: 'CTF binary/RE analizi: file/strings/xxd/readelf/nm/objdump/binwalk wrapper. ' +
    ...
    async execute(input) {
        ...
        if (!filePath)
            return { output: 'Hata: path boş olamaz.', isError: true };
        ...
        if (r.flagsFound.length > 0)
            lines.push(`│ 🎯 FLAG: ${r.flagsFound.join(', ')}`);
        lines.push('└─────────────────────────────────────────────────────────────┘');
