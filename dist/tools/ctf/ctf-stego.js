...
function bitsToString(bits) {
    ...
    for (let i = 0; i + 8 <= bits.length; i += 8) {
        ...
        if (byte >= 32 && byte < 127)
            ...
            break; // anlamsız byte geldi, dur
    }
    return result;
}
// isPrintable → isPrintableAscii (ctf-utils'ten import edildi)
const isPrintable = (s, threshold = 0.8) => isPrintableAscii(s, threshold);
...
function extractLSB(pixels, channels, bits = 1, step = 1) {
    const bitArray = [];
...
async function lsbAnalysis(filePath) {
...
async function alphaAnalysis(filePath) {
    const img = await loadPixels(filePath);
    if (!img)
        return null;
...
async function channelAnalysis(filePath, outDir) {
    const results = [];
    try {
        const jimpModule = await import('jimp');
        const Jimp = (jimpModule.default ?? jimpModule);
        ...
        for (const [chName, chIdx] of [['R', 0], ['G', 1], ['B', 2]]) {
...
async function histogramAnalysis(filePath) {
    const img = await loadPixels(filePath);
    if (!img)
        return null;
    const freq = {};
    ...
    const notes = [];
    if (lsbAnomaly)
        notes.push(`⚠️  LSB dağılımı çok dengeli (${(lsbRatio * 100).toFixed(1)}%) → stego şüphesi`);
    if (anomalies.length > 0)
        notes.push(`Baskın renkler: ${anomalies.slice(0, 3).join(', ')}`);
    if (notes.length > 0) {
        return {
...
export async function analyzeSteganography(filePath) {
    ...
    if (alphaResult)
        allResults.push(alphaResult);
    // Histogram
    const histResult = await histogramAnalysis(filePath);
    if (histResult)
        allResults.push(histResult);
...
// ─── Tool Tanımı ─────────────────────────────────────────────────────────────
export const ctfStegoTool = {
    name: 'ctf_stego',
    description: 'CTF görsel steganografi analizi: LSB (R/RGB/RGBA/2bit/step), Alpha kanal LSB, ' +
    ...
    async execute(input) {
        ...
        if (result.bestResult) {
            ...
            if (result.bestResult.data)
                lines.push(`│ Veri: ${result.bestResult.data.slice(0, 60)}`);
        }
