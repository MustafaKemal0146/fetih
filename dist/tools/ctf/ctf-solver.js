...
import { looksLikeFlag, isPrintableAscii } from './ctf-utils.js';
// ─── Yardımcı: Anlamlı mı? ───────────────────────────────────────────────────
...
function tryBase32(input) {
    ...
    try {
        ...
        if (isPrintableAscii(result, 0.7))
            return { decoded: result, technique: 'Base32' };
    }
    catch { /* ignore */ }
    return null;
}
function tryHex(input) {
    ...
    if (!/^[0-9a-f]+$/.test(clean) || clean.length % 2 !== 0 || clean.length < 4)
        return null;
    try {
        const decoded = Buffer.from(clean, 'hex').toString('utf8');
        if (isPrintableAscii(decoded, 0.7))
            return { decoded, technique: 'Hex' };
    }
    catch { /* ignore */ }
    return null;
}
function tryBinary(input) {
    const clean = input.trim().replace(/\s/g, '');
    ...
    try {
        ...
        if (isPrintableAscii(result, 0.8))
            return { decoded: result, technique: 'Binary' };
    }
    catch { /* ignore */ }
    return null;
}
function tryUrlDecode(input) {
    ...
    try {
        ...
        if (decoded !== input && isPrintableAscii(decoded))
            return { decoded, technique: 'URL Encoding' };
    }
    catch { /* ignore */ }
    return null;
}
function tryHtmlEntities(input) {
    if (!(/&[a-z]+;/.test(input) || /&#\d+;/.test(input) || /&#x[0-9a-f]+;/i.test(input)))
        return null;
    const decoded = input
    ...
    if (decoded !== input)
        return { decoded, technique: 'HTML Entities' };
    return null;
}
function tryAsciiDecimal(input) {
    const parts = input.trim().split(/\s+/);
    if (parts.length < 3)
        return null;
    if (!parts.every(p => /^\d+$/.test(p) && Number(p) >= 32 && Number(p) <= 126))
        return null;
    const decoded = parts.map(p => String.fromCharCode(Number(p))).join('');
    if (isPrintableAscii(decoded))
        return { decoded, technique: 'ASCII Decimal' };
    return null;
}
// ─── 2.2 Hash Kırma ──────────────────────────────────────────────────────────
const WORDLIST = [
...
function tryROT13(input) {
    ...
    if (decoded !== input && isMeaningful(decoded))
        return { decoded, technique: 'ROT13' };
    return null;
}
function tryROT47(input) {
    const decoded = input.replace(/[\x21-\x7e]/g, c => String.fromCharCode(((c.charCodeAt(0) - 33 + 47) % 94) + 33));
    if (decoded !== input && isMeaningful(decoded))
        return { decoded, technique: 'ROT47' };
    return null;
}
function tryCaesar(input) {
    if (!/[a-zA-Z]/.test(input))
        return null;
    ...
    for (let shift = 1; shift <= 25; shift++) {
        ...
        if (looksLikeFlag(decoded))
            return { decoded, technique: `Caesar (shift=${shift})` };
    }
    // Bulunamadıysa ilk anlamlı sonuca düş
    for (let shift = 1; shift <= 25; shift++) {
        const decoded = rot(input, shift);
        if (isMeaningful(decoded))
            return { decoded, technique: `Caesar (shift=${shift})` };
    }
    return null;
}
function tryAtbash(input) {
    if (!/[a-zA-Z]/.test(input))
        return null;
    ...
    if (decoded !== input && isMeaningful(decoded))
        return { decoded, technique: 'Atbash' };
    return null;
}
const MORSE_TABLE = {
    '.-': 'A', '-...': 'B', '-.-.': 'C', '-..': 'D', '.': 'E', '..-.': 'F',
...
function tryMorse(input) {
    if (!/^[.\-\s/]+$/.test(input.trim()))
        return null;
    const words = input.trim().split(/\s*\/\s*/);
    const decoded = words.map(word => word.trim().split(/\s+/).map(code => MORSE_TABLE[code] ?? '?').join('')).join(' ');
    if (!decoded.includes('?') && isPrintableAscii(decoded)) {
        return { decoded, technique: 'Morse Code' };
    }
    return null;
}
function tryPolybius(input) {
    const clean = input.trim().replace(/\s/g, '');
    if (!/^[1-5]{2,}$/.test(clean) || clean.length % 2 !== 0)
        return null;
    ...
    if (isPrintableAscii(decoded))
        return { decoded, technique: 'Polybius Square' };
    return null;
}
function tryBacon(input) {
    const clean = input.trim().replace(/\s/g, '').toUpperCase();
    if (!/^[AB]+$/.test(clean) || clean.length % 5 !== 0)
        return null;
    let decoded = '';
    ...
    if (isPrintableAscii(decoded))
        return { decoded, technique: 'Bacon Cipher' };
    return null;
}
function tryRailFence(input, rails) {
    ...
    for (let r = minRails; r < maxRails; r++) {
        ...
        if (isMeaningful(decoded))
            return { decoded, technique: `Rail Fence (rails=${r})` };
    }
    return null;
}
...
// ─── 2.4 Modern Kripto ───────────────────────────────────────────────────────
function tryXorSingleByte(input) {
...
function tryRSA(input) {
    ...
    if (!pMatch || !qMatch || !eMatch || !cMatch)
        return null;
    try {
...
// ─── 3. Steganografi ─────────────────────────────────────────────────────────
function tryWhitespaceStego(input) {
    if (!/[\t ]/.test(input))
        return null;
    ...
    if (isPrintableAscii(decoded, 0.8))
        return { decoded, technique: 'Whitespace Steganography' };
    return null;
}
// ─── Ana Orchestrator ─────────────────────────────────────────────────────────
const TECHNIQUES = [
...
export function solve(input, maxLayers = 7) {
    const layers = [];
    const alternatives = [];
    ...
    function recurse(data, depth, usedTechniques) {
        if (depth > maxLayers)
            return null;
        if (looksLikeFlag(data))
            return data;
        ...
        for (const fn of fns) {
            ...
            if (!result || usedTechniques.has(result.technique))
                continue;
        ...
        for (const fn of fns) {
            ...
            if (!result)
                continue;
            if (usedTechniques.has(result.technique))
                continue;
            layers.push(result.technique);
    ...
    if (!hasFlag) {
        ...
        if (topScore < 50)
            recommendations.push('Veri türü belirsiz — manuel inceleme gerekebilir');
        if (input.length > 100)
            recommendations.push('Uzun veri — çok katmanlı encoding olabilir');
    }
    if (finalResult) {
...
// ─── Tool Tanımı ─────────────────────────────────────────────────────────────
export const ctfSolverTool = {
    name: 'ctf_solver',
    description: 'CTF şifre/encoding çözücü. Base64, Hex, Binary, ROT13/47, Caesar, Atbash, Morse, ' +
