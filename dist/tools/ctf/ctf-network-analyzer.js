...
function analyzePcapText(content) {
...
function findCredentials(content) {
    const creds = [];
    ...
    for (const m of basicAuthMatches) {
        try {
            ...
            if (decoded.includes(':'))
                creds.push(`HTTP Basic Auth: ${decoded}`);
        }
        catch { /* ignore */ }
    }
    // FTP credentials
    ...
    if (ftpUser)
        creds.push(`FTP User: ${ftpUser[1]}`);
    if (ftpPass)
        creds.push(`FTP Pass: ${ftpPass[1]}`);
    // SMTP AUTH
...
function analyzeHttpHeaders(headerText) {
    const headers = {};
    const flagsInHeaders = [];
...
function parseNmapOutput(content) {
    const ports = [];
...
export function analyzeNetwork(input, filePath) {
    const result = {
        credentials_found: [],
        files_extracted: [],
        flags_found: [],
    ...
    if (/^[A-Za-z-]+:\s/m.test(input)) {
        ...
        if (hdrs)
            result.flags_found.push(...hdrs.flags_in_headers);
    }
...
// ─── Tool Tanımı ─────────────────────────────────────────────────────────────
export const ctfNetworkAnalyzerTool = {
    name: 'ctf_network_analyzer',
    description: 'CTF network analizi: PCAP dosyası, HTTP header, Nmap çıktısı analizi. ' +
    ...
    async execute(raw) {
        ...
        if (!input && !filePath)
            return { output: 'Hata: input veya file_path gerekli.', isError: true };
