...
function makeId(host, port) {
    return `${host}:${port}`;
}
async function connect(host, port, timeout = 10_000) {
    ensureCleanup();
    return new Promise((resolve, reject) => {
        const socket = new Socket();
        const session = { socket, buffer: Buffer.alloc(0), closed: false, host, port };
        socket.setTimeout(timeout);
        socket.on('data', (chunk) => { session.buffer = Buffer.concat([session.buffer, chunk]); });
...
async function recvUntil(session, pattern, timeout = 10_000) {
    ...
    while (true) {
        ...
        if (Date.now() - start > timeout)
            throw new Error(`recv_until timeout (${timeout}ms) — pattern bulunamadı`);
        await new Promise(r => setTimeout(r, 50));
    }
}
async function recvN(session, n, timeout = 5_000) {
    const start = Date.now();
...
async function recvAll(session, timeout = 2_000) {
...
function send(session, data) {
    if (session.closed)
        throw new Error('Bağlantı kapalı');
    const buf = typeof data === 'string' ? Buffer.from(data, 'utf8') : data;
    session.socket.write(buf);
}
// ─── Tool Tanımı ─────────────────────────────────────────────────────────────
export const pwnSessionTool = {
    name: 'pwn_session',
    description: 'PWN remote socket wrapper (pwntools remote() benzeri). Eylemler: ' +
        ...
        try {
            switch (action) {
                case 'connect': {
                    ...
                    if (!host || !port)
                        return { output: 'host ve port gerekli', isError: true };
                    const id = makeId(host, port);
                    if (sessions.has(id)) {
                        try {
                            sessions.get(id).socket.destroy();
                        }
                        catch { /* ignore */ }
                        sessions.delete(id);
                    }
                ...
                case 'sendline': {
                    ...
                    if (!session)
                        return { output: `Oturum bulunamadı: ${sessionId}. Önce connect.`, isError: true };
                ...
                case 'recv_until': {
                    const session = sessions.get(sessionId);
                    if (!session)
                        return { output: `Oturum bulunamadı: ${sessionId}`, isError: true };
                ...
                case 'close': {
                    const session = sessions.get(sessionId);
                    if (!session)
                        return { output: `Oturum yok: ${sessionId}`, isError: false };
                    try {
                        session.socket.destroy();
                    }
                    catch { /* ignore */ }
