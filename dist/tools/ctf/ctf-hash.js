...
export function detectHashFormat(hash) {
    const h = hash.trim();
    // Prefix-based
    if (/^\$2[abxy]\$/.test(h))
        return { name: 'bcrypt', hashcatMode: 3200, johnFormat: 'bcrypt', fastCrackable: false, description: 'bcrypt (yavaş, GPU önerilir)' };
    if (h.startsWith('$1$'))
        return { name: 'md5crypt', hashcatMode: 500, johnFormat: 'md5crypt', fastCrackable: false, description: 'MD5 crypt (Unix)' };
    if (h.startsWith('$5$'))
        return { name: 'sha256crypt', hashcatMode: 7400, johnFormat: 'sha256crypt', fastCrackable: false, description: 'SHA-256 crypt' };
    if (h.startsWith('$6$'))
        return { name: 'sha512crypt', hashcatMode: 1800, johnFormat: 'sha512crypt', fastCrackable: false, description: 'SHA-512 crypt' };
    if (h.startsWith('$argon2'))
        return { name: 'argon2', fastCrackable: false, description: 'Argon2 (memory-hard)' };
    if (h.startsWith('$y$'))
        return { name: 'yescrypt', fastCrackable: false, description: 'yescrypt' };
    if (h.startsWith('{SSHA}'))
        return { name: 'SSHA', hashcatMode: 111, fastCrackable: false, description: 'Salted SHA-1 (LDAP)' };
    if (/^[A-Za-z0-9+/]{27}$/.test(h))
        return { name: 'NTLM-base64?', fastCrackable: false, description: 'Base64 hash şüphesi' };
    // /etc/shadow benzeri
    if (h.includes(':')) {
        const parts = h.split(':');
...
// ─── Dahili hızlı crack ──────────────────────────────────────────────────────
function fastHash(algo, input) {
    return createHash(algo).update(input).digest('hex');
}
function tryWordlistCrack(target, format, wordlistPath, maxAttempts = 100_000) {
    const algo = (() => {
        switch (format.name) {
            case 'MD5': return 'md5';
    ...
    for (const word of words) {
        ...
        if (!clean)
            continue;
        if (fastHash(algo, clean) === targetLower)
            return { match: clean, attempts };
...
function buildExternalCommands(hash, format, wordlistPath) {
    const cmds = [];
...
export async function crackHash(hash) {
    ...
    if (format.fastCrackable) {
        ...
        if (cracked)
            notes.push(`🎯 ${attempts} denemede çözüldü`);
        else
            notes.push(`${attempts} kelime denendi, eşleşme yok`);
    }
    else {
    ...
    if (commandExists('hashcat'))
        notes.push('✓ hashcat kurulu — yukarıdaki komut hazır');
    if (commandExists('john'))
        notes.push('✓ john kurulu — yukarıdaki komut hazır');
    return {
        hash,
        format,
...
// ─── Tool Tanımı ─────────────────────────────────────────────────────────────
export const ctfHashTool = {
    name: 'ctf_hash',
    description: 'CTF hash crack — format auto-detect (MD5/SHA1/SHA224/256/384/512, NTLM, bcrypt, ' +
    ...
    async execute(input) {
        ...
        if (!hash)
            return { output: 'Hata: hash boş olamaz.', isError: true };
        ...
        if (r.cracked)
            lines.push(`│ 🎯 PLAINTEXT: ${r.cracked}`);
        else
            lines.push(`│ ✗ Çözülemedi (dahili)`);
        lines.push('└─────────────────────────────────────────────────────────────┘');
        if (r.notes.length > 0) {
            lines.push('\nNotlar:');
