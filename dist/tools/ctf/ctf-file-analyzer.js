...
export async function analyzeFile(filePath) {
    ...
    if (detectedType === 'ZIP') {
        ...
        if (encrypted.length > 0)
            notes.push(`🔒 ${encrypted.length} şifreli dosya var`);
    }
            ...
            const scan = (obj, path) => {
                ...
                if (typeof obj === 'string') {
                    ...
                    if (obj.length >= 16 && obj.length <= 2048) {
                        ...
                        if (hexLike || b64Like)
                            notes.push(`🔍 EXIF ${path}: şüpheli encoding (${hexLike ? 'hex' : 'base64'})`);
                    }
                }
                    ...
                    for (const [k, v] of Object.entries(obj)) {
                        if (k === 'thumbnail' || k === 'image')
                            continue; // binary buffer'ları atla
    ...
    if (b64Suspects.length > 0)
        notes.push(`🔍 ${b64Suspects.length} adet Base64 benzeri string bulundu`);
    return {
...
// ─── Tool Tanımı ─────────────────────────────────────────────────────────────
export const ctfFileAnalyzerTool = {
    name: 'ctf_file_analyzer',
    description: 'CTF dosya analizi: magic bytes tespiti, EXIF/metadata okuma, string extraction, ' +
    ...
    async execute(input) {
        ...
        if (result.mismatchWarning)
            lines.push(`│ ${result.mismatchWarning}`);
