...
function bigintSqrt(n) {
    if (n < 0n)
        throw new Error('negatif sayının karekökü yok');
    if (n < 2n)
        return n;
    // Newton-Raphson
    let x = n;
...
export function fermatFactor(n, maxIter = 1_000_000) {
    let a = isqrt(n);
    if (a * a < n)
        a += 1n;
...
export async function factordbLookup(n) {
    try {
        const res = await fetch(`http://factordb.com/api?query=${n.toString()}`, {
            signal: AbortSignal.timeout(8_000),
        });
        if (!res.ok)
            return { ok: false, factors: [], status: `HTTP ${res.status}` };
...
export function commonModulusAttack(n, e1, e2, c1, c2) {
...
export function smallECubeRoot(c, e = 3n) {
    if (e !== 3n)
        return null;
    // c^(1/3) tam sayı kök
    let lo = 0n, hi = c;
...
export function rsaDecrypt(c, p, q, e) {
    const n = p * q;
    const phi = (p - 1n) * (q - 1n);
...
// ─── Tool Tanımı ─────────────────────────────────────────────────────────────
export const ctfRsaTool = {
    name: 'ctf_rsa',
    description: 'CTF RSA toolkit — saldırı kütüphanesi. Eylemler: ' +
    ...
    async execute(input) {
        ...
        const get = (k) => input[k] ? parseBigInt(String(input[k])) : null;
        try {
            switch (action) {
                case 'fermat': {
                    const n = get('n');
                    if (!n)
                        return { output: 'n gerekli', isError: true };
                    const r = fermatFactor(n);
                    if (!r)
                        return { output: 'Fermat başarısız (p, q yeterince yakın değil)', isError: true };
                ...
                case 'wiener': {
                    ...
                    const e = get('e');
                    if (!n || !e)
                        return { output: 'n ve e gerekli', isError: true };
                    const d = wienerAttack(n, e);
                    if (d === null)
                        return { output: 'Wiener başarısız (d yeterince küçük değil)', isError: true };
                ...
                case 'common_modulus': {
                    ...
                    if (!n || !e1 || !e2 || !c1 || !c2)
                        return { output: 'n, e1, e2, c1, c2 gerekli', isError: true };
                    const m = commonModulusAttack(n, e1, e2, c1, c2);
                    if (m === null)
                        return { output: 'gcd(e1,e2) != 1 — saldırı uygulanamaz', isError: true };
                    let hex = m.toString(16);
                    if (hex.length % 2)
                        hex = '0' + hex;
                    const text = Buffer.from(hex, 'hex').toString('utf8');
                ...
                case 'small_e': {
                    ...
                    const e = get('e') ?? 3n;
                    if (!c)
                        return { output: 'c gerekli', isError: true };
                    const m = smallECubeRoot(c, e);
                    if (m === null)
                        return { output: 'Tam küp kök bulunamadı (m^e == c değil — padding olabilir)', isError: true };
                    let hex = m.toString(16);
                    if (hex.length % 2)
                        hex = '0' + hex;
                ...
                case 'decrypt': {
                    ...
                    if (!p || !q || !e || !c)
                        return { output: 'p, q, e, c gerekli', isError: true };
                    const r = rsaDecrypt(c, p, q, e);
                    if (!r)
                        return { output: 'Decrypt başarısız (modInverse yok)', isError: true };
                ...
                case 'auto': {
                    ...
                    if (!n || !e)
                        return { output: 'n ve e gerekli', isError: true };
                    const lines = ['# Otomatik RSA saldırı zinciri'];
                    // 1. factordb
