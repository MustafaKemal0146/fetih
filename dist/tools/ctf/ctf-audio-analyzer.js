...
function detectAudioFormat(buf) {
    ...
    if (head4 === 'OggS')
        return 'OGG';
    if (head4.startsWith('ID3') || (buf[0] === 0xFF && (buf[1] & 0xE0) === 0xE0))
        return 'MP3';
    if (head4to8 === 'ftyp')
        return 'M4A';
    return 'UNKNOWN';
...
function morseDecode(buf) {
    ...
    const states = []; // 1 = sinyal var, 0 = sessizlik
    let maxRms = 0;
    const rmsValues = [];
    ...
    const decoded = words.map(word => word.split(/\s+/).map(letter => MORSE_TABLE[letter] ?? '').join('')).join(' ');
...
// ─── 5. Spektogram + Vision ──────────────────────────────────────────────────
async function spectrogramVision(filePath) {
    ...
    try {
        const visionResult = await imageAnalyzeTool.execute({
            path: specPath,
            prompt: 'Bu bir ses dosyasının spektogramı. Görselde herhangi bir okunabilir METİN ' +
...
export async function analyzeAudio(filePath) {
    ...
    if (!bestFlag) {
        ...
        if (detectedFormat === 'WAV')
            recommendations.push('steghide extract -sf <file> deneyin');
    }
    return { path: filePath, detectedFormat, modules, bestFlag, recommendations };
}
// ─── Tool Tanımı ─────────────────────────────────────────────────────────────
export const ctfAudioAnalyzerTool = {
    name: 'ctf_audio_analyzer',
    description: 'CTF ses dosyası analizi: WAV LSB steganografi, DTMF tone decode (Goertzel), ' +
