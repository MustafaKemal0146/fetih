...
function listZipEntries(buf, maxEntries = 1000) {
    const entries = [];
    let i = 0;
...
function detectArchiveType(entries) {
    if (entries.some(e => e.name === 'AndroidManifest.xml' || e.name === 'classes.dex'))
        return 'APK';
    if (entries.some(e => e.name.startsWith('Payload/') && e.name.endsWith('.app/')))
        return 'IPA';
    if (entries.some(e => e.name === 'META-INF/MANIFEST.MF'))
        return 'JAR';
    return 'UNKNOWN';
}
function analyzeMobile(buf) {
    const result = {
...
function runApktool(filePath, outputDir) {
...
function runJadx(filePath, outputDir) {
...
// ─── Tool Tanımı ─────────────────────────────────────────────────────────────
export const ctfMobileTool = {
    name: 'ctf_mobile',
    description: 'CTF Mobile (APK/IPA) analizi. Eylemler: analyze (ZIP entries + manifest tespit + strings), ' +
    ...
    async execute(input) {
        ...
        if (action === 'analyze') {
            ...
            const interesting = r.entries.filter(e => /\.(xml|json|plist|properties|cfg|conf|txt|md)$/i.test(e.name) || e.name.includes('config')).slice(0, 20);
