 ...
 */
export const shodanTool = {
    name: 'shodan',
    description: 'Shodan API ile OSINT ve ağ keşfi yapar. IP adresi sorgulama, cihaz arama ve zafiyet tespiti için kullanılır. ' +
    ...
    async execute(input) {
        ...
        try {
            switch (action) {
                case 'host':
                    if (!query)
                        return { output: 'Hata: "host" işlemi için bir IP adresi gereklidir.', isError: true };
                    url = `${baseUrl}/shodan/host/${query}?${params.toString()}`;
                    break;
                case 'search':
                    if (!query)
                        return { output: 'Hata: "search" işlemi için bir arama terimi gereklidir.', isError: true };
