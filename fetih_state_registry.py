"""Registration/lookup mechanisms layered on top of SessionDB: Telegram DM
topic bindings and cross-platform session handoff state.
...
``self._lock``, ``self._execute_write`` and other SessionDB methods)
...
class SessionRegistryMixin:
    """Telegram DM topic-mode registry and cross-platform handoff registry."""

    # ── Telegram DM topic-mode registry ─────────────────────────────────

    def apply_telegram_topic_migration(self) -> None:
        """Create Telegram DM topic-mode tables on explicit /topic opt-in.

        This migration is deliberately not part of automatic SessionDB startup
        reconciliation. Operators must be able to upgrade FETIH, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one FETIH session.

        A FETIH session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a FETIH session is already bound to any Telegram DM topic.
