# Contributing to FETIH

Thank you for contributing to FETIH! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want FETIH to work everywhere.
...
Bundled skills (in `skills/`) ship with every FETIH install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `fetih skills browse` (labeled "official") and install it with `fetih skills install` (no third-party warning, builtin trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [FETIH Discord](https://discord.gg/FETIH). Users can install it with `fetih skills install`.

---

## Memory Providers: Ship as a Standalone Plugin

**We are no longer accepting new memory providers into this repo.** The set of built-in providers under `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) is closed. If you want to add a new memory backend, publish it as a **standalone plugin repo** that users install into `~/.fetih/plugins/` (or via a pip entry point).

Standalone memory plugins:

- Implement the same `MemoryProvider` ABC (`agent/memory_provider.py`) — `sync_turn`, `prefetch`, `shutdown`, and optionally `post_setup(fetih_home, config)` for setup-wizard integration
...
- Integrate with `fetih memory setup` via `post_setup()` — no need to touch core code
...
```bash
git clone --recurse-submodules https://github.com/MustafaKemal0146/fetih.git
cd fetih
...
```bash
mkdir -p ~/.fetih/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.fetih/config.yaml
touch ~/.fetih/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.fetih/.env
```
...
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/fetih" ~/.local/bin/fetih
...
├── cli.py                    # FETIHCLI class — interactive TUI, prompt_toolkit integration
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (fetih-cli, fetih-telegram, etc.)
├── fetih_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│
├── fetih_cli/               # CLI command implementations
│   ├── main.py                   # Entry point, argument parsing, command dispatch
│   ├── config.py                 # Config management, migration, env var definitions
│   ├── setup.py                  # Interactive setup wizard
│   ├── auth.py                   # Provider resolution, OAuth, FETIH Portal
...
│
├── skills/                   # Bundled skills (copied to ~/.fetih/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── tests/                    # Test suite
├── website/                  # Documentation site (github.com/MustafaKemal0146/fetih)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.fetih/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.fetih/`)

| Path | Purpose |
|------|---------|
| `~/.fetih/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.fetih/.env` | API keys and secrets |
| `~/.fetih/auth.json` | OAuth credentials (FETIH Portal) |
| `~/.fetih/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.fetih/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.fetih/state.db` | SQLite session database |
| `~/.fetih/sessions/` | JSON session logs |
| `~/.fetih/cron/` | Scheduled job data |
| `~/.fetih/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`fetih_state.py`) with full-text search and unique session titles. JSON logs go to `~/.fetih/sessions/`.
- **Ephemeral injection**: System prompts and prefill messages are injected at API call time, never persisted to the database or logs.
- **Provider abstraction**: The agent works with any OpenAI-compatible API. Provider resolution happens at init time (FETIH Portal OAuth, OpenRouter API key, or custom endpoint).
...
(for example `_FETIH_CORE_TOOLS` or a dedicated toolset); otherwise the tool
...
Four fields are supported under `metadata.fetih`:
...
The user may skip setup and keep loading the skill. FETIH only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `fetih setup` or update `~/.fetih/.env` locally.
...
2. **Tools referenced in SKILL.md prose must be native FETIH tools or MCP servers the skill explicitly expects.** When the skill needs a capability, point at the proper tool by name in backticks: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, `` `patch` ``, `` `search_files` ``, `` `vision_analyze` ``, `` `browser_navigate` ``, `` `delegate_task` ``, `` `image_generate` ``, `` `text_to_speech` ``, `` `cronjob` ``, `` `memory` ``, `` `skill_view` ``, `` `todo` ``, `` `execute_code` ``.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "FETIH" is the secondary collaborator. If the contributor's commit shows "FETIH" as author because they used FETIH to draft the skill, replace it with their actual name — credit the human, not the tool.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing FETIH tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `fetih --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

FETIH uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.fetih/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `fetih_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `fetih_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

FETIH runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
1. **Never call `os.kill(pid, 0)` for liveness checks.** `os.kill(pid, 0)`
   ...
   **Preferred:** use `psutil` (a core dependency — always available):
   ...
   If you specifically need the fetih wrapper (it has a stdlib fallback
...
2. **Use `shutil.which()` before shelling out — don't assume Windows has
   ...
   `fetih_cli/gateway.py::_scan_gateway_pids` for the pattern.
...
10. **Detached background daemons on Windows need `pythonw.exe`, NOT
    ...
    See `fetih_cli/gateway_windows.py::_spawn_detached` for the reference
    implementation.
...
16. **Two different quoting schemes in one command line.** `subprocess.run
    ...
    helpers and never cross them. See `fetih_cli/gateway_windows.py::
...
FETIH has terminal access. Security matters.
...
2. **Test manually**: Run `fetih` and exercise the code path you changed
...
- Use [GitHub Issues](https://github.com/MustafaKemal0146/fetih/issues)
- Include: OS, Python version, FETIH version (`fetih version`), full error traceback
...
- **Discord**: [discord.gg/FETIH](https://discord.gg/FETIH) — for questions, showcasing projects, and sharing skills
