...
_FETIH_CORE_TOOLS = [
    ...
    # spawned as a kanban worker (FETIH_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `fetih tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (FETIH_KANBAN_TASK env "
    ...
    # ==========================================================================
    # Full FETIH toolsets (CLI + messaging platforms)
    #
    ...
    "fetih-cli": {
        ...
        "tools": _FETIH_CORE_TOOLS,
        "includes": []
    },

    "fetih-cron": {
        # Mirrors fetih-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `fetih tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as fetih-cli; gated by `fetih tools`",
        "tools": _FETIH_CORE_TOOLS,
        "includes": []
    },
    ...
    "fetih-email": {
        "description": "Email bot toolset - interact with FETIH via email (IMAP/SMTP)",
        "tools": _FETIH_CORE_TOOLS,
        "includes": []
    },
    ...
    "fetih-sms": {
        "description": "SMS bot toolset - interact with FETIH via SMS (Twilio)",
        "tools": _FETIH_CORE_TOOLS,
        "includes": []
    },
    ...
    "fetih-gateway": {
        ...
        "tools": [],
        "includes": ["fetih-telegram", "fetih-discord", "fetih-whatsapp", "fetih-slack", "fetih-signal", "fetih-bluebubbles", "fetih-homeassistant", "fetih-email", "fetih-sms", "fetih-mattermost", "fetih-matrix", "fetih-dingtalk", "fetih-feishu", "fetih-wecom", "fetih-wecom-callback", "fetih-weixin", "fetih-qqbot", "fetih-webhook", "fetih-yuanbao"]
    }
}
...
def resolve_toolset(name: str, visited: Set[str] = None) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (fetih-<name>).
        # Gives them _FETIH_CORE_TOOLS plus any tools the plugin registered
        # into a toolset matching the platform name.
        if name.startswith("fetih-"):
            platform_name = name[len("fetih-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_FETIH_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
