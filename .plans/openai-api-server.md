# OpenAI-Compatible API Server for FETIH
...
SSE streaming. By exposing this endpoint, fetih becomes instantly
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.fetih/.env`
2. Run `fetih gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with fetih through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns fetih as a model) |
...
Authorization: Bearer fetih-api-key-here
...
But fetih sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "fetih"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured fetih model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of fetih's core prompt
...
Once implemented, fetih works as a drop-in backend for:
