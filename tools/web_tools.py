...
Backend is selected during ``fetih tools`` setup (web.backend in config.yaml).
When available, FETIH can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.fetih/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``fetih tools``).
...
def _is_nous_auxiliary_client(client: Any) -> bool:
    """Return True when the resolved auxiliary backend is FETIH Portal."""
    from urllib.parse import urlparse

    base_url = str(getattr(client, "base_url", "") or "")
    host = (urlparse(base_url).hostname or "").lower()
    return host == "github.com/MustafaKemal0146/fetih" or host.endswith(".github.com/MustafaKemal0146/fetih")
...
if __name__ == "__main__":
    ...
    if not nous_available:
        ...
        print("Set OPENROUTER_API_KEY, configure FETIH Portal, or set OPENAI_BASE_URL + OPENAI_API_KEY")
