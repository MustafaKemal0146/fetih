...
LOCAL_STT_COMMAND_ENV = "FETIH_LOCAL_STT_COMMAND"
LOCAL_STT_LANGUAGE_ENV = "FETIH_LOCAL_STT_LANGUAGE"
    ...
    if explicit:
        if provider == "local":
            ...
            logger.warning(
                ...
                "(install faster-whisper or set FETIH_LOCAL_STT_COMMAND)"
            )
            return "none"
...
def _transcribe_local_command(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="fetih-local-stt-") as output_dir:
...
def _transcribe_xai(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    if not api_key:
        return {
            "success": False,
            "transcript": "",
            "error": "No xAI credentials found. Configure xAI OAuth in `fetih model` or set XAI_API_KEY",
        }
    ...
    language = str(
        ...
        or os.getenv("FETIH_LOCAL_STT_LANGUAGE")
        or DEFAULT_LOCAL_STT_LANGUAGE
    ).strip()
