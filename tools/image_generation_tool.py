...
selectable via ``fetih tools`` → Image Generation; the active model is
    ...
    try:
        ...
        if status is not None and 400 <= status < 500:
            raise ValueError(
                ...
                f"the FETIH Portal's FAL proxy. Either:\n"
                f"  • Set FAL_KEY in your environment to use FAL.ai directly, or\n"
                f"  • Pick a different model via `fetih tools` → Image Generation."
            ) from exc
        raise
...
def _build_no_backend_setup_message() -> str:
    ...
    if managed_nous_tools_enabled():
        lines.append(
            ...
            "gateway enabled (`fetih setup`)"
        )
    lines.append(
        "  3. Configure a different image_gen provider via `fetih tools` "
        "→ Image Generation (run `fetih plugins list` to see installed "
        "backends)"
    )
    return "\n".join(lines)
...
def check_image_generation_requirements() -> bool:
    ...
    the historical behavior: shipping fetih with a FAL key configured
