...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("FETIH_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return os.getenv("FETIH_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set FETIH_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind FETIH_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds FETIH_SESSION_PLATFORM via
    ...
    """
    if env_var_enabled("FETIH_CRON_SESSION"):
        return False
...
# via shell expansions like $HOME or $FETIH_HOME.
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_FETIH_ENV_PATH = (
    r'(?:~\/\.fetih/|'
    r'(?:\$home|\$\{home\})/\.fetih/|'
    r'(?:\$fetih_home|\$\{fetih_home\})/)'
    r'\.env\b'
)
...
DANGEROUS_PATTERNS = [
    ...
    (r'\bfetih\s+gateway\s+(stop|restart)\b', "stop/restart fetih gateway (kills running agents)"),
    (r'\bfetih\s+update\b', "fetih update (restarts gateway, kills running agents)"),
    # Gateway protection: never start gateway outside systemd management
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart fetih-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart fetih-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(fetih|gateway|cli\.py)\b', "kill fetih/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill fetih` but not
    # `kill -9 $(pgrep -f fetih)` because the substitution is opaque
...
def check_dangerous_command(command: str, env_type: str,
    ...
    if is_truthy_value(os.getenv("FETIH_YOLO_MODE")) or is_current_session_yolo_enabled():
        return {"approved": True, "message": None}
    ...
    is_cli = env_var_enabled("FETIH_INTERACTIVE")
    ...
    if is_gateway or env_var_enabled("FETIH_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    if is_truthy_value(os.getenv("FETIH_YOLO_MODE")) or is_current_session_yolo_enabled() or approval_mode == "off":
        return {"approved": True, "message": None}

    is_cli = env_var_enabled("FETIH_INTERACTIVE")
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("FETIH_EXEC_ASK")
