...
# Must match fetih_cli.runtime_provider.RUNTIME_PROVIDER_TYPE_CUSTOM.
...
# (fetih-* prefixed), and scenario toolsets.
#
...
def _expand_parent_toolsets(parent_toolsets: set) -> set:
    ...
    When a parent uses a composite toolset like ``fetih-cli`` (which bundles
...
def _build_child_agent(
    ...
    model on OpenRouter while the parent runs on FETIH Portal).
    """
    ...
    if toolsets:
        ...
        # Expand composite toolsets (e.g. fetih-cli) so that individual
    ...
    # Extract parent's API key so subagents inherit auth (e.g. FETIH Portal).
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under ``~/.fetih/logs/subagent-<sid>-<ts>.log``
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'fetih auth'."
        )
...
def _load_config() -> dict:
    ...
    to the persistent config (fetih_cli/config.py load_config()) so that
        ...
        "properties": {
            ...
            "acp_command": {
                ...
                "description": (
                    ...
                    "Leave empty to use the parent's default transport (FETIH subagents)."
                ),
            },
            "acp_args": {
                "type": "array",
