...
~/.fetih/skills/. Existing skills (bundled, hub-installed, or user-created)
...
def _guard_agent_created_enabled() -> bool:
    ...
    on via `fetih config set skills.guard_agent_created true`.
    """
    try:
...
# All skills live in ~/.fetih/skills/ (single source of truth)
    ...
    try:
        ...
        if rec.get("pinned"):
            return (
                ...
                f"`fetih curator unpin {name}` if they want to delete it. "
...
def _find_skill(name: str) -> Optional[Dict[str, Any]]:
    ...
    Searches the local skills dir (~/.fetih/skills/) first, then any
...
SKILL_MANAGE_SCHEMA = {
    ...
    "description": (
        ...
        f"New skills go to {display_fetih_home()}/skills/; existing skills can be modified wherever they live.\n\n"
        ...
        "will refuse with a message pointing the user to `fetih curator unpin <name>`. "
