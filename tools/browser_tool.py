...
def _browser_candidate_path_dirs() -> list[str]:
    ...
    fetih_node_bin = str(fetih_home / "node" / "bin")
    fetih_node_root = str(fetih_home / "node")
    fetih_nm_bin = str(fetih_home / "node_modules" / ".bin")
    return [fetih_node_bin, fetih_node_root, fetih_nm_bin, *list(_discover_homebrew_node_dirs()), *_SANE_PATH_DIRS]
...
# plugins drop in under ``~/.fetih/plugins/browser/<vendor>/``.
...
def _get_cloud_provider() -> Optional[CloudBrowserProvider]:
    ...
    browser plugins (``~/.fetih/plugins/browser/<vendor>/``) participate
...
def _lightpanda_fallback_reason(engine: str, command: str, result: Dict[str, Any]) -> Optional[str]:
    ...
    the fallback result so CLI/TUI/gateway users can see when FETIH silently
...
def _run_chrome_fallback_command(
    ...
    def _run_tmp(cmd: str, cmd_args: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            #   in the FETIH CLI where sys.stdout and sys.stderr both report
...
def _socket_safe_tmpdir() -> str:
    ...
    append ``agent-browser-fetih_…`` the resulting socket path exceeds the
...
def _emergency_cleanup_all_sessions():
    ...
    crashed fetih processes — this way every clean fetih exit sweeps
    ...
    # Sweep orphans from other crashed fetih processes.  Safe even if we
    # never used the browser — uses owner_pid liveness to avoid reaping
    # daemons owned by other live fetih processes.
    try:
...
def _write_owner_pid(socket_dir: str, session_name: str) -> None:
    """Record the current fetih PID as the owner of a browser socket dir.

    Written atomically to ``<socket_dir>/<session_name>.owner_pid`` so the
    orphan reaper can distinguish daemons owned by a live fetih process
...
def _reap_orphaned_browser_sessions():
    ...
    daemons whose owning fetih process is no longer alive.

    Ownership detection priority:
      1. ``<session>.owner_pid`` file (written by current code) — if the
         referenced fetih PID is alive, leave the daemon alone regardless
         of whether it's in *this* process's ``_active_sessions``.  This is
         cross-process safe: two concurrent fetih instances won't reap each
         other's daemons.
    ...
    socket_dirs += glob.glob(os.path.join(tmpdir, "agent-browser-fetih_*"))
    ...
    for socket_dir in socket_dirs:
        ...
        if owner_alive is True:
            # Owner is alive — this session belongs to a live fetih process.
            continue
...
def _find_agent_browser() -> str:
    ...
    Checks in order: current PATH, Homebrew/common bin dirs, FETIH-managed
    ...
    # Build an extended search PATH including FETIH-managed Node, macOS
    ...
    try:
        ...
        if ensure_dependency("browser"):
            ...
            if not recheck:
                fetih_nm = str(get_fetih_home() / "node_modules" / ".bin")
                recheck = shutil.which("agent-browser", path=fetih_nm)
            if not recheck:
                fetih_node_bin = str(get_fetih_home() / "node" / "bin")
                recheck = shutil.which("agent-browser", path=fetih_node_bin)
            if not recheck:
                fetih_node_root = str(get_fetih_home() / "node")
                recheck = shutil.which("agent-browser", path=fetih_node_root)
            if recheck:
...
def _run_browser_command(
    ...
    try:
        ...
        os.makedirs(task_socket_dir, mode=0o700, exist_ok=True)
        # Record this fetih PID as the session owner (cross-process safe
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        recordings_dir = fetih_home / "browser_recordings"
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> str:
    ...
    from fetih_constants import get_fetih_dir
    screenshots_dir = get_fetih_dir("cache/screenshots", "browser_screenshots")
        ...
        try:
            ...
            if _requires_real_termux_browser_install(browser_cmd):
                ...
                if _running_in_docker():
                    ...
                    print("       docker pull ghcr.io/mustafakemal0146/fetih:latest")
                else:
