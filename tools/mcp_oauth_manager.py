...
`mcp_tool.py`, and `fetih_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# FETIHMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_fetih_provider_class() -> Optional[type]:
    ...
    class FETIHMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(self, *args: Any, server_name: str = "", **kwargs: Any):
            ...
            self._fetih_server_name = server_name
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`FETIHTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, FETIHTokenStorage)
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._fetih_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import FETIHTokenStorage
                        if isinstance(storage, FETIHTokenStorage):
                            storage.save_oauth_metadata(asm)
                        logger.debug(
                            "MCP OAuth '%s': pre-flight ASM discovered "
                            "token_endpoint=%s",
                            self._fetih_server_name, asm.token_endpoint,
                        )
                        break
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import FETIHTokenStorage
            if not isinstance(storage, FETIHTokenStorage):
                return
    ...
    return FETIHMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_FETIH_PROVIDER_CLS: Optional[type] = _make_fetih_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`FETIHMCPOAuthProvider` directly using the helpers
        ...
        storage = FETIHTokenStorage(server_name)
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``fetih mcp remove <name>`` and (indirectly) by
        ``fetih mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
