#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the FETIH Skills Hub.
...
Used by fetih_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            fetih_meta = metadata.get("fetih", metadata.get("hermes", {}))
            if isinstance(fetih_meta, dict):
                tags = fetih_meta.get("tags", [])
        if not tags:
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by FETIH) but not activated
    by default — they don't appear in the system prompt and aren't copied to
    ~/.fetih/skills/ during setup.  They are discoverable via the Skills Hub
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                fetih_meta = meta_block.get("fetih", {})
                if isinstance(fetih_meta, dict):
                    tags = fetih_meta.get("tags", [])
...
# ---------------------------------------------------------------------------
# FETIH centralized index source
# ---------------------------------------------------------------------------

FETIH_INDEX_URL = "https://github.com/MustafaKemal0146/fetih/docs/api/skills-index.json"
FETIH_INDEX_CACHE_FILE = INDEX_CACHE_DIR / "fetih-index.json"
FETIH_INDEX_TTL = 6 * 3600  # 6 hours


def _load_fetih_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for FETIH_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    if FETIH_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - FETIH_INDEX_CACHE_FILE.stat().st_mtime
            if age < FETIH_INDEX_TTL:
                return json.loads(FETIH_INDEX_CACHE_FILE.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from docs site
    try:
        resp = httpx.get(FETIH_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
            logger.debug("FETIH index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("FETIH index fetch failed: %s", e)
        return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        FETIH_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        FETIH_INDEX_CACHE_FILE.write_text(json.dumps(data))
    except OSError:
        pass
...
class FETIHIndexSource(SkillSource):
    """Skill source backed by the centralized FETIH Skills Index.
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "fetih-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "fetih-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        FETIHIndexSource(auth=auth), # Centralized index (search + resolved install paths)
