...
can break on OS updates. Pin the installed version via `FETIH_CUA_DRIVER_
...
PINNED_CUA_DRIVER_VERSION = os.environ.get("FETIH_CUA_DRIVER_VERSION", "0.5.0")

_CUA_DRIVER_CMD = os.environ.get("FETIH_CUA_DRIVER_CMD", "cua-driver")
...
def cua_driver_install_hint() -> str:
    return (
        ...
        "  fetih computer-use install\n"
        "Or run the upstream installer directly:\n"
        '  /bin/bash -c "$(curl -fsSL '
        'https://raw.githubusercontent.com/trycua/cua/main/libs/cua-driver/scripts/install.sh)"\n'
        "Or run `fetih tools` and enable the Computer Use toolset to install it automatically."
    )
...
class CuaDriverBackend(ComputerUseBackend):
    ...
    def capture(self, mode: str = "som", app: Optional[str] = None) -> CaptureResult:
        ...
        Maps fetih `capture(mode, app)` → cua-driver `list_windows` +
