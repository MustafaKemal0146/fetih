...
Lets the LLM write a Python script that calls FETIH tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `fetih_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `fetih_tools.py` with file-based RPC stubs
...
# available on every platform FETIH itself runs on.
...
def generate_fetih_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the fetih_tools.py stub module.
...
"""Auto-generated FETIH tools RPC stubs."""
...
def _connect():
    ...
    FETIH_RPC_SOCKET can be either:
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["FETIH_RPC_SOCKET"]
...
"""Auto-generated FETIH tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("FETIH_RPC_DIR") or os.path.join(tempfile.gettempdir(), "fetih_rpc")
_seq = 0
...
def _execute_remote(
    ...
    The script and the generated fetih_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/fetih_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_fetih_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/fetih_tools.py", tools_src)
        ...
        env_prefix = (
            f"FETIH_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("FETIH_TIMEZONE", "").strip()
        if tz:
            env_prefix += f" TZ={tz}"
...
def execute_code(
    ...
    # --- Set up temp directory with fetih_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="fetih_sandbox_")
    ...
    # selector from FETIH_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"fetih_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated fetih_tools module.
        ...
        tools_src = generate_fetih_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "fetih_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        #   connect.  FETIH_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        child_env["FETIH_RPC_SOCKET"] = rpc_endpoint
        ...
        # Ensure the fetih-agent root is importable in the sandbox so
        # repo-root modules are available to child scripts.  We also prepend
        # the staging tmpdir so ``from fetih_tools import ...`` resolves even
        # when the subprocess CWD is not tmpdir (project mode).
        _fetih_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        _pp_parts = [tmpdir, _fetih_root]
        if _existing_pp:
            _pp_parts.append(_existing_pp)
        child_env["PYTHONPATH"] = os.pathsep.join(_pp_parts)
        # Inject user's configured timezone so datetime.now() in sandboxed
        # code reflects the correct wall-clock time.  Only TZ is set —
        # FETIH_TIMEZONE is an internal FETIH setting and must not leak
        # into child processes.
        _tz_name = os.getenv("FETIH_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("FETIH_TIMEZONE", None)

        # Per-profile HOME isolation: redirect system tool configs into
        # {FETIH_HOME}/home/ when that directory exists.
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.fetih/.env')).
...
def _load_config() -> dict:
    ...
    ``fetih --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (fetih-agent's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``fetih tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and fetih-agent's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.fetih/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that can call FETIH tools programmatically. "
        ...
        f"Available via `from fetih_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        ...
        "Also available (no import needed — built into fetih_tools):\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from fetih_tools import {import_str}` "
