...
This module provides the glue:
    - ``FETIHTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "FETIH"
"""
...
def _get_token_dir() -> Path:
    ...
    Uses FETIH_HOME so each profile gets its own OAuth tokens.
    Layout: ``FETIH_HOME/mcp-tokens/``
    """
    try:
        from fetih_constants import get_fetih_home
        base = Path(get_fetih_home())
    except ImportError:
        base = Path(os.environ.get("FETIH_HOME", str(Path.home() / ".fetih")))
...
# ---------------------------------------------------------------------------
# FETIHTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class FETIHTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        FETIH_HOME/mcp-tokens/<server_name>.json         -- tokens
        FETIH_HOME/mcp-tokens/<server_name>.client.json   -- client info
        FETIH_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # FETIH records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to FETIH.</p></body></html>"
            ) if code else (
...
async def _redirect_handler(authorization_url: str) -> None:
    ...
    if _oauth_port and (os.getenv("SSH_CLIENT") or os.getenv("SSH_TTY")):
        print(
            ...
            f"\n"
            f"  Then open the URL above. See: https://github.com/MustafaKemal0146/fetih/docs/guides/oauth-over-ssh\n",
            file=sys.stderr,
        )
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "FETIH")
    scope = cfg.get("scope")
