...
it is automatically downloaded from GitHub releases to $FETIH_HOME/bin/tirith.
...
def _get_fetih_home() -> str:
    """Return the FETIH home directory, respecting FETIH_HOME env var."""
    return str(get_fetih_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_fetih_home(), ".tirith-install-failed")
...
def _fetih_bin_dir() -> str:
    """Return $FETIH_HOME/bin, creating it if needed."""
    d = os.path.join(_get_fetih_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $FETIH_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_fetih_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $FETIH_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $FETIH_HOME/bin/tirith
    ...
    fetih_bin = os.path.join(_fetih_bin_dir(), "tirith")
    if os.path.isfile(fetih_bin) and os.access(fetih_bin, os.X_OK):
        _resolved_path = fetih_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return fetih_bin
