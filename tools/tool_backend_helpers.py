...
def managed_nous_tools_enabled() -> bool:
    ...
    """
    try:
        from fetih_cli.auth import get_nous_auth_status

        status = get_nous_auth_status()
        if not status.get("logged_in"):
            return False

        from fetih_cli.models import check_nous_free_tier
...
def fal_key_is_configured() -> bool:
    ...
    Consults both ``os.environ`` and ``~/.fetih/.env`` (via
    ``fetih_cli.config.get_env_value`` when available) so tool-side
