...
Uses the Vercel Python SDK to run commands in cloud sandboxes through FETIH'
shared ``BaseEnvironment`` shell contract. When persistence is enabled, the
backend stores task-scoped snapshot metadata under ``FETIH_HOME`` and restores
...
def _snapshot_store_path() -> Path:
    return get_fetih_home() / _SNAPSHOT_STORE_NAME
...
class VercelSandboxEnvironment(BaseEnvironment):
    ...
    def _configure_attached_sandbox(self, *, requested_cwd: str) -> None:
        ...
        if self._remote_home == "/":
            container_base = "/.fetih"
        else:
            container_base = f"{self._remote_home.rstrip('/')}/.fetih"
    ...
    def _vercel_bulk_download(self, dest_tar_path: Path) -> None:
        remote_fetih = (
            "/.fetih"
            if self._remote_home == "/"
            else f"{self._remote_home.rstrip('/')}/.fetih"
        )
        archive_member = remote_fetih.lstrip("/")
        remote_tar = f"/tmp/.fetih_sync.{os.getpid()}.tar"
        sandbox = self._sandbox
        if sandbox is None:
            raise RuntimeError("Vercel sandbox is not attached")
