...
provider** — turn on a backend by enabling a plugin (``fetih plugins
enable video_gen/<name>``) and selecting it in ``fetih tools`` → Video
Generation.
...
VIDEO_GENERATE_SCHEMA: Dict[str, Any] = {
    ...
    # skill at github/fetih-agent-dev/references/dynamic-tool-schemas.md.
        ...
        "properties": {
            ...
            "model": {
                ...
                "description": (
                    ...
                    "configured ``video_gen.model`` (set via `fetih tools` "
...
def _missing_provider_error(configured: Optional[str]) -> str:
    if configured:
        msg = (
            ...
            f"registered that name. Run `fetih plugins list` to see "
            f"installed video gen backends, or `fetih tools` → Video "
    ...
    msg = (
        "No video generation backend is configured. Run `fetih tools` → "
...
# `fetih tools` or `/skills`, the schema rebuilds automatically.
...
_GENERIC_DESCRIPTION = (
    ...
    "`fetih tools` → Video Generation; the agent does not pick them. "
...
def _build_dynamic_video_schema() -> Dict[str, Any]:
    ...
    if not configured:
        parts.append(
            ...
            "until the user picks one via `fetih tools` → Video Generation."
        )
        return {"description": "\n".join(parts)}
