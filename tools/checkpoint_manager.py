...
-----------------------------------------------------------------------------
    ...
    ~/.fetih/checkpoints/
        store/                          — single bare-ish git repo
            ...
            refs/fetih/<hash16>        — per-project branch tip
...
CHECKPOINT_BASE = get_fetih_home() / "checkpoints"

# Single shared store directory under CHECKPOINT_BASE.
_STORE_DIRNAME = "store"
_REFS_PREFIX = "refs/fetih"
...
DEFAULT_EXCLUDES = [
    ...
    ".hg/",
    ".svn/",
    # Worktrees (FETIH convention — don't recursively snapshot siblings)
    ".worktrees/",
...
_GIT_TIMEOUT: int = max(10, min(60, int(os.getenv("FETIH_CHECKPOINT_TIMEOUT", "30"))))
...
def _git_env(
    ...
    The shared store is internal FETIH infrastructure — it must NOT inherit
...
def _migrate_legacy_store(base: Path) -> Optional[Path]:
    ...
    cleared with ``fetih checkpoints clear-legacy``.
    ...
    if legacy_root is not None:
        logger.info(
            ...
            "Clear with `fetih checkpoints clear-legacy` when safe.",
            legacy_root,
        )
    return legacy_root
...
def _init_store(store: Path, working_dir: str) -> Optional[str]:
    ...
    cfg_wd = str(base)
    _run_git(["config", "user.email", "fetih@local"], store, cfg_wd)
    _run_git(["config", "user.name", "FETIH Checkpoint"], store, cfg_wd)
...
# look for ``HEAD``/``info/exclude``/``FETIH_WORKDIR``.  In v2 we also write
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    # Compat marker for tests that look at FETIH_WORKDIR
    # (write in addition to the JSON metadata).
    try:
        (shadow_repo / "FETIH_WORKDIR").write_text(
...
# ---------------------------------------------------------------------------
# Public helpers for `fetih checkpoints` CLI
# ---------------------------------------------------------------------------
