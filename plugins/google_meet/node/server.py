...
Launched by ``fetih meet node run``.

Token handling
--------------
On first boot we mint 32 hex chars of entropy and persist them at
``$FETIH_HOME/workspace/meetings/node_token.json``. Subsequent boots
...
via ``fetih meet node approve <name> <url> <token>``.
...
def _default_token_path() -> Path:
    return Path(get_fetih_home()) / "workspace" / "meetings" / "node_token.json"
...
class NodeServer:
    ...
    def __init__(
        ...
        display_name: str = "fetih-meet-node",
    ) -> None:
        self.host = host
        self.port = port
