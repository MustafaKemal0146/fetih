...
Let the fetih agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where fetih runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `fetih meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `fetih meet` CLI |
...
| `cli.py` | `fetih meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$FETIH_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `fetih meet node {run,list,approve,remove,status,ping}` |
...
fetih meet install                                      # pip + Chromium
fetih meet setup                                        # preflight
fetih meet auth                                         # optional
fetih meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
fetih meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.fetih/.env
fetih meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
fetih meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
fetih meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.fetih/.env
fetih meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, fetih will **not** switch your system audio input automatically — the
...
fetih plugins enable google_meet
fetih meet node run --display-name my-mac --host 0.0.0.0 --port 18789
