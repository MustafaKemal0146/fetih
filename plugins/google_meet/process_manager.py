...
session-scoped state file under ``$FETIH_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $FETIH_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_fetih_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this fetih install, leave it first —
    ...
    env = os.environ.copy()
    env["FETIH_MEET_URL"] = url
    env["FETIH_MEET_OUT_DIR"] = str(out)
    env["FETIH_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["FETIH_MEET_HEADED"] = "1"
    if auth_state:
        env["FETIH_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["FETIH_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if FETIH_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["FETIH_MEET_MODE"] = mode
    if realtime_model:
        env["FETIH_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["FETIH_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["FETIH_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["FETIH_MEET_REALTIME_KEY"] = realtime_api_key
