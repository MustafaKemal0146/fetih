# FETIH Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: fetih-achievements plugin
...
### `GET /api/plugins/fetih-achievements/achievements`
...
### `POST /api/plugins/fetih-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/fetih-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/fetih-achievements/overview`
...
- Persist plugin data under:
  - `~/.fetih/plugins/fetih-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.fetih/plugins/fetih-achievements/scan_checkpoint.json`
...
- `~/.fetih/plugins/fetih-achievements/`
