...
- Per-user tokens (keyed by sender email):
    ``${FETIH_HOME}/google_chat_user_tokens/<sanitized_email>.json``
- Legacy single-user token (fallback, untouched for backward compat):
    ``${FETIH_HOME}/google_chat_user_token.json``
- Per-user pending OAuth state during /setup-files start → exchange:
    ``${FETIH_HOME}/google_chat_user_oauth_pending/<sanitized_email>.json``
- Legacy pending state:
    ``${FETIH_HOME}/google_chat_user_oauth_pending.json``
- Shared OAuth client (one per host):
    ``${FETIH_HOME}/google_chat_user_client_secret.json``
"""
...
# Use the project's FETIH_HOME helper so the token follows the user's
# profile (e.g. tests can override via FETIH_HOME=/tmp/...).
try:
    from fetih_constants import display_fetih_home, get_fetih_home
except (ModuleNotFoundError, ImportError):
    # Fallback for environments where fetih_constants isn't importable
...
def _fetih_home() -> Path:
    """Resolve FETIH_HOME at call time (NOT module import).

    Tests and ``FETIH_HOME=...`` env overrides need this to be late-
...
# ``ls ~/.fetih/google_chat_user_tokens/`` trivial.
...
def _legacy_token_path() -> Path:
    return _fetih_home() / "google_chat_user_token.json"


def _user_tokens_dir() -> Path:
    return _fetih_home() / "google_chat_user_tokens"


def _legacy_pending_path() -> Path:
    return _fetih_home() / "google_chat_user_oauth_pending.json"


def _user_pending_dir() -> Path:
    return _fetih_home() / "google_chat_user_oauth_pending"
...
def _client_secret_path() -> Path:
    return _fetih_home() / "google_chat_user_client_secret.json"
...
def load_user_credentials(email: Optional[str] = None) -> Optional[Any]:
    ...
    try:
        ...
        logger.warning(
            ...
            "attachment delivery is disabled. Install fetih-agent[google_chat]."
        )
        return None
...
def install_deps() -> bool:
    ...
    try:
        ...
        print("  pip install 'fetih-agent[google_chat]'")
        return False
...
def store_client_secret(path: str) -> None:
    """Validate and copy the user's OAuth client_secret.json into FETIH_HOME."""
...
def exchange_auth_code(code: str, email: Optional[str] = None) -> None:
    ...
    rel_label = (
        f"{display_fetih_home()}/google_chat_user_tokens/{_sanitize_email(email)}.json"
        if email
        else f"{display_fetih_home()}/google_chat_user_token.json"
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Google Chat user-OAuth setup for FETIH (native attachment delivery)"
    )
