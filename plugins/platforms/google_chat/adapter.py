...
# ``model_tools`` import time, so ``fetih status``, ``fetih chat``, etc.
...
# ``fetih_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "FETIH is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a fetih MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${FETIH_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _fetih_home = _get_fetih_home()
        except (ModuleNotFoundError, ImportError):
            _fetih_home = _Path.home() / ".fetih"
        self._thread_count_store = _ThreadCountStore(
            _fetih_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "FETIH is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("FETIH_HOME", str(_Path.home() / ".fetih"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${FETIH_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a fetih MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'FETIH is thinking…' marker message.
        ...
        "FETIH is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "FETIH is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "FETIH is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "FETIH is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "FETIH is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``fetih setup``.

    The setup wizard at ``fetih_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.fetih/.env`` so the next
    gateway restart picks them up.
    """
    from fetih_cli.cli_output import (
        print_info,
        print_success,
        print_warning,
        prompt,
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. fetih-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.fetih/.env")
    print_info("Restart the gateway: fetih gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``fetih cron`` running as a
    separate process from ``fetih gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the FETIH plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'fetih-agent[google_chat]'",
        ...
        # the "FETIH is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'FETIH is thinking…' marker message "
