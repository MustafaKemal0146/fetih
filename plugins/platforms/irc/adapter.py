"""
IRC Platform Adapter for FETIH.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the FETIH agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "fetih-bot")
    ...
    async def connect(self) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :FETIH")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :FETIH shutting down")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        if command == "433":
            # Retry with incrementing suffix: fetih_, fetih_1, fetih_2...
...
def check_requirements() -> bool:
    ...
    # fetih setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `fetih gateway setup` flow for the IRC platform.

    Lazy-imports ``fetih_cli.setup`` helpers so the plugin stays importable
    ...
    print_info("Connect FETIH to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    channel = prompt(
        "Channel to join (e.g. #fetih — comma-separate for multiple)",
    ...
    print()
    print_success("IRC configuration saved to ~/.fetih/.env")
    print_info("Restart the gateway for changes to take effect: fetih gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "fetih-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "fetih-bot"
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :FETIH (cron)")
...
def register(ctx):
    """Plugin entry point: called by the FETIH plugin system."""
