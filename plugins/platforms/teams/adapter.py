"""
Microsoft Teams platform adapter for FETIH.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "FETIH"}),
            )
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        fetih_action = data.get("fetih_action", "")
        session_key = data.get("session_key", "")

        if not fetih_action or not session_key:
        ...
        choice = choice_map.get(fetih_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
        ...
        card = (
            ...
            .with_actions([
                ExecuteAction(
                    title="Allow Once",
                    verb="fetih_approve",
                    data={**btn_data_base, "fetih_action": "approve_once"},
                    style="positive",
                ),
                ExecuteAction(
                    title="Allow Session",
                    verb="fetih_approve",
                    data={**btn_data_base, "fetih_action": "approve_session"},
                ),
                ExecuteAction(
                    title="Always Allow",
                    verb="fetih_approve",
                    data={**btn_data_base, "fetih_action": "approve_always"},
                ),
                ExecuteAction(
                    title="Deny",
                    verb="fetih_approve",
                    data={**btn_data_base, "fetih_action": "deny"},
                    style="destructive",
                ),
            ])
        )
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"FETIH\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
    ...
    print()
    print_success("Teams configuration saved to ~/.fetih/.env")
