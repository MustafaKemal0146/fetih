"""SimpleX Chat platform adapter (FETIH plugin).
...
This adapter ships as a FETIH platform plugin under
``plugins/platforms/simplex/``. The FETIH plugin loader scans the
...
discoverable and `fetih setup` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside FETIH — no
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        payload = {
            "corrId": f"fetih-snd-{int(time.time() * 1000)}",
            "cmd": cmd_str,
        }
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``fetih setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / home channel.
    Writes to ``~/.fetih/.env`` via ``fetih_cli.config``.
    """
    print()
    ...
    try:
        ...
        print("fetih_cli.config not available; set SIMPLEX_* vars manually in ~/.fetih/.env")
        return
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        # env-only setups show up in `fetih gateway status` without
