"""
LINE Messaging API platform adapter for FETIH.
...
to FETIH. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``FETIH_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            fetih_home = Path.home().joinpath(".fetih").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``fetih status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``fetih status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``fetih setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.fetih/.env`` via ``fetih_cli.config``.
    """
    print()
    ...
    try:
        from fetih_cli.config import get_env_var, set_env_var
    except ImportError:
        print("fetih_cli.config not available; set LINE_* vars manually in ~/.fetih/.env")
        return
