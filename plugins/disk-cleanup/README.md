...
Auto-tracks and cleans up ephemeral files created during FETIH
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$FETIH_HOME` and `/tmp/fetih-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `FETIH_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under FETIH_HOME | always | Never |
...
- `is_safe_path()` rejects anything outside `FETIH_HOME` or `/tmp/fetih-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$FETIH_HOME/disk-cleanup/` is itself excluded
- `$FETIH_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
