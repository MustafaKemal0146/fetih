"""disk_cleanup — ephemeral file cleanup for FETIH.
...
Rules:
  ...
  - empty dirs    → always delete (under FETIH_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly FETIH_HOME and /tmp/fetih-*
Never touches: ~/.fetih/logs/ or any system directory.
"""
...
try:
    ...
    def get_fetih_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("FETIH_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".fetih").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$FETIH_HOME/logs/``."""
    return get_fetih_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$FETIH_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under FETIH_HOME or ``/tmp/fetih-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    fetih_home = get_fetih_home()
    try:
        path.resolve().relative_to(fetih_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/fetih-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("fetih-"):
        return True
    return False
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside FETIH_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under FETIH_HOME (but leave FETIH_HOME itself and
    ...
    _PROTECTED_TOP_LEVEL = {
        ...
        "fetih-agent", "backups", "profiles", ".worktrees",
    }
    empty_removed = 0
    try:
        for dirpath in sorted(fetih_home.rglob("*"), reverse=True):
            if not dirpath.is_dir() or dirpath == fetih_home:
                continue
            try:
                rel_parts = dirpath.relative_to(fetih_home).parts
            except ValueError:
                continue
...
def guess_category(path: Path) -> Optional[str]:
    ...
    fetih_home = get_fetih_home()
    try:
        rel = path.resolve().relative_to(fetih_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "fetih-agent",
        }:
            return None
        ...
        # Path isn't under FETIH_HOME (e.g. /tmp/fetih-*) — fall through.
        pass
