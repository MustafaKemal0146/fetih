/**
 * FETIH Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.fetih/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__FETIH_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__FETIH_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "fetih-kanban-dot-triage",
    todo: "fetih-kanban-dot-todo",
    ready: "fetih-kanban-dot-ready",
    running: "fetih-kanban-dot-running",
    blocked: "fetih-kanban-dot-blocked",
    done: "fetih-kanban-dot-done",
    archived: "fetih-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-fetih-task";
  ...
  const DOCS_URL = "https://github.com/MustafaKemal0146/fetih/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://github.com/MustafaKemal0146/fetih/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "fetih.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="fetih-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "fetih-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "fetih-kanban-md",
  ...
  // standard `drop` event and our `fetih-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("fetih-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (col !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("fetih-kanban-column--drop");
          if (col) col.classList.add("fetih-kanban-column--drop");
          lastTarget = col;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("fetih-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          lastTarget.dispatchEvent(new CustomEvent("fetih-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        const token = window.__FETIH_SESSION_TOKEN__ || "";
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "fetih-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "fetih-kanban-attention",
        "fetih-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "fetih-kanban-attention-bar" },
        h("span", { className: "fetih-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "fetih-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "fetih-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "fetih-kanban-attention-row",
                  "fetih-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "fetih-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "fetih-kanban-attention-row-id" }, task.id),
                h("span", { className: "fetih-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "fetih-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "fetih-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "fetih-kanban-diag-action-btn",
      action.suggested ? "fetih-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " fetih-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".fetih-kanban-drawer-comment-row input, .fetih-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "fetih-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("fetih-kanban-diag", sevClass) },
      h("div", { className: "fetih-kanban-diag-header" },
        h("span", { className: "fetih-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "fetih-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "fetih-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "fetih-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "fetih-kanban-diag-data-row" },
                  h("span", { className: "fetih-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "fetih-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "fetih-kanban-diag-data-row" },
                h("span", { className: "fetih-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "fetih-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "fetih-kanban-diag-reassign-row" },
            h("span", { className: "fetih-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "fetih-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "fetih-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "fetih-kanban-diag-msg",
              msg.ok ? "fetih-kanban-diag-msg--ok" : "fetih-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "fetih-kanban-section" },
      h("div", { className: "fetih-kanban-section-head-row" },
        h("span", { className: "fetih-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "fetih-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "fetih-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "fetih-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "fetih-kanban-docs-link",
      title: "Open FETIH Kanban docs in a new tab",
      "aria-label": "FETIH Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `fetih kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "fetih-kanban-msg-ok" : "fetih-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "fetih-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "fetih-kanban-boardswitcher" },
      h("div", { className: "fetih-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "fetih-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "fetih-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "fetih-kanban-dialog-title" },
        ...
        h("div", { className: "fetih-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned FETIH profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "fetih-kanban-bulk" },
      h("span", { className: "fetih-kanban-bulk-count" },
      ...
      h("div", { className: "fetih-kanban-bulk-priority",
      ...
      h("div", { className: "fetih-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different FETIH profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "fetih-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h("input", {
          type: "checkbox",
  ...
  function BoardColumns(props) {
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".fetih-kanban-card");
      if (!card) return;
    ...
    return h("div", { className: "fetih-kanban-columns", onDragStart: handleDragStart, onDragEnd: handleDragEnd },
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("fetih-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("fetih-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "fetih-kanban-column",
        dragOver ? "fetih-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "fetih-kanban-column-header",
                 title: colHelp || "" },
        h("input", {
          type: "checkbox",
          className: "fetih-kanban-col-check",
        ...
        h("span", { className: cn("fetih-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "fetih-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "fetih-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "fetih-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "fetih-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "fetih-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "fetih-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "fetih-kanban-lane" },
                  h("div", { className: "fetih-kanban-lane-head" },
                    h("span", { className: "fetih-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "fetih-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "fetih-kanban-card--stale-red";
    if (age >= tier.amber) return "fetih-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".fetih-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "fetih-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "fetih-kanban-card",
        props.selected ? "fetih-kanban-card--selected" : "",
        props.failed ? "fetih-kanban-card--failed" : "",
        props.draggingSource ? "fetih-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "fetih-kanban-card-content" },
          h("div", { className: "fetih-kanban-card-row" },
            h("label", {
              className: "fetih-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h("input", {
                type: "checkbox",
                className: "fetih-kanban-card-check",
            ...
            h("span", { className: "fetih-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or fetih kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "fetih-kanban-warning-badge",
                    "fetih-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "fetih-kanban-needs-assignee",
          ...
          h("div", { className: "fetih-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "fetih-kanban-card-row fetih-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "fetih-kanban-assignee",
                            title: `Assigned to FETIH profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "fetih-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "fetih-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "fetih-kanban-ago",
  ...
  function InlineCreate(props) {
    ...
    return h("div", { className: "fetih-kanban-inline-create" },
      h("textarea", {
        value: title,
      ...
      h("div", { className: "flex gap-2" },
        h(Input, {
          ...
          title: props.columnName === "triage"
            ? "FETIH profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
            : "FETIH profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "fetih-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "fetih-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "fetih-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "fetih-kanban-drawer-close",
        ...
        data ? h("div", { className: "fetih-kanban-drawer-comment-row" },
          h(Input, {
            value: newComment,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "fetih-kanban-drawer-body" },
      h("div", { className: "fetih-kanban-drawer-title" },
        h("span", { className: cn("fetih-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "fetih-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "fetih-kanban-drawer-meta" },
      ...
      t.result ? h("div", { className: "fetih-kanban-section" },
        h("div", { className: "fetih-kanban-section-head" }, tx(i18n, "result", "Result")),
        h(MarkdownBlock, { source: t.result, enabled: props.renderMarkdown }),
      ) : null,
      h("div", { className: "fetih-kanban-section" },
        h("div", { className: "fetih-kanban-section-head" },
          `${tx(i18n, "comments", "Comments")} (${comments.length})`),
        comments.length === 0
          ? h("div", { className: "text-xs text-muted-foreground" },
              tx(i18n, "noComments", "— no comments —"))
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "fetih-kanban-comment" },
                h("div", { className: "fetih-kanban-comment-head" },
                  h("span", { className: "fetih-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "fetih-kanban-comment-ago" },
      ...
      h("div", { className: "fetih-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            ...
            className: cn(
              "fetih-kanban-event",
              isDiag ? "fetih-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "fetih-kanban-event-header" },
                  h("span", { className: "fetih-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "fetih-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "fetih-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "fetih-kanban-event-header-plain" },
                  h("span", { className: "fetih-kanban-event-kind" }, e.kind),
                  h("span", { className: "fetih-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "fetih-kanban-event-phantom-row" },
                  h("span", { className: "fetih-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "fetih-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "fetih-kanban-event-payload" },
    ...
    return h("div", { className: "fetih-kanban-section" },
      h("div", { className: "fetih-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "fetih-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `fetih-kanban-run--${r.outcome || r.status || "ended"}`
          : "fetih-kanban-run--active";
        return h("div", { key: r.id, className: cn("fetih-kanban-run", outcomeClass) },
          h("div", { className: "fetih-kanban-run-head" },
            h("span", { className: "fetih-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "fetih-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "fetih-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "fetih-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "fetih-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "fetih-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "fetih-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "fetih-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "fetih-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "fetih-kanban-pre fetih-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "fetih-kanban-meta-row" },
      h("span", { className: "fetih-kanban-meta-label" }, props.label),
      h("span", { className: "fetih-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "fetih-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "fetih-kanban-meta-row" },
        h("span", { className: "fetih-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "fetih-kanban-meta-value fetih-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "fetih-kanban-meta-row" },
        h("span", { className: "fetih-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "fetih-kanban-meta-value fetih-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "fetih-kanban-section" },
      h("div", { className: "fetih-kanban-section-head-row" },
        h("span", { className: "fetih-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "fetih-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "fetih-kanban-section" },
      h("div", { className: "fetih-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "fetih-kanban-deps-row" },
        h("span", { className: "fetih-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "fetih-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "fetih-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "fetih-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "fetih-kanban-dep-chip-x",
      ...
      h("div", { className: "fetih-kanban-deps-row" },
        h("span", { className: "fetih-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "fetih-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "fetih-kanban-actions" },
        specifyButton,
        decomposeButton,
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "fetih-kanban-section" },
      ...
      h("div", { className: "fetih-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "fetih-kanban-home-sub fetih-kanban-home-sub--on"
              : "fetih-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__FETIH_PLUGINS__ && typeof window.__FETIH_PLUGINS__.register === "function") {
    window.__FETIH_PLUGINS__.register("kanban", KanbanPage);
  }
})();
