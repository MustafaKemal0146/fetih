...
1. Bundled providers: ``plugins/memory/<name>/`` (shipped with fetih-agent)
2. User-installed providers: ``$FETIH_HOME/plugins/<name>/``
...
def _get_user_plugins_dir() -> Optional[Path]:
    """Return ``$FETIH_HOME/plugins/`` or None if unavailable."""
    try:
        from fetih_constants import get_fetih_home
        d = get_fetih_home() / "plugins"
...
def load_memory_provider(name: str) -> Optional["MemoryProvider"]:
    ...
    (``$FETIH_HOME/plugins/<name>/``) directories.  Bundled takes
...
def _load_provider_from_dir(provider_dir: Path) -> Optional["MemoryProvider"]:
    ...
    module_name = f"plugins.memory.{name}" if _is_bundled else f"_fetih_user_memory.{name}"
...
def discover_plugin_cli_commands() -> List[dict]:
    ...
    module_name = f"plugins.memory.{active_provider}.cli" if _is_bundled else f"_fetih_user_memory.{active_provider}.cli"
    try:
