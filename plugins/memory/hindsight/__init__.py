...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID                — memory bank identifier (default: fetih)
...
Or via $FETIH_HOME/hindsight/config.json (profile-scoped), falling back to
...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    so FETIH can degrade gracefully instead of repeatedly trying to start
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $FETIH_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_fetih_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "fetih": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "fetih"),
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    """Return the Hindsight embedded profile name for this FETIH config."""
    profile = config.get("profile", "fetih")
    return str(profile or "fetih")
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    Supported placeholders (each is sanitized before substitution):
      {profile}   — active FETIH profile name (from agent_identity)
      {workspace} — FETIH workspace name (from agent_workspace)
      {platform}  — "cli", "telegram", "discord", etc.
      {user}      — platform user id (gateway sessions)
      {session}   — current session id

    Missing/empty placeholders are rendered as the empty string and then
    collapsed — e.g. ``fetih-{user}`` with no user becomes ``fetih``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between FETIH and the User"
    ...
    def save_config(self, values, fetih_home):
        """Write config to $FETIH_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(fetih_home) / "hindsight"
    ...
    def post_setup(self, fetih_home: str, config: dict) -> None:
        ...
        from fetih_cli.memory_setup import _curses_select
        ...
        provider_config.setdefault("bank_id", "fetih")
        ...
        self.save_config(provider_config, fetih_home)
        ...
        if mode == "local_embedded":
            ...
            config_path = Path(fetih_home) / "hindsight" / "config.json"
            try:
            ...
            if not llm_api_key:
                llm_api_key = _load_simple_env(Path(fetih_home) / ".env").get("HINDSIGHT_LLM_API_KEY", "")
            if not llm_api_key:
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "fetih"},
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: fetih-{profile}", "default": ""},
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between FETIH and the User"},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "fetih"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "fetih"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        banks = cfg_get(self._config, "banks", "fetih", default={})
        static_bank_id = self._config.get("bank_id") or banks.get("bankId", "fetih")
        ...
        self._retain_context = self._config.get("retain_context", "conversation between FETIH and the User")
        ...
        if self._client is not None:
            try:
                if self._mode == "local_embedded":
                    ...
                    # When FETIH created/used that client on the shared async loop,
