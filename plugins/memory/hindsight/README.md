...
```bash
fetih memory setup    # select "hindsight"
```
...
```bash
fetih config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.fetih/.env
```
...
FETIH spins up a local Hindsight daemon with built-in PostgreSQL. Requires an LLM API key for memory extraction and synthesis. The daemon starts automatically in the background on first use and stops after 5 minutes of inactivity.
...
Daemon startup logs: `~/.fetih/logs/hindsight-embed.log`
Daemon runtime logs: `~/.hindsight/profiles/<profile>.log`

To open the Hindsight web UI (local embedded mode only):
```bash
hindsight-embed -p fetih ui start
```
...
Config file: `~/.fetih/hindsight/config.json`
...
| Key | Default | Description |
|-----|---------|-------------|
| `bank_id` | `fetih` | Memory bank name (static fallback used when `bank_id_template` is unset or resolves empty) |
| `bank_id_template` | — | Optional template to derive the bank name dynamically. Placeholders: `{profile}`, `{workspace}`, `{platform}`, `{user}`, `{session}`. Example: `fetih-{profile}` isolates memory per active FETIH profile. Empty placeholders collapse cleanly (e.g. `fetih-{user}` with no user becomes `fetih`). |
...
| `retain_context` | `conversation between FETIH and the User` | Context label for retained memories |
...
The LLM API key is stored in `~/.fetih/.env` as `HINDSIGHT_LLM_API_KEY`.
