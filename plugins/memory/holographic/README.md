...
```bash
fetih memory setup    # select "holographic"
```

Or manually:
```bash
fetih config set memory.provider holographic
```

## Config

Config in `config.yaml` under `plugins.fetih-memory-store`:

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$FETIH_HOME/memory_store.db` | SQLite database path |
