...
```bash
fetih honcho setup    # full interactive wizard (cloud or local)
fetih memory setup    # generic picker, also works
```

Or manually:
```bash
fetih config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.fetih/.env
```
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$FETIH_HOME/honcho.json` | Profile-local (isolated FETIH instances) |
| 2 | `~/.fetih/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active FETIH profile: `fetih` (default) or `fetih.<profile>`.
...
| 4 | `per-session` strategy | FETIH session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`fetih`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`fetih`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-fetih`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening fetih in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — FETIH session ID (timestamp + hex). Every `fetih` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple FETIH profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`fetih`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active FETIH profile: `fetih` (default) or `fetih.<profile>` (e.g. `fetih -p coder` → host key `fetih.coder`).
...
| `FETIH_HONCHO_HOST` | Host key override |

## CLI Commands

| Command | Description |
|---------|-------------|
| `fetih honcho setup` | Full interactive setup wizard |
| `fetih honcho status` | Show resolved config for active profile |
| `fetih honcho enable` / `disable` | Toggle Honcho for active profile |
| `fetih honcho mode <mode>` | Change recall or observation mode |
| `fetih honcho peer --user <name>` | Update user peer name |
| `fetih honcho peer --ai <name>` | Update AI peer name |
| `fetih honcho tokens --context <N>` | Set context token budget |
| `fetih honcho tokens --dialectic <N>` | Set dialectic max chars |
| `fetih honcho map <name>` | Map current directory to a session name |
| `fetih honcho sync` | Create host blocks for all FETIH profiles |
