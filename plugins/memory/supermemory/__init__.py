...
def _load_supermemory_config(fetih_home: str) -> dict:
    config = _default_config()
    config_path = Path(fetih_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, fetih_home: str) -> None:
    config_path = Path(fetih_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `fetih memory setup`.
        # All other options are documented for $FETIH_HOME/supermemory.json
    ...
    def save_config(self, values, fetih_home):
        ...
        _save_supermemory_config(sanitized, fetih_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from fetih_constants import get_fetih_home
        self._fetih_home = kwargs.get("fetih_home") or str(get_fetih_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._fetih_home)
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        metadata = {"source": "fetih", "type": "conversation_turn"}
        ...
        def _run():
            try:
                self._client.add_memory(
                    content.strip(),
                    metadata={"source": "fetih_memory", "target": target, "type": "explicit_memory"},
