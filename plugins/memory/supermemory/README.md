...
```bash
fetih memory setup    # select "supermemory"
```

Or manually:

```bash
fetih config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.fetih/.env
```

## Config

Config file: `$FETIH_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `fetih` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `fetih-{identity}` → `fetih-coder`). |
...
Use `{identity}` in the `container_tag` to scope memories per FETIH profile:

```json
{
  "container_tag": "fetih-{identity}"
}
```

For a profile named `coder`, this resolves to `fetih-coder`. The default profile resolves to `fetih-default`. Without `{identity}`, all profiles share the same container.
