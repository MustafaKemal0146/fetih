...
```bash
fetih memory setup    # select "byterover"
```

Or manually:
```bash
fetih config set memory.provider byterover
# Optional cloud sync:
echo "BRV_API_KEY=your-key" >> ~/.fetih/.env
```
...
Working directory: `$FETIH_HOME/byterover/` (profile-scoped).
