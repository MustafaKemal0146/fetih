...
## Client ID
- Need to register a "Desktop app" OAuth client on a FETIH GCP project
...
## Token Lifecycle
- Store at `~/.fetih/gemini_oauth.json` (NOT sharing with `~/.gemini/oauth_creds.json`)
...
2. `fetih_cli/auth.py` — Add ProviderConfig for "gemini" with auth_type="oauth_google"
3. `fetih_cli/models.py` — Add Gemini model catalog
4. `fetih_cli/runtime_provider.py` — Add gemini branch (read OAuth token, build OpenAI client)
5. `fetih_cli/main.py` — Add `_model_flow_gemini()`, add to provider choices
6. `fetih_cli/setup.py` — Add gemini auth flow (trigger browser OAuth)
...
## Prerequisites
- FETIH GCP project with Desktop OAuth client registered
- OR: accept user-provided client_id via FETIH_GEMINI_CLIENT_ID env var
...
- fetih Copilot OAuth: `fetih_cli/main.py` `_copilot_device_flow()` (different flow type but same lifecycle pattern)
