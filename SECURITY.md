# FETIH Security Policy

This document describes FETIH's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/MustafaKemal0146/fetih/security/advisories/new)
or **security@github.com/MustafaKemal0146/fetih**. Do not open public issues for
security vulnerabilities. **FETIH does not operate a bug
bounty program.**
...
- Environment details (`fetih version`, commit SHA, OS, Python
  version).
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted FETIH access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in FETIH's documentation
...
FETIH supports two OS-level isolation postures. They address
...
FETIH supports this in two ways:

- **FETIH's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, FETIH's in-process heuristics
...
FETIH filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in FETIH
itself. Bugs in FETIH's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   FETIH does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, FETIH's own documentation, or reasonable
  operator expectations would predict — including cases where
  FETIH has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  fetih-home security, and similar. Reports against those
...
- FETIH includes supply-chain guards for MCP server
...
- **Channel:** the GHSA thread or email correspondence with
  security@github.com/MustafaKemal0146/fetih.
