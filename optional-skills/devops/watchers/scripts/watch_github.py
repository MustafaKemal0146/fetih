...
Usage (via cron with --no-agent):
    ...
    fetih cron create fetih-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$FETIH_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name fetih-issues --repo MustafaKemal0146/fetih --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in ~/.fetih/.env to avoid the 60 req/hr
