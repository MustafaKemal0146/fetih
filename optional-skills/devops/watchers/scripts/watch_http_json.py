...
Usage (via cron with --no-agent):
    ...
    fetih cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$FETIH_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "FETIH-Watcher/1.0"})
