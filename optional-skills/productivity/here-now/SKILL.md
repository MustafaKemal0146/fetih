...
- Skill helper paths:
  - `${FETIH_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${FETIH_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${FETIH_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client fetih
```
...
```bash
PUBLISH="${FETIH_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client fetih
```
...
```bash
DRIVE="${FETIH_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `fetih`)    |
