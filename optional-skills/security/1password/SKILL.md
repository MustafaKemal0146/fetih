...
version: 1.0.0
author: arceus77-7, enhanced by FETIH
license: MIT
...
- `tmux` available for stable authenticated sessions during FETIH terminal calls (desktop app flow only)
...
### Service Account (recommended for FETIH)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.fetih/.env` (the skill will prompt for this on first load).
...
## FETIH Execution Pattern (desktop app flow)

FETIH terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/fetih-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/fetih-op.sock"
