---
name: honcho
description: Configure and use Honcho memory with FETIH -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.
version: 2.0.0
author: FETIH
license: MIT
...
# Honcho Memory for FETIH

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every FETIH profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/hermes#running-honcho-locally-with-fetih

### Verify

```bash
fetih honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this FETIH profile's AI peer
...
Honcho models conversations as interactions between **peers**. FETIH creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this FETIH instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each FETIH run |
| `global` | Single session across all directories |

Manual override: `fetih honcho map my-project-name`
...
Each FETIH profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
```bash
fetih profile create coder --clone
# creates host block fetih.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `fetih.coder` host block in `honcho.json`
...
```bash
fetih honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for FETIH when Honcho memory is active.
...
Config file: `$FETIH_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `fetih honcho setup`. Ensure `memory.provider: honcho` is in `~/.fetih/config.yaml`.

### Memory not persisting across sessions
Check `fetih honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `fetih profile create <name> --clone`. For existing profiles: `fetih honcho sync`.
...
| Command | Description |
|---------|-------------|
| `fetih honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `fetih honcho status` | Show resolved config, connection test, peer info for active profile |
| `fetih honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `fetih honcho disable` | Disable Honcho for the active profile |
| `fetih honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `fetih honcho peers` | Show peer identities across all profiles |
| `fetih honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `fetih honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `fetih honcho sessions` | List known directory-to-session-name mappings |
| `fetih honcho map <name>` | Map current working directory to a Honcho session name |
| `fetih honcho identity` | Seed AI peer identity or show both peer representations |
| `fetih honcho sync` | Create host blocks for all FETIH profiles that don't have one yet |
| `fetih honcho migrate` | Step-by-step migration guide from OpenClaw native memory to FETIH + Honcho |
| `fetih memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `fetih memory status` | Show active memory provider and config |
| `fetih memory off` | Disable external memory provider |
