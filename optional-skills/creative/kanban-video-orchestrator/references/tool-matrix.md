...
Maps each role archetype to the FETIH skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public fetih
...
## FETIH skills relevant to video production

### Visual / rendering skills (`fetih/skills/creative/`)
...
### Design / pre-production skills (`fetih/skills/creative/`)
...
### Audio / media skills (`fetih/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`fetih/skills/devops/`)
...
These are **not** FETIH skills but external CLIs / APIs that profiles invoke.
...
## Built-in FETIH tools for media review

These are native FETIH tools — not invoked via terminal but through their own
...
key is present in `~/.fetih/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every FETIH profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `~/.fetih/.env` → environment variable.
...
When a new FETIH-public video skill ships:
