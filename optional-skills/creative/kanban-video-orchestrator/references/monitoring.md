...
fetih kanban watch --tenant <project-slug>

# Snapshot of the board
fetih kanban list --tenant <project-slug>
fetih kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
fetih kanban stats --tenant <project-slug>

# Visual dashboard (browser)
fetih dashboard

# Inspect a specific task (includes comments + events)
fetih kanban show <task-id>

# Follow a single task's event stream
fetih kanban tail <task-id>
```

Verify available subcommands with `fetih kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `fetih kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `fetih kanban block <id>` then `fetih kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `fetih kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `fetih kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `fetih profile list` (profile exists?), `fetih status` (gateway/dispatcher up?), and verify tenant. |
...
fetih kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
fetih kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(fetih kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
fetih kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
fetih kanban block <task-id>
fetih kanban archive <task-id>

# Diagnose what happened
fetih kanban show <task-id>      # task body, comments, recent events
fetih kanban tail <task-id>      # follow the live event stream
fetih kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `fetih kanban create` for the director
...
while true; do
    clear
    fetih kanban list --tenant <slug>
    echo "---"
    fetih kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `fetih kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `fetih profile list` and start
