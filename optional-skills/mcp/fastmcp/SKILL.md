...
- smoke-test a server with the FastMCP CLI before wiring it into FETIH or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to FETIH. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.fetih/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.fetih/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is FETIH integration, either:

- configure the server in `~/.fetih/config.yaml` using the `native-mcp` skill, or
...
### FETIH cannot see the deployed server

The server-building part may be correct while the FETIH config is not. Load the `native-mcp` skill and configure the server in `~/.fetih/config.yaml`, then restart FETIH.
