...
import upstream classes into FETIH itself.
...
```bash
mkdir -p ~/.fetih/cache/darwinian-evolver && cd ~/.fetih/cache/darwinian-evolver
...
```bash
cd ~/.fetih/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.fetih/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.fetih/skills/research/darwinian-evolver
DE_DIR=~/.fetih/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside FETIH core.
   Custom driver scripts under `~/.fetih/skills/...` are user-side and fine.
