...
This is an optional third-party workflow, not a FETIH core capability.

Important expectations:
- Parallel is a paid service with a free tier, not a fully free local tool.
- It overlaps with FETIH native `web_search` / `web_extract`, so do not prefer it by default for ordinary lookups.
...
Prefer FETIH native `web_search` / `web_extract` for quick one-off lookups when Parallel is not specifically requested.
...
7. Prefer FETIH native tools unless the user wants Parallel specifically or needs Parallel-only workflows.
...
## Recommended FETIH usage patterns
...
- Do not silently choose Parallel when FETIH native tools are already sufficient.
