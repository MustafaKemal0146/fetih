...
description: Migrate a user's OpenClaw customization footprint into FETIH. Imports FETIH-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.
version: 1.0.0
author: FETIH (FETIH)
license: MIT
...
metadata:
  hermes:
    tags: [Migration, OpenClaw, FETIH, Memory, Persona, Import]
    related_skills: [fetih]
---

# OpenClaw -> FETIH Migration

Use this skill when a user wants to move their OpenClaw setup into FETIH with minimal manual cleanup.
...
```bash
fetih claw migrate              # Full interactive migration
...
fetih claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `fetih setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_fetih.py` to:

- import `SOUL.md` into the FETIH home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into FETIH memory entries
- merge OpenClaw command approval patterns into FETIH `command_allowlist`
- migrate FETIH-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS` and `MESSAGING_CWD`
- copy OpenClaw skills into `~/.fetih/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen FETIH workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.fetih/tts/`
- archive non-secret docs that do not have a direct FETIH destination
...
- `~/.fetih/skills/migration/openclaw-migration/scripts/openclaw_to_fetih.py`

Do not guess a shorter path like `~/.fetih/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.fetih/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of FETIH-compatible secrets, currently:
...
FETIH CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in FETIH. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a FETIH workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing FETIH skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.fetih/skills/migration/openclaw-migration/scripts/openclaw_to_fetih.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.fetih/skills/migration/openclaw-migration/scripts/openclaw_to_fetih.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.fetih/skills/migration/openclaw-migration/scripts/openclaw_to_fetih.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.fetih/skills/migration/openclaw-migration/scripts/openclaw_to_fetih.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.fetih/skills/migration/openclaw-migration/scripts/openclaw_to_fetih.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.fetih/skills/migration/openclaw-migration/scripts/openclaw_to_fetih.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of FETIH unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty FETIH targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean FETIH destination. Unsupported auth blobs must still be reported as skipped.
...
- FETIH persona state imported
- FETIH memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.fetih/skills/openclaw-imports/`
