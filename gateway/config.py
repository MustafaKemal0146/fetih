...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_fetih_home() / "sessions")
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.fetih/config.yaml (primary user-facing config)
    3. ~/.fetih/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    _home = get_fetih_home()
    gw_data: dict = {}
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            if not isinstance(streaming_cfg, dict):
                ...
                # ``fetih config set gateway.streaming.*``
