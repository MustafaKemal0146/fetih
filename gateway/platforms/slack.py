...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        tokens_file = get_fetih_home() / "slack_tokens.json"
        ...
        try:
            ...
            # /model work directly without /fetih prefix). A single regex
            ...
            # manifest (see `fetih slack manifest`). In Socket Mode, Slack
            ...
            from fetih_cli.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/fetih$")

            @self._app.command(_slash_pattern)
            async def handle_fetih_command(ack, command):
            ...
            for _action_id in (
                "fetih_approve_once",
                "fetih_approve_session",
                "fetih_approve_always",
                "fetih_deny",
            ):
            ...
            for _action_id in (
                "fetih_confirm_once",
                "fetih_confirm_always",
                "fetih_confirm_cancel",
            ):
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = f":thread: FETIH handoff — *{(name or 'session').strip()[:80]}*"
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own FETIH session — matching the per-thread behavior channels already
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "fetih_approve_once",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Allow Session"},
                            "action_id": "fetih_approve_session",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Allow"},
                            "action_id": "fetih_approve_always",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "fetih_confirm_once",
                            "value": value,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Approve"},
                            "action_id": "fetih_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
                            ...
                            "style": "danger",
                            "action_id": "fetih_confirm_cancel",
                            "value": value,
                        },
                    ],
                },
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "fetih_confirm_once": "once",
            "fetih_confirm_always": "always",
            "fetih_confirm_cancel": "cancel",
        }
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "fetih_approve_once": "once",
            "fetih_approve_session": "session",
            "fetih_approve_always": "always",
            "fetih_deny": "deny",
        }
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/fetih <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/fetih
        ...
        if slash_name in {"fetih", ""}:
            # Legacy /fetih <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            from fetih_cli.commands import slack_subcommand_map
            subcommand_map = slack_subcommand_map()
            subcommand_map["compact"] = "/compress"
            # Guard against whitespace-only text where ``text`` is truthy but
            # ``text.split()`` returns ``[]`` (e.g. user sends ``/fetih   ``).
        ...
        # "Running /cmd…" ack shown by handle_fetih_command).
        # Only stash for COMMAND events (text starts with "/") — free-form
        # questions via "/fetih <question>" must produce public replies so
