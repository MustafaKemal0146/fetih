...
class WhatsAppAdapter(BasePlatformAdapter):
    ...
    DEFAULT_REPLY_PREFIX = "⚕ *FETIH*\n────────────\n"
    
    # Default bridge location relative to the fetih-agent install
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._session_path: Path = Path(config.extra.get(
            "session_path",
            get_fetih_dir("platforms/whatsapp/session", "whatsapp/session")
        ))
    ...
    async def connect(self) -> bool:
        ...
        if not check_whatsapp_requirements():
            ...
            self._set_fatal_error(
                ...
                "Node.js is not installed — install Node.js and re-run `fetih gateway`.",
                retryable=False,
            )
            return False
        ...
        # clear "run fetih whatsapp" message instead of the watcher
        ...
        if not creds_path.exists():
            logger.warning(
                ...
                "Run `fetih whatsapp` to pair, or remove WHATSAPP_ENABLED from "
                "your .env to disable.",
                self.name, creds_path,
            )
            self._set_fatal_error(
                "whatsapp_not_paired",
                "WhatsApp enabled but not paired — run `fetih whatsapp` to pair.",
                retryable=False,
            )
            return False
        ...
        try:
            ...
            if data.get("status") != "connected":
                ...
                for attempt in range(15):
                    ...
                    print(f"[{self.name}]   If session expired, re-pair: fetih whatsapp")
