"""
Email platform adapter for the FETIH gateway.

Allows users to interact with FETIH by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "fetih-agent" "version" "{_fetih_version}" '
            '"vendor" "FETIH" '
            '"support-email" "noreply@github.com/MustafaKemal0146/fetih")',
        )
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def _send_email(
        ...
        msg_id = f"<fetih-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"
