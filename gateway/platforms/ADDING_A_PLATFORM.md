...
There are two ways to add a platform to the FETIH gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.fetih/plugins/` (or under `plugins/platforms/`
...
**zero changes to core FETIH code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `fetih gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `fetih_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the FETIH core.
...
And add it to the `fetih-gateway` composite:

```python
"fetih-gateway": {
    "includes": [..., "fetih-your-platform"]
}
```
...
## 12. Status Display (`fetih_cli/status.py`)
...
## 13. Gateway Setup Wizard (`fetih_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ fetih_cli/ toolsets.py \
  --include="*.py" -l | sort -u
