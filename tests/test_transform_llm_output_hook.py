...
import fetih_cli.plugins as plugins_mod
from fetih_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(fetih_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <fetih_home>/plugins/<name> and opt it in."""
    plugin_dir = fetih_home / "plugins" / name
...
def test_hook_receives_expected_kwargs(tmp_path, monkeypatch):
    ...
    fetih_home = tmp_path / "fetih_test"
    fetih_home.mkdir(exist_ok=True)
    _make_enabled_plugin(
        fetih_home, "capture_hook",
        register_body=(
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("FETIH_HOME", str(tmp_path / "fetih_empty"))
