...
(SQLITE_PROTOCOL — typical on NFS/SMB), FETIH must fall back to
``journal_mode=DELETE`` so ``state.db`` / ``kanban.db`` remain usable.

Without this fallback, users on NFS-mounted ``FETIH_HOME`` silently lose
...
def _reset_last_init_error():
    ...
    fetih_state._set_last_init_error(None)
    yield
    fetih_state._set_last_init_error(None)
...
def _reset_wal_fallback_warned_paths():
    ...
    fetih_state._wal_fallback_warned_paths.clear()
    yield
    fetih_state._wal_fallback_warned_paths.clear()
...
class TestApplyWalWithFallback:
    ...
    def test_falls_back_to_delete_on_locking_protocol(self, tmp_path, caplog):
        ...
        with caplog.at_level("WARNING", logger="fetih_state"):
    ...
    def test_warning_deduplicated_per_db_label(self, tmp_path, caplog):
        ...
        connection on every operation — see fetih_cli/kanban_db.py).
...
class TestGetLastInitError:
    ...
    def test_success_does_not_clear_prior_error(self, tmp_path):
        ...
        """
        fetih_state._set_last_init_error("OperationalError: locking protocol")
    ...
    def test_captures_cause_on_failed_init(self, tmp_path):
        ...
        with patch("fetih_state.sqlite3.connect", side_effect=gated_connect):
...
class TestFormatSessionDbUnavailable:
    ...
    def test_includes_cause(self):
        ...
        fetih_state._set_last_init_error("OperationalError: generic SQLite error")
