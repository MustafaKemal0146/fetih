"""Shared fixtures for the fetih-agent test suite.
...
2. **Isolated FETIH_HOME.** FETIH_HOME points to a per-test tempdir so
   code reading ``~/.fetih/*`` via ``get_fetih_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".fetih"`` instead of the canonical
   ``get_fetih_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No FETIH_SESSION_* inheritance** — the agent's current gateway
...
# FETIH_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_FETIH_BEHAVIORAL_VARS = frozenset({
    "FETIH_YOLO_MODE",
    "FETIH_INTERACTIVE",
    "FETIH_QUIET",
    "FETIH_TOOL_PROGRESS",
    ...
    "FETIH_SESSION_CHAT_ID",
    "FETIH_SESSION_CHAT_NAME",
    "FETIH_SESSION_THREAD_ID",
    "FETIH_SESSION_SOURCE",
    "FETIH_SESSION_KEY",
    "FETIH_GATEWAY_SESSION",
    "FETIH_PLATFORM",
    "FETIH_MODEL",
    "FETIH_INFERENCE_MODEL",
    "FETIH_INFERENCE_PROVIDER",
    "FETIH_TUI_PROVIDER",
    "FETIH_MANAGED",
    "FETIH_DEV",
    "FETIH_CONTAINER",
    ...
    "FETIH_TIMEZONE",
    "FETIH_REDACT_SECRETS",
    "FETIH_BACKGROUND_NOTIFICATIONS",
    "FETIH_EXEC_ASK",
    "FETIH_HOME_MODE",
    "FETIH_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.fetih/kanban.db instead of the per-test FETIH_HOME.
    "FETIH_KANBAN_DB",
    "FETIH_KANBAN_BOARD",
    "FETIH_KANBAN_WORKSPACES_ROOT",
    "FETIH_KANBAN_LOGS_ROOT",
    "FETIH_KANBAN_TASK",
    "FETIH_KANBAN_WORKSPACE",
    "FETIH_TENANT",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and FETIH_HOME to per-test tempdirs so code that
    reads ``~/.fetih/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral FETIH_* vars that could change test semantics.
    for name in _FETIH_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect FETIH_HOME to a per-test tempdir. Code that reads
    #    ``~/.fetih/*`` via ``get_fetih_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.fetih/*`` via
    #    ``Path.home() / ".fetih"`` instead of ``get_fetih_home()``
    #    is a bug to fix at the callsite.
    fake_fetih_home = tmp_path / "fetih_test"
    fake_fetih_home.mkdir()
    (fake_fetih_home / "sessions").mkdir()
    ...
    #    ~/.fetih/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import fetih_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_fetih_home(_hermetic_environment):
...
def _reset_module_state():
    ...
    for _logger_name in ("tools", "run_agent", "trajectory_compressor", "cron", "fetih_cli"):
...
@pytest.fixture()
def mock_config():
    """Return a minimal fetih config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``fetih-gateway`` process
...
# correlated with a ``tests/fetih_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> fetih-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart fetih-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting fetih/python patterns are also blocked.
    """
    ...
    def _matches_fetih_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _FETIH_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_fetih_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch fetih-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m fetih_cli.main".
                if (
                    "fetih" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live fetih-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting fetih/python could hit the live gateway. "
