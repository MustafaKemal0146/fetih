...
class TestToolsetConsistency:
    ...
    def test_fetih_platforms_share_core_tools(self):
        """All fetih-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on fetih-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["fetih-cli", "fetih-telegram", "fetih-discord", "fetih-whatsapp", "fetih-slack", "fetih-signal", "fetih-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_fetih_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("fetih-whatsapp")

    def test_fetih_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("fetih-api-server")
