...
minimal ``FETIHCLI`` stub (pattern used elsewhere in tests/cli).
"""
...
def fetih_home(tmp_path, monkeypatch):
    """Isolated FETIH_HOME so SessionDB.state_meta writes stay hermetic."""
    home = tmp_path / ".fetih"
    home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.setenv("FETIH_HOME", str(home))

    # Bust the goal module's DB cache so it re-resolves FETIH_HOME each test.
    from fetih_cli import goals
...
def _make_cli_with_goal(session_id: str, goal_text: str = "build a thing"):
    """Build a minimal FETIHCLI stub with an active goal wired in."""
    from cli import FETIHCLI
    from fetih_cli.goals import GoalManager

    cli = FETIHCLI.__new__(FETIHCLI)
...
class TestInterruptAutoPause:
    def test_interrupted_turn_pauses_goal_and_skips_continuation(self, fetih_home):
        ...
        with patch("fetih_cli.goals.judge_goal") as judge_mock:
    ...
    def test_interrupted_turn_is_resumable(self, fetih_home):
        ...
        with patch("fetih_cli.goals.judge_goal"):
            cli._maybe_continue_goal_after_turn()
...
class TestEmptyResponseSkip:
    def test_empty_response_does_not_invoke_judge(self, fetih_home):
    ...
    def test_no_assistant_message_skipped(self, fetih_home):
...
class TestHealthyTurnStillRuns:
    ...
    def test_clean_response_marks_done_when_judge_says_done(self, fetih_home):
...
class TestInterruptFlagLifecycle:
    def test_chat_resets_flag_at_entry(self, fetih_home):
        ...
        src = inspect.getsource(FETIHCLI.chat)
