...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("fetih/fetih-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned fetih/* and pr-* branches."""

    def test_prunes_orphaned_fetih_branch(self, git_repo):
        """fetih/fetih-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "fetih/fetih-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "fetih/fetih-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "fetih/fetih-deadbeef" in result.stdout
        ...
        assert "fetih/fetih-deadbeef" in orphaned
        ...
        assert "fetih/fetih-deadbeef" not in result.stdout
