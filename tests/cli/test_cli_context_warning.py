...
def _isolate(tmp_path, monkeypatch):
    """Isolate FETIH_HOME so tests don't touch real config."""
    home = tmp_path / ".fetih"
    home.mkdir()
...
def cli_obj(_isolate):
    """Create a minimal FETIHCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = FETIHCLI.__new__(FETIHCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["fetih-core"]
        obj.compact = False
