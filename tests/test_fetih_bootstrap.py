"""Tests for fetih_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every FETIH entry point
(fetih, fetih-agent, fetih-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every FETIH entry point imports fetih_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported fetih_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("fetih_bootstrap", None)
    import fetih_bootstrap  # noqa: WPS433
    return fetih_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the FETIH CLI
...
class TestEntryPointsImportBootstrap:
    """Every FETIH entry point must import fetih_bootstrap as its
    ...
    # Entry points that invoke FETIH as a process.  Each one must
    # import fetih_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "fetih_cli/main.py",   # fetih CLI (console_script)
        "run_agent.py",          # fetih-agent (console_script)
        "acp_adapter/entry.py",  # fetih-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import fetih_bootstrap' and that
        ...
        half-finished ``fetih update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``fetih_bootstrap`` as a top-level module) leaves fetih
        ...
        """
        # Resolve relative to the fetih-agent repo root.  Tests live
        # at tests/test_fetih_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # fetih start even when fetih_bootstrap hasn't been
        ...
        assert first_import_name == "fetih_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'fetih_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import fetih_bootstrap' line to be the first import."
        )
