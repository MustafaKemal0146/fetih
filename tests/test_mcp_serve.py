"""
Tests for mcp_serve — FETIH MCP server.
...
def _isolate_fetih_home(tmp_path, monkeypatch):
    """Redirect FETIH_HOME to a temp directory."""
    monkeypatch.setenv("FETIH_HOME", str(tmp_path))
    try:
        import fetih_constants
        monkeypatch.setattr(fetih_constants, "get_fetih_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking fetih_state schema."""
