...
Verifies:
  - Tools are gated on FETIH_KANBAN_TASK: a normal chat session sees
...
def test_kanban_tools_hidden_without_env_var(monkeypatch, tmp_path):
    """Normal `fetih chat` sessions (no FETIH_KANBAN_TASK) must have
    zero kanban_* tools in their schema."""
    monkeypatch.delenv("FETIH_KANBAN_TASK", raising=False)
    home = tmp_path / ".fetih"
    home.mkdir()
    ...
    schema = registry.get_definitions(set(resolve_toolset("fetih-cli")), quiet=True)
...
def test_kanban_tools_visible_with_env_var(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("FETIH_KANBAN_TASK", "t_fake")
    home = tmp_path / ".fetih"
    home.mkdir()
    monkeypatch.setenv("FETIH_HOME", str(home))
...
def test_worker_with_kanban_toolset_still_hides_board_routing(monkeypatch, tmp_path):
    ...
    in its config, the FETIH_KANBAN_TASK env var means it's a focused
...
def worker_env(monkeypatch, tmp_path):
    """Simulate being a worker: FETIH_HOME isolated, FETIH_KANBAN_TASK set
    after we've created the task."""
    home = tmp_path / ".fetih"
    home.mkdir()
    monkeypatch.setenv("FETIH_HOME", str(home))
    monkeypatch.setenv("FETIH_PROFILE", "test-worker")
    ...
    try:
        ...
        conn.close()
    monkeypatch.setenv("FETIH_KANBAN_TASK", tid)
    return tid
...
def test_complete_metadata_round_trips_through_show(worker_env):
    ...
    handoff = {
        "changed_files": ["fetih_cli/kanban.py"],
...
def test_comment_happy_path(worker_env):
    ...
    try:
        ...
        # Author defaults to FETIH_PROFILE env we set in the fixture
...
def test_comment_ignores_caller_supplied_author(worker_env):
    ...
    derived from ``FETIH_PROFILE`` so a worker can't forge a comment
    under an authoritative-looking name like `fetih-system``` and
    ...
    from tools import kanban_tools as kt
    out = kt._handle_comment({
        "task_id": worker_env, "body": "hi", "author": "fetih-system",
    })
    ...
    try:
        comments = kb.list_comments(conn, worker_env)
        # Author comes from FETIH_PROFILE in the fixture, not the
        # caller-supplied "fetih-system" override.
        assert comments[0].author == "test-worker"
    finally:
        conn.close()
...
def test_kanban_guidance_not_in_normal_prompt(monkeypatch, tmp_path):
    """A normal chat session (no FETIH_KANBAN_TASK) must NOT have
...
def test_kanban_guidance_in_worker_prompt(monkeypatch, tmp_path):
    """A worker session (FETIH_KANBAN_TASK set) MUST have the full
...
# ---------------------------------------------------------------------------
#
# A worker process has FETIH_KANBAN_TASK set to its own task id. The
...
# are unrestricted.
#
# Orchestrator profiles (no FETIH_KANBAN_TASK in env) are intentionally
...
def test_worker_can_comment_on_foreign_task(worker_env):
    ...
    # FETIH_PROFILE — never to a caller-controlled string.
    conn = kb.connect()
    try:
...
def test_worker_complete_rejects_stale_run_id(worker_env, monkeypatch):
    ...
    from fetih_cli import kanban_db as kb
    import fetih_cli.kanban_db as _kb
    ...
    from tools import kanban_tools as kt
    monkeypatch.setenv("FETIH_KANBAN_RUN_ID", str(run1.id))
    ...
    monkeypatch.setenv("FETIH_KANBAN_RUN_ID", str(run2.id))
...
def test_orchestrator_complete_any_task_allowed(monkeypatch, tmp_path):
    """Orchestrator profiles (no FETIH_KANBAN_TASK) can still complete
