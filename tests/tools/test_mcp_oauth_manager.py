...
def test_fetih_provider_subclass_exists():
    """FETIHMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _FETIH_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _FETIH_PROVIDER_CLS is not None
    assert issubclass(_FETIH_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_fetih_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns FETIHMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _FETIH_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("FETIH_HOME", str(tmp_path))
    ...
    assert _FETIH_PROVIDER_CLS is not None
    assert isinstance(provider, _FETIH_PROVIDER_CLS)
    assert provider._fetih_server_name == "srv"
