...
Covers:
- :class:`FETIHTokenStorage` ``.meta.json`` roundtrip (save / load / remove)
- The production manager provider
  (:class:`tools.mcp_oauth_manager.FETIHMCPOAuthProvider`) restoring metadata
...
from tools.mcp_oauth import FETIHTokenStorage
from tools.mcp_oauth_manager import _FETIH_PROVIDER_CLS
...
# ---------------------------------------------------------------------------
# FETIHTokenStorage metadata roundtrip
# ---------------------------------------------------------------------------
...
class TestMetadataStorage:
    def test_save_and_load_roundtrip(self, tmp_path, monkeypatch):
        ...
        storage = FETIHTokenStorage("example-server")
    ...
    def test_remove_deletes_meta_file(self, tmp_path, monkeypatch):
        monkeypatch.setenv("FETIH_HOME", str(tmp_path))
        storage = FETIHTokenStorage("cleanup-server")
...
# ---------------------------------------------------------------------------
# Manager-path provider (FETIHMCPOAuthProvider) — production code path
# ---------------------------------------------------------------------------


def _manager_provider_with_context(storage: FETIHTokenStorage, **context_attrs):
    ...
    """
    if _FETIH_PROVIDER_CLS is None:
        pytest.skip("MCP SDK auth not available")
    provider = _FETIH_PROVIDER_CLS.__new__(_FETIH_PROVIDER_CLS)
    provider._fetih_server_name = context_attrs.get("server_name", "srv")
    context = MagicMock()
...
class TestManagerOAuthProviderMetadata:
    def test_initialize_restores_metadata_from_disk(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("FETIH_HOME", str(tmp_path))
        storage = FETIHTokenStorage("mgr-srv")
        ...
        with patch.object(
            _FETIH_PROVIDER_CLS.__bases__[0], "_initialize", new=AsyncMock()
        ):
    ...
    def test_initialize_skips_restore_when_in_memory_present(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("FETIH_HOME", str(tmp_path))
        storage = FETIHTokenStorage("mgr-srv2")
    ...
    def test_persist_metadata_if_changed_writes_on_first_discover(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("FETIH_HOME", str(tmp_path))
        storage = FETIHTokenStorage("persist-srv")
        assert storage.load_oauth_metadata() is None
    ...
    def test_persist_metadata_noop_when_unchanged(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("FETIH_HOME", str(tmp_path))
        storage = FETIHTokenStorage("noop-srv")
        ...
        with patch.object(
            FETIHTokenStorage, "save_oauth_metadata"
        ) as save_spy:
            provider._persist_oauth_metadata_if_changed()
    ...
    def test_async_auth_flow_persists_on_completion(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("FETIH_HOME", str(tmp_path))
        storage = FETIHTokenStorage("flow-srv")
        ...
        with patch.object(
            _FETIH_PROVIDER_CLS.__bases__[0],
            "async_auth_flow",
