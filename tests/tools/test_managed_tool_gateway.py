...
def test_resolve_managed_tool_gateway_derives_vendor_origin_from_shared_domain():
    ...
    assert result.gateway_origin == "https://firecrawl-gateway.github.com/MustafaKemal0146/fetih"
...
def test_resolve_managed_tool_gateway_is_disabled_without_subscription():
    with patch.dict(os.environ, {"TOOL_GATEWAY_DOMAIN": "github.com/MustafaKemal0146/fetih"}, clear=False), \
...
def test_read_nous_access_token_refreshes_expiring_cached_token(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "fetih_cli.auth.resolve_nous_access_token",
