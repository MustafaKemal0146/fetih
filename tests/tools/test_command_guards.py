...
def _clean_state():
    ...
    saved = {}
    for k in ("FETIH_INTERACTIVE", "FETIH_GATEWAY_SESSION", "FETIH_EXEC_ASK", "FETIH_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["FETIH_INTERACTIVE"] = "1"
        session_key = os.getenv("FETIH_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No FETIH_INTERACTIVE or FETIH_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
