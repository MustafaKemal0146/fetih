...
class TestWriteDenyExactPaths:
    ...
    def test_fetih_env(self):
        # ``.env`` under the active FETIH_HOME (profile-aware, not just
        # ``~/.fetih``) must be write-denied. The hermetic test conftest
        # points FETIH_HOME at a tempdir — resolve via get_fetih_home()
        # to match the denylist.
        from fetih_constants import get_fetih_home
        path = str(get_fetih_home() / ".env")
        assert _is_write_denied(path) is True
...
class TestWriteAllowed:
    ...
    def test_fetih_config_not_env(self):
        path = os.path.join(str(Path.home()), ".fetih", "config.yaml")
