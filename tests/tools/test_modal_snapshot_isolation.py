...
def _restore_tool_modules():
    original_fetih_home = os.environ.get("FETIH_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_fetih_home is None:
            os.environ.pop("FETIH_HOME", None)
        else:
            os.environ["FETIH_HOME"] = original_fetih_home
        _reset_modules(("tools", "fetih_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    fetih_cli = types.ModuleType("fetih_cli")
    fetih_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["fetih_cli"] = fetih_cli
    fetih_home = tmp_path / "fetih-home"
    os.environ["FETIH_HOME"] = str(fetih_home)
    sys.modules["fetih_cli.config"] = types.SimpleNamespace(
        get_fetih_home=lambda: fetih_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="fetih-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": fetih_home / "modal_snapshots.json",
