...
class TestCronjobRequirements:
    def test_requires_no_crontab_binary(self, monkeypatch):
        ...
        # because fetih uses an internal scheduler, not system crontab.
    ...
    def test_accepts_gateway_session(self, monkeypatch):
        ...
        monkeypatch.setenv("FETIH_GATEWAY_SESSION", "1")
        monkeypatch.delenv("FETIH_EXEC_ASK", raising=False)
    ...
    def test_rejects_false_like_interactive_env(self, monkeypatch, false_like_value):
        monkeypatch.setenv("FETIH_INTERACTIVE", false_like_value)
        monkeypatch.delenv("FETIH_GATEWAY_SESSION", raising=False)
        monkeypatch.delenv("FETIH_EXEC_ASK", raising=False)
        assert check_cronjob_requirements() is False
    ...
    @pytest.mark.parametrize(
        "var_name",
        ["FETIH_INTERACTIVE", "FETIH_GATEWAY_SESSION", "FETIH_EXEC_ASK"],
    )
    @pytest.mark.parametrize("false_like_value", ["0", "false", "no", "off"])
    def test_rejects_false_like_any_session_env(
        ...
        for v in ("FETIH_INTERACTIVE", "FETIH_GATEWAY_SESSION", "FETIH_EXEC_ASK"):
