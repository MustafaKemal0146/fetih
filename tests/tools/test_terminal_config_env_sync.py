...
at startup, by THREE separate code paths:
  ...
  2. gateway/run.py    -> ``_terminal_env_map`` dict (gateway / messaging
                          platforms)
  3. fetih_cli/config.py:save_config_value
                       -> ``_config_to_env_sync`` dict (one-shot when the
                          user runs ``fetih config set …``)
...
dicts) keeps the test independent of the user's ~/.fetih/config.yaml and
mirrors the pattern used in tests/fetih_cli/test_config_drift.py.
"""
...
def _save_config_env_sync_keys() -> set[str]:
    """terminal config keys bridged by ``fetih config set foo bar``."""
    from fetih_cli import config as hc_config
...
def test_save_config_set_supports_critical_bridged_keys():
    """``fetih config set terminal.X true`` must propagate to .env for
    ...
    assert not missing, (
        f"`fetih config set terminal.X` doesn't sync these load-bearing "
        f"keys to .env: {sorted(missing)}.  Add them to _config_to_env_sync "
        f"in fetih_cli/config.py:set_config_value."
    )
