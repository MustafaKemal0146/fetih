...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._fetih_bin_dir", return_value="/nonexistent"), \
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_fetih_bin(self):
        """After _INSTALL_FAILED, manual install in FETIH_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        fetih_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(fetih_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(fetih_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._fetih_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == fetih_bin
            assert _tirith_mod._resolved_path == fetih_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# FETIH_HOME isolation
# ---------------------------------------------------------------------------

class TestFETIHHomeIsolation:
    def test_fetih_bin_dir_respects_fetih_home(self):
        """_fetih_bin_dir must use FETIH_HOME, not hardcoded ~/.fetih."""
        from tools.tirith_security import _fetih_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"FETIH_HOME": tmpdir}):
            result = _fetih_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_fetih_home(self):
        """_failure_marker_path must use FETIH_HOME, not hardcoded ~/.fetih."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"FETIH_HOME": "/custom/fetih"}):
            result = _failure_marker_path()
        assert result == "/custom/fetih/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets FETIH_HOME; verify it's active."""
        fetih_home = os.getenv("FETIH_HOME")
        assert fetih_home is not None, "FETIH_HOME should be set by conftest"
        assert "fetih_test" in fetih_home, "Should point to test temp dir"

    def test_get_fetih_home_fallback(self):
        """Without FETIH_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_fetih_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove FETIH_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".fetih")
            result = _get_fetih_home()
