...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        fetih_home.mkdir()
        (fetih_home / "token.json").write_text("{}")
        ...
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(fetih_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.fetih/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        fetih_home = tmp_path / ".fetih"
        fetih_home.mkdir()
        (fetih_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        fetih_home = tmp_path / ".fetih"
        fetih_home.mkdir()
        (fetih_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        fetih_home = tmp_path / ".fetih"
        fetih_home.mkdir()
        (fetih_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.fetih/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        fetih_home = tmp_path / ".fetih"
        (fetih_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"FETIH_HOME": str(fetih_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.fetih")

        assert mounts[0]["container_path"] == "/home/user/.fetih/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.fetih/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.fetih/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape FETIH_HOME."""
        monkeypatch.setenv("FETIH_HOME", str(tmp_path / ".fetih"))
        (tmp_path / ".fetih").mkdir()

        # Create a sensitive file one level above fetih_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        fetih_home = tmp_path / ".fetih"
        fetih_home.mkdir()
        monkeypatch.setenv("FETIH_HOME", str(fetih_home))

        # Create a fake sensitive file outside fetih_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside FETIH_HOME must still be registered."""
        fetih_home = tmp_path / ".fetih"
        fetih_home.mkdir()
        monkeypatch.setenv("FETIH_HOME", str(fetih_home))
        (fetih_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_fetih_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of FETIH_HOME must be allowed."""
        fetih_home = tmp_path / ".fetih"
        fetih_home.mkdir()
        subdir = fetih_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside FETIH_HOME pointing outside must be rejected."""
        fetih_home = tmp_path / ".fetih"
        fetih_home.mkdir()
        monkeypatch.setenv("FETIH_HOME", str(fetih_home))

        # Create a sensitive file outside fetih_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside fetih_home pointing outside
        symlink = fetih_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes FETIH_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, fetih_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape FETIH_HOME."""
        fetih_home = tmp_path / ".fetih"
        fetih_home.mkdir()
        monkeypatch.setenv("FETIH_HOME", str(fetih_home))
        ...
        sensitive.write_text("{}")
        self._write_config(fetih_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(fetih_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside FETIH_HOME via config must still mount."""
        fetih_home = tmp_path / ".fetih"
        fetih_home.mkdir()
        monkeypatch.setenv("FETIH_HOME", str(fetih_home))

        (fetih_home / "oauth.json").write_text("{}")
        self._write_config(fetih_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        fetih_home = tmp_path / ".fetih"
        fetih_home.mkdir()
        (fetih_home / "cache" / "documents").mkdir(parents=True)
        (fetih_home / "cache" / "audio").mkdir(parents=True)
        monkeypatch.setenv("FETIH_HOME", str(fetih_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.fetih/cache/documents" in paths
        assert "/root/.fetih/cache/audio" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.fetih/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        fetih_home = tmp_path / ".fetih"
        fetih_home.mkdir()
        # Use legacy dir name — get_fetih_dir prefers old if it exists
        (fetih_home / "document_cache").mkdir()
        (fetih_home / "image_cache").mkdir()
        monkeypatch.setenv("FETIH_HOME", str(fetih_home))

        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(fetih_home / "document_cache") in host_paths
        assert str(fetih_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.fetih/cache/documents" in container_paths
        assert "/root/.fetih/cache/images" in container_paths

    def test_empty_fetih_home(self, tmp_path, monkeypatch):
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        fetih_home = tmp_path / ".fetih"
        doc_dir = fetih_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        fetih_home = tmp_path / ".fetih"
        ss_dir = fetih_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.fetih/cache/screenshots/session_abc/screen1.png"
