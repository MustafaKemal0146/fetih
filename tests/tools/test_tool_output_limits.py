...
class TestDefaults:
    ...
    def test_get_limits_returns_defaults_when_config_not_a_dict(self):
        ...
        with patch("fetih_cli.config.load_config", return_value="not a dict"):
            limits = tol.get_tool_output_limits()
    ...
    def test_get_limits_returns_defaults_when_load_config_raises(self):
        ...
        with patch("fetih_cli.config.load_config", side_effect=_boom):
            limits = tol.get_tool_output_limits()
...
class TestOverrides:
    def test_user_config_overrides_all_three(self):
        ...
        with patch("fetih_cli.config.load_config", return_value=cfg):
            limits = tol.get_tool_output_limits()
        assert limits == {
            "max_bytes": 100_000,
            "max_lines": 5000,
...
class TestDefaultConfigHasSection:
    """The DEFAULT_CONFIG in fetih_cli.config must expose tool_output so
    that ``fetih setup`` and default installs stay in sync with the
    helpers here."""
