...
Run from the PR worktree:
    ...
    cd ~/.fetih/fetih-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.fetih/fetih-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.fetih/fetih-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a fetih-agent checkout"
)
assert (PR_DIR / "tools" / "browser_tool.py").exists(), (
    f"PR_DIR={PR_DIR} doesn't look like a fetih-agent checkout"
)
...
# Isolated FETIH_HOME for the config write.
