"""Tests for fetih_constants module."""
...
from fetih_constants import (
    ...
    get_default_fetih_root,
    is_container,
    parse_reasoning_effort,
)


class TestGetDefaultFETIHRoot:
    """Tests for get_default_fetih_root() — Docker/custom deployment awareness."""

    def test_no_fetih_home_returns_native(self, tmp_path, monkeypatch):
        """When FETIH_HOME is not set, returns ~/.fetih."""
        monkeypatch.delenv("FETIH_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_fetih_root() == tmp_path / ".fetih"

    def test_fetih_home_is_native(self, tmp_path, monkeypatch):
        """When FETIH_HOME = ~/.fetih, returns ~/.fetih."""
        native = tmp_path / ".fetih"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("FETIH_HOME", str(native))
        assert get_default_fetih_root() == native

    def test_fetih_home_is_profile(self, tmp_path, monkeypatch):
        """When FETIH_HOME is a profile under ~/.fetih, returns ~/.fetih."""
        native = tmp_path / ".fetih"
        profile = native / "profiles" / "coder"
        profile.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("FETIH_HOME", str(profile))
        assert get_default_fetih_root() == native

    def test_fetih_home_is_docker(self, tmp_path, monkeypatch):
        """When FETIH_HOME points outside ~/.fetih (Docker), returns FETIH_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("FETIH_HOME", str(docker_home))
        assert get_default_fetih_root() == docker_home

    def test_fetih_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any FETIH_HOME outside ~/.fetih is treated as the root."""
        custom = tmp_path / "my-fetih-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("FETIH_HOME", str(custom))
        assert get_default_fetih_root() == custom

    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (FETIH_HOME=<root>/profiles/<name>),
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("FETIH_HOME", str(profile))
        assert get_default_fetih_root() == docker_root
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(fetih_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(fetih_constants, "_container_detected", True)
        assert is_container() is True
