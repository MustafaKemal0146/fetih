...
def _isolate(tmp_path, monkeypatch):
    """Redirect FETIH_HOME and clear module caches."""
    fetih_home = tmp_path / ".fetih"
    fetih_home.mkdir()
    monkeypatch.setenv("FETIH_HOME", str(fetih_home))
    # Write a minimal config so load_config doesn't fail
    (fetih_home / "config.yaml").write_text("model:\n  default: test-model\n")


def _write_config(tmp_path, config_dict):
    """Write a config.yaml to the test FETIH_HOME."""
    import yaml
...
class TestResolveProviderClientMainAlias:
    ...
    def test_main_resolves_github_copilot_alias(self, tmp_path):
        ...
        with (
            patch("fetih_cli.auth.resolve_api_key_provider_credentials", return_value={
