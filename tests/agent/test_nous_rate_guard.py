"""Tests for agent/nous_rate_guard.py — cross-session FETIH Portal rate limit guard."""
...
def rate_guard_env(tmp_path, monkeypatch):
    ...
    fetih_home = str(tmp_path / ".fetih")
    os.makedirs(fetih_home, exist_ok=True)
    monkeypatch.setenv("FETIH_HOME", fetih_home)
...
class TestIsGenuineNousRateLimit:
    ...
    FETIH Portal multiplexes upstreams (DeepSeek, Kimi, MiMo, FETIH).
