...
2. ``fetih lsp status`` surfaces a ``Backend warnings`` section when
...
def test_install_npm_works_without_extras(tmp_path, monkeypatch):
    ...
    install_targets = [c for c in cmd if not c.startswith("-") and c not in {
        "install", "--prefix", str(install_mod.fetih_lsp_bin_dir().parent),
        "/usr/bin/npm",
    }]
...
# ---------------------------------------------------------------------------
# Fix 2: ``fetih lsp status`` surfaces shellcheck-missing for bash
# ---------------------------------------------------------------------------
