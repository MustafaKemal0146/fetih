...
""",
        ...
        from fetih_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        fetih_home = tmp_path / ".fetih"
        fetih_home.mkdir()
        (fetih_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("FETIH_HOME", str(fetih_home))
        from fetih_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from fetih_cli.plugins import PluginContext, PluginManifest, PluginManager
