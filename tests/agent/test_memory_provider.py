...
class TestUserInstalledProviderDiscovery:
    """Memory providers installed to $FETIH_HOME/plugins/ should be found.
    ...
    def test_load_user_plugin(self, tmp_path, monkeypatch):
        """load_memory_provider() can load from $FETIH_HOME/plugins/."""
        from plugins.memory import load_memory_provider
        self._make_user_memory_plugin(tmp_path, "myexternal")
        monkeypatch.setattr(
            "plugins.memory._get_user_plugins_dir",
...
class TestSetupFieldFiltering:
    ...
    memory setup wizard in fetih_cli/memory_setup.py.
...
class TestOnMemoryWriteBridge:
    ...
    def test_memory_manager_tool_injection_deduplicates(self):
        ...
        on providers that enforce unique names (e.g. Xiaomi MiMo via FETIH Portal).

        Regression test for: duplicate mnemosyne_recall / mnemosyne_remember /
        mnemosyne_stats in tools array → 400 from FETIH Portal.
        """
        mgr = MemoryManager()
