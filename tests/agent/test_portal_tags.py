"""Tests for agent.portal_tags — FETIH Portal request tag contract."""

from __future__ import annotations


def test_fetih_client_tag_includes_current_version():
    """The client tag must reflect fetih_cli.__version__ verbatim."""
    from fetih_cli import __version__
    from agent.portal_tags import fetih_client_tag

    assert fetih_client_tag() == f"client=fetih-client-v{__version__}"


def test_fetih_client_tag_format():
    """The client tag has the exact shape FETIH Portal expects."""
    from agent.portal_tags import fetih_client_tag

    tag = fetih_client_tag()
    assert tag.startswith("client=fetih-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    """Every FETIH Portal request gets BOTH the product tag and the version tag."""
    from agent.portal_tags import fetih_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=fetih-agent" in tags
    assert fetih_client_tag() in tags
    assert len(tags) == 2
