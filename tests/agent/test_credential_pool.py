...
def _write_auth_store(tmp_path, payload: dict) -> None:
    ...
    (fetih_home / "auth.json").write_text(json.dumps(payload, indent=2))
...
def test_round_robin_strategy_rotates_priorities(tmp_path, monkeypatch):
    ...
    config_path = tmp_path / "fetih" / "config.yaml"
...
def test_explicit_reset_timestamp_overrides_default_429_ttl(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "fetih_cli.auth._import_codex_cli_tokens",
        lambda: None,
    )
    _write_auth_store(
        tmp_path,
...
def test_mark_exhausted_and_rotate_persists_status(tmp_path, monkeypatch):
    ...
    auth_payload = json.loads((tmp_path / "fetih" / "auth.json").read_text())
...
def test_load_pool_prefers_dotenv_over_stale_os_environ(tmp_path, monkeypatch):
    ...
    from a parent shell) must NOT shadow the fresh key in ~/.fetih/.env when
    ...
    # User edited ~/.fetih/.env with the fresh key
    (fetih_home / ".env").write_text(
...
def test_load_pool_falls_back_to_os_environ_when_dotenv_empty(tmp_path, monkeypatch):
    """When ~/.fetih/.env does not define OPENROUTER_API_KEY (typical Docker /
    ...
    (fetih_home / ".env").write_text("SOME_OTHER_VAR=unrelated\n")
...
def test_nous_pool_terminal_refresh_removes_device_code_entry(tmp_path, monkeypatch):
    monkeypatch.setenv("FETIH_HOME", str(tmp_path / "fetih"))
    monkeypatch.setenv("FETIH_SHARED_AUTH_DIR", str(tmp_path / "shared"))
    _write_auth_store(
        tmp_path,
        {
            "version": 1,
    ...
    from fetih_cli import auth as auth_mod
...
def test_load_pool_removes_stale_file_backed_singleton_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "agent.anthropic_adapter.read_fetih_oauth_credentials",
        lambda: None,
    )
    monkeypatch.setattr(
...
def test_singleton_seed_does_not_clobber_manual_oauth_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("ANTHROPIC_TOKEN", raising=False)
    monkeypatch.delenv("CLAUDE_CODE_OAUTH_TOKEN", raising=False)
    monkeypatch.setattr("fetih_cli.auth.is_provider_explicitly_configured", lambda pid: True)
    _write_auth_store(
        tmp_path,
        {
            "version": 1,
    ...
    assert {entry.source for entry in entries} == {"manual:fetih_pkce", "fetih_pkce"}
...
def test_get_custom_provider_pool_key(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("FETIH_HOME", str(tmp_path / "fetih"))
    (tmp_path / "fetih").mkdir(parents=True, exist_ok=True)
    import yaml
    config_path = tmp_path / "fetih" / "config.yaml"
    config_path.write_text(yaml.dump({
        "custom_providers": [
...
def test_load_pool_seeds_copilot_via_gh_auth_token(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "fetih_cli.copilot_auth.resolve_copilot_token",
...
def test_load_pool_seeds_qwen_oauth_via_cli_tokens(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "fetih_cli.auth.resolve_qwen_runtime_credentials",
        lambda **kw: {
            "provider": "qwen-oauth",
        ...
        tmp_path,
        {
            ...
            "providers": {
                "nous": {
                    ...
                    "client_id": "fetih-cli",
                    "portal_base_url": "https://github.com/MustafaKemal0146/fetih",
                    "inference_base_url": "https://inference.github.com/MustafaKemal0146/fetih/v1",
                    "token_type": "Bearer",
...
def test_sync_codex_entry_from_auth_store_adopts_newer_tokens(tmp_path, monkeypatch):
    ...
    # Simulate `fetih auth openai-codex` replacing the token pair on disk.
...
def test_codex_exhausted_entry_recovers_via_auth_store_sync(tmp_path, monkeypatch):
    ...
    rate-limit reset the user ran `fetih model` to reauth, but the pool
    ...
    # Simulate `fetih model` / `fetih auth` refreshing the tokens.
...
def test_is_terminal_xai_oauth_refresh_error():
    from fetih_cli.auth import AuthError, _is_terminal_xai_oauth_refresh_error
...
def test_is_terminal_codex_oauth_refresh_error():
    from fetih_cli.auth import AuthError, _is_terminal_codex_oauth_refresh_error
