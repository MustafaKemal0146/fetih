...
def sample_repo(tmp_path: Path) -> Path:
    ...
    repo.mkdir()
    _git(repo, "init")
    _git(repo, "config", "user.name", "FETIH Tests")
...
@pytest.mark.asyncio
async def test_blocks_sensitive_home_and_fetih_paths(tmp_path: Path, monkeypatch):
    from agent.context_references import preprocess_context_references_async

    monkeypatch.setenv("HOME", str(tmp_path))
    monkeypatch.setenv("FETIH_HOME", str(tmp_path / ".fetih"))

    fetih_env = tmp_path / ".fetih" / ".env"
    fetih_env.parent.mkdir(parents=True)
    fetih_env.write_text("API_KEY=super-secret\n", encoding="utf-8")
    ...
    result = await preprocess_context_references_async(
        "read @file:.fetih/.env and @file:.ssh/id_rsa",
        cwd=tmp_path,
