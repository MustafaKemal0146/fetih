...
Note: Tests that import ``fetih_cli.auth`` or ``fetih_cli.runtime_provider``
...
class TestProviderAliases:
    ...
    def test_aws_alias(self):
        from fetih_cli.models import _PROVIDER_ALIASES
...
class TestProviderLabels:
    ...
    def test_bedrock_label(self):
        from fetih_cli.models import _PROVIDER_LABELS
...
class TestResolveProvider:
    ...
    def test_auto_detect_with_aws_credentials(self, monkeypatch):
        ...
        with patch("fetih_cli.auth._load_auth_store", return_value={}):
...
class TestRuntimeProvider:
    ...
    def test_bedrock_runtime_resolution(self, monkeypatch):
        ...
        with patch("fetih_cli.runtime_provider.resolve_provider", return_value="bedrock"), \
             patch("fetih_cli.runtime_provider._get_model_config", return_value={"provider": "bedrock"}):
    ...
    def test_bedrock_runtime_no_credentials_raises_on_auto_detect(self, monkeypatch):
        ...
        with patch("fetih_cli.runtime_provider.resolve_provider", return_value="bedrock"), \
             patch("fetih_cli.runtime_provider._get_model_config", return_value={"provider": "bedrock"}), \
             patch("fetih_cli.runtime_provider.resolve_requested_provider", return_value="auto"), \
...
class TestProvidersModule:
    """Verify bedrock is wired into fetih_cli/providers.py."""

    def test_bedrock_alias_in_providers(self):
        from fetih_cli.providers import ALIASES
...
class TestPackaging:
    ...
    def test_bedrock_is_not_eager_installed_by_all_extra(self):
        extras = self._optional_dependencies()
        assert "fetih-agent[bedrock]" not in extras["all"]
