...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. FETIH passes a narrow app-server config override
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("FETIH_HOME", "/users/alice/.fetih/profiles/backend-worker")
        monkeypatch.setenv("FETIH_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "FETIH_KANBAN_DB",
            "/users/alice/.fetih/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.fetih/kanban/boards/smoke"]'
            in cmd
        )
