...
class TestFETIHConstantsFallback:
    """Tests for _fetih_home.py fallback when fetih_constants is unavailable."""

    HELPER_PATH = (
        Path(__file__).resolve().parents[2]
        / "skills/productivity/google-workspace/scripts/_fetih_home.py"
    )

    def _load_helper(self, monkeypatch):
        """Load _fetih_home.py with fetih_constants blocked."""
        monkeypatch.setitem(sys.modules, "fetih_constants", None)
        spec = importlib.util.spec_from_file_location("_fetih_home_test", self.HELPER_PATH)
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        return module

    def test_fallback_uses_fetih_home_env_var(self, monkeypatch, tmp_path):
        """When fetih_constants is missing, FETIH_HOME comes from env var."""
        monkeypatch.setenv("FETIH_HOME", str(tmp_path / "custom-fetih"))
        module = self._load_helper(monkeypatch)
        assert module.get_fetih_home() == tmp_path / "custom-fetih"

    def test_fallback_defaults_to_dot_fetih(self, monkeypatch):
        """When fetih_constants is missing and FETIH_HOME unset, default to ~/.fetih."""
        monkeypatch.delenv("FETIH_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.get_fetih_home() == Path.home() / ".fetih"

    def test_fallback_ignores_empty_fetih_home(self, monkeypatch):
        """Empty/whitespace FETIH_HOME is treated as unset."""
        monkeypatch.setenv("FETIH_HOME", "  ")
        module = self._load_helper(monkeypatch)
        assert module.get_fetih_home() == Path.home() / ".fetih"

    def test_fallback_display_fetih_home_shortens_path(self, monkeypatch):
        """Fallback display_fetih_home() uses ~/ shorthand like the real one."""
        monkeypatch.delenv("FETIH_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.display_fetih_home() == "~/.fetih"

    def test_fallback_display_fetih_home_profile_path(self, monkeypatch):
        """Fallback display_fetih_home() handles profile paths under ~/."""
        monkeypatch.setenv("FETIH_HOME", str(Path.home() / ".fetih/profiles/coder"))
        module = self._load_helper(monkeypatch)
        assert module.display_fetih_home() == "~/.fetih/profiles/coder"

    def test_fallback_display_fetih_home_custom_path(self, monkeypatch):
        """Fallback display_fetih_home() returns full path for non-home locations."""
        monkeypatch.setenv("FETIH_HOME", "/opt/fetih-custom")
        module = self._load_helper(monkeypatch)
        assert module.display_fetih_home() == "/opt/fetih-custom"

    def test_delegates_to_fetih_constants_when_available(self):
        """When fetih_constants IS importable, _fetih_home delegates to it."""
        spec = importlib.util.spec_from_file_location(
            "_fetih_home_happy", self.HELPER_PATH
        )
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        import fetih_constants
        assert module.get_fetih_home is fetih_constants.get_fetih_home
        assert module.display_fetih_home is fetih_constants.display_fetih_home
