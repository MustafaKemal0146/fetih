...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    fetih_state = ModuleType("fetih_state")
    fetih_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "fetih_state", fetih_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "FETIH"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "FETIH"
