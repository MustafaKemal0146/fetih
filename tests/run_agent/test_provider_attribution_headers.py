...
def test_ai_gateway_base_url_applies_attribution_headers(mock_openai):
    ...
    assert headers["HTTP-Referer"] == "https://github.com/MustafaKemal0146/fetih"
    assert headers["X-Title"] == "FETIH"
    assert headers["User-Agent"].startswith("FETIHAgent/")
...
def test_nvidia_cloud_base_url_applies_billing_origin_header(mock_openai):
    ...
    assert agent._client_kwargs["default_headers"]["X-BILLING-INVOKE-ORIGIN"] == "FETIHAgent"
...
def test_routed_client_preserves_openai_sdk_custom_headers(mock_openai):
    ...
    routed_client = SimpleNamespace(
        api_key="test-key",
        base_url="https://integrate.api.nvidia.com/v1",
        _custom_headers={"X-BILLING-INVOKE-ORIGIN": "FETIHAgent"},
    )
...
def test_openrouter_headers_include_response_cache_when_enabled(mock_openai):
    ...
    with patch("fetih_cli.config.load_config", return_value={
