...
FETIH preserves the callable end-to-end so the SDK refreshes tokens
...
class TestTruncateTokenCallable:
    def test_callable_returns_placeholder(self):
        ...
        ``"<function ...>"``."""
        from fetih_cli.web_server import _truncate_token
...
class TestCliEnsureRuntimeCredentialsCallable:
    ...
    ``FETIHCLI`` instance) — the predicate change is the load-bearing
...
class TestInlinedDisplayMasks:
    ...
    def test_cli_show_config_handles_callable(self):
        """``cli.FETIHCLI.show_config`` previously did
        ...
        assert "is_token_provider(self.api_key)" in src, (
            "cli.FETIHCLI.show_config must guard self.api_key via "
            "is_token_provider so callable Entra ID providers don't "
            "crash /config."
        )
        assert '"Microsoft Entra ID"' in src, (
            "cli.FETIHCLI.show_config must print the static "
