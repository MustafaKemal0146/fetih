...
class TestTryActivateFallback:
    ...
    def test_activates_nous_fallback(self):
        """FETIH Portal fallback should use OAuth credentials and chat_completions mode."""
        agent = _make_agent(
            fallback_model={"provider": "nous", "model": "nous-fetih-3"},
        )
        mock_client = _mock_resolve(
        ...
        with patch(
            "agent.auxiliary_client.resolve_provider_client",
            return_value=(mock_client, "nous-fetih-3"),
        ):
            result = agent._try_activate_fallback()
            assert result is True
            assert agent.model == "nous-fetih-3"
