...
FETIH_HOME scoping, and tick() serialization for profile jobs.
"""
...
def isolated_cron_profile_home(tmp_path, monkeypatch):
    """Create an isolated FETIH root with a named profile and temp cron store."""
    root = tmp_path / "fetih-root"
    profile_home = root / "profiles" / "support"
    profile_home.mkdir(parents=True)
    (root / "cron").mkdir(parents=True)

    monkeypatch.setenv("FETIH_HOME", str(root))
...
class TestCronjobToolProfile:
    ...
    def test_schema_advertises_profile(self):
        ...
        assert "fetih profile" in desc_lower
        assert "context-local" in desc_lower
        assert "subprocess" in desc_lower
        assert "temporarily sets fetih_home" not in desc_lower
...
class TestRunJobProfileContext:
    ...
    def _install_agent_stubs(monkeypatch, observed: dict):
        ...
        class FakeAgent:
            def __init__(self, **kwargs):
                ...
                observed["env_home_during_init"] = os.environ.get("FETIH_HOME")
                observed["profile_env_only_during_init"] = os.environ.get(
                    "FETIH_PROFILE_TEST_ONLY"
                )
                observed["profile_env_shared_during_init"] = os.environ.get(
                    "FETIH_PROFILE_TEST_SHARED"
                )
                observed["fetih_home_during_init"] = str(get_fetih_home())
                observed["scheduler_home_during_init"] = str(sched._get_fetih_home())
                observed["skip_context_files"] = kwargs.get("skip_context_files")

            def run_conversation(self, *_a, **_kw):
                from fetih_constants import get_fetih_home

                observed["env_home_during_run"] = os.environ.get("FETIH_HOME")
                ...
                observed["fetih_home_during_run"] = str(get_fetih_home())
                observed["scheduler_home_during_run"] = str(sched._get_fetih_home())
        ...
        from fetih_cli import runtime_provider as runtime_provider
        ...
        monkeypatch.setattr(sched, "_fetih_home", None)
        monkeypatch.setenv("FETIH_CRON_TIMEOUT", "0")
    ...
    def test_run_job_sets_and_restores_profile_home(
        ...
        assert observed["fetih_home_during_init"] == str(profile_home.resolve())
        assert observed["fetih_home_during_run"] == str(profile_home.resolve())
        ...
        assert os.environ["FETIH_HOME"] == str(root)
        assert sched._get_fetih_home() == root
    ...
    def test_profile_dotenv_environment_is_restored(
        ...
        observed: dict = {}
        self._install_agent_stubs(monkeypatch, observed)
        monkeypatch.setenv("FETIH_PROFILE_TEST_SHARED", "outer")
        monkeypatch.delenv("FETIH_PROFILE_TEST_ONLY", raising=False)

        def fake_load_dotenv(path, *_a, **_kw):
            observed.setdefault("dotenv_paths", []).append(str(path))
            os.environ["FETIH_PROFILE_TEST_SHARED"] = "profile-value"
            os.environ["FETIH_PROFILE_TEST_ONLY"] = "profile-only"
            os.environ["FETIH_CRON_TIMEOUT"] = "123"
            return True
        ...
        assert os.environ["FETIH_PROFILE_TEST_SHARED"] == "outer"
        assert "FETIH_PROFILE_TEST_ONLY" not in os.environ
        assert os.environ["FETIH_CRON_TIMEOUT"] == "0"
        assert os.environ["FETIH_HOME"] == str(root)
        assert sched._get_fetih_home() == root
    ...
    def test_no_agent_profile_uses_profile_scripts_dir_and_restores_env(
        ...
        (scripts_dir / "print_home.py").write_text(
            "import os\nprint(os.environ.get('FETIH_HOME', ''))\n",
            encoding="utf-8",
        )
        monkeypatch.setattr(sched, "_fetih_home", None)
    ...
    def test_run_job_without_profile_leaves_fetih_home_untouched(
        self, isolated_cron_profile_home, monkeypatch
    ):
        import cron.scheduler as sched
        ...
        assert observed["fetih_home_during_init"] == str(root)
        assert os.environ["FETIH_HOME"] == str(root)
