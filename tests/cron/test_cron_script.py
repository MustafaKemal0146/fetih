...
- Path resolution (absolute, relative to FETIH_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp FETIH_HOME."""
    fetih_home = tmp_path / ".fetih"
    fetih_home.mkdir()
    (fetih_home / "cron").mkdir()
    (fetih_home / "cron" / "output").mkdir()
    (fetih_home / "scripts").mkdir()
    monkeypatch.setenv("FETIH_HOME", str(fetih_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "FETIH_DIR", fetih_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", fetih_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", fetih_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", fetih_home / "cron" / "output")
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.fetih/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("FETIH_SESSION_PLATFORM") is None
        assert os.environ.get("FETIH_SESSION_CHAT_ID") is None
        assert os.environ.get("FETIH_SESSION_CHAT_NAME") is None
