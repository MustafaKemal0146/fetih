...
being persisted to ~/.fetih/discord_threads.json.
"""
...
class TestDiscordThreadPersistence:
    ...
    def _make_adapter(self, tmp_path):
        """Build a minimal DiscordAdapter with FETIH_HOME pointed at tmp_path."""
        from gateway.config import PlatformConfig
        from gateway.platforms.discord import DiscordAdapter

        config = PlatformConfig(enabled=True, token="test-token")
        with patch.dict(os.environ, {"FETIH_HOME": str(tmp_path)}):
    ...
    def test_missing_fetih_home_does_not_crash(self, tmp_path):
        """Load/save tolerate missing directories."""
        fake_home = tmp_path / "nonexistent" / "deep"
        with patch.dict(os.environ, {"FETIH_HOME": str(fake_home)}):
