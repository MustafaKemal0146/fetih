...
def _redirect_cache(tmp_path, monkeypatch):
    """Point document cache to tmp_path so tests don't touch ~/.fetih."""
    monkeypatch.setattr(
...
class TestAppMentionHandler:
    ...
    def test_app_mention_registered_on_connect(self):
        ...
        # covering every COMMAND_REGISTRY entry (e.g. /fetih, /btw, /stop,
        ...
        for expected in ("/fetih", "/btw", "/stop", "/model", "/help"):
...
class TestBangPrefixCommands:
    ...
    async def test_bang_with_bot_suffix_resolves(self, adapter):
        """``!stop@fetih`` matches the get_command() ``@suffix`` stripping."""
        await adapter._handle_slack_message(self._make_event("!stop@fetih"))

        msg_event = adapter.handle_message.call_args[0][0]
        assert msg_event.text.startswith("/stop@fetih")
        assert msg_event.message_type == MessageType.COMMAND
...
class TestSlashCommands:
    ...
    # instead of as /fetih subcommands. This is the Discord/Telegram parity
    # fix: the slash name itself becomes the command.
    # ------------------------------------------------------------------

    @pytest.mark.asyncio
    async def test_native_btw_slash(self, adapter):
        """/btw with args must dispatch to /background, not /fetih btw."""
        command = {
            "command": "/btw",
    ...
    @pytest.mark.asyncio
    async def test_legacy_fetih_prefix_still_works(self, adapter):
        """Backward compat: /fetih btw foo must still route to /btw foo.

        Old workspace manifests only declared /fetih as the single slash.
    ...
    @pytest.mark.asyncio
    async def test_legacy_fetih_freeform_question(self, adapter):
        """/fetih <free-form text> must stay as the raw text (non-command)."""
        command = {
            "command": "/fetih",
...
class TestSlashEphemeralAck:
    ...
    @pytest.mark.asyncio
    async def test_legacy_fetih_slash_stashes_context(self, adapter):
        """Legacy /fetih <subcommand> also stashes context."""
        command = {
            "command": "/fetih",
            "text": "help",
            "user_id": "U_H",
    ...
    @pytest.mark.asyncio
    async def test_freeform_fetih_question_does_not_stash_context(self, adapter):
        """Free-form /fetih <question> must NOT route agent reply ephemeral."""
        command = {
            "command": "/fetih",
