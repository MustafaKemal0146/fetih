...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      fetih-agent = self'.packages.default;
      fetihVenv = fetih-agent.fetihVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "fetih-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${fetihVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "fetih-cross-eval" { } (
          if failures != [] then
        ...
        package-contents = pkgs.runCommand "fetih-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${fetih-agent}/bin/fetih || (echo "FAIL: fetih binary missing"; exit 1)
          test -x ${fetih-agent}/bin/fetih-agent || (echo "FAIL: fetih-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${fetih-agent}/bin/fetih version 2>&1 | grep -qi "fetih" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "fetih-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in fetih fetih-agent fetih-acp; do
            test -x ${fetih-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "fetih-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking fetih --help ==="
          ${fetih-agent}/bin/fetih --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${fetih-agent}/bin/fetih --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "fetih-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${fetih-agent}/share/fetih-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${fetih-agent}/share/fetih-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "FETIH_BUNDLED_SKILLS" ${fetih-agent}/bin/fetih || \
            (echo "FAIL: FETIH_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: FETIH_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "fetih-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${fetih-agent}/share/fetih-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${fetih-agent}/share/fetih-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "FETIH_BUNDLED_PLUGINS" ${fetih-agent}/bin/fetih || \
            (echo "FAIL: FETIH_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: FETIH_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-tui = pkgs.runCommand "fetih-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${fetih-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${fetih-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "FETIH_TUI_DIR" ${fetih-agent}/bin/fetih || \
            (echo "FAIL: FETIH_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: FETIH_TUI_DIR set in wrapper"
        ...
        # Verify FETIH_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        fetih-node = pkgs.runCommand "fetih-node-version" { } ''
          set -e
          echo "=== Checking FETIH_NODE in wrapper ==="
          grep -q "FETIH_NODE" ${fetih-agent}/bin/fetih || \
            (echo "FAIL: FETIH_NODE not set in wrapper"; exit 1)
          echo "PASS: FETIH_NODE present in wrapper"

          FETIH_NODE=$(sed -n "s/^export FETIH_NODE='\(.*\)'/\1/p" ${fetih-agent}/bin/fetih)
          test -x "$FETIH_NODE" || (echo "FAIL: FETIH_NODE=$FETIH_NODE not executable"; exit 1)
          echo "PASS: FETIH_NODE executable at $FETIH_NODE"

          NODE_MAJOR=$("$FETIH_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All FETIH_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify FETIH_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "fetih-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(FETIH_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking FETIH_MANAGED guards ==="
          check_blocked "config set" ${fetih-agent}/bin/fetih config set model foo
          check_blocked "config edit" ${fetih-agent}/bin/fetih config edit
        ...
        extra-python-packages = let
          ...
          fetihWithExtra = fetih-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "fetih-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${fetihWithExtra}/bin/fetih || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${fetihWithExtra}/bin/fetih || \
          ...
          if grep -q "PYTHONPATH" ${fetih-agent}/bin/fetih; then
        ...
        extra-dependency-groups = let
          fetihWithGroups = fetih-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "fetih-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${fetihWithGroups}"
          echo "venv: ${fetihWithGroups.fetihVenv}"
        ...
        in pkgs.runCommand "fetih-config-roundtrip" {
          ...
          merge_and_load() {
            local fetih_home="$1"
            export FETIH_HOME="$fetih_home"
            ${configMergeScript} ${nixSettings} "$fetih_home/config.yaml"
            ${fetihVenv}/bin/python3 -c '
import json, sys
