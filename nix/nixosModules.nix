# nix/nixosModules.nix — NixOS module for fetih-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: fetih runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $FETIH_HOME/.env
# and read by fetih at startup — no container recreation needed for env changes.
#
# Tool resolution: the fetih wrapper uses --suffix PATH for nix store tools,
...
#
# Usage:
#   services.fetih-agent = {
#     enable = true;
#     settings.model = "anthropic/claude-sonnet-4";
#     environmentFiles = [ config.sops.secrets."fetih/env".path ];
#   };
#
{ inputs, ... }: {
  ...
  let
    cfg = config.services.fetih-agent;
    effectivePackage =
    ...
    fetih-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-fetih-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "fetih-config-attrs";
      description = "FETIH YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "fetih-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "fetih-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'FETIH_DOC_EOF'\n${value}\nFETIH_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "fetih-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/fetih";
    ...
    # fetih user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "fetih-container-entrypoint" ''
      set -eu

      FETIH_UID="''${FETIH_UID:?FETIH_UID must be set}"
      FETIH_GID="''${FETIH_GID:?FETIH_GID must be set}"

      # ── Group: ensure a group with GID=$FETIH_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$FETIH_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="fetih"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$FETIH_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$FETIH_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$FETIH_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$FETIH_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="fetih"
        TARGET_HOME="/home/fetih"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$FETIH_UID" -g "$FETIH_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$FETIH_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$FETIH_UID:$FETIH_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure FETIH_HOME is owned by the target user.
      ...
      if [ -n "''${FETIH_HOME:-}" ] && [ -d "$FETIH_HOME" ]; then
        find "$FETIH_HOME" \! -user "$FETIH_UID" -exec chown "$FETIH_UID:$FETIH_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/fetih-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/fetih-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/fetih ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/fetih
        chmod 0440 /etc/sudoers.d/fetih
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$FETIH_UID" --regid="$FETIH_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $FETIH_HOME/.env.
    ...
    # Default: /var/lib/fetih/workspace → /data/workspace.
  ...
  in {
    options.services.fetih-agent = with lib; {
      enable = mkEnableOption "FETIH Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = fetih-agent;
        description = "The fetih-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/fetih";
        description = "State directory. Contains .fetih/ subdir (FETIH_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative FETIH config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $FETIH_HOME/.env at activation time.
          FETIH reads this file on every startup via load_fetih_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $FETIH_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $FETIH_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `fetih gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the fetih user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the fetih plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. FETIH discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          fetih_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as fetih (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in fetih-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the fetih CLI to environment.systemPackages and export
          FETIH_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.fetih symlink to the service
            stateDir. These users are automatically added to the fetih group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.fetih-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the fetih CLI to system PATH and export FETIH_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.fetih/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.FETIH_HOME = "${cfg.stateDir}/.fetih";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.fetih-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the fetih user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.fetih-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed fetih
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure fetih is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.fetih        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.fetih/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.fetih/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.fetih/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.fetih/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.fetih/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."fetih-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.fetih
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.fetih ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.fetih ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.fetih -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.fetih/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.fetih/$_subdir"
            chmod 2770 "${cfg.stateDir}/.fetih/$_subdir"
            find "${cfg.stateDir}/.fetih/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m 0640 -D ${configFile} ${cfg.stateDir}/.fetih/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.fetih/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.fetih/config.yaml
            chmod 0640 ${cfg.stateDir}/.fetih/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.fetih/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.fetih/.managed
          chmod 0644 ${cfg.stateDir}/.fetih/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.fetih/.container-mode <<'FETIH_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    fetih_bin=${containerDataDir}/current-package/bin/fetih
    FETIH_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.fetih/.container-mode
            chmod 0644 ${cfg.stateDir}/.fetih/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.fetih/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.fetih";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.fetih" ]; then
                  rm -f "${symlinkPath}"
                  echo "fetih-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.fetih -> stateDir/.fetih for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.fetih";
                target = "${cfg.stateDir}/.fetih";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "fetih-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.fetih/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.fetih/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.fetih/auth.json
              fi
            ''}
          ''}
          ...
          # FETIH reads $FETIH_HOME/.env at startup via load_fetih_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.fetih/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'FETIH_NIX_ENV_EOF'
    ${envFileContent}
    FETIH_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.fetih/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.fetih/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.fetih/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.fetih-agent = {
          description = "FETIH Agent Gateway";
          ...
          serviceConfig = {
            ...
            # $FETIH_HOME/.env by the activation script. load_fetih_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/fetih"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the fetih group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.fetih-agent = {
          description = "FETIH Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              FETIH_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              FETIH_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env FETIH_UID="$FETIH_UID" \
                --env FETIH_GID="$FETIH_GID" \
                --env FETIH_HOME=${containerDataDir}/.fetih \
                --env FETIH_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/fetih gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
