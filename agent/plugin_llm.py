...
Plugins built on FETIH often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing FETIH
...
:class:`~fetih_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors FETIH' main config
...
handles every provider, fallback chain, and per-task override FETIH
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`fetih_cli.plugins.PluginContext`
