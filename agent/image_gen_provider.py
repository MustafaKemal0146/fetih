...
as ``kind: backend``) or ``~/.fetih/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
def _images_cache_dir() -> Path:
    """Return ``$FETIH_HOME/cache/images/``, creating parents as needed."""
    from fetih_constants import get_fetih_home

    path = get_fetih_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$FETIH_HOME/cache/images/``.
