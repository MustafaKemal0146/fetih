...
Data resolution order (like TypeScript OpenCode):
  ...
  2. Disk cache (~/.fetih/models_dev_cache.json)
...
# ---------------------------------------------------------------------------
# Provider ID mapping: FETIH ↔ models.dev
# ---------------------------------------------------------------------------

# FETIH provider names → models.dev provider IDs
...
# Reverse mapping: models.dev → FETIH (built lazily)
...
def _get_cache_path() -> Path:
    ...
    return get_fetih_home() / "models_dev_cache.json"
...
def fetch_models_dev(force_refresh: bool = False) -> Dict[str, Any]:
    ...
    When ``force_refresh=True`` (used by ``fetih config refresh``, the
...
def _get_provider_models(provider: str) -> Optional[Dict[str, Any]]:
    """Resolve a FETIH provider ID to its models dict from models.dev.
...
def list_provider_models(provider: str) -> List[str]:
    ...
    """
    from fetih_cli.models import normalize_provider
...
# Gemma models whose TPM quotas are too small for normal FETIH agent traffic.
...
def get_provider_info(provider_id: str) -> Optional[ProviderInfo]:
    ...
    Accepts either a FETIH provider ID (e.g. "kilocode") or a models.dev
    ID (e.g. "kilo").  Returns None if the provider is not in the catalog.
    """
    # Resolve FETIH ID → models.dev ID
...
def get_model_info(
    ...
    Accepts FETIH or models.dev provider ID.  Tries exact match then
