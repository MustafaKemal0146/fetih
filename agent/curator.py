...
def _state_file() -> Path:
    return get_fetih_home() / "skills" / ".curator_state"
...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.fetih/config.yaml. Tolerates missing file."""
    try:
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``fetih update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``fetih curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `fetih curator run --dry-run` to preview now"
            )
            save_state(state)
...
CURATOR_REVIEW_PROMPT = (
    "You are running as FETIH' background skill CURATOR. This is an "
    ...
    "into ~/.fetih/skills/.archive/) is the maximum destructive action. "
    ...
    "to find: fetih-config-*, fetih-dashboard-*, gateway-*, codex-*, "
    ...
    "~/.fetih/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.fetih/logs/curator/``)
    ...
    authored skill data in ``~/.fetih/skills/``.

    ``ensure_fetih_home()`` pre-creates this dir on every CLI launch and
    ...
    that bypasses both.
    """
    root = get_fetih_home() / "logs" / "curator"
    try:
...
def _parse_structured_summary(
    ...
    # Prefer PyYAML when available — every fetih install already has it
...
def _build_rename_summary(
    *,
    ...
    Format::
        ...
        full report: fetih curator status
        keep an umbrella stable: fetih curator pin document-tools
    ...
    lines.append("full report: fetih curator status")
    ...
    if consolidated:
        ...
        if umbrellas:
            example = umbrellas[0]
            lines.append(
                f"keep an umbrella stable: fetih curator pin {example}"
            )
    return "\n".join(lines)
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.fetih/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.fetih/skills/.archive/` for "
            "safety and can be restored via `fetih curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.fetih/skills/.archive/`. "
            "Restore any via `fetih curator restore <name>`._\n"
        )
        SHOW = 50
    ...
    lines.append("- Restore an archived skill: `fetih curator restore <name>`")
    lines.append("- All archives live under `~/.fetih/skills/.archive/` and are recoverable by `mv`")
...
def run_curator_review(
    ...
    # real pass out. We still record a summary so `fetih curator status`
    ...
    def _llm_pass():
        ...
        # recorded in state so `fetih curator status` can point at it.
        try:
            after_report = skill_usage.agent_created_report()
        except Exception:
            after_report = []
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``fetih model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `fetih model` → auxiliary
