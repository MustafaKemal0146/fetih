...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - fetih_home (str): The active FETIH_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.fetih``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "fetih").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'fetih memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], fetih_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'fetih memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``fetih_home`` is the active FETIH_HOME directory path.
