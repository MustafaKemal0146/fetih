...
as ``kind: backend``) or ``~/.fetih/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``fetih tools``. Defaults to ``name.title()``."""
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        """Return catalog entries for ``fetih tools`` model picker.
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        """Return provider metadata for the ``fetih tools`` picker."""
        return {
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``fetih tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$FETIH_HOME/cache/videos/``, creating parents as needed."""
    from fetih_constants import get_fetih_home

    path = get_fetih_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$FETIH_HOME/cache/videos/``.
