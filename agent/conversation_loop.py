...
from fetih_logging import set_session_context
...
def run_conversation(
    ...
    # ``fetih logs --session <id>`` can filter a single conversation.
    from fetih_logging import set_session_context
    ...
    # are reused.  The system prompt is FETIH's territory; plugins
    ...
    # all run inside Codex). Default FETIH path is bypassed entirely.
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        # cache prefix.  The system prompt is reserved for FETIH internals.
        #
        # FETIH invariant: the system prompt is built ONCE per session
        ...
        while retry_count < max_retries:
            ...
            if agent.provider == "nous":
                try:
                    ...
                    if _nous_remaining is not None and _nous_remaining > 0:
                        _nous_msg = (
                            f"FETIH Portal rate limit active — "
            ...
            try:
                ...
                if env_var_enabled("FETIH_DUMP_REQUESTS"):
                ...
                if (
                    ...
                    print(f"{agent.log_prefix}     • Re-authenticate: fetih login --provider nous")
                    print(f"{agent.log_prefix}     • Check credits / billing: https://github.com/MustafaKemal0146/fetih")
                ...
                if (
                    ...
                    if is_token_provider(key):
                        ...
                        print(f"{agent.log_prefix}   Run `fetih doctor` for credential-chain diagnostics, or")
                    ...
                    from fetih_constants import display_fetih_home as _dhh_fn
                    _dhh = _dhh_fn()
                    print(f"{agent.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for FETIH-managed OAuth/setup tokens")
                    ...
                    print(f"{agent.log_prefix}     • Legacy cleanup: fetih config set ANTHROPIC_TOKEN \"\"")
                    print(f"{agent.log_prefix}     • Clear stale keys: fetih config set ANTHROPIC_API_KEY \"\"")
                ...
                # ── FETIH Portal: record rate limit & skip retries ─────
                ...
                #
                # IMPORTANT: FETIH Portal multiplexes multiple upstream
                # providers (DeepSeek, Kimi, MiMo, FETIH).  A 429 can
                ...
                # which FETIH' system prompt + tool schemas alone exceed.
                ...
                if (
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      request at ~8K tokens. FETIH' system prompt + tool schemas baseline",
                        force=True,
                    )
                    agent._vprint(
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      Use the `copilot` provider with a Copilot subscription token (`fetih",
                        force=True,
                    )
                    agent._vprint(
                    ...
                    if classified.is_auth or classified.reason == FailoverReason.billing:
                        if _provider in {"openai-codex", "xai-oauth"} and status_code == 401:
                            if _provider == "openai-codex":
                                ...
                                agent._vprint(f"{agent.log_prefix}      2. Then run `fetih auth` to re-authenticate.", force=True)
                            else:
                                agent._vprint(f"{agent.log_prefix}   💡 xAI OAuth token was rejected (HTTP 401). To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      re-authenticate with xAI Grok OAuth (SuperGrok Subscription) from `fetih model`.", force=True)
                        else:
                            agent._vprint(f"{agent.log_prefix}   💡 Your API key was rejected by the provider. Check:", force=True)
                            agent._vprint(f"{agent.log_prefix}      • Is the key valid? Run: fetih setup", force=True)
    ...
    if final_response is None and (
        ...
        _kanban_task = os.environ.get("FETIH_KANBAN_TASK")
        if _kanban_task:
            try:
