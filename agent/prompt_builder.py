...
from fetih_constants import get_fetih_home, get_skills_dir, is_wsl
...
_FETIH_MD_NAMES = (".fetih.md", "FETIH.md")


def _find_fetih_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.fetih.md`` or ``FETIH.md``.
...
FETIH_AGENT_HELP_GUIDANCE = (
    "If the user asks about configuring, setting up, or using FETIH "
    "itself, load the `fetih-agent` skill with skill_view(name='fetih-agent') "
    "before answering. Docs: https://github.com/MustafaKemal0146/fetih/docs"
)
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.fetih/kanban.db`. Your task id is in "
    "`$FETIH_KANBAN_TASK`; your workspace is `$FETIH_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $FETIH_KANBAN_WORKSPACE` before "
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `fetih kanban <verb>` for board operations. Use "
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the FETIH WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where FETIH itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host FETIH runs on.
    """
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where FETIH itself is running. The host OS, home, and cwd "
                f"of the FETIH process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where FETIH "
...
def _skills_prompt_snapshot_path() -> Path:
    return get_fetih_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot FETIH itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `fetih-agent` skill "
            "first. It has the actual commands (e.g. `fetih config set …`, `fetih tools`, "
            "`fetih setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: fetih setup, fetih setup tools, fetih setup terminal, fetih status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md() -> Optional[str]:
    """Load SOUL.md from FETIH_HOME and return its content, or None.
    ...
    """
    try:
        from fetih_cli.config import ensure_fetih_home
        ensure_fetih_home()
    except Exception as e:
        logger.debug("Could not ensure FETIH_HOME before loading SOUL.md: %s", e)

    soul_path = get_fetih_home() / "SOUL.md"
...
def _load_fetih_md(cwd_path: Path) -> str:
    """.fetih.md / FETIH.md — walk to git root."""
    fetih_md_path = _find_fetih_md(cwd_path)
    if not fetih_md_path:
        return ""
    try:
        content = fetih_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = fetih_md_path.name
        try:
            rel = str(fetih_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(result, ".fetih.md")
    except Exception as e:
        logger.debug("Could not read %s: %s", fetih_md_path, e)
        return ""
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .fetih.md / FETIH.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from FETIH_HOME is independent and always included when present.
    ...
    # SOUL.md from FETIH_HOME only — skip when already loaded as identity
    if not skip_soul:
