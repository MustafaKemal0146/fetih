"""Anthropic Messages API adapter for FETIH.

Translates between FETIH's internal OpenAI-style message format and
...
# FETIH effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    FETIH historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by FETIH)
    ...
    # 1. FETIH-managed OAuth/setup token env var
    ...
    # This remains as a compatibility fallback for pre-migration FETIH configs.
...
def read_fetih_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read FETIH-managed OAuth credentials from ~/.fetih/.anthropic_oauth.json."""
    if _FETIH_OAUTH_FILE.exists():
        try:
            data = json.loads(_FETIH_OAUTH_FILE.read_text(encoding="utf-8"))
            if data.get("accessToken"):
                return data
        except (json.JSONDecodeError, OSError, IOError) as e:
            logger.debug("Failed to read FETIH OAuth credentials: %s", e)
    return None
    ...
    *base_url* is a Kimi / Moonshot host), unsigned thinking blocks
    ...
    for m in messages:
        ...
        if role == "assistant":
            ...
            # can validate the message history.  See fetih-agent#13848.
            #
    ...
    # are preserved.  See fetih-agent#13848 (Kimi) and #16748 (DeepSeek).
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("FETIH", "Claude Code")
                text = text.replace("FETIH agent", "Claude Code")
                text = text.replace("fetih-agent", "claude-code")
                text = text.replace("FETIH", "Anthropic")
                block["text"] = text
    ...
    # silently hides reasoning text that FETIH surfaces in its CLI. We
