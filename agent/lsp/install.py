...
appropriate.  All installs go to a FETIH-owned bin staging dir,
``<FETIH_HOME>/lsp/bin/``, so we don't pollute the user's global
toolchain.
...
- ``manual`` — never install; if a binary is missing, the server is
  silently skipped and the user is told about it via ``fetih lsp
  status``.
...
# ``<FETIH_HOME>/lsp/bin/`` first, then on PATH.
#
# Optional fields:
...
def fetih_lsp_bin_dir() -> Path:
    """Return the FETIH-owned bin staging dir for LSP servers."""
    home = os.environ.get("FETIH_HOME")
    if home is None:
        home = os.path.join(os.path.expanduser("~"), ".fetih")
    p = Path(home) / "lsp" / "bin"
...
def _install_npm(
    ...
    if npm is None:
        ...
        return None
    staging = fetih_lsp_bin_dir().parent  # <FETIH_HOME>/lsp/
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    """Install a Python package into a fetih-owned target dir.
    ...
    console script.
    """
    pip_target = fetih_lsp_bin_dir().parent / "python-packages"
    ...
    if bin_path.exists():
        link = fetih_lsp_bin_dir() / bin_name
        if not link.exists():
            try:
...
def detect_status(pkg: str) -> str:
    ...
    Used by the ``fetih lsp status`` CLI to give users a quick
