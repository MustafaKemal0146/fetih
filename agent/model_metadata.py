...
def _resolve_requests_verify() -> bool | str:
    ...
    by default. FETIH also honours FETIH_CA_BUNDLE (its own convention)
    ...
    """
    for env_var in ("FETIH_CA_BUNDLE", "REQUESTS_CA_BUNDLE", "SSL_CERT_FILE"):
...
# Minimum context length required to run FETIH.  Models with fewer
...
DEFAULT_CONTEXT_LENGTHS = {
    ...
    # so these hardcoded fallbacks prevent FETIH from probing-down to
...
_URL_TO_PROVIDER: Dict[str, str] = {
    ...
    "inference-api.github.com/MustafaKemal0146/fetih": "nous",
    ...
    # (often 8K) makes it unusable for FETIH' system prompt, but mapping
...
def _get_context_cache_path() -> Path:
    ...
    return get_fetih_home() / "context_length_cache.yaml"
        ...
        with httpx.Client(timeout=3.0, headers=headers) as client:
            ...
            if server_type == "ollama":
                ...
                if resp.status_code == 200:
                    ...
                    # FETIH grow conversations past the runtime limit and Ollama
...
def _resolve_nous_context_length(
    ...
    """Resolve FETIH Portal model context length.
...
def get_model_context_length(
    ...
    if base_url and provider != "lmstudio":
        ...
        if cached is not None:
            ...
            # Invalidate stale 32k cache entries for Kimi-family models.
                ...
                _invalidate_cached_context_length(model, base_url)
            # FETIH Portal: the portal /v1/models endpoint is authoritative.
            ...
            # cost amortise to ~0 within a process.
                ...
                logger.debug(
                    "Bypassing persistent cache for %s@%s (third-party portal authoritative)",
                    model, base_url,
                )
    ...
    if effective_provider in {"copilot", "copilot-acp", "github-copilot"}:
        try:
            from fetih_cli.models import get_copilot_model_context
...
def estimate_request_tokens_rough(
    ...
    *,
    ...
    Includes the major payload buckets FETIH sends to providers:
