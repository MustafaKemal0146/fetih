...
def dump_api_request_debug(
    ...
    *,
    ...
    try:
        ...
        if env_var_enabled("FETIH_DUMP_REQUEST_STDOUT"):
...
def anthropic_prompt_cache_policy(
    ...
    *,
    ...
    # FETIH Portal proxies to OpenRouter behind the scenes — identical
    # OpenAI-wire envelope cache_control semantics. Treat it as an
    # OpenRouter-equivalent endpoint for caching layout purposes.
    is_third_party_portal = "nousresearch" in eff_base_url.lower()
    ...
    if is_native_anthropic:
        return True, True
    if (is_openrouter or is_third_party_portal) and is_claude:
        return True, False
    # FETIH Portal Qwen (e.g. qwen3.6-plus) takes the same envelope-layout
    ...
    if is_third_party_portal and "qwen" in model_lower:
        return True, False
...
def switch_model(agent, new_model, new_provider, api_key='', base_url='', api_mode=''):
    ...
    if hasattr(agent, "context_compressor") and agent.context_compressor:
        ...
        try:
            from fetih_cli.config import load_config, get_compatible_custom_providers
            _sm_cfg = load_config()
...
def invoke_tool(agent, function_name: str, function_args: dict, effective_task_id: str,
    ...
    if not pre_tool_block_checked:
        try:
            from fetih_cli.plugins import get_pre_tool_call_block_message
