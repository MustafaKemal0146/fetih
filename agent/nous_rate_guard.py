"""Cross-session rate limit guard for FETIH Portal.

Writes rate limit state to a shared file so all sessions (CLI, gateway,
cron, auxiliary) can check whether FETIH Portal is currently rate-limited
...
(3 SDK retries x 3 FETIH retries), and every one of those calls counts
...
def _state_path() -> str:
    ...
    try:
        ...
        base = os.path.join(os.path.expanduser("~"), ".fetih")
...
def record_nous_rate_limit(
    *,
    ...
    """Record that FETIH Portal is rate-limited.
...
def nous_rate_limit_remaining() -> Optional[float]:
    """Check if FETIH Portal is currently rate-limited.
...
def is_genuine_nous_rate_limit(
    *,
    ...
    """Decide whether a 429 from FETIH Portal is a real account rate limit.

    FETIH Portal multiplexes multiple upstream providers (DeepSeek, Kimi,
    MiMo, FETIH, ...) behind one endpoint.  A 429 can mean either:
