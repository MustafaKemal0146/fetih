...
from fetih_cli.timeouts import get_provider_request_timeout, get_provider_stale_timeout
...
def build_api_kwargs(agent, api_messages: list) -> dict:
    ...
    _is_ext_portal = "nousresearch" in agent._base_url_lower
    ...
    _ant_max = None
    if (_is_or or _is_ext_portal) and "claude" in (agent.model or "").lower():
        try:
    ...
    if _is_qwen:
        _qwen_meta = {
            "sessionId": agent.session_id or "fetih",
...
def try_activate_fallback(agent, reason: "FailoverReason | None" = None) -> bool:
    ...
    try:
        ...
        if not fb_api_key_hint:
            ...
            # _normalize_custom_provider_entry in fetih_cli/config.py).
...
def interruptible_streaming_api_call(agent, api_kwargs: dict, *, on_first_delta=None):
    ...
    def _call_chat_completions():
        ...
        # wins over the FETIH_API_TIMEOUT env default if the user set it.
        _provider_timeout_cfg = get_provider_request_timeout(agent.provider, agent.model)
        _base_timeout = (
            _provider_timeout_cfg
            if _provider_timeout_cfg is not None
            else float(os.getenv("FETIH_API_TIMEOUT", 1800.0))
        )
        # Read timeout: config wins here too.  Otherwise use
        # FETIH_STREAM_READ_TIMEOUT (default 120s) for cloud providers.
        if _provider_timeout_cfg is not None:
            _stream_read_timeout = _provider_timeout_cfg
        else:
            _stream_read_timeout = float(os.getenv("FETIH_STREAM_READ_TIMEOUT", 120.0))
            # Local providers (Ollama, llama.cpp, vLLM) can take minutes for
            # prefill on large contexts before producing the first token.
            # Auto-increase the httpx read timeout unless the user explicitly
            # overrode FETIH_STREAM_READ_TIMEOUT.
    ...
    def _call():
        ...
        _max_stream_retries = int(os.getenv("FETIH_STREAM_RETRIES", 2))
    ...
    if _cfg_stale is not None:
        ...
        _stream_stale_timeout_base = float(os.getenv("FETIH_STREAM_STALE_TIMEOUT", 180.0))
    # Local providers (Ollama, oMLX, llama-cpp) can take 300+ seconds
    # for prefill on large contexts.  Disable the stale detector unless
    # the user explicitly set FETIH_STREAM_STALE_TIMEOUT.
