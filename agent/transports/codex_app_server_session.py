...
Owns one Codex thread per FETIH session. Drives `turn/start`, consumes
...
# FETIH' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_FETIH_TO_CODEX_PERMISSION_PROFILE = {
...
class CodexAppServerSession:
    """One Codex thread per FETIH session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _FETIH_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("FETIH_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="fetih",
            client_title="FETIH",
            client_version=_get_fetih_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but FETIH' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into FETIH' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own fetih-tools callback we
            # auto-accept — the user already approved FETIH' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map FETIH approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    FETIH returns 'once', 'session', 'always', or 'deny'.
...
def _get_fetih_version() -> str:
    """Best-effort FETIH version string for codex's userAgent line."""
    try:
        from importlib.metadata import version

        return version("fetih-agent")
