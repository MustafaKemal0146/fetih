...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time FETIH starts and finds ``~/.openclaw/``.

    Points users at ``fetih claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``fetih claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to FETIH, run "
        "`fetih claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `fetih claw cleanup` (renames it to ~/.openclaw.pre-migration — "
