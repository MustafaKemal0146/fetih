"""Lightweight internationalization (i18n) for FETIH static user-facing messages.

Scope (thin slice, by design): only the highest-impact static strings shown
to the user by FETIH itself -- approval prompts, a handful of gateway slash
...
Language resolution order:
    ...
    2. ``FETIH_LANGUAGE`` environment variable (for tests / quick override)
...
def _load_catalog(lang: str) -> dict[str, str]:
    ...
    try:
        import yaml  # PyYAML is already a fetih dependency
...
def reset_language_cache() -> None:
    ...
    Call after :func:`fetih_cli.config.save_config` if a running process
...
def get_language() -> str:
    ...
    env_lang = os.environ.get("FETIH_LANGUAGE")
    if env_lang:
