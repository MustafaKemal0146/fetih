...
from fetih_cli.config import get_env_value, load_env
import fetih_cli.auth as auth_mod
...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``fetih model`` / ``fetih auth`` which
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another FETIH process
    ...
    def _sync_nous_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        avoiding a "refresh token reuse" revocation on the FETIH Portal.
        """
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        ``fetih`` invocation that defaults to the active provider
        (e.g. setup wizard, ``fetih auth status``) would land on
        ...
        try:
            if self.provider == "anthropic":
                ...
                refreshed = refresh_anthropic_oauth_pure(
                    entry.refresh_token,
                    use_json=entry.source.endswith("fetih_pkce"),
                )
                updated = replace(
                    entry,
                ...
                # refresh_token — single-use tokens consumed by another FETIH
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            ...
                            use_json=synced.source.endswith("fetih_pkce"),
                        )
                        updated = replace(
                            synced,
                            access_token=refreshed["access_token"],
            ...
            if self.provider == "xai-oauth":
                ...
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    self._persist()
                    return None
            # For openai-codex: same race as xAI/nous — another FETIH process
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other FETIH profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
            ...
            # re-authed via `fetih model` / `fetih auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `fetih model` ->
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `fetih auth remove <provider> <N>` is stable across all source types.
    try:
        from fetih_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
        def _is_suppressed(_p, _s):  # type: ignore[misc]
            return False

    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, FETIH PKCE)
        ...
        from agent.anthropic_adapter import read_claude_code_credentials, read_fetih_oauth_credentials

        for source_name, creds in (
            ("fetih_pkce", read_fetih_oauth_credentials()),
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # `fetih auth add nous --label <name>`).  Fall back to the
        ...
        # FETIH auth store or env vars, so resolve them here.
        # Use refresh_if_expiring=False to avoid network calls during
        # pool loading / provider discovery.
        try:
            from fetih_cli.auth import resolve_qwen_runtime_credentials
        ...
        # MiniMax OAuth tokens live in ~/.fetih/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `fetih auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `fetih auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the FETIH auth store.  Without this gate the removal is instantly
        ...
        # FETIH owns its own Codex auth state — we do NOT auto-import from
        ...
        # When the user logs in via ``fetih model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``fetih auth list`` reflects the logged-in state and so the pool
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Prefer ~/.fetih/.env over os.environ — the user's config file is the
    # authoritative source for FETIH credentials. Stale env vars from parent
    ...
    # Honour user suppression — `fetih auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.fetih/.env.
    ...
    try:
        from fetih_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
        def _is_source_suppressed(_p, _s):  # type: ignore[misc]
            return False
    if provider == "openrouter":
        # Prefer ~/.fetih/.env over os.environ
...
def _prune_stale_seeded_entries(entries: List[PooledCredential], active_sources: Set[str]) -> bool:
    retained = [
        ...
        or not (
            ...
            or entry.source in {"claude_code", "fetih_pkce"}
        )
    ]
    if len(retained) == len(entries):
        return False
