"""Unified removal contract for every credential source FETIH reads from.

FETIH seeds its credential pool from many places:

    env:<VAR>     — os.environ / ~/.fetih/.env
    claude_code   — ~/.claude/.credentials.json
    fetih_pkce   — ~/.fetih/.anthropic_oauth.json
    ...
    manual        — user ran `fetih auth add`
...
unify here is **removal**:
    ...
    ``fetih auth remove <provider> <N>`` must make the pool entry stay gone.

Before this module, every source had an ad-hoc removal branch in
``auth_remove_command``, and several sources had no branch at all — so
``auth remove`` silently reverted on the next ``load_pool()`` call for
qwen-cli, nous device_code (partial), fetih_pkce, copilot gh_cli, and
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.fetih/.env  → clear it
    ...
    """
    from fetih_cli.config import get_env_path, remove_env_value
    ...
    if shell_exported:
        result.hints.extend([
            ...
            "launchd plist, etc.) or it will keep being visible to FETIH.",
            f"  The pool entry is now suppressed — FETIH will ignore "
            f"{env_var} until you run `fetih auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    work.  We just suppress it so FETIH stops reading it.
    """
    return RemovalResult(hints=[
        "Suppressed claude_code credential — it will not be re-seeded.",
        "Note: Claude Code credentials still live in ~/.claude/.credentials.json",
        "Run `fetih auth add anthropic` to re-enable if needed.",
    ])


def _remove_fetih_pkce(provider: str, removed) -> RemovalResult:
    """~/.fetih/.anthropic_oauth.json is ours — delete it outright."""
    from fetih_constants import get_fetih_home

    result = RemovalResult()
    oauth_file = get_fetih_home() / ".anthropic_oauth.json"
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared FETIH Anthropic OAuth credentials")
...
def _remove_nous_device_code(provider: str, removed) -> RemovalResult:
    ...
    user's next `fetih login` run from writing providers.nous again
    before they decide to.  Suppression forces them to go through
    `fetih auth add nous` to re-engage, which is the documented re-add
...
def _remove_xai_oauth_loopback_pkce(provider: str, removed) -> RemovalResult:
    ...
    Without this step, ``fetih auth remove xai-oauth <N>`` silently undoes
    ...
    Belt-and-braces against the manual entry path: ``fetih auth add
    ...
    if _clear_auth_store_provider(provider):
        result.cleaned.append(f"Cleared {provider} OAuth tokens from auth store")
    result.hints.append(
        "Run `fetih model` → xAI Grok OAuth (SuperGrok Subscription) to re-authenticate if needed."
    )
    return result
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    the FETIH auth store is not enough — _seed_from_singletons() would
    ...
    via ``fetih auth add openai-codex``), but in both cases the re-seed
    ...
    """
    from fetih_cli.auth import suppress_credential_source
    ...
    result.hints.extend([
        ...
        "Run `fetih auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `fetih auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    FETIH stops picking the token up.
    """
    ...
    return RemovalResult(hints=[
        ...
        "Run `fetih auth add copilot` to re-enable if needed.",
    ])
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        provider="anthropic", source_id="fetih_pkce",
        remove_fn=_remove_fetih_pkce,
        description="~/.fetih/.anthropic_oauth.json",
    ))
    register(RemovalStep(
