    ...
    *max_len* controls truncation.  ``None`` (default) defers to the global
    ...
    if tool_name == "todo":
        ...
        if todos_arg is None:
            ...
            return f"updating {len(todos_arg)} görev"
        else:
            return f"planning {len(todos_arg)} görev"
...
def _render_inline_unified_diff(diff: str) -> list[str]:
    """Render unified diff lines in FETIH' inline transcript style."""
    rendered: list[str] = []
    from_file = None
    to_file = None
...
class KawaiiSpinner:
    ...
    def _animate(self):
        ...
        while self.running:
            if os.getenv("FETIH_SPINNER_PAUSE"):
                time.sleep(0.1)
                continue
...
def _detect_tool_failure(tool_name: str, result: str | None) -> tuple[bool, str]:
    ...
    like ``" [çıkış 1]"`` for terminal failures, or ``" [hata]"`` for generic
...
def get_cute_tool_message(
    ...
    if tool_name == "web_search":
        return _wrap(f"┊ 🔍 arıyor   {_trunc(args.get('query', ''), 42)}  {dur}")
    if tool_name == "web_extract":
        ...
        if urls:
            ...
            return _wrap(f"┊ 📄 alıyor   {_trunc(domain, 35)}{extra}  {dur}")
        return _wrap(f"┊ 📄 alıyor   sayfalar  {dur}")
    ...
    if tool_name == "process":
        ...
        labels = {"list": "işlem listesi", "poll": f"bekle {sid}", "log": f"kayıt {sid}",
                  "wait": f"bekle {sid}", "kill": f"durdur {sid}", "write": f"yaz {sid}", "submit": f"gönder {sid}"}
        return _wrap(f"┊ ⚙️  proc      {labels.get(action, f'{action} {sid}')}  {dur}")
    if tool_name == "read_file":
        return _wrap(f"┊ 📖 okuyor   {_path(args.get('path', ''))}  {dur}")
    if tool_name == "write_file":
        return _wrap(f"┊ ✍️  yazıyor  {_path(args.get('path', ''))}  {dur}")
    if tool_name == "patch":
        return _wrap(f"┊ 🔧 yamalıyor {_path(args.get('path', ''))}  {dur}")
    if tool_name == "search_files":
        pattern = _trunc(args.get("pattern", ""), 35)
        target = args.get("target", "content")
        verb = "buluyor" if target == "files" else "tarıyor"
        return _wrap(f"┊ 🔎 {verb:9} {pattern}  {dur}")
    if tool_name == "browser_navigate":
        url = args.get("url", "")
        domain = url.replace("https://", "").replace("http://", "").split("/")[0]
        return _wrap(f"┊ 🌐 gidiyor  {_trunc(domain, 35)}  {dur}")
    if tool_name == "browser_snapshot":
        mode = "tam" if args.get("full") else "özet"
        return _wrap(f"┊ 📸 ekran    {mode}  {dur}")
    if tool_name == "browser_click":
        return _wrap(f"┊ 👆 tıklıyor {args.get('ref', '?')}  {dur}")
    if tool_name == "browser_type":
        return _wrap(f"┊ ⌨️  giriyor  \"{_trunc(args.get('text', ''), 30)}\"  {dur}")
    if tool_name == "browser_scroll":
        d = args.get("direction", "down")
        arrow = {"down": "↓", "up": "↑", "right": "→", "left": "←"}.get(d, "↓")
        return _wrap(f"┊ {arrow}  kaydırıyor {d}  {dur}")
    if tool_name == "browser_back":
        return _wrap(f"┊ ◀️  geri     {dur}")
    if tool_name == "browser_press":
        return _wrap(f"┊ ⌨️  basıyor  {args.get('key', '?')}  {dur}")
    if tool_name == "browser_get_images":
        return _wrap(f"┊ 🖼️  görseller alıyor  {dur}")
    if tool_name == "browser_vision":
        return _wrap(f"┊ 👁️  görüntü  analiz ediyor  {dur}")
    if tool_name == "todo":
        todos_arg = args.get("todos")
        merge = args.get("merge", False)
        if todos_arg is None:
            return _wrap(f"┊ 📋 görev    okunuyor  {dur}")
        elif merge:
            return _wrap(f"┊ 📋 görev    güncelleniyor {len(todos_arg)} görev  {dur}")
        else:
            return _wrap(f"┊ 📋 görev    {len(todos_arg)} görev  {dur}")
    if tool_name == "session_search":
        return _wrap(f"┊ 🔍 anımsıyor \"{_trunc(args.get('query', ''), 35)}\"  {dur}")
    if tool_name == "memory":
        action = args.get("action", "?")
        target = args.get("target", "")
        if action == "add":
            return _wrap(f"┊ 🧠 bellek   +{target}: \"{_trunc(args.get('content', ''), 30)}\"  {dur}")
        elif action == "replace":
            old = args.get("old_text") or ""
            old = old if old else "<missing old_text>"
            return _wrap(f"┊ 🧠 bellek   ~{target}: \"{_trunc(old, 20)}\"  {dur}")
        elif action == "remove":
            old = args.get("old_text") or ""
            old = old if old else "<missing old_text>"
            return _wrap(f"┊ 🧠 bellek   -{target}: \"{_trunc(old, 20)}\"  {dur}")
        return _wrap(f"┊ 🧠 bellek   {action}  {dur}")
    if tool_name == "skills_list":
        return _wrap(f"┊ 📚 beceriler listesi {args.get('category', 'all')}  {dur}")
    if tool_name == "skill_view":
        return _wrap(f"┊ 📚 beceri   {_trunc(args.get('name', ''), 30)}  {dur}")
    if tool_name == "image_generate":
        return _wrap(f"┊ 🎨 üretiyor {_trunc(args.get('prompt', ''), 35)}  {dur}")
    if tool_name == "text_to_speech":
        return _wrap(f"┊ 🔊 konuşuyor {_trunc(args.get('text', ''), 30)}  {dur}")
    if tool_name == "vision_analyze":
        return _wrap(f"┊ 👁️  görüntü  {_trunc(args.get('question', ''), 30)}  {dur}")
    if tool_name == "mixture_of_agents":
        return _wrap(f"┊ 🧠 düşünüyor {_trunc(args.get('user_prompt', ''), 30)}  {dur}")
    if tool_name == "send_message":
        return _wrap(f"┊ 📨 gönderiyor {args.get('target', '?')}: \"{_trunc(args.get('message', ''), 25)}\"  {dur}")
    if tool_name == "cronjob":
        ...
        if action == "create":
            ...
            return _wrap(f"┊ ⏰ zamanlayıcı oluştur {_trunc(label, 24)}  {dur}")
        if action == "list":
            return _wrap(f"┊ ⏰ zamanlayıcı listeleniyor  {dur}")
        return _wrap(f"┊ ⏰ zamanlayıcı {action} {args.get('job_id', '')}  {dur}")
    if tool_name == "execute_code":
        code = args.get("code", "")
        first_line = code.strip().split("\n")[0] if code.strip() else ""
        return _wrap(f"┊ 🐍 çalıştırıyor {_trunc(first_line, 35)}  {dur}")
    if tool_name == "delegate_task":
        tasks = args.get("tasks")
        if tasks and isinstance(tasks, list):
            return _wrap(f"┊ 🔀 devrediyor {len(tasks)} paralel görev  {dur}")
        return _wrap(f"┊ 🔀 devrediyor {_trunc(args.get('goal', ''), 35)}  {dur}")
