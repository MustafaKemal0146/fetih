"""Centralized logging setup for FETIH.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.fetih/logs/`` (profile-aware via ``get_fetih_home()``).
...
from fetih_constants import get_config_path, get_fetih_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_fetih_session_injector", False):
    ...
    _session_record_factory._fetih_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``fetih logs --component``.
COMPONENT_PREFIXES = {
    "gateway": ("gateway",),
...
def setup_logging(
    *,
    ...
    """Configure the FETIH logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    fetih_home
        Override for the FETIH home directory.  Falls back to
        ``get_fetih_home()`` (profile-aware).
    log_level
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_fetih_verbose", False):
                return

    handler = logging.StreamHandler()
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._fetih_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    inherit the fetih group. However, both _open() (initial creation) and
