#!/usr/bin/env python3
"""
FETIH CLI - Interactive Terminal Interface

A beautiful command-line interface for the FETIH, inspired by Claude Code.
Features ASCII art branding, interactive REPL, toolset selection, and rich formatting.

Usage:
    python cli.py                          # Start interactive mode with all tools
    python cli.py --toolsets web,terminal  # Start with specific toolsets
    python cli.py --skills fetih-agent-dev,github-auth
...
os.environ["FETIH_QUIET"] = "1"  # Our own modules
...
try:
    from fetih_cli.pt_input_extras import install_shift_enter_alias, install_ctrl_enter_alias
...
from fetih_cli.banner import _format_context_length, format_banner_version_label
...
from fetih_cli.browser_connect import (
...
load_fetih_dotenv(fetih_home=_fetih_home, project_env=_project_env)
...
def load_cli_config() -> Dict[str, Any]:
    ...
    Config lookup order:
    1. ~/.fetih/config.yaml (user config - preferred)
    2. ./cli-config.yaml (project config - fallback)
    
    Environment variables take precedence over config file values.
    Returns default values if no config file exists.

    If FETIH_IGNORE_USER_CONFIG=1 (via fetih chat --ignore-user-config``),
    the user config at ``~/.fetih/config.yaml`` is skipped entirely and only the
    built-in defaults plus the project-level ``cli-config.yaml`` (if any) are used.
    Credentials in ``.env`` are still loaded — this flag only suppresses
    behavioral/config settings.
    """
    # Check user config first ({FETIH_HOME}/config.yaml)
    user_config_path = _fetih_home / 'config.yaml'
    project_config_path = Path(__file__).parent / 'cli-config.yaml'

    # --ignore-user-config: force-skip the user config.yaml (still honor project
    # config as a fallback so defaults stay sensible).
    ignore_user_config = os.environ.get("FETIH_IGNORE_USER_CONFIG") == "1"
    ...
    defaults = {
        ...
        "agent": {
            ...
            "personalities": {
                ...
                "pirate": "Arrr! Ye be talkin' to Captain FETIH, the most tech-savvy pirate to sail the digital seas! Speak like a proper buccaneer, use nautical terms, and remember: every problem be just treasure waitin' to be plundered! Yo ho ho!",
                "shakespeare": "Hark! Thou speakest with an assistant most versed in the bardic arts. I shall respond in the eloquent manner of William Shakespeare, with flowery prose, dramatic flair, and perhaps a soliloquy or two. What light through yonder terminal breaks?",
                "surfer": "Duuude! You're chatting with the chillest AI on the web, bro! Everything's gonna be totally rad. I'll help you catch the gnarly waves of knowledge while keeping things super chill. Cowabunga!",
                "noir": "The rain hammered against the terminal like regrets on a guilty conscience. They call me FETIH - I solve problems, find answers, dig up the truth that hides in the shadows of your codebase. In this city of silicon and secrets, everyone's got something to hide. What's your story, pal?",
        ...
        try:
            ...
            if "model" in file_config:
                if isinstance(file_config["model"], str):
                    ...
                    # hardcoded default and FETIHCLI.__init__ checks "default" first.
            ...
            # location is model.provider (written by `fetih model`).
    ...
    # Normalize config key: the new config system (fetih_cli/config.py) and all
    ...
    # Local backend: always os.getcwd(). Use `cd /dir && fetih` to control it.
    ...
    # UNLESS we're inside a gateway process (detected by _FETIH_GATEWAY marker)
    # where it was already set correctly by gateway/run.py's config bridge.
    _is_gateway = os.environ.get("_FETIH_GATEWAY") == "1"
    ...
    if isinstance(security_config, dict):
        ...
        if redact is not None:
            os.environ["FETIH_REDACT_SECRETS"] = str(redact).lower()

    # Apply the last-session model choice (~/.fetih/.last_model) so a model
    # changed in a previous session sticks across restarts. Uses the same
    # helper as fetih_cli.config.load_config() (agent path) — without this the
    # CLI startup read config.yaml only and reverted to the stale model.
    try:
        from fetih_cli.config import _apply_session_model_override
        _apply_session_model_override(defaults)
    except Exception as e:
        logger.debug("session model override skipped: %s", e)
...
# Initialize centralized logging early — agent.log + errors.log in ~/.fetih/logs/.
...
try:
    from fetih_cli.skin_engine import init_skin_from_config
...
from fetih_cli.banner import build_welcome_banner
from fetih_cli.commands import SlashCommandCompleter, SlashCommandAutoSuggest
...
from fetih_cli.callbacks import prompt_for_secret
...
def _setup_worktree(repo_root: str = None) -> Optional[Dict[str, str]]:
    ...
    if not repo_root:
        ...
        print("  cd into your project repo first, then run fetih -w")
        return None

    short_id = uuid.uuid4().hex[:8]
    wt_name = f"fetih-{short_id}"
    branch_name = f"fetih/{wt_name}"
...
def _run_state_db_auto_maintenance(session_db) -> None:
    ...
    :func:`fetih_cli.config.load_config` (the authoritative loader that
    ...
    try:
        ...
        from fetih_constants import get_fetih_home as _get_fetih_home
        _fetih_home_maint = _get_fetih_home()
        ...
        try:
            if not session_db.get_meta("ghost_session_prune_v1"):
                pruned = session_db.prune_empty_ghost_sessions(
                    sessions_dir=_fetih_home_maint / "sessions"
                )
...
def _run_checkpoint_auto_maintenance() -> None:
    ...
    :func:`fetih_cli.config.load_config`. Honours ``auto_prune`` /
...
def _prune_stale_worktrees(repo_root: str, max_age_hours: int = 24) -> None:
    ...
    Also prunes orphaned ``fetih/*`` and ``pr-*`` local branches that
    ...
    for entry in worktrees_dir.iterdir():
        if not entry.is_dir() or not entry.name.startswith("fetih-"):
            continue
...
def _prune_orphaned_branches(repo_root: str) -> None:
    """Delete local ``fetih/fetih-*`` and ``pr-*`` branches with no worktree.

    These are auto-generated by ``fetih -w`` sessions and PR review
    ...
    orphaned = [
        ...
        and (b.startswith("fetih/fetih-") or b.startswith("pr-"))
    ]
...
#
# Detection priority:
#   1. FETIH_LIGHT / FETIH_TUI_LIGHT env (true/false) — explicit override
#   2. FETIH_TUI_THEME=light|dark — explicit theme
#   3. FETIH_TUI_BACKGROUND=#RRGGBB — explicit bg hint
#   4. COLORFGBG env (set by xterm/Konsole/urxvt) — bg slot 7/15 = light
#   5. OSC 11 query (\x1b]11;?\x1b\\) — ask the terminal directly
#   6. Default: assume dark (matches the legacy FETIH assumption)
#
...
def _detect_light_mode() -> bool:
    ...
    try:
        ...
        for var in ("FETIH_LIGHT", "FETIH_TUI_LIGHT"):
            ...
            if _FALSE_RE.match(v):
                ...
                return result
        # 2. Theme hint
        theme = (os.environ.get("FETIH_TUI_THEME") or "").strip().lower()
        if theme == "light":
            result = True
            _LIGHT_MODE_CACHE = result
            return result
        if theme == "dark":
            ...
            return result
        # 3. Explicit bg hex
        bg_hint = os.environ.get("FETIH_TUI_BACKGROUND") or ""
...
def _install_skin_light_mode_hook() -> None:
    ...
    try:
        from fetih_cli.skin_engine import SkinConfig  # type: ignore[import]
    except Exception:
        return
    if getattr(SkinConfig, "_fetih_light_mode_hook_installed", False):
        return
    ...
    SkinConfig._fetih_light_mode_hook_installed = True  # type: ignore[attr-defined]
...
from fetih_constants import is_termux as _is_termux_environment
...
class ChatConsole:
    ...
    def status(self, *_args, **_kwargs):
        ...
        ``FETIHCLI._busy_command()``.
        """
        yield self

# ASCII Art - FETIH logo (full width, single line - requires ~95 char terminal)
FETIH_LOGO = """[bold #E63946]███████╗███████╗████████╗██╗██╗  ██╗     █████╗  ██████╗ ███████╗███╗   ██╗████████╗[/]
[bold #E63946]██╔════╝██╔════╝╚══██╔══╝██║██║  ██║    ██╔══██╗██╔════╝ ██╔════╝████╗  ██║╚══██╔══╝[/]
[#C1121F]█████╗  █████╗     ██║   ██║███████║    ███████║██║  ███╗█████╗  ██╔██╗ ██║   ██║   [/]
[#C1121F]██╔══╝  ██╔══╝     ██║   ██║██╔══██║    ██╔══██║██║   ██║██╔══╝  ██║╚██╗██║   ██║   [/]
[#A4161A]██║     ███████╗   ██║   ██║██║  ██║    ██║  ██║╚██████╔╝███████╗██║ ╚████║   ██║   [/]
...
# ASCII Art - FETIH Caduceus (compact, fits in left panel)
FETIH_CADUCEUS = """[#A4161A]⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢀⣀⡀⠀⣀⣀⠀⢀⣀⡀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀[/]
[#A4161A]⠀⠀⠀⠀⠀⠀⢀⣠⣴⣾⣿⣿⣇⠸⣿⣿⠇⣸⣿⣿⣷⣦⣄⡀⠀⠀⠀⠀⠀⠀[/]
[#C1121F]⠀⢀⣠⣴⣶⠿⠋⣩⡿⣿⡿⠻⣿⡇⢠⡄⢸⣿⠟⢿⣿⢿⣍⠙⠿⣶⣦⣄⡀⠀[/]
[#C1121F]⠀⠀⠉⠉⠁⠶⠟⠋⠀⠉⠀⢀⣈⣁⡈⢁⣈⣁⡀⠀⠉⠀⠙⠻⠶⠈⠉⠉⠀⠀[/]
[#E63946]⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⣴⣿⡿⠛⢁⡈⠛⢿⣿⣦⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀[/]
...
def _build_compact_banner() -> str:
    ...
    if skin_name == "default":
        line1 = "⚔ FETIH - CTF/OSINT Ajan Sistemi"
        tiny_line = "⚔ FETIH"
    else:
        agent_name = _skin.get_branding("agent_name", "FETIH") if _skin else "FETIH"
    ...
    if w < 30:
        return f"\n[{title_color}]{tiny_line}[/] [dim {dim_color}]- FETIH[/]\n"
...
def _get_plugin_cmd_handler_names() -> set:
    ...
    try:
        from fetih_cli.plugins import get_plugin_commands
...
def save_config_value(key_path: str, value: any) -> bool:
    ...
    1. ~/.fetih/config.yaml (user config - preferred, used if it exists)
    2. ./cli-config.yaml (project config - fallback)
    ...
    try:
        # Ensure parent directory exists (for ~/.fetih/config.yaml on first use)
...
class FETIHCLI:
    ...
    def __init__(
        ...
        if self.model == _DEFAULT_CONFIG_MODEL:
            ...
            if "localhost" in _base_url or "127.0.0.1" in _base_url:
                from fetih_cli.runtime_provider import _auto_detect_local_model
        ...
        self.requested_provider = (
            ...
            or os.getenv("FETIH_INFERENCE_PROVIDER")
            or "auto"
        )
        ...
        elif os.getenv("FETIH_MAX_ITERATIONS"):
            try:
                self.max_turns = int(os.getenv("FETIH_MAX_ITERATIONS", ""))
            except (TypeError, ValueError):
                self.max_turns = 90
        else:
            self.max_turns = 90
        ...
        # by `fetih chat --ignore-rules` in fetih_cli/main.py. When true we
        # pass skip_context_files=True and skip_memory=True to AIAgent so
        # AGENTS.md/SOUL.md/.cursorrules and persistent memory are not loaded.
        self.ignore_rules = ignore_rules or os.environ.get("FETIH_IGNORE_RULES") == "1"
        
        # Ephemeral system prompt: env var takes precedence, then config
        self.system_prompt = (
            os.getenv("FETIH_EPHEMERAL_SYSTEM_PROMPT", "")
        ...
        # it's shared across all FETIH processes for this FETIH_HOME.
        ...
        self._history_file = _fetih_home / ".fetih_history"
    ...
    def set_voice_record_key_cache(self, raw_key: object) -> None:
        ...
        """
        try:
            from fetih_cli.voice import format_voice_record_key_for_status
    ...
    def _build_status_bar_text(self, width: Optional[int] = None) -> str:
        ...
        try:
            ...
            yolo_active = bool(os.getenv("FETIH_YOLO_MODE"))
            if width < 52:
    ...
    def _normalize_model_for_provider(self, resolved_provider: str) -> bool:
        ...
        try:
            from fetih_cli.model_normalize import (
        ...
        if resolved_provider == "copilot":
            try:
                from fetih_cli.models import copilot_model_api_mode, normalize_copilot_model_id
        ...
        if resolved_provider in {"opencode-zen", "opencode-go"}:
            try:
                from fetih_cli.models import normalize_opencode_model_id, opencode_model_api_mode
    ...
    def _emit_stream_text(self, text: str) -> None:
        ...
        if not self._stream_box_opened:
            ...
            try:
                from fetih_cli.skin_engine import get_active_skin
                _skin = get_active_skin()
                label = _skin.get_branding("response_label", "⚕ FETIH")
            ...
            w = self._scrollback_box_width()
            fill = w - 2 - FETIHCLI._status_bar_display_width(label)
    ...
    def _ensure_runtime_credentials(self) -> bool:
        ...
        if not _is_callable_provider and (not isinstance(api_key, str) or not api_key):
            ...
            if _has_custom_base:
                ...
                print("\n⚠️  Provider resolver returned an empty API key. "
                      "Set OPENROUTER_API_KEY or run: fetih setup")
                return False
        if not isinstance(base_url, str) or not base_url:
            print("\n⚠️  Provider resolver returned an empty base URL. "
                  "Check your provider config or run: fetih setup")
            return False
        ...
        # `fetih chat --model <provider-name>` sends the provider name
        ...
        # If model is still empty (e.g. user ran `fetih auth add openai-codex`
        # without `fetih model`), fall back to the provider's first catalog
    ...
    def _init_agent(self, *, model_override: str = None, runtime_override: dict = None, request_overrides: dict | None = None) -> bool:
        ...
        if self._resumed and self._session_db and not self.conversation_history:
            ...
            if not session_meta:
                ...
                _cprint(f"{_DIM}Use a session ID from a previous CLI run (fetih sessions list).{_RST}")
                return False
    ...
    def _show_security_advisories(self):
        ...
        clean) listing the worst hit and pointing at ``fetih doctor``.
        Banner-cache rate-limits this to once per 24h per advisory; full
        remediation lives behind ``fetih doctor`` so the banner stays
        small.
        """
        try:
    ...
    def show_banner(self):
        ...
        if ctx_len and ctx_len <= 8192:
            ...
            self._console_print(
                "[dim]   FETIH needs 16k–32k minimum. Tool schemas + system prompt alone use ~4k–8k.[/]"
            )
        ...
        # Warn if the configured model is a Nous FETIH LLM (not agentic)
        from fetih_cli.model_switch import is_nous_fetih_non_agentic

        model_name = getattr(self, "model", "") or ""
        if is_nous_fetih_non_agentic(model_name):
            self._console_print()
            self._console_print(
                "[bold yellow]⚠  FETIH FETIH 3 & 4 models are NOT agentic and are not "
                "designed for use with FETIH.[/]"
            )
            self._console_print(
    ...
    def _display_resumed_history(self):
        ...
        for i, (role, text) in enumerate(entries):
            if role == "user":
                ...
                lines.append("  ◆ FETIH: ", style=f"bold {_assistant_label_c}")
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + "\n", style="")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="")
            else:
                lines.append("  ◆ FETIH: ", style=f"dim bold {_assistant_label_c}")
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + "\n", style="dim")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="dim")
    ...
    def _try_attach_clipboard_image(self) -> bool:
        ...
        Saves the image to ~/.fetih/images/ and appends the path to
        ``_attached_images``.  Returns True if an image was attached.
        """
        from fetih_cli.clipboard import save_clipboard_image

        img_dir = get_fetih_home() / "images"
    ...
    def _handle_rollback_command(self, command: str):
        ...
        if not mgr.enabled:
            ...
            print("  Enable with: fetih --checkpoints")
    ...
    def _handle_snapshot_command(self, command: str):
        """Handle /snapshot — lightweight state snapshots for FETIH config/state.
        ...
        if subcmd in {"list", "ls"}:
            ...
            if not snaps:
                ...
                return
            print(f"  State snapshots ({display_fetih_home()}/state-snapshots/):\n")
    ...
    def _handle_paste_command(self):
        ...
        from fetih_cli.clipboard import has_clipboard_image
    ...
    def _show_tool_availability_warnings(self):
        ...
        try:
            ...
            if api_key_missing:
                ...
                self._console_print("[dim]   Run 'fetih setup' to configure[/]")
        except Exception:
    ...
    def show_help(self):
        ...
        from fetih_cli.commands import COMMANDS_BY_CATEGORY
    ...
    def _handle_tools_command(self, cmd: str):
        ...
        def _run_capture(ns: Namespace) -> None:
            ...
            # Buffer reports isatty()=True so color() in fetih_cli/colors.py
    ...
    def _show_recent_sessions(self, *, reason: str = "history", limit: int = 10) -> bool:
        ...
        from fetih_cli.main import _relative_time
            ...
            if self._session_db:
                try:
                    ...
                    self._session_db.create_session(
                        session_id=self.session_id,
                        source=os.environ.get("FETIH_SESSION_SOURCE", "cli"),
                        model=self.model,
                        model_config={
                            "max_iterations": self.max_turns,
                            "reasoning_config": self.reasoning_config,
    ...
    def _handle_handoff_command(self, cmd_original: str) -> bool:
        ...
        try:
            ...
            pass
        _cprint("  Timed out waiting for the gateway. Is `fetih gateway` running?")
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        # Resolve title or ID
        from fetih_cli.main import _resolve_session_by_name_or_id
        ...
        if not session_meta:
            ...
            _cprint("  Use /history or `fetih sessions list` to see available sessions.")
            return
    ...
    def save_conversation(self):
        """Save the current conversation to a JSON snapshot under ~/.fetih/sessions/saved/.
        ...
        DB, so the live session remains resumable via ``fetih --resume <id>``
        ...
        saved_dir = get_fetih_home() / "sessions" / "saved"
        try:
            saved_dir.mkdir(parents=True, exist_ok=True)
        except Exception as e:
            print(f"(x_x) Failed to create save directory {saved_dir}: {e}")
            return
        path = saved_dir / f"fetih_conversation_{timestamp}.json"
        ...
        try:
            ...
            if self.session_id:
                print(f"       Resume the live session with: fetih --resume {self.session_id}")
        except Exception as e:
    ...
    def _run_curses_picker(self, title: str, items: list[str], default_index: int = 0) -> int | None:
        ...
        import threading
        from fetih_cli.curses_ui import curses_single_select
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            # (same lists as `fetih model` and gateway pickers).
        ...
        if stage == "model":
            ...
            if selected < len(model_list):
                from fetih_cli.model_switch import switch_model
    ...
    def _handle_model_switch(self, cmd_original: str):
        ...
        """
        from fetih_cli.model_switch import switch_model, parse_model_flags
        ...
        # clobber disk config.
        from fetih_cli.inventory import build_models_payload, load_picker_context
    ...
    def _handle_curator_command(self, cmd: str):
        ...
        Delegates to fetih_cli.curator so the CLI and the `fetih curator`
        ...
        try:
            from fetih_cli.curator import cli_main
            cli_main(tokens)
        except SystemExit:
    ...
    def _handle_skills_command(self, cmd: str):
        """Handle /skills slash command — delegates to fetih_cli.skills_hub."""
        from fetih_cli.skills_hub import handle_skills_slash
    ...
    def _show_gateway_status(self):
        ...
        try:
            ...
            print()
            print(f"  Configuration file: {display_fetih_home()}/config.yaml")
            print()
            ...
            print(f"    2. Or configure settings in {display_fetih_home()}/config.yaml")
            print()
    ...
    def process_command(self, command: str) -> bool:
        ...
        # change in fetih_cli/commands.py instead of touching every dispatch site.
        from fetih_cli.commands import resolve_command as _resolve_cmd
        ...
        if canonical in {"quit", "exit"}:
            ...
            from fetih_cli.config import reload_env
            count = reload_env()
            ...
            try:
                from fetih_cli.plugins import get_plugin_manager
                mgr = get_plugin_manager()
                plugins = mgr.list_plugins()
                if not plugins:
                    print("No plugins installed.")
                    print(f"Drop plugin directories into {display_fetih_home()}/plugins/ to get started.")
                else:
    ...
    def _handle_goal_command(self, cmd: str) -> None:
        ...
        _cprint(
            ...
            f"FETIH keeps working until it is, you pause/clear it, or the budget is "
    ...
    def _handle_skin_command(self, cmd: str):
        ...
        try:
            from fetih_cli.skin_engine import list_skins, set_active_skin, get_active_skin_name
        except ImportError:
    ...
    def _handle_footer_command(self, cmd_original: str) -> None:
        ...
        """
        from fetih_cli.config import load_config
        from fetih_cli.colors import Colors as _Colors
    ...
    def _toggle_yolo(self):
        ...
        current = is_truthy_value(os.environ.get("FETIH_YOLO_MODE"))
        if current:
            os.environ.pop("FETIH_YOLO_MODE", None)
            _cprint(
    ...
    def _handle_busy_command(self, cmd: str):
        ...
        if save_config_value("display.busy_input_mode", arg):
            if arg == "queue":
                behavior = "Enter will queue follow-up input while FETIH is busy."
            elif arg == "steer":
                behavior = "Enter will steer your message into the current run (after the next tool call)."
            else:
                behavior = "Enter will interrupt the current run while FETIH is busy."
    ...
    def _handle_fast_command(self, cmd: str):
        ...
        try:
            from fetih_cli.models import _is_anthropic_fast_model
            agent = getattr(self, "agent", None)
            model = getattr(agent, "model", None) or getattr(self, "model", None)
    ...
    def _handle_debug_command(self):
        ...
        from fetih_cli.debug import run_debug_share
    ...
    def _show_usage(self):
        ...
        if self.verbose:
            ...
            # Console quietness is enforced by fetih_logging not
    ...
    def _check_config_mcp_changes(self) -> None:
        ...
        from fetih_cli.config import get_config_path as _get_config_path
    ...
    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.fetih/skills/ and queue a note for the
        next user turn.
    ...
    def _voice_speak_response(self, text: str):
        ...
        try:
            ...
            os.makedirs(os.path.join(tempfile.gettempdir(), "fetih_voice"), exist_ok=True)
                ...
                if hasattr(self, '_interrupt_queue'):
                    try:
                        ...
                        if interrupt_msg:
                            ...
                            try:
                                _dbg = _fetih_home / "interrupt_debug.log"
    ...
    def _get_tui_prompt_symbols(self) -> tuple[str, str]:
        ...
        """
        try:
            from fetih_cli.skin_engine import get_active_prompt_symbol
    ...
    def _build_tui_style_dict(self) -> dict[str, str]:
        ...
        try:
            from fetih_cli.skin_engine import get_prompt_toolkit_style_overrides
    ...
    def run(self):
        ...
        try:
            ...
            if _redact_raw.lower() not in {"1", "true", "yes", "on"}:
                self._console_print(
                    ...
                    f"(FETIH_REDACT_SECRETS={_redact_raw}). "
        ...
        # after an OpenClaw→FETIH migration (especially migrations done by
        ...
        try:
            ...
            if not is_seen(self.config, OPENCLAW_RESIDUE_FLAG) and detect_openclaw_residue():
                ...
                try:
                    from fetih_cli.config import get_config_path as _get_cfg_path_resid
        ...
        if _preserve_ctrl_enter_newline():
            ...
            def handle_ctrl_enter_newline(event):
                ...
                conflicting FETIH binding. See issue #22379.
                """
                event.current_buffer.insert_text('\n')
        ...
        def handle_ctrl_z(event):
            ...
            from prompt_toolkit.application import run_in_terminal
            from fetih_cli.skin_engine import get_active_skin
            agent_name = get_active_skin().get_branding("agent_name", "FETIH")
        ...
        # Dynamic prompt: shows FETIH symbol when agent is working,
        ...
        def _get_clarify_display():
            ...
            box_width = _panel_box_width("FETIH needs your input", preview_lines)
            inner_text_width = max(8, box_width - 2)
            ...
            lines.append(('class:clarify-title', 'FETIH needs your input'))
            lines.append(('class:clarify-border', ' ' + ('─' * max(0, box_width - len("FETIH needs your input") - 3)) + '╮\n'))
            if not use_compact_chrome:
        ...
        def _get_model_picker_display():
            ...
            try:
                ...
                term_rows = shutil.get_terminal_size((100, 24)).lines
            scroll_offset, visible = FETIHCLI._compute_model_picker_viewport(
        ...
        try:
            ...
            if not getattr(_pt_renderer, "_fetih_osd_patched", False):
                ...
                _pt_renderer._fetih_osd_patched = True
        except Exception:
            pass
        ...
        def _signal_handler(signum, frame):
            ...
            Grace window (``FETIH_SIGTERM_GRACE``, default 1.5 s) gives
            ...
            try:
                if getattr(self, "agent", None) and getattr(self, "_agent_running", False):
                    ...
                    try:
                        _grace = float(os.getenv("FETIH_SIGTERM_GRACE", "1.5"))
                    except (TypeError, ValueError):
                        _grace = 1.5
                    if _grace > 0:
                        time.sleep(_grace)
        ...
        try:
            ...
            # delivers spurious CTRL_C_EVENT to the fetih process when
        ...
        try:
            ...
            print(
                ...
                "Try reinstalling Python via pyenv or Homebrew, then re-run: fetih setup"
            )
            _run_cleanup()
...
def main(
    ...
    """
    FETIH CLI - Interactive AI Assistant
    ...
    if gateway:
        ...
        print("Starting FETIH Gateway (messaging platforms)...")
    ...
    # Default to fetih-cli toolset which includes cronjob management tools
    toolsets_list = None
    if toolsets:
    ...
    # mode registers its own inside FETIHCLI.run(), but `-q` runs
    ...
    # so main unwinds normally.  FETIH_SIGTERM_GRACE overrides the 1.5 s
        ...
        if quiet:
            ...
            if cli._ensure_runtime_credentials():
                ...
                if cli._init_agent(
                    ...
                    # machine-readable (no styled "FETIH" box, no tool-gen
            ...
            # Single-query mode (`fetih chat -q "…"`): skip the welcome
